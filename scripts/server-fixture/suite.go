...
// mmdb/places.yml is the place list the server's location seeder reads
// (connect/GEOMAP.md §4.1), exactly as server/cli/geolite2export writes it:
// this is the exporter's output for a handful of synthetic networks, and
// suite_test.go regenerates it through the exporter to keep the two identical.
// It names real GeoNames places, with a city under a subdivision, a city
// under none (sg), and one whose networks carry two coordinates, so a
// portable DB test can seed countries, regions and cities by geoname id
// without the production-sized database.
const suiteFixturePlaces = `# The canonical place list (connect/GEOMAP.md §4), generated by
