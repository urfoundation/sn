...
func newStatsSnapshotWriteTest(t *testing.T) (*StatsEngine, string, []byte, []byte) {
	...
	if err != nil || bytes.Equal(before, after) {
		t.Fatalf("distinct actual counter snapshots: %v", err)
	}
...
func statsSnapshotWriteTestRetarget(dir string, data []byte) error {
	...
	if data != nil {
		return os.WriteFile(filepath.Join(dir, "stats.json"), data, 0o600)
	}
	return nil
}
...
func TestStatsSnapshotWriteExactRoundTripAndUnlockedObservers(t *testing.T) {
	...
	stats.writeHooks.snapshotIO.after = func(stage string, file *os.File) error {
		stages[stage]++
		if !stats.mu.TryLock() {
			return errors.New("snapshot observer retained Stats.mu")
		}
		stats.mu.Unlock()
		_ = stats.ProviderIDs()
		if strings.HasSuffix(stage, "-closed") {
			if _, err := file.Stat(); !errors.Is(err, os.ErrClosed) {
				return errors.New("observer preceded actual Close")
			}
		}
		return nil
	}
	...
	if !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), expected) || stages["temporary-closed"] != 1 || stages["directory-closed"] != 1 || stages["directory-synced"] != 1 {
		t.Fatalf("native snapshot did not retain exact codec/close ownership: %+v", stages)
	}
	info, err := os.Stat(filepath.Join(dir, "stats.json"))
	if err != nil || info.Mode().Perm() != 0o600 {
		t.Fatalf("private snapshot mode: %v", err)
	}
	loaded := NewStatsEngine(stats.cfg)
	if err := loaded.Load(dir); err != nil || !reflect.DeepEqual(loaded.snapshotStats(), stats.snapshotStats()) {
		t.Fatalf("actual restart differs: %v", err)
	}
}
...
func TestStatsSnapshotWriteBeforeSnapshotParentRetargetRefused(t *testing.T) {
	...
	stats.writeHooks.step = func(operation, stage string) {
		...
		steps++
		if err := statsSnapshotWriteTestRetarget(dir, marker); err != nil {
			t.Fatal(err)
		}
	}
	err := stats.Save(dir)
	if steps != 1 || err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), marker) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir+"-preserved", "stats.json")), before) {
		t.Fatalf("snapshot pre-write parent retarget escaped retained authority: steps=%d error=%v", steps, err)
	}
}
...
func TestStatsSnapshotWriteBeforeSnapshotLeafReplacementRefused(t *testing.T) {
	...
	err := stats.Save(dir)
	if steps != 1 || err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, path), before) || !bytes.Equal(statsSnapshotWriteTestRead(t, path+"-preserved"), before) {
		t.Fatalf("same-byte replacement snapshot was overwritten: steps=%d error=%v", steps, err)
	}
}
...
func TestStatsSnapshotWriteTemporaryCloseRetargetCannotRedirectCleanup(t *testing.T) {
	...
	stats.writeHooks.snapshotIO.after = func(stage string, file *os.File) error {
		...
		if err := statsSnapshotWriteTestRetarget(dir, before); err != nil {
			return err
		}
		return os.WriteFile(filepath.Join(dir, temporary), marker, 0o600)
	}
	err := stats.Save(dir)
	if closed != 1 || err == nil || temporary == "" || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, temporary)), marker) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir+"-preserved", "stats.json")), before) {
		t.Fatalf("temporary Close retarget redirected publication or cleanup: closed=%d error=%v", closed, err)
	}
	if _, err := os.Lstat(filepath.Join(dir+"-preserved", temporary)); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("owned original temporary was not cleaned: %v", err)
	}
}
...
func TestStatsSnapshotWriteChangedTemporaryCannotPublishOrCleanForeign(t *testing.T) {
	for _, kind := range []string{"regular", "symlink", "fifo"} {
		...
		stats.writeHooks.snapshotIO.after = func(stage string, file *os.File) error {
			...
			changes++
			if temporary == "" {
				return errors.New("actual temporary was not observed")
			}
			...
			case "symlink":
				return os.Symlink(filepath.Base(temporary)+"-preserved", temporary)
			default:
				return unix.Mkfifo(temporary, 0o600)
			}
		}
		err := stats.Save(dir)
		if changes != 1 || err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) {
			t.Fatalf("changed temporary reached snapshot rename: kind=%s changes=%d error=%v", kind, changes, err)
		}
		if _, err := os.Lstat(temporary); err != nil {
			t.Fatalf("foreign %s temporary was deleted: %v", kind, err)
		}
		if kind == "regular" && !bytes.Equal(statsSnapshotWriteTestRead(t, temporary), marker) {
			t.Fatal("foreign regular temporary changed")
		}
	}
}
...
func TestStatsSnapshotWritePostRenameRetargetRefused(t *testing.T) {
	...
	err := stats.Save(dir)
	if changed != 1 || err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir+"-preserved", "stats.json")), expected) {
		t.Fatalf("post-rename parent retarget was acknowledged: changed=%d error=%v", changed, err)
	}
}
...
func TestStatsSnapshotWriteDirectorySyncRetargetRefused(t *testing.T) {
	...
	err := stats.Save(dir)
	if synced != 1 || err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir+"-preserved", "stats.json")), expected) {
		t.Fatalf("directory Sync retarget escaped final custody: synced=%d error=%v", synced, err)
	}
}
...
func TestStatsSnapshotWriteFinalDirectoryCloseRetargetRefused(t *testing.T) {
	...
	stats.writeHooks.snapshotIO.after = func(stage string, file *os.File) error {
		...
		closed++
		if _, err := file.Stat(); !errors.Is(err, os.ErrClosed) {
			return errors.New("directory callback preceded actual Close")
		}
		return statsSnapshotWriteTestRetarget(dir, before)
	}
	err := stats.Save(dir)
	if closed != 1 || err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir+"-preserved", "stats.json")), expected) {
		t.Fatalf("nil-returning final directory Close retarget escaped: closed=%d error=%v", closed, err)
	}
}
...
func TestStatsSnapshotWriteFinalCloseLeafReplacementAndAbsenceRefused(t *testing.T) {
	for _, replace := range []bool{false, true} {
		...
		err := stats.Save(dir)
		if closed != 1 || err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, path+"-preserved"), expected) {
			t.Fatalf("final Close changed leaf was acknowledged: replace=%t closed=%d error=%v", replace, closed, err)
		}
		if !replace && !errors.Is(err, os.ErrNotExist) {
			t.Fatalf("late missing snapshot cause lost: %v", err)
		}
	}
}
...
func TestStatsSnapshotWriteAllRootsCheckedAfterLastObservableClose(t *testing.T) {
	...
	err = stats.Save(dir)
	if closed != 3 || retargeted != 1 || err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir+"-preserved", "stats.json")), expected) {
		t.Fatalf("last owner Close changed an earlier closed child unnoticed: closes=%d retargeted=%d error=%v", closed, retargeted, err)
	}
}
...
func TestStatsSnapshotWritePrimaryAndEverySecondaryCloseRemainJoined(t *testing.T) {
	...
	err := stats.Save(dir)
	if closed != 2 || !errors.Is(err, primary) || !errors.Is(err, temporaryClose) || !errors.Is(err, directoryClose) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) {
		t.Fatalf("snapshot primary/secondary Close causes were dropped: closes=%d error=%v", closed, err)
	}
}
...
func TestStatsSnapshotWriteActualClosedDescriptorAndCleanupErrorJoined(t *testing.T) {
	...
	err := stats.Save(dir)
	if closed != 1 || !errors.Is(err, os.ErrClosed) || !errors.Is(err, secondary) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) {
		t.Fatalf("real closed-descriptor failure lost secondary cleanup cause: closes=%d error=%v", closed, err)
	}
}
...
func TestStatsSnapshotWriteMixedCancellationAndDurabilityRemainJoined(t *testing.T) {
	...
	stats.writeHooks.snapshotIO.after = func(stage string, _ *os.File) error {
		if stage == "directory-synced" {
			cancel()
			return errors.Join(context.Canceled, syncFailure)
		}
		if stage == "directory-closed" {
			return closeFailure
		}
	...
	owner.release()
	if !errors.Is(err, context.Canceled) || !errors.Is(err, syncFailure) || !errors.Is(err, closeFailure) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), expected) {
		t.Fatalf("mixed cancellation discarded durable or cleanup cause: %v", err)
	}
}
...
func TestStatsSnapshotWriteCancellationBeforeCreationHasNoMutation(t *testing.T) {
	...
	defer cancel()
	steps := 0
	stats.writeHooks.step = func(_, stage string) {
	...
	err = stats.saveOwned(dir, owner.persist)
	owner.release()
	if steps != 1 || !errors.Is(err, context.Canceled) {
		t.Fatalf("pre-write cancellation result differs: %d/%v", steps, err)
	}
	entries, err := os.ReadDir(ancestor)
	if err != nil || len(entries) != 0 {
		t.Fatalf("canceled snapshot created or repaired state: %v", err)
	}
}
