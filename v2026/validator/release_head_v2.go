...
func (self *ReleaseSteerer) gatherHeadV2(ctx context.Context, snapshot *ReleaseSnapshot, subnetEpoch, nativeBlock uint64, nativeHash string, hotkeyUIDs map[[32]byte]uint16, inputs []ReleaseMeasurementInput, options ReleaseMeasurementV2Options) (result releaseHeadResult, resultErr error) {
	...
	owner, maxEntries, ownerBudget, err := headEMA.ownReleaseHeadEMAV2(ctx, options.MaxHeadEntries, releaseHeadV2Budget{limit: options.MaxControlBytes})
	if err != nil {
		return result, err
	}
	defer func() {
	...
	options.MaxHeadEntries, options.MaxControlBytes = maxEntries, ownerBudget.limit
	...
	budget, err = reserveReleaseHeadV2OperatorControls(ctx, options, budget)
	if err != nil {
		return result, err
	}
	...
	ema, head, budget, err := owner.previewFleets(ctx, subnetEpoch, fleets, draft.Policy.Steering.HeadScoreEMA, options.MaxHeadEntries, budget)
	if err != nil {
		return result, err
	}
