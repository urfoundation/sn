...
func (self *statsWriteOwner) persistChecked(write statsSnapshotWrite, check func() error) (resultErr error) {
	if self.snapshot == nil {
		...
		self.snapshot = owner
		if err != nil {
			return errors.Join(err, self.finishSnapshot())
		}
	}
	defer func() { resultErr = errors.Join(resultErr, self.finishSnapshot()) }()
	if err := self.snapshot.admit(write); err != nil {
		return err
	}
	...
	if err := self.snapshot.check(); err != nil {
		return err
	}
	if self.hooks.writeSnapshot != nil {
		if err := self.hooks.writeSnapshot(self.snapshot, write); err != nil {
			return err
		}
		if !self.snapshot.written {
			return errors.New("statistics snapshot callback did not complete the owned writer")
		}
		return nil
	}
