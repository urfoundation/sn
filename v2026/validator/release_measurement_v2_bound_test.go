...
func TestReleaseMeasurementV2ArtifactBoundExactCeilingRetainsRealReplay(t *testing.T) {
	...
	if maxReleaseMeasurementArtifactBytes != releaseMeasurementEnvelopeMaxArtifactSize {
		t.Fatal("compact admission no longer uses the original envelope ceiling")
	}
	if maxReleaseMeasurementArtifactBytes != 64*1024*1024 {
		t.Fatal("compact admission ceiling is no longer exactly 64 MiB")
	}
