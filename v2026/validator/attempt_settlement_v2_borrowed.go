...
func borrowAttemptSettlementV2Snapshot(stats *StatsEngine) attemptSettlementV2BorrowedSnapshot {
	...
	version := 4
	if stats.attemptLedger != nil || stats.attemptLastAppliedSequence != 0 || stats.attemptSettlementFirstSequence != 0 || stats.attemptEgressFirstSequence != 0 {
		version = 5
	}
	...
	if stats.settlementEpochKnown {
		...
		borrowed.SettlementEpoch = &epoch
	}
	return borrowed
}
...
func (self attemptSettlementV2BorrowedSnapshot) admitCensus(ctx context.Context, operator AttemptSettlementV2OperatorOptions) error {
	...
	for _, size := range []int{len(self.Window), len(self.Ema), len(self.EmaPPM), len(self.Egress)} {
		if uint64(size) > maxProviders {
			return errors.New("compact borrowed provider census exceeds its bound before copying")
		}
	}
	...
	add := func() error {
		...
		if providers >= maxProviders {
			return errors.New("compact borrowed provider union exceeds its bound before copying")
		}
		providers++
		return nil
	}
	for _, window := range self.Window {
		if window == nil {
			return errors.New("compact borrowed provider window is nil")
		}
		if err := add(); err != nil {
			return err
		}
	...
	for id := range self.Ema {
		...
		if _, present := self.Window[id]; !present {
			if err := add(); err != nil {
				return err
			}
		}
	...
	for id, values := range self.Egress {
		...
		_, window := self.Window[id]
		_, ema := self.Ema[id]
		_, ppm := self.EmaPPM[id]
		if !window && !ema && !ppm {
			if err := add(); err != nil {
				return err
			}
		}
		if uint64(len(values)) > maxHashes-hashes {
			return errors.New("compact borrowed egress census exceeds its bound before copying")
		}
...
func (self *attemptSettlementV2JSONCount) borrowedMap(value reflect.Value, depth uint64, egress bool) error {
	...
	if uint64(value.Len()) > self.limit-self.used {
		return errors.New("compact borrowed map census exceeds its byte allowance")
	}
	...
	for iterator.Next() {
		...
		if err := self.add(38 + colon); err != nil {
			return err
		}
		if egress {
			...
			if uint64(hashes.Len()) > (self.limit-self.used)/68 {
				return errors.New("compact borrowed hashes exceed their byte allowance")
			}
			for index := 0; index < hashes.Len(); index++ {
				...
				if err := self.line(depth + 2); err != nil {
					return err
				}
				if err := self.add(68); err != nil {
					return err
				}
			}
			if hashes.Len() != 0 {
			...
			if value.Type() == reflect.TypeFor[attemptSettlementV2BorrowedWindow]() && iterator.Value().IsNil() {
				return errors.New("compact borrowed provider window is nil")
			}
...
func encodeAttemptSettlementV2Engine(ctx context.Context, stats *StatsEngine, limit uint64) ([]byte, error) {
	...
	if uint64(len(encoded)) != size {
		return nil, errors.New("compact borrowed snapshot differs from its admitted exact size")
	}
	return encoded, nil
}
...
func admitAttemptSettlementV2OwnedBorrowed(ctx context.Context, batch *attemptSettlementV2Owners, authority AttemptSettlementV2Options) error {
	for _, participant := range batch.ordered {
		...
		if err := borrowed.admitCensus(ctx, authority.Operators[participant.NoID]); err != nil {
			return fmt.Errorf("no_id %d: %w", participant.NoID, err)
		}
		if _, err := countAttemptSettlementV2JSON(ctx, borrowed, batch.persistence.MaxSnapshotBytes, true, true); err != nil {
			return fmt.Errorf("no_id %d borrowed snapshot: %w", participant.NoID, err)
		}
	}
	return ctx.Err()
}
