...
func headEMAStoreV2BitSum(values ...uint64) (uint64, error) {
	...
	for _, value := range values {
		if value > ^uint64(0)-total {
			return 0, errors.New("bounded head EMA rational growth overflow")
		}
		total += value
	}
	return total, nil
...
func headEMAStoreV2DecimalBits(ctx context.Context, value string) (uint64, error) {
	if len(value) == 0 {
		return 0, errors.New("bounded head EMA rational text is empty")
	}
	for index := range len(value) {
		...
		if value[index] < '0' || value[index] > '9' {
			return 0, errors.New("bounded head EMA rational text is not unsigned decimal")
		}
	}
	if value == "0" {
		return 0, nil
	}
	if uint64(len(value)) > (^uint64(0)-2)/10 {
		return 0, errors.New("bounded head EMA decimal growth overflow")
	}
...
func headEMAStoreV2TextFraction(ctx context.Context, value RationalJSON) (headEMAStoreV2Fraction, error) {
	...
	if err != nil {
		return headEMAStoreV2Fraction{}, err
	}
	denominator, err := headEMAStoreV2DecimalBits(ctx, value.Denominator)
	if err != nil || denominator == 0 {
		return headEMAStoreV2Fraction{}, errors.Join(errors.New("bounded head EMA denominator is absent or zero"), err)
	}
...
func headEMAStoreV2RatFraction(value *big.Rat) (headEMAStoreV2Fraction, error) {
	if value == nil || value.Sign() < 0 {
		return headEMAStoreV2Fraction{}, errors.New("head EMA raw score is nil or negative")
	}
...
func (self headEMAStoreV2Fraction) reserve(budget *headEMAStoreV2Budget, owners uint64) error {
	...
	if err := budget.charge(words, wordBytes*owners); err != nil {
		return err
	}
	return nil
}
...
func (self headEMAStoreV2Fraction) reserveText(budget *headEMAStoreV2Budget) error {
	for _, length := range [2]uint64{self.numerator, self.denominator} {
		...
		if err := budget.charge(digits, 4); err != nil {
			return err
		}
	}
	return nil
...
func (self headEMAStoreV2Fraction) multiply(numerator, denominator uint64) (headEMAStoreV2Fraction, error) {
	if numerator == 0 || self.numerator == 0 {
		return headEMAStoreV2Fraction{denominator: 1}, nil
	}
...
func (self *HeadEMAStore) admitHeadEMAStoreV2KnownWithLock(ctx context.Context, operation headEMAStoreV2Operation, epoch uint64, alpha protocol.Rational, limits HeadEMAStoreV2Limits) error {
	...
	if operation > headEMAStoreV2Fold {
		return errors.New("bounded head EMA operation is unknown")
	}
	...
	if operation != headEMAStoreV2Fold && self.lastSubnetEpoch != nil {
		if epoch < *self.lastSubnetEpoch {
			return errors.New("head EMA epoch regressed")
		}
		if epoch == *self.lastSubnetEpoch {
...
func chargeHeadEMAStoreV2Records(ctx context.Context, budget *headEMAStoreV2Budget, records []HeadEMAMeasurement, copies uint64) error {
	if err := budget.charge(uint64(len(records)), uint64(reflect.TypeFor[HeadEMAMeasurement]().Size())*copies); err != nil {
		return err
	}
	for _, record := range records {
		...
		for _, value := range [3]RationalJSON{record.Raw, record.Prior, record.Next} {
			if err := budget.charge(uint64(len(value.Numerator)), copies); err != nil {
				return err
			}
			if err := budget.charge(uint64(len(value.Denominator)), copies); err != nil {
				return err
			}
		}
	}
...
func (self *HeadEMAStore) admitHeadEMAStoreV2WithLock(ctx context.Context, operation headEMAStoreV2Operation, epoch uint64, raw map[FleetScoreKey]*big.Rat, records []HeadEMAMeasurement, alpha protocol.Rational, limits HeadEMAStoreV2Limits) (headEMAStoreV2Budget, error) {
	...
	if err := self.admitHeadEMAStoreV2KnownWithLock(ctx, operation, epoch, alpha, limits); err != nil {
		return budget, err
	}
	if uint64(len(raw)) > limits.MaxEntries || uint64(len(records)) > limits.MaxEntries {
		return budget, errors.New("bounded head EMA input census exceeds its allowance")
	}
	...
	if err := budget.charge(uint64(len(self.path)), 8+2*uint64(reflect.TypeFor[string]().Size())); err != nil {
		return budget, err
	}
	if operation != headEMAStoreV2Preview {
		...
		if err := budget.charge(1, 4096+4*uint64(reflect.TypeFor[attemptPrivateFileState]().Size())+1024); err != nil {
			return budget, err
		}
		...
		if err := budget.charge(uint64(len(self.path))+24, 6); err != nil {
			return budget, err
		}
	}
	entryWidth := uint64(reflect.TypeFor[headEMAEntry]().Size()) + uint64(reflect.TypeFor[string]().Size())
	if err := budget.charge(uint64(len(self.values)), 2*entryWidth); err != nil {
		return budget, err
	}
	for key, value := range self.values {
		...
		for _, text := range [3]string{key, value.Numerator, value.Denominator} {
			if err := budget.charge(uint64(len(text)), 2); err != nil {
				return budget, err
			}
		}
	}
	if err := chargeHeadEMAStoreV2Records(ctx, &budget, self.lastFold, 2); err != nil {
		return budget, err
	}
	if err := chargeHeadEMAStoreV2Records(ctx, &budget, records, 2); err != nil {
		return budget, err
	}
	inputCount := uint64(len(raw))
	if operation == headEMAStoreV2Commit {
		inputCount = uint64(len(records))
	}
	planWidth := uint64(reflect.TypeFor[FleetScoreKey]().Size()) + uint64(reflect.TypeFor[headEMAStoreV2Fraction]().Size())
	if err := budget.charge(inputCount, planWidth+uint64(reflect.TypeFor[*big.Rat]().Size())+uint64(reflect.TypeFor[big.Rat]().Size())); err != nil {
		return budget, err
	}
	if err := budget.charge(inputCount+uint64(len(self.values)), 156+planWidth+uint64(reflect.TypeFor[headEMAStoreV2UIDBound]().Size())+2); err != nil {
		return budget, err
	}
	...
	if operation == headEMAStoreV2Commit {
		for _, record := range records {
			...
			for _, value := range [3]RationalJSON{record.Raw, record.Prior, record.Next} {
				...
				if err := bound.reserve(&budget, 16); err != nil {
					return budget, err
				}
			}
			if record.HasRaw {
				if _, found := current[record.Key]; found {
					return budget, errors.New("head EMA transcript duplicates a raw identity")
				}
	...
	for key := range current {
		...
		if _, found := self.values[key.String()]; !found {
			if union >= limits.MaxEntries {
				return budget, errors.New("bounded head EMA current/prior union exceeds its allowance")
			}
			union++
		}
	}
	...
	if sameEpoch {
		union = uint64(len(self.lastFold))
	}
	...
	if err := budget.charge(union, 4*156+3*uint64(reflect.TypeFor[HeadEMAMeasurement]().Size())+2*entryWidth+2048); err != nil {
		return budget, err
	}
	...
	plan := func(key FleetScoreKey, current, prior headEMAStoreV2Fraction, hasRaw, hasPrior bool) error {
		...
		if hasPrior {
			...
			for _, value := range [2]headEMAStoreV2Fraction{left, right} {
				if err := value.reserve(&budget, 16); err != nil {
					return err
				}
			}
	...
	if sameEpoch {
		for _, record := range self.lastFold {
			...
			if record.HasRaw {
				...
				if err := addUID(record.Key.UID, bound); err != nil {
					return budget, err
				}
			}
		}
		...
		for _, entry := range self.values {
			...
			if !found {
				value = headEMAStoreV2Fraction{denominator: 1}
			}
			if err := plan(entry.Key, value, prior, found, true); err != nil {
				return budget, err
			}
		}
		for key, value := range current {
			...
			if _, found := self.values[key.String()]; !found {
				if err := plan(key, value, headEMAStoreV2Fraction{denominator: 1}, true, false); err != nil {
					return budget, err
				}
			}
		}
...
func checkHeadEMAStoreV2Completed(ctx context.Context, store *HeadEMAStore, output map[uint16]*big.Rat, records []HeadEMAMeasurement, reservation headEMAStoreV2Budget) error {
	if reservation.used > reservation.limit {
		return errors.New("bounded head EMA completed plan exceeds its allowance")
	}
	actual := headEMAStoreV2Budget{limit: reservation.used}
	if err := actual.charge(1, headEMAStoreV2FixedControlBytes()); err != nil {
		return err
	}
	if err := actual.charge(uint64(len(store.path)), 1); err != nil {
		return err
	}
	if err := actual.charge(uint64(len(store.values)), uint64(reflect.TypeFor[headEMAEntry]().Size())+uint64(reflect.TypeFor[string]().Size())); err != nil {
		return err
	}
	for key, value := range store.values {
		...
		for _, text := range [3]string{key, value.Numerator, value.Denominator} {
			if err := actual.charge(uint64(len(text)), 1); err != nil {
				return err
			}
		}
	}
	if err := chargeHeadEMAStoreV2Records(ctx, &actual, store.lastFold, 1); err != nil {
		return err
	}
	if err := chargeHeadEMAStoreV2Records(ctx, &actual, records, 1); err != nil {
		return err
	}
	if err := actual.charge(uint64(len(output)), 2+uint64(reflect.TypeFor[*big.Rat]().Size())+uint64(reflect.TypeFor[big.Rat]().Size())); err != nil {
		return err
	}
	for _, value := range output {
		...
		if err := bound.reserve(&actual, 1); err != nil {
			return err
		}
	}
	return ctx.Err()
