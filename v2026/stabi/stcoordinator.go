...
// the contract method with ID 0x18673428.  This method will panic if any
// invalid/nil inputs are passed.
//
// Solidity: function registerOperator(uint256 noId, bytes32 coldkey, bytes32 poolHotkey, bytes32 depositHotkey, address depositSigner, address rootSigner, uint64 effectiveEpoch) payable returns(uint16 uid)
func (sTCoordinator *STCoordinator) PackRegisterOperator(noId *big.Int, coldkey [32]byte, poolHotkey [32]byte, depositHotkey [32]byte, depositSigner common.Address, rootSigner common.Address, effectiveEpoch uint64) []byte {
	enc, err := sTCoordinator.abi.Pack("registerOperator", noId, coldkey, poolHotkey, depositHotkey, depositSigner, rootSigner, effectiveEpoch)
	if err != nil {
		panic(err)
	}
	return enc
...
// the contract method with ID 0x18673428.  This method will return an error
// if any inputs are invalid/nil.
//
// Solidity: function registerOperator(uint256 noId, bytes32 coldkey, bytes32 poolHotkey, bytes32 depositHotkey, address depositSigner, address rootSigner, uint64 effectiveEpoch) payable returns(uint16 uid)
func (sTCoordinator *STCoordinator) TryPackRegisterOperator(noId *big.Int, coldkey [32]byte, poolHotkey [32]byte, depositHotkey [32]byte, depositSigner common.Address, rootSigner common.Address, effectiveEpoch uint64) ([]byte, error) {
	return sTCoordinator.abi.Pack("registerOperator", noId, coldkey, poolHotkey, depositHotkey, depositSigner, rootSigner, effectiveEpoch)
}

// UnpackRegisterOperator is the Go binding that unpacks the parameters returned
// from invoking the contract method with ID 0x18673428.
//
// Solidity: function registerOperator(uint256 noId, bytes32 coldkey, bytes32 poolHotkey, bytes32 depositHotkey, address depositSigner, address rootSigner, uint64 effectiveEpoch) payable returns(uint16 uid)
