// Synthetic461 responses exercise the complete authenticated schedule path.
package crv4
...
func TestRuntimeArtifactMetadataValidatorScheduleRuntime461(t *testing.T) {
	for _, example := range []struct {
		...
		identity.version.SpecVersion = 461
		...
		if err != nil || result.SubnetEpochIndex != 77 || result.Stake.Identity.UID != 1 ||
			...
			t.Fatalf("runtime461 %s lost authenticated schedule semantics: %+v error=%v", example.name, result, err)
		}
		if identity.chain.Meta != metadata || identity.chain.Runtime != runtime {
			t.Fatal("runtime461 schedule changed the signing binding")
		}
	}
}

// Layout compatibility does not authorize another artifact or unknown version.
func TestRuntimeArtifactMetadataValidatorStakeRuntime461RejectsForeignIdentity(t *testing.T) {
	for _, example := range []struct {
		name   string
		...
		fixture.identity.version.SpecVersion = 461
		...
		if err == nil || result != (ValidatorStakeObservation{}) || fixture.runtimeCalls != 0 {
			t.Fatalf("%s reached the461 stake decoder: %+v error=%v calls=%d", example.name, result, err, fixture.runtimeCalls)
		}
	}
}
