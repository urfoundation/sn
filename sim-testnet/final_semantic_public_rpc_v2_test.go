...
func newFinalV2PublicRPCFixture(t *testing.T) *finalV2PublicRPCFixture {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if batch {
			...
			if err := json.Unmarshal(raw, &calls[0]); err != nil {
				t.Error(err)
				return
			}
		}
		...
		for _, call := range calls {
			...
			switch call.Method {
			case "eth_chainId":
				result = fmt.Sprintf(`"%s"`, hexutil.EncodeUint64(testnetChainID))
			case "eth_getBlockByHash", "eth_getBlockByNumber":
				result = fmt.Sprintf(`{ "number" : "0x64", "hash" : %q }`, f.evmHead.Hash)
			case "eth_call":
				result = fmt.Sprintf(`"0x%064x"`, f.answer.Load())
				if n := f.payloadBytes.Load(); n != 0 {
					result = `"0x` + strings.Repeat("00", int(n)) + `"`
				}
			case "eth_getCode":
				result = `"0x6000"`
			default:
				t.Errorf("unexpected source method %s", call.Method)
				http.Error(writer, "unadmitted call", 400)
				return
			}
		...
		if batch {
			fmt.Fprintf(writer, "[%s]", strings.Join(replies, ","))
		} else {
			fmt.Fprint(writer, replies[0])
		}
	}))
...
func TestFinalPublicValidatorSourcesV2KeepsDetachedEvidenceAuthority(t *testing.T) {
	...
	for _, fault := range []string{"plan", "config", "window", "terminal", "nested-source"} {
		t.Run(fault, func(t *testing.T) {
			...
			switch fault {
			case "plan":
				changed.PlanHash = common.Hash{0x44}.Hex()
			case "config":
				changed.ConfigHash = common.Hash{0x44}.Hex()
			case "window":
				changed.Window.FirstEpoch++
			case "terminal":
				changed.NativeTerminalHead.Number++
			case "nested-source":
				changed.ExitCriteria[0].Assertions[0].Observed++
			}
...
func TestFinalPublicValidatorSourcesV2RetainsRealBatchesAndCanonicalHeads(t *testing.T) {
	...
	if err != nil || netuid != 55 {
		t.Fatalf("actual typed canonical reader: netuid=%d err=%v", netuid, err)
	}
	...
	if err := facade.BatchCallContext(t.Context(), batch); err != nil {
		t.Fatal(err)
	}
	...
	for _, exchange := range exchanges {
		if exchange.Chain != "evm" || exchange.PinnedHead != f.evmHead {
			t.Fatal("EVM result lost its exact EVM checkpoint")
		}
		requestHash, responseHash, err := finalRPCExchangeHashes(exchange)
		if err != nil || exchange.RequestHash != requestHash || exchange.ResponseHash != responseHash {
			t.Fatalf("transcript hashes changed: %v", err)
		}
		if exchange.Method == "eth_getBlockByHash" && bytes.Contains(exchange.Result, []byte(`{ "number" :`)) {
			originalHeader = true
		}
	}
	if !originalHeader {
		t.Fatal("original RPC result bytes were normalized before custody")
	}
	first := bytes.Clone(exchanges[0].Result)
	exchanges[0].Result[0] ^= 1
	again, err := recorder.finish()
	if err != nil || !bytes.Equal(again[0].Result, first) {
		t.Fatal("returned transcript aliases the completed audit")
	}
	...
	if err := other.CallContext(t.Context(), &answer, batch[0].Method, batch[0].Args...); err != nil {
		t.Fatal(err)
	}
	if f.calls.Load() <= beforeCalls {
		t.Fatal("source transcript scope leaked across invocations")
	}
}
...
func TestFinalPublicValidatorSourcesV2RefusesWritesUnpinnedAndConflictingReads(t *testing.T) {
	...
	for _, call := range []struct {
		...
		if err := facade.CallContext(t.Context(), &result, call.method, call.args...); err == nil || f.calls.Load() != before {
			t.Fatalf("unadmitted method reached real transport: %s", call.method)
		}
	}
	recorder := newFinalV2RPCRecorder(t.Context(), f.reader, 64*1024, 1024*1024)
	facade := finalV2PublicReadFacade(t, t.Context(), recorder)
	...
	if err := facade.CallContext(t.Context(), &result, "eth_call", args...); err != nil {
		t.Fatal(err)
	}
	f.answer.Store(56)
	if err := facade.CallContext(t.Context(), &result, "eth_call", args...); err == nil {
		t.Fatal("changed immutable source answer was admitted")
	}
	if got, err := recorder.finish(); err == nil || got != nil {
		t.Fatal("conflicting source produced a partial successful transcript")
	}
	...
	if err := foreign.CallContext(t.Context(), &result, "eth_call", args...); err == nil {
		t.Fatal("foreign hash was assigned the real canonical height")
	}
}

type finalV2NativeReadTestClient struct {
	*gsrpcgeth.Client
	endpoint string
}
...
func TestFinalPublicValidatorSourcesV2KeepsNativeClockAndNullStorage(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "chain_getBlockHash":
			result = fmt.Sprintf("%q", f.nativeHead.Hash)
		case "chain_getFinalizedHead":
			result = fmt.Sprintf("%q", common.Hash{0x33}.Hex())
		case "state_getStorage":
			result = "null"
		default:
			t.Errorf("unexpected native method %s", call.Method)
			http.Error(writer, "unexpected", 400)
			return
		}
		writer.Header().Set("Content-Type", "application/json")
	...
	var raw *json.RawMessage
	if err := client.CallContext(t.Context(), &raw, "state_getStorage", "0x0102", f.nativeHead.Hash); err != nil {
		t.Fatal(err)
	}
	if raw != nil {
		t.Fatal("actual absent storage was not JSON null")
	}
	if err := recorder.retainNative(t.Context(), validatorpkg.ReleaseEvidenceV2NativeRead{Schema: "urnetwork-validator-native-read-v2", Method: "state_getStorage", Parameters: params, Result: []byte("null")}); err != nil {
		t.Fatal(err)
	}
	exchanges, err := recorder.finish()
	if err != nil {
	...
	var absent bool
	for _, exchange := range exchanges {
		if exchange.Chain != "substrate" || exchange.PinnedHead != f.nativeHead || exchange.PinnedHead.Number == f.evmHead.Number {
			t.Fatal("native read inherited the EVM numeric clock")
		}
		if exchange.Method == "state_getStorage" && bytes.Equal(exchange.Result, []byte("null")) {
			absent = true
		}
	}
	if !absent {
		t.Fatal("valid absent native storage was dropped")
	}
	before := len(exchanges)
	finalized, _ := json.Marshal(common.Hash{0x33}.Hex())
	if err := recorder.retainNative(t.Context(), validatorpkg.ReleaseEvidenceV2NativeRead{Schema: "urnetwork-validator-native-read-v2", Method: "chain_getFinalizedHead", Parameters: []byte("[]"), Result: finalized}); err != nil {
		t.Fatal(err)
	}
	if got, err := recorder.finish(); err != nil || len(got) != before {
		t.Fatal("moving finality was mislabeled as immutable historical state")
	}
	if err := recorder.retainNative(t.Context(), validatorpkg.ReleaseEvidenceV2NativeRead{Schema: "urnetwork-validator-native-read-v2", Method: "state_getStorage", Parameters: []byte(`["0x0102"]`), Result: []byte("null")}); err == nil {
		t.Fatal("unpinned native storage gained historical authority")
	}
}
...
func TestFinalPublicValidatorSourcesV2CancelsAndJoinsActiveTransport(t *testing.T) {
	...
	case <-time.After(2 * time.Second):
		t.Fatal("actual RPC body never entered")
	}
	cancel()
	select {
	case err := <-returned:
	...
	case <-time.After(2 * time.Second):
		t.Fatal("cancelled facade did not return")
	}
	select {
	case <-joined:
	case <-time.After(2 * time.Second):
		t.Fatal("actual source request was not joined")
	}
	if got, err := recorder.finish(); err == nil || got != nil {
		t.Fatal("cancelled source yielded a transcript")
	}
}
...
func TestFinalPublicValidatorSourcesV2BoundsRejectPartialReadResults(t *testing.T) {
	...
	if got, err := recorder.finish(); err == nil || got != nil {
		t.Fatal("oversized source response left a successful partial transcript")
	}
	bounded := newFinalV2RPCRecorder(t.Context(), f.reader, 64*1024, 1)
	limited := finalV2PublicReadFacade(t, t.Context(), bounded)
	if err := limited.CallContext(t.Context(), &output, "eth_chainId"); err == nil || output != "unchanged" {
		t.Fatal("aggregate byte refusal exposed an uncaptured source fact")
	}
	if got, err := bounded.finish(); err == nil || got != nil {
		t.Fatal("exhausted transcript allowance produced a partial success")
	}
}
