...
scenarioLoop:
	for (!scenarioAcceptanceIntervalObserved(window, current) || !scenarioAssertionsComplete(cfg, definition, assertions) || !faultsComplete(faults) || (options.FleetLifecycle != nil && !options.FleetLifecycle.Complete()) || (options.Adversaries != nil && !options.Adversaries.Ready())) && options.Now().Before(deadline) {
		...
		if boundaryCommitted {
			provisionalTerminalFailure = scenarioProvisionalFailureCompletion(cfg, definition, window, current, faults, assertions, options)
			if provisionalTerminalFailure != "" {
				break
			}
		}
	...
	if boundaryCommitted && acceptanceIncomplete && !provisionalTerminalComplete && provisionalTerminalFailure == "" {
		if terminalErr == nil {
	...
	if provisionalTerminalFailure != "" {
		assertions = append(assertions, AssertionRecord{
			ID: "provisional_failed_interval_complete", Passed: false,
			Message:   "complete signed interval and fault cleanup observed; immutable failure retained: " + provisionalTerminalFailure + "; final_acceptance=false",
			StartedAt: started.UTC().Format(time.RFC3339Nano), CompletedAt: completed.UTC().Format(time.RFC3339Nano), DurationSeconds: completed.Sub(started).Seconds(), ObservationHash: current.ObservationHash,
		})
	}
