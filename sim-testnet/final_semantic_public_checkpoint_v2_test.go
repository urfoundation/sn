...
func TestFinalPublicValidatorSourcesV2NativeCheckpointAuthenticatesExactPublicState(t *testing.T) {
	...
	compact := func(n uint64) []byte {
		t.Helper()
		raw, err := codec.Encode(gsrpctypes.NewUCompactFromUInt(n))
		if err != nil {
			t.Fatal(err)
		}
		return raw
	...
	stakes := compact(3)
	for _, stake := range []uint64{0, 150, 9} {
		stakes = append(stakes, compact(stake)...)
	}
	...
	for field := 1; field <= 76; field++ {
		if value, ok := fields[field]; ok {
			metagraph = append(metagraph, 1)
			metagraph = append(metagraph, value...)
		} else {
			metagraph = append(metagraph, 0)
		}
	}
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "chain_getHeader":
			...
			parentHash := parent
			if number == 99 {
				parentHash = common.Hash{0x25}.Hex()
			}
			if number == 101 {
				parentHash = f.nativeHead.Hash
			}
			if number == 90 {
				parentHash = payoutParent
			}
			if number == 89 {
				parentHash = common.Hash{0x27}.Hex()
			}
		...
		case "state_getRuntimeVersion":
			result = f.reader.runtimeVersion
		case "state_getStorageHash":
			result = f.reader.runtimeCodeHash
			if mode.Load() == 3 {
		...
		case "state_getStorage":
			...
			if key == mappingKey.Hex() {
				if lastHash == parent && mode.Load() != 2 {
					result = nil
				} else {
					result = f.evmHead.Hash
				}
				if lastHash == f.nativeHead.Hash && mode.Load() == 1 {
					result = common.Hash{0x55}.Hex()
				}
			} else {
				var ok bool
				result, ok = storage[key]
				if !ok || lastHash != f.nativeHead.Hash {
					t.Errorf("unexpected native storage %s at %s", key, lastHash)
					http.Error(writer, "wrong storage", 400)
					return
				}
				if key == updatesKey && mode.Load() == 4 {
					result = "0x00"
				}
			}
		case "state_call":
			result = fmt.Sprintf("0x%x", metagraph)
		default:
	...
	read := func(wantUID uint16) (FinalNativeCheckpointV2, []FinalRPCExchange, error) {
		return f.reader.NativeCheckpointV2(t.Context(), target, f.evmHead, wantUID, fmt.Sprintf("0x%x", hotkey))
	}
	got, exchanges, err := read(1)
	if err != nil {
		t.Fatalf("actual public checkpoint failed: %v", err)
	}
	...
	for _, exchange := range exchanges {
		if exchange.Chain == "evm" {
			seenEVM = true
			if exchange.PinnedHead != f.evmHead {
				t.Fatal("EVM exchange inherited native identity")
			}
		}
		if exchange.Chain == "substrate" {
			seenNative = true
			if exchange.PinnedHead.Hash == f.evmHead.Hash {
				t.Fatal("native exchange inherited EVM identity")
			}
		}
		if exchange.Method == "state_call" {
			seenRuntime = true
		}
		if exchange.Method == "state_getStorage" && exchange.PinnedHead.Hash == parent && string(exchange.Result) == "null" {
			seenAbsent = true
		}
	}
	...
	for _, fault := range []uint32{1, 2, 3, 4} {
		...
		if err == nil || !reflect.DeepEqual(partial, FinalNativeCheckpointV2{}) || raw != nil {
			t.Fatalf("fault %d issued partial checkpoint authority: %v", fault, err)
		}
	}
	mode.Store(0)
	if partial, raw, err := read(2); err == nil || !reflect.DeepEqual(partial, FinalNativeCheckpointV2{}) || raw != nil {
		t.Fatal("wrong validator UID issued public authority")
	}
	before := nativeCalls.Load()
	got.Weights.Values[0] = 1
	fresh, _, err := read(1)
	if err != nil || fresh.Weights.Values[0] != 65535 || nativeCalls.Load() <= before {
		t.Fatalf("invocation reused another caller's mutable checkpoint: %v", err)
	}
}
