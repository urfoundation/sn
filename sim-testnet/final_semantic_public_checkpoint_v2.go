...
func (self *PublicFinalSemanticChainReader) NativeCheckpointV2(ctx context.Context, evidence *FinalSemanticEvidence, at ChainHead, validatorUID uint16, hotkey string) (observation FinalNativeCheckpointV2, exchanges []FinalRPCExchange, resultErr error) {
	...
	defer func() {
		...
		if resultErr != nil {
			observation, exchanges = FinalNativeCheckpointV2{}, nil
		}
	}()
	...
	if err != nil || key == ([32]byte{}) {
		return observation, nil, errors.Join(errors.New("public native checkpoint hotkey is invalid"), err)
	}
	...
	if _, err := evmReader.CanonicalEVMHead(ctx, at); err != nil {
		return observation, nil, err
	}
