...
func (self *coordinatorRepairCarryRPC) GetCode(ctx context.Context, address common.Address, selector finalEVMBlockSelector) (hexutil.Bytes, error) {
	self.calls.Add(1)
	if _, err := self.selected(selector); err != nil { return nil, err }
	code := bytes.Clone(self.code[address])
	if self.problem == "runtime" && address == self.upgrade.Implementation && len(code) > 0 { code[0] ^= 1 }
	if self.problem == "immutable" && address == self.plan.Deployment.SettlementVault && len(code) > 0 { code[0] ^= 1 }
	return code, ctx.Err()
}
...
func (self *coordinatorRepairCarryRPC) GetStorageAt(_ context.Context, address common.Address, slot string, selector finalEVMBlockSelector) (hexutil.Bytes, error) {
	self.calls.Add(1)
	if _, err := self.selected(selector); err != nil { return nil, err }
	if address != self.plan.Deployment.CoordinatorProxy || slot != erc1967ImplementationSlot { return nil, errors.New("repair requested another storage slot") }
	active := self.upgrade.Implementation
	if self.problem == "activation" { active = self.plan.CoordinatorUpgrade.Implementation }
...
func (self *coordinatorRepairCarryRPC) Call(_ context.Context, message ValidatorEvidenceInstallRPCMessage, selector json.RawMessage) (hexutil.Bytes, error) {
	self.calls.Add(1)
	var number string
	if err := json.Unmarshal(selector, &number); err != nil { return nil, err }
	if number != "0x65" && number != "0x6e" || message.To != self.plan.Deployment.CoordinatorProxy { return nil, errors.New("repair identity used another checkpoint") }
	for method, value := range self.identity {
		if bytes.Equal(message.Data, crypto.Keccak256([]byte(method+"()"))[:4]) {
			result := bytes.Clone(value)
			if self.problem == "custody" && method == "selfColdkey" { result[0] ^= 1 }
			return result, nil
		}
	}
...
func (self *coordinatorRepairCarryRPC) GetTransactionCount(_ context.Context, address common.Address, selector string) (hexutil.Uint64, error) {
	self.calls.Add(1)
	if address != common.HexToAddress(self.plan.Roles.Deployer) || selector != "pending" && selector != "0x6e" { return 0, errors.New("repair nonce read escaped its owner") }
	nonce := self.upgrade.DeployerNonce+1
	if self.problem == "nonce" || self.problem == "pending" && selector == "pending" { nonce++ }
...
func (self *coordinatorRepairCarryRPC) GetTransactionReceipt(ctx context.Context, hash common.Hash) (*types.Receipt, error) {
	...
	if receipt != nil && hash == self.creation { receipt.ContractAddress = self.upgrade.Implementation }
	if receipt != nil && self.problem == "receipt" { receipt.Status = types.ReceiptStatusFailed }
	return receipt, err
}
...
func (self *coordinatorRepairCarryRPC) client(t *testing.T) (*ethclient.Client, string) {
	...
	if err := server.RegisterName("eth", self); err != nil { t.Fatal(err) }
...
func newCoordinatorRepairCarryFixture(t *testing.T) coordinatorRepairCarryFixture {
	...
	for key, value := range identity { encodedIdentity[key] = hex.EncodeToString(value) }
	...
	if err != nil { t.Fatal(err) }
	if err := validateCoordinatorRepairRequest(source, &reference.Request); err != nil { t.Fatal(err) }
	...
	for index, action := range []Action{deploy, activate} {
		...
		if index == 1 { label, data, txNonce, to = "testnet-owner", activateData, 19, &source.Deployment.CoordinatorProxy }
		...
		if err != nil { t.Fatal(err) }
		if err := atomicWrite(filepath.Join(e.stateDir, "transactions", stringsTrim0x(transaction.Hash().Hex())+".rlp"), wire, 0o600); err != nil { t.Fatal(err) }
		...
		if index == 0 { base.creation = transaction.Hash() }
		for _, stage := range []JournalStage{StageIntent, StageBroadcast, StageIncluded, StageFinalized} {
			...
			if stage == StageBroadcast { entry.Signer, entry.Nonce, entry.TransactionHash = crypto.PubkeyToAddress(key.PublicKey).Hex(), strconv.FormatUint(txNonce, 10), transaction.Hash().Hex() }
			if stage == StageIncluded || stage == StageFinalized { entry.TransactionHash, entry.BlockNumber, entry.BlockHash = transaction.Hash().Hex(), head.Number, head.Hash }
			if err := e.journal.Append(entry); err != nil { t.Fatal(err) }
		}
		entries := e.journal.Entries()
	...
	if err != nil { t.Fatal(err) }
	for name, value := range map[string]any{"request.json": reference.Request, "result.json": reference.Result} { if err := writeCoordinatorRepairFile(filepath.Join(e.stateDir, coordinatorRepairDirectory, name), value); err != nil { t.Fatal(err) } }
	for name, raw := range map[string][]byte{"artifact.json": artifactRaw, "budget.json": budgetRaw} { if err := atomicWrite(filepath.Join(e.stateDir, coordinatorRepairDirectory, name), raw, 0o600); err != nil { t.Fatal(err) } }
	code := map[common.Address][]byte{}
	for address, raw := range e.payloads.ExpectedRuntime { code[address] = bytes.Clone(raw) }
...
func TestCoordinatorRepairCarryReplaysExactFinalizedCorrectionWithoutWrites(t *testing.T) {
	...
	if err != nil || observed == nil || !reflect.DeepEqual(observed.reference, fixture.reference) || observed.deployerNonce != fixture.reference.Request.Request.Upgrade.DeployerNonce+1 { t.Fatalf("exact completed correction was not authenticated: %v", err) }
	if fixture.reader.calls.Load() == 0 || fixture.independent.calls.Load() == 0 || fixture.reader.sends.Load() != 0 || fixture.independent.sends.Load() != 0 || !reflect.DeepEqual(before, e.journal.Entries()) { t.Fatal("carry omitted independent reads or changed transaction history") }
	for _, problem := range []string{"runtime", "immutable", "activation", "custody", "receipt", "nonce", "pending"} {
		t.Run(problem, func(t *testing.T) {
			...
			if got, err := authenticateCoordinatorRepairCarry(t.Context(), e.cfg, e.stateDir, e.plan, before, e.deployer.client, e.independentEVM); err == nil || got != nil { t.Fatalf("independent %s mismatch issued authority: %v", problem, err) }
		})
	}
	fixture.independent.fault = "reorg"
	if got, err := authenticateCoordinatorRepairCarry(t.Context(), e.cfg, e.stateDir, e.plan, before, e.deployer.client, e.independentEVM); err == nil || got != nil { t.Fatal("reorg admitted") }
}
...
func TestCoordinatorRepairCarryRejectsChangedSourceAndIncompleteHistory(t *testing.T) {
	...
	e := fixture.executor
	entries := e.journal.Entries()
	if _, err := readCoordinatorRepairCarry(e.stateDir, e.plan, entries); err != nil { t.Fatal(err) }
	for _, name := range []string{"artifact.json", "budget.json", "request.json", "result.json"} {
		t.Run(name, func(t *testing.T) {
			...
			if err != nil { t.Fatal(err) }
			defer func(){ if err := os.WriteFile(path, raw, 0o600); err != nil { t.Error(err) } }()
			changed := bytes.Clone(raw)
			changed[len(changed)/2] ^= 1
			if err := os.WriteFile(path, changed, 0o600); err != nil { t.Fatal(err) }
			if got, err := readCoordinatorRepairCarry(e.stateDir, e.plan, entries); err == nil || got != nil { t.Fatal("changed retained bytes admitted") }
		})
	}
	if got, err := readCoordinatorRepairCarry(e.stateDir, e.plan, entries[:len(entries)-1]); err == nil || got != nil { t.Fatal("missing finalization admitted") }
	changed := append([]JournalEntry(nil), entries...)
	changed[len(changed)-1].BlockNumber++
	if got, err := readCoordinatorRepairCarry(e.stateDir, e.plan, changed); err == nil || got != nil { t.Fatal("changed canonical journal receipt admitted") }
	copyPlan := *e.plan
	copyPlan.PlanHash = "0x"+strings.Repeat("ab",32)
	if got, err := readCoordinatorRepairCarry(e.stateDir, &copyPlan, entries); err == nil || got != nil { t.Fatal("cross-lineage source admitted") }
}
...
func TestCoordinatorRepairCarryKeepsOriginalProbeAndNextNonceSeparate(t *testing.T) {
	...
	if err := configureCoordinatorUpgradeNonce(payloads, oldUpgrade.DeployerNonce+3); err != nil { t.Fatal(err) }
	if err := configureFleetBatcherNonce(payloads, oldUpgrade.DeployerNonce+1); err != nil { t.Fatal(err) }
	...
	if err := validateCoordinatorUpgradePayloadBaseline(baseline, retained, coordinatorRepairBaselinePayloads(payloads, carry)); err != nil { t.Fatalf("original probe/batcher proof lost: %v", err) }
	if err := validateCoordinatorUpgradeBaseline(baseline, retained, payloads.CoordinatorUpgrade); err == nil { t.Fatal("unadopted later CREATE was accepted as the original adjacent probe proof") }
	if payloads.CoordinatorUpgrade.DeployerNonce != oldUpgrade.DeployerNonce+3 { t.Fatal("historical baseline view changed current correction") }
	...
	if ok, err := coordinatorUpgradeMigrationNonceMatches(prior, migration, payloads, observation.deployerNonce, nil); err != nil || !ok { t.Fatalf("exact authenticated next nonce rejected: %v", err) }
	if ok, _ := coordinatorUpgradeMigrationNonceMatches(prior, migration, payloads, observation.deployerNonce+1, nil); ok { t.Fatal("another consumed nonce admitted") }
	other := *observation
	prior.coordinatorRepairObserved = &other
	if ok, _ := coordinatorUpgradeMigrationNonceMatches(prior, migration, payloads, observation.deployerNonce, nil); ok { t.Fatal("cross-observation migration admitted") }
}
