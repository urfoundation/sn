...
func finalV2RPCString(raw json.RawMessage) (string, error) {
	var value string
	if json.Unmarshal(raw, &value) != nil || value == "" { return "", errors.New("public V2 RPC parameter is not a string") }
	return value, nil
}
...
func finalV2RPCHash(raw json.RawMessage) (string, error) {
	...
	if !validCanonicalHashHex(value) { return "", errors.New("public V2 RPC hash is not canonical") }
	return value, nil
}
...
func finalV2EVMReadMethod(method string, params []json.RawMessage) error {
	switch method {
	case "eth_chainId":
		if len(params) == 0 { return nil }
	case "eth_getBlockByNumber", "eth_getBlockByHash":
		if len(params) == 2 && bytes.Equal(params[1], []byte("false")) {
			if method == "eth_getBlockByHash" { _, err := finalV2RPCHash(params[0]); return err }
			if string(params[0]) == `"finalized"` { return nil }
			_, err := finalV2RPCNumber(params[0]); return err
		}
	case "eth_call", "eth_getCode":
		if len(params) == 2 { _, err := finalV2EVMSelector(params[1]); return err }
	case "eth_getStorageAt":
		if len(params) == 3 { _, err := finalV2EVMSelector(params[2]); return err }
	case "eth_getLogs":
		if len(params) == 1 {
			var filter struct { BlockHash string `json:"blockHash"`; FromBlock string `json:"fromBlock"`; ToBlock string `json:"toBlock"` }
			if err := json.Unmarshal(params[0], &filter); err != nil { return err }
			if filter.BlockHash != "" && filter.FromBlock == "" && filter.ToBlock == "" && validCanonicalHashHex(filter.BlockHash) { return nil }
			from, fromErr := hexutil.DecodeUint64(filter.FromBlock)
			to, toErr := hexutil.DecodeUint64(filter.ToBlock)
			if filter.BlockHash == "" && fromErr == nil && toErr == nil && from <= to { return nil }
		}
	case "eth_getTransactionReceipt":
		if len(params) == 1 { _, err := finalV2RPCHash(params[0]); return err }
	}
...
func (self *finalV2RPCRecorder) cachedHead(chain, hash string) (ChainHead, bool) {
	...
	if chain == "evm" { heads = self.evmHeads }
	head, found := heads[hash]
	return head, found
}

func (self *finalV2RPCRecorder) rememberHead(chain string, head ChainHead) error {
	if err := verifyFinalHead("public V2 canonical", head); err != nil { return err }
	self.mu.Lock()
	defer self.mu.Unlock()
	heads, numbers := self.nativeHeads, self.nativeNumbers
	if chain == "evm" { heads, numbers = self.evmHeads, self.evmNumbers }
	if old, found := heads[head.Hash]; found && old != head { return errors.New("public V2 hash changed its block number") }
	if old, found := numbers[head.Number]; found && old != head.Hash { return errors.New("public V2 canonical block changed within source observation") }
...
func (self *finalV2RPCRecorder) evmHead(ctx context.Context, hash string, header json.RawMessage) (ChainHead, error) {
	if cached, found := self.cachedHead("evm", hash); found { return cached, nil }
	if !validCanonicalHashHex(hash) { return ChainHead{}, errors.New("public V2 EVM head hash is invalid") }
	var err error
	if len(header) == 0 {
			header, _, err = self.sourceRaw(ctx, "evm", ChainHead{}, "eth_getBlockByHash", hash, false)
		if err != nil { return ChainHead{}, err }
	}
	head, err := finalEVMBlock(header)
	if err != nil || head.Hash != hash { return ChainHead{}, errors.Join(errors.New("public V2 EVM header identity differs"), err) }
	...
	if err != nil || observed != head { return ChainHead{}, errors.Join(errors.New("public V2 EVM block is not canonical"), err) }
	if err := self.retain(exchange); err != nil { return ChainHead{}, err }
	params, _ := json.Marshal([]any{hash, false})
	if err := self.retain(FinalRPCExchange{Chain: "evm", Method: "eth_getBlockByHash", Params: params, PinnedHead: head, Result: header}); err != nil { return ChainHead{}, err }
...
func (self *finalV2RPCRecorder) nativeHead(ctx context.Context, hash string, header json.RawMessage) (ChainHead, error) {
	if cached, found := self.cachedHead("substrate", hash); found { return cached, nil }
	if !validCanonicalHashHex(hash) { return ChainHead{}, errors.New("public V2 native head hash is invalid") }
	var err error
	if len(header) == 0 {
	...
	if err != nil || included != "" && !strings.EqualFold(included, hash) { return ChainHead{}, errors.Join(errors.New("public V2 native header identity differs"), err) }
	...
	if err != nil || observed != hash { return ChainHead{}, errors.Join(errors.New("public V2 native block is not canonical"), err) }
	if err := self.retain(exchange); err != nil { return ChainHead{}, err }
	params, _ := json.Marshal([]any{hash})
	if err := self.retain(FinalRPCExchange{Chain: "substrate", Method: "chain_getHeader", Params: params, PinnedHead: head, Result: header}); err != nil { return ChainHead{}, err }
...
func (self *finalV2RPCRecorder) retainEVM(ctx context.Context, method string, params []json.RawMessage, raw []byte) error {
	...
	case "eth_chainId":
		...
		if decodeErr != nil || chainID != self.reader.evidence.ChainID { return errors.New("public V2 EVM chain identity differs") }
		head = self.reader.evidence.EVMTerminalHead
	case "eth_getBlockByNumber", "eth_getBlockByHash":
		if method == "eth_getBlockByNumber" && string(params[0]) == `"finalized"` { return nil }
		observed, decodeErr := finalEVMBlock(raw)
		if decodeErr != nil { return decodeErr }
		if method == "eth_getBlockByNumber" {
			want, decodeErr := finalV2RPCNumber(params[0])
			if decodeErr != nil || want != observed.Number { return errors.New("public V2 EVM header changed its requested number") }
		} else {
			want, decodeErr := finalV2RPCHash(params[0])
			if decodeErr != nil || want != observed.Hash { return errors.New("public V2 EVM header changed its requested hash") }
		}
		var exactHashResponse json.RawMessage
		if method == "eth_getBlockByHash" { exactHashResponse = raw }
	...
	case "eth_getLogs":
		var filter struct { BlockHash string `json:"blockHash"`; ToBlock string `json:"toBlock"` }
		if err := json.Unmarshal(params[0], &filter); err != nil { return err }
		if filter.BlockHash != "" { head, err = self.evmHead(ctx, filter.BlockHash, nil) } else {
			...
			if readErr != nil || numberErr != nil || observed.Number != want { return errors.New("public V2 log range terminal header differs") }
	...
	case "eth_getTransactionReceipt":
		var receipt struct { BlockHash string `json:"blockHash"`; BlockNumber string `json:"blockNumber"`; TransactionHash string `json:"transactionHash"` }
		if err := json.Unmarshal(raw, &receipt); err != nil { return err }
		want, decodeErr := finalV2RPCHash(params[0])
		number, numberErr := hexutil.DecodeUint64(receipt.BlockNumber)
		if decodeErr != nil || numberErr != nil || !strings.EqualFold(receipt.TransactionHash, want) { return errors.New("public V2 receipt changed its requested identity") }
		head, err = self.evmHead(ctx, strings.ToLower(receipt.BlockHash), nil)
		if err == nil && head.Number != number { return errors.New("public V2 receipt changed its canonical inclusion number") }
	default:
		return errors.New("public V2 EVM transcript received a non-read method")
	}
	if err != nil { return err }
	if params == nil { params = []json.RawMessage{} }
...
func (self *finalV2RPCRecorder) retainNative(ctx context.Context, read validatorpkg.ReleaseEvidenceV2NativeRead) (resultErr error) {
	...
	if read.Schema != "urnetwork-validator-native-read-v2" || json.Unmarshal(read.Parameters, &params) != nil { return errors.New("public V2 native read wire is invalid") }
	var hash string
	var head ChainHead
	...
	case "chain_getFinalizedHead":
		if len(params) != 0 { return errors.New("public V2 finalized native query has arguments") }
		hash, err = finalV2RPCHash(read.Result)
		if err != nil { return err }
		self.mu.Lock(); self.liveNativeHeads[hash] = true; self.mu.Unlock()
		return nil
	case "chain_getBlockHash":
		if len(params) != 1 { return errors.New("public V2 native canonical hash query is not height pinned") }
		number, decodeErr := finalV2RPCNumber(params[0])
		if decodeErr != nil { return decodeErr }
		hash, err = finalV2RPCHash(read.Result)
		if err != nil { return err }
		if number == 0 {
			if !strings.EqualFold(hash, self.reader.evidence.GenesisHash) { return errors.New("public V2 native genesis differs") }
			head = self.reader.evidence.NativeTerminalHead
		} else {
			head, err = self.nativeHead(ctx, hash, nil)
			if err == nil && head.Number != number { return errors.New("public V2 native canonical query changed its requested number") }
		}
	case "chain_getHeader", "chain_getBlock", "state_getRuntimeVersion", "state_getMetadata":
		if len(params) != 1 { return errors.New("public V2 native historical read is not hash pinned") }
		hash, err = finalV2RPCHash(params[0])
		if err != nil { return err }
		if read.Method == "chain_getHeader" {
			self.mu.Lock(); live := self.liveNativeHeads[hash]; self.mu.Unlock()
			if live { return nil }
			header = read.Result
		}
	...
	case "state_getStorage", "state_getStorageHash", "state_call":
		want := 2
		if read.Method == "state_call" { want = 3 }
		if len(params) != want { return errors.New("public V2 native state read is not hash pinned") }
