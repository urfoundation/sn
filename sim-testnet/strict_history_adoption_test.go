...
func TestStrictHistoryAdoptionOptionsKeepWritesAndProvisionalSeparate(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		if _, _, err := parseCLI(args); err == nil {
			t.Fatalf("incomplete or competing adoption admitted: %v", args)
		}
	}
}
...
func TestStrictHistoryAdoptionCapturesOriginalSourceAndBindsExactLaunch(t *testing.T) {
	...
	if provisionalResumeEnabled(fixture.cfg) || !reflect.DeepEqual(before, validatorNamespaceTreeSnapshot(t, fixture.stateDir)) {
		t.Fatal("strict adoption enabled provisional mode or migrated the retained namespace")
	}
	...
	for id := 1; id <= 2; id++ {
		...
		if err := atomicWrite(configPath, configBytes, 0o600); err != nil {
			t.Fatal(err)
		}
		specs = append(specs, ProcessSpec{ID: fmt.Sprintf("validator-%d", id), Role: "validator", Args: []string{"__validator", "--config=" + configPath}})
	}
	if err := attachStrictHistoryAdoption(fixture.cfg, fixture.stateDir, revised, specs); err != nil {
		t.Fatal(err)
	}
	for index, spec := range specs {
		if len(spec.Args) != 4 || !strings.HasPrefix(spec.Args[2], "--strict-history-adoption=") || spec.Args[3] != "--strict-history-adoption-sha256="+bytesSHA256(fixture.cfg.strictHistoryAdoption.requests[index]) {
			t.Fatal("validator argv lost its exact request owner")
		}
		child, err := os.ReadFile(strings.TrimPrefix(spec.Args[2], "--strict-history-adoption="))
		if err != nil || !bytes.Equal(child, fixture.cfg.strictHistoryAdoption.requests[index]) {
			t.Fatalf("child handoff bytes changed: %v", err)
		}
	}
	before = validatorNamespaceTreeSnapshot(t, fixture.stateDir)
	...
	if err := prepareSignedAttemptStateNamespaces(fixture.cfg, fixture.stateDir); err == nil {
		t.Fatal("changed request reached namespace preparation")
	}
	if err := atomicWrite(path, raw, 0o600); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(before, validatorNamespaceTreeSnapshot(t, fixture.stateDir)) {
		t.Fatal("rejected request changed retained coordinator, inputs or signed history")
	}
	var unknown map[string]any
	if err := json.Unmarshal(bundle.Validators[0], &unknown); err != nil {
		t.Fatal(err)
	}
	...
	bad = append(bad, '\n')
	if err := atomicWrite(path, bad, 0o600); err != nil {
		t.Fatal(err)
	}
