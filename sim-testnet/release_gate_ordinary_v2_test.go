...
func TestProducerGateStateSelectionOrdinaryRuntimeV2RetainsOwnerAndReplay(t *testing.T) {
	for _, edge := range []struct{ path, caller, callee string }{
		...
		if !releaseClosureFunctionCalls(t, edge.path, edge.caller)[edge.callee] {
			t.Errorf("%s lost ordinary runtime edge %s", edge.caller, edge.callee)
		}
	}
	for _, caller := range []string{"detachReleaseStatsMeasurementV2Owned", "reconcileReleaseStatsCutV2Owned"} {
		...
		if calls["BuildCut"] || calls["VerifyReleaseStatsMeasurement"] {
			t.Errorf("compact runtime %s acquired a legacy replay bypass", caller)
		}
	}
}
