...
func TestRuntime459ConfigHashPreserves455And458Domains(t *testing.T) {
	...
	for _, spec := range []uint32{458, 459} {
	...
	cfg := &ResolvedConfig{Public: &public, Release: runtime459ReviewedTestLock()}
	if err := validateRuntimeConfigIdentityPlan(cfg, plan); err != nil { t.Fatal(err) }
	public.Chain.ExpectedRuntimeSpec = 458
	if err := validateRuntimeConfigIdentityPlan(cfg, plan); err == nil { t.Fatal("historical458 expectation gained current459 plan authority") }
	cfg.Release = runtime458ReviewedTestLock()
	if err := validateRuntimeConfigIdentityPlan(cfg, plan); err == nil { t.Fatal("historical458 lock gained current plan authority") }
	for _, spec := range []uint32{456, 457, 460} {
		public.Chain.ExpectedRuntimeSpec = spec
		if _, err := releaseConfigHash(config, &public, hyper); err == nil { t.Fatalf("unreviewed%d acquired the original hash", spec) }
	}
}
...
func TestRuntime459RestartsOriginal458CompanionWithoutRewritingApproval(t *testing.T) {
	...
	if err != nil || restarted == nil { t.Fatalf("current459 cannot reopen original458 companion: %v", err) }
	...
	for _, actionId := range []string{validatorEvidenceDeployActionID, validatorEvidenceAnchorActionID} {
		if !finalJSONEqual(actionByID(t, restarted, actionId), actionByID(t, fixture.original, actionId)) { t.Fatalf("restart changed original %s intent or spend", actionId) }
	}
	if restarted.MaximumSpend != fixture.current.MaximumSpend || restarted.Limits != fixture.current.Limits { t.Fatal("restart changed approved budgets") }
	for _, item := range []struct {
		...
		if err != nil || plan == nil || !finalJSONEqual(plan.ValidatorEvidenceSource, fixture.original.ValidatorEvidenceSource) { t.Fatalf("%s lost original458 source: %v", item.name, err) }
		foreign := *fixture.current
		foreign.PriorPlanHashes = nil
		if plan, err := item.load(fixture.stateDir, &foreign, fixture.original.PlanHash); plan != nil || err == nil { t.Fatalf("%s admitted an unapproved ancestor", item.name) }
	}
	archived, err := os.ReadFile(filepath.Join(fixture.stateDir, "plans", stringsTrim0x(fixture.original.PlanHash)+".json"))
	if err != nil || !bytes.Equal(archived, fixture.originalWire) { t.Fatal("restart rewrote the original approval") }
	if err := atomicWrite(filepath.Join(fixture.stateDir, "plan.json"), fixture.originalWire, 0o600); err != nil { t.Fatal(err) }
	if plan, err := loadPersistedPlan(fixture.config, fixture.stateDir); plan != nil || !errors.Is(err, errPersistedPlanIdentityMismatch) { t.Fatalf("original458 plan became current authority: %v", err) }
}
...
func TestRuntime459Archived458LockRejectsCrossArtifactAndProvenanceDrift(t *testing.T) {
	...
	if err := validateReviewedRuntimeIdentity(lock); err == nil { t.Fatal("historical458 lock became current authority") }
	for _, change := range []func(*ReleaseRuntimeLock){
		func(runtime *ReleaseRuntimeLock) { runtime.SourceRefName = runtime459ReviewedTestLock().Runtime.SourceRefName },
		func(runtime *ReleaseRuntimeLock) { runtime.SourceCommit = runtime459ReviewedTestLock().Runtime.SourceCommit },
		func(runtime *ReleaseRuntimeLock) { runtime.CodeHash = runtime459ReviewedTestLock().Runtime.CodeHash },
		func(runtime *ReleaseRuntimeLock) { runtime.MetadataHash = runtime459ReviewedTestLock().Runtime.MetadataHash },
		func(runtime *ReleaseRuntimeLock) { runtime.CompressedWasmSHA256 = runtime459ReviewedTestLock().Runtime.CompressedWasmSHA256 },
		...
		if err := validateValidatorEvidenceHistoricalReleaseLock(&changed); err == nil { t.Fatal("mixed458 provenance acquired historical authority") }
	}
}
