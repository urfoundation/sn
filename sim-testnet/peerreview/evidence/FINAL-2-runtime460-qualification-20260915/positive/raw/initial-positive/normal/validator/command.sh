...
set -u -o pipefail
umask 077
capture=/mnt/data/sn-testnet/qualification/runtime460-affected-matrix-20260915-r1/terra/normal/validator
source=/mnt/data/sn-testnet/qualification/runtime460-20260915-r1/workspace/sn
compiler=/mnt/data/sn-testnet/qualification/runtime460-affected-matrix-20260915-r1/terra/compile/validator-normal
binary="/mnt/data/sn-testnet/qualification/runtime460-affected-matrix-20260915-r1/terra/compile/validator-normal/validator.normal.test"
selector=/mnt/data/sn-testnet/qualification/runtime460-affected-matrix-20260915-r1/terra/selectors/validator.selector
expected_tests=/mnt/data/sn-testnet/qualification/runtime460-affected-matrix-20260915-r1/terra/selectors/validator.expected-tests.txt
...
printf '%s\n' 'env GOMAXPROCS=4 GOCACHE=/mnt/data/sn-testnet/gocache TMPDIR=private GOTMPDIR=private timeout --foreground --kill-after=15s 600s go tool test2json -t validator.normal.test -test.v -test.run=exact-union-selector -test.count=1 -test.timeout=10m -test.parallel=4' >"$capture/command.txt"
...
if [ "$(cat "$capture/source.commit.before")" = "$expected" ] && [ ! -s "$capture/source.status.before" ] && [ "$expected_binary" = "$actual_binary" ] && [ -d "$source/validator" ]; then
