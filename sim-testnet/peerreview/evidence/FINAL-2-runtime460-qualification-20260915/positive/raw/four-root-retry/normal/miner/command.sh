...
set -u -o pipefail
umask 077
capture=/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/normal/miner
source=/mnt/data/sn-testnet/qualification/runtime460-20260915-r1/metadata-constant-repair/workspace/sn
compiler=/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/compile/miner-normal
binary="/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/compile/miner-normal/miner.normal.test"
selector=/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/selectors/miner.selector
expected_tests=/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/selectors/miner.expected-tests.txt
expected=7989fa78faae81b82138c79e43dded16aad449dc
status=125
...
printf '%s\n' 'env GOMAXPROCS=4 GOCACHE=/mnt/data/sn-testnet/gocache TMPDIR=private GOTMPDIR=private timeout --foreground --kill-after=15s 600s go tool test2json -t miner.normal.test -test.v -test.run=exact-union-selector -test.count=1 -test.timeout=10m -test.parallel=4' >"$capture/command.txt"
...
if [ "$(cat "$capture/source.commit.before")" = "$expected" ] && [ ! -s "$capture/source.status.before" ] && [ "$expected_binary" = "$actual_binary" ] && [ -d "$source/miner" ]; then
