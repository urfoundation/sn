...
set -u -o pipefail
umask 077
capture=/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/normal/crv4
source=/mnt/data/sn-testnet/qualification/runtime460-20260915-r1/metadata-constant-repair/workspace/sn
compiler=/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/compile/crv4-normal
binary="/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/compile/crv4-normal/crv4.normal.test"
selector=/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/selectors/crv4.selector
expected_tests=/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/selectors/crv4.expected-tests.txt
...
printf '%s\n' 'env GOMAXPROCS=4 GOCACHE=/mnt/data/sn-testnet/gocache TMPDIR=private GOTMPDIR=private timeout --foreground --kill-after=15s 600s go tool test2json -t crv4.normal.test -test.v -test.run=exact-union-selector -test.count=1 -test.timeout=10m -test.parallel=4' >"$capture/command.txt"
...
if [ "$(cat "$capture/source.commit.before")" = "$expected" ] && [ ! -s "$capture/source.status.before" ] && [ "$expected_binary" = "$actual_binary" ] && [ -d "$source/crv4" ]; then
