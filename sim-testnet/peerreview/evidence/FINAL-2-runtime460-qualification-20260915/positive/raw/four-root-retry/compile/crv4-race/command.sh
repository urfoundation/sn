...
set -u -o pipefail
umask 077
capture=/mnt/data/sn-testnet/qualification/runtime460-failed-root-retry-20260915-r1/terra/compile/crv4-race
...
printf '%s\n' 'env GOMAXPROCS=4 GOFLAGS=-mod=readonly GOPROXY=off GOSUMDB=off GOWORK=off GOENV=off GOTOOLCHAIN=local GOVCS=*:off GOMODCACHE=/home/by/go/pkg/mod GOCACHE=/mnt/data/sn-testnet/gocache TMPDIR=private GOTMPDIR=private timeout --foreground --kill-after=15s 600s go test -c -race -p=1 -o crv4.race.test ./crv4' >"$capture/command.txt"
