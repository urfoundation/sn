#!/usr/bin/env bash
set -u -o pipefail
# Prepared release-lock-preview capture from the retained strict-resume owner.
# Requires a concrete REQUEST.json and late operands; run in the clean isolated source before publication.
umask 077
native_capture='/mnt/data/sn-testnet/qualification/pf01-pf02-release-lock-preview-20260916-r1'
...
env GOMAXPROCS=4 'GOFLAGS=-mod=readonly -p=1' GOENV=off GOWORK=off GOPROXY=off GOSUMDB=off GOTOOLCHAIN=local 'GOVCS=*:off' GOMODCACHE=/home/by/go/pkg/mod GOCACHE=/home/by/urnetwork/temp/sn-carried-fleet-qualification-20260913/runtime/gocache GOTMPDIR=/mnt/data/sn-testnet/qualification/pf01-pf02-release-lock-preview-20260916-r1/gotmp TMPDIR=/mnt/data/sn-testnet/qualification/pf01-pf02-release-lock-preview-20260916-r1/tmp "$native_cli" release-lock --config "$native_source/sim-testnet/testnet.yml" --sn-repo "$native_source" --server-repo /home/by/urnetwork/temp/sn-warp-admission-integration-20260913/workspace-physical/server --operator-proxy-repo /home/by/urnetwork/temp/sn-warp-admission-integration-20260913/workspace-physical/operator-proxy --vault-repo "$native_vault" --platform-config-repo "$native_config" > "$native_capture/stdout.yml" 2> "$native_capture/stderr"
native_exit=$?
