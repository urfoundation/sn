...
func TestRetainedChainCensus(t *testing.T) {
	manifestPath := os.Getenv("RETAINED_CHAIN_CENSUS_MANIFEST")
	if manifestPath == "" {
	...
	runErr := func() error {
		...
		data = append(data, '\n')
		if err := os.WriteFile(filepath.Join(root, "replayed-census.json"), data, 0o600); err != nil {
			return err
		}
	...
	if err := os.WriteFile(filepath.Join(root, "census-replay-receipt.json"), append(data, '\n'), 0o600); err != nil {
		t.Fatal(err)
	}
	if runErr != nil {
		t.Fatal(runErr)
