...
func TestCoordinatorRepairCarryRuntimeIdentityRequiresExplicitReviewedPin(t *testing.T) {
	...
	for _, version := range []uint32{451, 454, 458, 459, 460} {
	...
	for _, item := range []struct {
		...
		{name: "future", change: func(public *PublicManifest) { public.Chain.ExpectedRuntimeSpec = 461 }},
...
func TestCoordinatorRepairCarryRuntimeIdentityPreservesHashAndInputOwnership(t *testing.T) {
	...
	if err != nil || !bytes.Equal(before, after) || current.Public.Chain.ExpectedRuntimeSpec != 460 || !bytes.Contains(after, []byte(`"ConfigIdentityRuntimeSpec":455`)) {
...
// when a caller recomputes the outer plan hash. Runtime authority stays460.
...
// source and current460 configuration. No signed source bytes are rewritten.
func TestCoordinatorRepairCarryRuntimeIdentityAuthenticatesOriginalRepair(t *testing.T) {
	...
	public.Chain.ExpectedRuntimeSpec, public.Chain.ConfigIdentityRuntimeSpec = 460, 455
	current.Public, current.Release = &public, testReleaseLockFixture(t)
