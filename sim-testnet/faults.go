...
type FaultProcessEvidence struct {
	...
	PID      int    `json:"pid"`
	// A numeric pid alone cannot authorize escalation after a delayed exit.
	// Older evidence remains readable but grants no forced-restart authority.
...
func advanceFaultsWithConditions(ctx context.Context, head ChainHead, specs []scenarioFaultSpec, records []ScenarioFaultRecord, driver scenarioFaultDriver, activateReady, restoreReady func(scenarioFaultSpec) (bool, error)) error {
	for i := range records {
		...
		case "active":
			...
			if err != nil {
				if processRestartPending(specs[i], err) || faultCompletionPending(specs[i], "enable", err) {
