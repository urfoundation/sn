...
func newFinalNativeRewardHistoryFixtureV2(t *testing.T) *finalNativeRewardHistoryFixtureV2 {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "chain_getBlockHash":
			...
			if mode == 1 && number == 100 { result = common.Hash{0x34}.Hex() }
		case "chain_getFinalizedHead": result = finalized
		case "chain_getHeader":
			number := uint64(100)
			if lastHash == finalized { number = 110; if mode == 2 { number = 99 } } else if lastHash != f.head.Hash { t.Errorf("unexpected historical header %s", lastHash); return }
		...
		case "state_getRuntimeVersion": result = f.runtime.Version
		case "state_getStorageHash": result = f.runtime.CodeHash; if mode == 3 { result = common.Hash{0x36}.Hex() }
		...
		case "state_getStorage":
			...
			value, ok := values[key]
			if !ok || lastHash != f.head.Hash { t.Errorf("unbound historical storage %s at %s", key, lastHash); return }
			result = value
			if mode == 4 && key == emissionKey { result = "0x00" }
			if mode == 7 && key == countKey { result = "0x0000" }
		case "state_queryStorageAt":
			f.queries.Add(1)
			if mode == 8 { close(f.started); <-request.Context().Done(); close(f.joined); return }
			var keys []string; if err := json.Unmarshal(call.Params[0], &keys); err != nil { t.Error(err); return }
			if lastHash != f.head.Hash { t.Error("UID batch lost its exact historical head"); return }
			changes := make([][2]any, 0, len(keys))
			for _, key := range keys { value, ok := values[key]; if !ok { t.Errorf("unexpected UID storage %s", key); return }; changes = append(changes, [2]any{key, value}) }
			block := f.head.Hash
			if mode == 5 { block = common.Hash{0x37}.Hex() }
			if mode == 6 { changes = changes[:len(changes)-1] }
			result = []any{map[string]any{"block": block, "changes": changes}}
		default: t.Errorf("unexpected historical native method %s", call.Method); return
		}
		if call.Method == "state_getRuntimeVersion" || call.Method == "state_getStorageHash" || call.Method == "state_getMetadata" {
			if lastHash != f.head.Hash { t.Error("historical native runtime read used unpinned/latest metadata"); return }
		}
		raw, err := json.Marshal(result); if err != nil { t.Error(err); return }
...
func TestFinalNativeRewardV2ReadsExactHistoricalVectorsAndUIDStake(t *testing.T) {
	...
	for _, mode := range []uint32{1, 2, 3, 4, 5, 6, 7} {
		f.mode.Store(mode)
		if result, err := readFinalNativeRewardAtV2(t.Context(), f.native, f.head, 521, f.runtime); err == nil || result != nil { t.Fatalf("fault %d returned a historical reward observation: %+v %v", mode, result, err) }
	}
	f.mode.Store(0)
	got.TotalHotkeyAlphaRao[0] = "999"
	if fresh, err := readFinalNativeRewardAtV2(t.Context(), f.native, f.head, 521, f.runtime); err != nil || !reflect.DeepEqual(fresh, want) { t.Fatalf("historical read reused another caller's mutable result: %+v %v", fresh, err) }
}
...
func TestFinalNativeRewardV2CancellationJoinsHistoricalBatch(t *testing.T) {
	...
	go func() { result, err := readFinalNativeRewardAtV2(ctx, f.native, f.head, 521, f.runtime); if result != nil { err = errors.New("canceled read returned a partial reward") }; done <- err }()
	select { case <-f.started: case <-time.After(10*time.Second): t.Fatal("historical UID batch did not start") }
	cancel()
	select { case err := <-done: if !errors.Is(err, context.Canceled) { t.Fatalf("historical batch cancellation differs: %v", err) }; case <-time.After(10*time.Second): t.Fatal("historical reader failed to join canceled batch") }
	select { case <-f.joined: case <-time.After(10*time.Second): t.Fatal("historical HTTP batch remained live after cancellation") }
}
