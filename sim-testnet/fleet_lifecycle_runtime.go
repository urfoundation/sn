...
func (self *liveFleetLifecycle) BeginPhase(phase, runID string) error {
	...
	if targetEffective == 0 || targetEffective != companionEffective {
		return errors.New("fleet lifecycle approved takeovers do not share one effective epoch")
	}
	for _, expected := range []struct {
		...
		for member := 1; member <= self.cfg.Config.Topology.ClientsPerHeadFleet; member++ {
			binding, readErr := loadFleetLifecycleBindingEvidenceForPlan(self.executor.plan, self.stateDir, expected.variant, member)
	...
	self.evidence = &FleetLifecycleEvidence{
		...
		TakeoverEffectiveEpoch: targetEffective, LaunchPrune: &launch, Renewal: cloneFleetLifecycleRenewal(self.executor.plan.FleetLifecycleRenewal),
	}
	self.phase = phase
	self.resumeValidated = false
	return self.write()
...
func (self *liveFleetLifecycle) installedEffectiveEpoch(variantName string) (uint64, error) {
	var result uint64
	for member := 1; member <= self.cfg.Config.Topology.ClientsPerHeadFleet; member++ {
		evidence, err := loadFleetLifecycleBindingEvidenceForPlan(self.executor.plan, self.stateDir, variantName, member)
		if err != nil {
			return 0, err
		}
		if result == 0 {
