...
func collectHistoricalFleetCheckpoints(ctx context.Context, client *ethclient.Client, calls []historicalFleetGenerationOneCall, independent bool) []error {
	...
	var headErr error
	if client == nil {
		headErr = errors.New("historical fleet EVM reader is unavailable")
	} else {
		finalized, headErr = finalizedEVMHead(ctx, client)
	}
	if headErr != nil {
		for index := range calls {
			failures[index] = fmt.Errorf("checkpoint blocked by finalized EVM head: %w", headErr)
		}
		return failures
	}
	...
	for index, call := range calls {
		if call.record == nil || call.observe == nil || (len(call.data) != 0 && call.address == (common.Address{})) {
			failures[index] = errors.New("historical fleet call evidence is incomplete")
			continue
		}
		...
		if checkpoint.Number == 0 || checkpoint.Hash == "" {
			failures[index] = errors.New("historical EVM checkpoint is incomplete")
			continue
		}
		checkpoints[index] = checkpoint
		if _, exists := positions[checkpoint.Number]; !exists {
			positions[checkpoint.Number] = len(numbers)
			numbers = append(numbers, checkpoint.Number)
		}
	}
	...
	for index, checkpoint := range checkpoints {
		...
		if headerErrors[position] != nil {
			failures[index] = fmt.Errorf("historical EVM block %d: %w", checkpoint.Number, headerErrors[position])
			continue
		}
		ready, err := checkpointVisibility(checkpoint, finalized, heads[position].Hash)
		if err != nil {
			failures[index] = fmt.Errorf("historical EVM block %d: %w", checkpoint.Number, err)
		} else if !ready {
			failures[index] = fmt.Errorf("historical EVM block %d is not finalized (head %d)", checkpoint.Number, finalized.Number)
		}
	}
	return failures
}
