...
func finalHistoricalArtifactPlanTest(t *testing.T, plan *SetupPlan) {
	...
	if err := rebindValidatorEvidencePlan(plan); err != nil { t.Fatal(err) }
	plan.PlanHash, err = plan.hash()
	if err != nil { t.Fatal(err) }
	if err := validatePlanBudget(plan); err != nil { t.Fatalf("original artifact approval prerequisite: %v", err) }
}
...
func TestFinalSemanticHistoricalRuntimeNativeArchiveRetainsOriginalArtifactAuthority(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if _, err := decodePersistedPlanBytes(wire); err == nil || !strings.Contains(err.Error(), "fresh validator evidence source differs") { t.Fatalf("original artifact became a current source: %v", err) }
	...
	if err != nil || opened.PlanHash != original.PlanHash || current.PlanHash != original.PlanHash || !finalJSONEqual(opened.ValidatorEvidenceSource, original.ValidatorEvidenceSource) { t.Fatalf("closed original native archive: %v", err) }
	successor := *original
	...
	opened, current, err = archive.nativeActionPlan(original.PlanHash, action.ID, action.IntentHash)
	if err != nil || opened.PlanHash != original.PlanHash || current.PlanHash != successor.PlanHash { t.Fatalf("closed original ancestor authority: %v", err) }
	archive.files[priorPath] = successorBytes
	if _, _, err := archive.nativeActionPlan(original.PlanHash, action.ID, action.IntentHash); err == nil { t.Fatal("closed archive accepted a substituted ancestor path") }
	archive.files[priorPath] = wire
	if _, _, err := archive.nativeActionPlan("0x" + strings.Repeat("14", 32), action.ID, action.IntentHash); err == nil { t.Fatal("closed archive accepted an unapproved source") }
	changed := bytes.Clone(wire)
	changed[0] = '!'
	archive.files["launch-foundation/plan.json"] = changed
	if _, _, err := archive.nativeActionPlan(original.PlanHash, action.ID, action.IntentHash); err == nil { t.Fatal("warm archived plan cache ignored changed bytes") }
}
...
func TestFinalSemanticHistoricalRuntimeLockRetainsExactOriginalProvenance(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if _, err := canonicalReleaseLockBytes(lock); err == nil { t.Fatal("current lock writer admitted original455") }
	...
	if err != nil || !finalJSONEqual(opened, lock) { t.Fatalf("closed original455 release lock: %v", err) }
	if _, err := finalReleaseRuntimeRootsForPlan(fixture.original, opened); err != nil { t.Fatalf("original runtime root census: %v", err) }
	for _, test := range []struct { name string; mutate func(*ReleaseRuntimeLock) }{
		...
		if err != nil { t.Fatal(err) }
		if _, err := verifyFinalReleaseLockArtifact(evidence, fixture.original, encoded); err == nil { t.Fatalf("changed original %s lock passed", test.name) }
	}
	other := *fixture.original
	other.ReleaseLockHash = "0x" + strings.Repeat("24", 32)
	if _, err := verifyFinalReleaseLockArtifact(evidence, &other, wire); err == nil { t.Fatal("original lock escaped canonical approval hash") }
}
...
func TestFinalSemanticHistoricalRuntimeRenewalReconstructsOriginalApproval(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if _, err := appendFleetRenewalPlan(base, fixture.renewal); err == nil || !strings.Contains(err.Error(), "fresh validator evidence source differs") { t.Fatalf("historical artifact became live renewal authority: %v", err) }
	approved, err := appendFleetRenewalPlanForHistory(base, fixture.renewal, true)
	if err != nil { t.Fatalf("reconstruct original renewal approval: %v", err) }
	...
	if err != nil || retained.PlanHash != approved.PlanHash || round.Approval.ContentHash != bytesSHA256(approvedBytes) { t.Fatalf("archived original renewal source: %v", err) }
	retained, err = finalFleetLifecycleRenewalApproval(approved, archive.files)
	if err != nil || retained == nil || retained.PlanHash != approved.PlanHash { t.Fatalf("archived original lifecycle renewal: %v", err) }
	changed := cloneFleetRenewalForTest(t, fixture.renewal)
	changed.MaximumFeePerGasWei++
	if _, _, err := source.renewalApproval(changed); err == nil { t.Fatal("historical renewal accepted changed fee authority") }
	if after, err := json.Marshal(base); err != nil || !bytes.Equal(baseBytes, after) { t.Fatalf("historical reconstruction changed original bytes: %v", err) }
}
