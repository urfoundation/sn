...
func newEvidenceRelayRefreshTest(t *testing.T) (*runtimeEvidenceProvisionV2TestFixture, *Executor) {
	...
	for _, request := range requests {
		...
		for index, previous := range current.Retained {
			...
			if priorSlot == slot {
				current.Retained[index] = request
			}
		}
	}
...
func evidenceRelayRefreshRequestTest(t *testing.T, executor *Executor) EvidenceRelayContinuation {
	...
	if err := current.validateClocks(work); err != nil {
		t.Fatal(err)
	}
	return current
}
...
func TestEvidenceRelayContinuationRefreshPreservesEveryApprovalAndDebit(t *testing.T) {
	...
	if err := executor.journal.Append(JournalEntry{DeploymentID: first.DeploymentID, PlanHash: first.PlanHash, ActionID: ordinary.ID, IntentHash: ordinary.IntentHash, Stage: StageFinalized, TransactionHash: common.Hash{0x81}.Hex(), BlockNumber: 100, BlockHash: common.Hash{0x82}.Hex()}); err != nil {
		t.Fatal(err)
	}
	...
	if err != nil || owner != first.PlanHash || action.Spend.EVMGasWei != "25000000000000000" {
		t.Fatal("initial continuation admission changed", err)
	}
	...
	for generation := 0; generation < 3; generation++ {
		...
		if current.NewSlots != 1019 || current.HistoricalLiabilityWei != "125000000000000000" || len(current.Debits) != 2 || !reflect.DeepEqual(previous.Actions, refreshed.Actions) || !refreshed.allowedPlanHashes()[first.PlanHash] || !refreshed.allowedPlanHashes()[current.ActivationPlanHash] {
			t.Fatal("refresh reset liability, actions, or ancestry")
		}
		if err := writeRunInputs(fixture.cfg, fixture.stateDir, refreshed, fixture.roles); err != nil {
			t.Fatal(err)
		}
		executor.plan = refreshed
		if err := validateEvidenceRelayContinuationSource(fixture.stateDir, refreshed, executor.journal.Entries()); err != nil {
			t.Fatalf("generation %d lost its authenticated predecessor: %v", generation, err)
		}
		retained, retainedOwner, err := executor.admitOwnedEvidenceRelayAction(t.Context(), next)
		if err != nil || retainedOwner != first.PlanHash || retained.IntentHash != action.IntentHash {
			t.Fatal("refresh reassigned an existing 25 gwei retry", err)
		}
		for _, debit := range current.Debits {
			...
			if err != nil || originalOwner != debit.PlanHash || retained.Spend.EVMGasWei != debit.AllowanceWei {
				t.Fatal("refresh rewrote an admitted signed liability", err)
			}
		}
	}
	retainedJournal, err := os.ReadFile(journalPath)
	if err != nil || !bytes.Equal(journalBytes, retainedJournal) {
		t.Fatal("refresh changed finalized history or re-debited a retry", err)
	}
	...
	if err != nil || !bytes.Equal(firstBytes, retainedBytes) {
		t.Fatal("refresh replaced the original approval or its fixed end", err)
	}
	...
	if err != nil || newOwner != executor.plan.PlanHash || newAction.Spend.EVMGasWei != "25000000000000000" {
		t.Fatal("refreshed approval failed to own a bounded new subject", err)
	}
	remaining := evidenceRelayRefreshRequestTest(t, executor)
	if remaining.NewSlots != 1018 || remaining.HistoricalLiabilityWei != "150000000000000000" {
		t.Fatal("new subject was not charged cumulatively")
	}
}
...
func TestEvidenceRelayContinuationRefreshRejectsOmittedCheckpointDebit(t *testing.T) {
	...
	if err := writeRunInputs(fixture.cfg, fixture.stateDir, valid, fixture.roles); err != nil {
		t.Fatal(err)
	}
	if err := validateEvidenceRelayContinuationSource(fixture.stateDir, valid, executor.journal.Entries()); err != nil {
		t.Fatal(err)
	}
	for index, debit := range current.Debits {
		if debit.ActionID == admitted.ID {
			current.Debits = append(current.Debits[:index], current.Debits[index+1:]...)
			break
		}
	}
	current.NewSlots, current.HistoricalLiabilityWei, err = current.remainingSlots()
	...
	if err != nil {
		t.Fatal("control must remain arithmetically valid before journal authentication", err)
	}
	if err := validateEvidenceRelayContinuationSource(fixture.stateDir, omitted, executor.journal.Entries()); err == nil || !strings.Contains(err.Error(), "omitted an admitted checkpoint liability") {
		t.Fatal("refresh refunded a post-predecessor journal admission", err)
	}
}
...
func TestEvidenceRelayContinuationRefreshRejectsChangedAuthorityAndHistory(t *testing.T) {
	...
	for _, sample := range []struct {
		name   string
		mutate func(*EvidenceRelayContinuation)
	}{
		...
		if _, err := appendEvidenceRelayContinuationPlan(executor.plan, current); err == nil {
			t.Fatalf("refresh accepted changed %s", sample.name)
		}
	}
	current := evidenceRelayRefreshRequestTest(t, executor)
	base := *executor.plan
	...
	if err := validateEvidenceRelayContinuationRefresh(&base, &current); err == nil {
		t.Fatal("refresh reinterpreted a 50 gwei approval")
	}
}
...
func TestEvidenceRelayContinuationRefreshCountsContinuedDebitCensus(t *testing.T) {
	...
	if err != nil || slots != 767 || liability != "6425000000000000000" {
		t.Fatal("refresh lost cumulative continued slots", slots, liability, err)
	}
	current.Schema = evidenceRelayContinuationSchema
	if _, _, err := current.remainingSlots(); err == nil {
		t.Fatal("legacy first continuation gained a larger original debit census")
	}
}
...
func TestEvidenceRelayContinuationRefreshRunwayRetainsCompleteWork(t *testing.T) {
	...
	if err := validateEvidenceRelayContinuationRunway(&current, cutoff); err != nil {
		t.Fatal(err)
	}
	cfg := runtimeEvidenceLaunchConfigTest(t)
	bounds := cfg.Config.ValidatorEvidenceV2[0].Evidence.Bounds
	if err := validateEvidenceRelayContinuationCapacity(cfg, bounds, current.EndBlock-cutoff-1, current.Sources[0].Capacity); err != nil {
		t.Fatal("control capacity must still admit the shortened runtime", err)
	}
	for _, block := range []uint64{cutoff + 1, current.EndBlock, current.EndBlock + 1, current.EVMHead.Number - 1} {
		if err := validateEvidenceRelayContinuationRunway(&current, block); err == nil {
			t.Fatalf("startup admitted incomplete full work at %d", block)
		}
	}
}
