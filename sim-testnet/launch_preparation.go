...
func validateLaunchPreparationOptions(command string, options cliOptions) error {
	if !options.PrepareOnly { return nil }
	if command != "setup" && command != "launch" && command != "resume" { return errors.New("--prepare-only requires setup, launch or resume") }
	if !options.Apply || !validCanonicalHashHex(options.PlanHash) { return errors.New("--prepare-only requires --apply and the exact approved --plan-hash for reversible host preparation") }
	return nil
}
...
func (self *launchPreparationReport) add(name string, err error) bool {
	...
	if err != nil { check.Detail = err.Error(); self.Ready = false }
...
func (self *launchPreparationReport) Error() error {
	...
	for _, check := range self.Checks {
		if check.Hard && !check.OK { failures = append(failures, fmt.Errorf("%s: %s", check.Name, check.Detail)) }
	}
...
func collectLaunchRuntimePreparation(report *launchPreparationReport, command string, executor *Executor) {
	if report == nil || executor == nil || executor.plan == nil { return }
	if command != "launch" && command != "resume" {
		action, err := executor.planAction("config.render")
		if err != nil { report.Checks = append(report.Checks, Check{Name: "launch-runtime-inputs", Hard: false, Detail: "deferred to launch/resume: config.render is absent"}); return }
		if _, verified := executor.verifiedActionEntry(action); verified { report.Checks = append(report.Checks, Check{Name: "launch-runtime-inputs", Hard: false, Detail: "deferred to launch/resume: config.render is already verified; future namespace/adoption authority does not block approved setup repair"}); return }
		var pending []string
		for _, dependency := range action.DependsOn { if !executor.actionVerified(dependency) { pending = append(pending, dependency) } }
		if len(pending) != 0 { report.Checks = append(report.Checks, Check{Name: "launch-runtime-inputs", Hard: false, Detail: "blocked until approved setup prerequisites complete: "+strings.Join(pending, ", ")}); return }
	}
	...
	if !report.add("runtime-deployment-inputs", err) { report.blocked("reserved-attempt-uploads", "runtime-deployment-inputs"); return }
...
func finishLaunchPreparation(report *launchPreparationReport, publish func(*launchPreparationReport, error) error, apply func() error) error {
	if report == nil || publish == nil || apply == nil { return errors.New("launch preparation gate is incomplete") }
	err := report.Error()
...
func collectOrderedReadOnlyAudits(ctx context.Context, count, workers int, audit func(int) error) []error {
	if ctx == nil || count < 0 || workers <= 0 || audit == nil { return []error{errors.New("read-only audit configuration is invalid")} }
	errs := make([]error, count)
	for first := 0; first < count; first += workers {
		last := min(first+workers, count)
		var wait sync.WaitGroup
		for index := first; index < last; index++ {
			if err := ctx.Err(); err != nil { errs[index] = fmt.Errorf("audit %d blocked by canceled preparation: %w", index, err); continue }
			wait.Add(1)
...
func (self *Executor) preparationPayloadReadersError() error {
	if self == nil || self.deployer == nil || self.deployer.client == nil { return errors.New("blocked by deployer EVM reader") }
	if self.preparationIncomplete && independentRPCRequired(self.cfg) && self.independentEVM == nil { return errors.New("blocked by independent EVM reader") }
	return nil
}
...
func newLaunchPreparationExecutor(ctx context.Context, cfg *ResolvedConfig, stateDir string, plan *SetupPlan, journal *Journal, roles *RoleSecrets) (*Executor, error) {
	...
	if err := validateOwnedRPCPlan(cfg, plan); err != nil { return nil, err }
	if err := validateRuntimeConfigIdentityPlan(cfg, plan); err != nil { return nil, err }
	if err := validateExecutionRPCConfiguration(cfg); err != nil { return nil, fmt.Errorf("execution RPC configuration: %w", err) }
	if err := validateCampaignRPCTransport(cfg, runtimeCfg); err != nil { return nil, err }
	...
	if err != nil { failures = append(failures, fmt.Errorf("operational native reader: %w", err)) }
	if independentRPCRequired(runtimeCfg) {
		self.independentSubstrate, err = DialIndependentSubstrateManager(runtimeCfg)
		if err != nil { failures = append(failures, fmt.Errorf("independent native reader: %w", err)) }
		...
		if err == nil {
			...
			if chainErr != nil { err = chainErr } else if chainId.Uint64() != testnetChainID { err = fmt.Errorf("independent EVM chain id=%d, want %d", chainId.Uint64(), testnetChainID) }
			if err != nil { self.independentEVM.Close(); self.independentEVM = nil }
		}
		if err != nil { failures = append(failures, fmt.Errorf("independent EVM reader: %w", err)) }
	}
	if roles == nil {
	...
	for _, role := range []struct { name string; target **EvmTxManager }{
		...
		*role.target, err = DialEvmTxManager(ctx, runtimeCfg, stateDir, journal, roles, role.name)
		if err != nil { failures = append(failures, fmt.Errorf("%s EVM reader: %w", role.name, err)) }
	}
	for operator := 1; operator <= runtimeCfg.Config.Topology.Operators; operator++ {
		...
		if err != nil { failures = append(failures, fmt.Errorf("%s EVM reader: %w", role, err)); continue }
