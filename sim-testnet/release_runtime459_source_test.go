...
func TestRuntime459SourceAttestationPreservesScopeAndChangedNativeDependencies(t *testing.T) {
	...
	if hex.EncodeToString(digest[:]) != "e8a920ba20bd44204856b64f517f18f4e9b6e42abaca84531a449063cd6e1bcf" {
		t.Fatal("runtime459 exact source scope changed")
	}
	paths := map[string]bool{}
	for _, row := range strings.Split(strings.TrimSpace(string(raw)), "\n") {
		fields := strings.Fields(row)
		if len(fields) != 2 || paths[fields[1]] {
			t.Fatal("runtime459 source rows are malformed or duplicated")
		}
		paths[fields[1]] = true
	}
	if len(paths) != 71 {
		t.Fatal("runtime459 source scope is incomplete")
	}
	...
	for _, row := range strings.Split(strings.TrimSpace(string(prior)), "\n") {
		if !paths[strings.Fields(row)[1]] {
			t.Fatal("runtime459 omitted previously reviewed native scope")
		}
	}
	for _, path := range []string{"Cargo.lock", "pallets/subtensor/src/migrations/migrate_fix_root_pot_shortfall.rs", "pallets/subtensor/src/migrations/mod.rs", "pallets/subtensor/src/rpc_info/delegate_info.rs", "pallets/subtensor/src/staking/move_stake.rs", "pallets/subtensor/src/staking/remove_stake.rs", "pallets/subtensor/src/staking/set_children.rs", "pallets/subtensor/src/subnets/registration.rs", "pallets/subtensor/src/tests/destroy_alpha_tests.rs"} {
		if !paths[path] {
			t.Fatalf("runtime459 omitted changed native dependency %s", path)
		}
	}
...
func TestRuntime459ArtifactCheckerRequiresOwnedObservationAndCompleteHistory(t *testing.T) {
	...
	for _, item := range []struct {
		...
		{name: "foreign observation", change: func(_ map[string]any, artifacts []any) {
			artifacts[6].(map[string]any)["observation_rpc_url"] = "http://archive.example"
		}},
		{name: "future version", change: func(_ map[string]any, artifacts []any) { artifacts[6].(map[string]any)["spec_version"] = 460 }},
		{name: "missing458", change: func(manifest map[string]any, artifacts []any) {
			manifest["artifacts"] = append(artifacts[:5:5], artifacts[6])
		}},
	} {
		...
		if item.change == nil {
			if _, err := os.Stat(filepath.Join(dir, "cargo")); err != nil {
				t.Fatalf("reviewed459 did not reach the exact-genesis boundary: %s %v", output, err)
			}
