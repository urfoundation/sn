...
func TestProducerGateStateSelectionLiveHeadV2RetainsReplayAndMath(t *testing.T) {
	...
	if calls["BuildCut"] || calls["AttemptCutEgressClaims"] || calls["VerifyReleaseStatsMeasurement"] || calls["FindUidByHotkey"] || calls["CommitForEpoch"] || calls["releaseRawHeadScores"] { t.Fatal("compact live head acquired a legacy/current-height, unplanned rational or early-publication bypass") }
}
