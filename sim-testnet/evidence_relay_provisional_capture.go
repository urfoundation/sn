...
func validateProvisionalRelayCaptureMarker(plan *SetupPlan) error {
	...
	if record.Schema != "urnetwork-sim-provisional-resume-v1" || !record.Provisional || record.FinalAcceptance || !record.ReadOnly || record.Command != "relay-continuation" || record.Scenario != "" || record.PlanHash != plan.EvidenceRelayContinuation.SourcePlanHash || !plan.allowedPlanHashes()[record.PlanHash] || !validCanonicalHashHex(record.ConfigHash) || record.DeploymentID != plan.DeploymentID || !validCanonicalHashHex(record.ReleaseLockHash) || !filepath.IsAbs(capture.RecordPath) || filepath.Clean(capture.RecordPath) != capture.RecordPath || !releaseSHA256.MatchString(capture.RecordSHA256) || !releaseSHA256.MatchString(record.Driver.ExecutableSHA256) || !releaseGitCommit.MatchString(record.Driver.Build.Revision) || record.Driver.Build.PackagePath != "github.com/urfoundation/sn/sim-testnet" || record.Driver.Build.ModulePath != "github.com/urfoundation/sn/v2026" || !filepath.IsAbs(record.Driver.ExecutablePath) || plan.OwnedRPCAuthority == "" || plan.ChainID != testnetChainID || plan.GenesisHash != testnetGenesis {
