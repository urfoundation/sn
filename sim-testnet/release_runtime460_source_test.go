...
func TestRuntime460SourceAttestationPreservesReviewed459Scope(t *testing.T) {
	...
	if hex.EncodeToString(digest[:]) != "98c2aac04434d2ec67931479cf646918571970718078c50f711486c8db889644" { t.Fatal("runtime460 exact source scope changed") }
	...
	if len(rows) != 71 || len(rows) != len(priorRows) { t.Fatal("runtime460 lost retained source scope") }
	...
	for index, row := range rows {
		...
		if len(fields) != 2 || len(previous) != 2 || fields[1] != previous[1] { t.Fatal("runtime460 changed the retained source corpus") }
		if fields[0] != previous[0] {
			if _, ok := changed[fields[1]]; !ok { t.Fatalf("unreviewed source delta: %s", fields[1]) }
			changed[fields[1]] = true
		}
	}
	for path, found := range changed { if !found { t.Fatalf("runtime460 omitted source delta %s", path) } }
	...
	for _, required := range []string{"for current_spec in 455 458 459 460", "runtime-v460-source.sha256", "current_commit=\"8d5f20ec1a5e5d90295d43046dacdefc54aaed06\"", "SUBTENSOR_RUNTIME460_SOURCE", "expected_metadata_files=24"} {
		if !strings.Contains(string(checker), required) { t.Fatalf("runtime460 source checker omits %s", required) }
	}
}
