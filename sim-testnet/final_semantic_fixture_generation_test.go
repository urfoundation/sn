...
func attachFinalSemanticFixtureGeneration(t *testing.T, source *FinalSemanticEvidence, artifacts map[string][]byte) {
	t.Helper()
	attachFinalSemanticFixtureGenerationWithWorkObserver(t, source, artifacts, nil)
}

// Observe bounded calldata preparation without changing journal chronology,
// source ownership, signature verification or derived-artifact publication.
func attachFinalSemanticFixtureGenerationWithWorkObserver(t *testing.T, source *FinalSemanticEvidence, artifacts map[string][]byte, observer finalSemanticFixtureWorkObserver) {
	t.Helper()
	attachFinalSemanticFixtureGenerationWithWorkControl(t, source, artifacts, finalSemanticFixtureWorkControl{ctx: t.Context(), observer: observer})
}

// Independent fleet signing and calldata preparation own their mutable state.
// Journal chronology and the complete production rebuild stay serial.
func attachFinalSemanticFixtureGenerationWithWorkControl(t *testing.T, source *FinalSemanticEvidence, artifacts map[string][]byte, work finalSemanticFixtureWorkControl) {
	t.Helper()
	work.namespaceWork = work.namespaceWork.forInvocation()
	...
	fleetJobs, fleetErr := prepareFinalSemanticFixtureFleets(finalSemanticFixtureFleetContext{
		cfg: cfg, roles: roles, actions: actionByID, deploymentID: source.DeploymentID, planHash: current.PlanHash,
		chainID: source.ChainID, netuid: source.Netuid, coordinator: coordinator,
	}, work)
	if fleetErr != nil {
		t.Fatal(fleetErr)
	}
	// The workers own only pending facts. Preserve the original chronology:
	// every commitment precedes every initial binding in the journal.
	for _, job := range fleetJobs {
		for _, mutation := range job.commitments {
			approved := action(mutation.actionID)
			finalized(current.PlanHash, approved, mutation.transactionHash, mutation.nativeHead)
			verified(current.PlanHash, approved, postcondition(current.PlanHash, approved, mutation.nativeHead, mutation.evmHead, map[string]any{"fixture_action": approved.ID}))
		}
	}
	for _, job := range fleetJobs {
		for _, mutation := range job.bindings {
			approved := action(mutation.actionID)
			finalized(current.PlanHash, approved, mutation.transactionHash, mutation.nativeHead)
			verified(current.PlanHash, approved, postcondition(current.PlanHash, approved, mutation.nativeHead, mutation.evmHead, map[string]any{"fixture_action": approved.ID}))
		}
	...
	for _, job := range fleetJobs {
		for _, version := range job.versions {
			versions[key(int(job.fleet), version.manifest.Generation)] = version
		}
	...
	calldataJobs := make([]finalSemanticFixtureCalldataJob, finalFleetGenerationBatchCount)
	if err := work.run(finalSemanticFixtureGenerationCalldata, len(calldataJobs), 4, func(ctx context.Context, index int) error {
		stageWork := work
		stageWork.ctx = ctx
		job, err := prepareFinalSemanticFixtureGenerationCalldata(generationSource, uint64(index+1), stageWork, index)
		if err == nil {
			calldataJobs[index] = job
		}
		return err
	}); err != nil {
		t.Fatal(err)
	}
	owners := make([]*finalSemanticFixtureArtifacts, len(calldataJobs))
	for index := range calldataJobs {
		owners[index] = &calldataJobs[index].artifacts
	}
	joined, err := joinFinalSemanticFixtureArtifacts(artifacts, owners)
	if err != nil {
		t.Fatal(err)
	}
	// No receipt or derived object is published until every expected input and
	// every collision across the complete preparation result has been checked.
	receiptPaths := map[string]bool{}
	for _, job := range calldataJobs {
		for _, receipt := range job.receipts {
			if receiptPaths[receipt.path] || !bytes.Equal(files[receipt.path], receipt.prior) {
				t.Fatalf("fixture calldata receipt %s ownership changed", receipt.path)
			}
	...
	publishFinalSemanticFixtureArtifacts(artifacts, joined)
	for _, job := range calldataJobs {
		for _, receipt := range job.receipts {
			files[receipt.path] = append([]byte(nil), receipt.data...)
		}
	}
	...
	sealedFiles, err := finalFleetGenerationArtifactFilesWithWork(source, artifacts[source.FleetGeneration.Artifact.URI], work.namespaceWork, "fixture-postconditions")
	if err != nil {
		t.Fatal(err)
	}
	...
	cache, err := loadFinalSemanticArtifactUses(work.ctx, uses, func(_ context.Context, locator FinalArtifactLocator) ([]byte, error) {
	...
	if err := verifyFinalHistoricalCoordinatorReceiptArtifactsWithWork(source, current, cache, finalHistoricalCoordinatorArtifactWork{namespaceWork: work.namespaceWork}); err != nil {
