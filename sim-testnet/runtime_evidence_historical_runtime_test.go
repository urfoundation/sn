...
func newRuntimeEvidenceHistoricalRuntimeTest(t *testing.T) *runtimeEvidenceActivationRpcV2TestFixture {
	...
	if err != nil || metadataHash != "0x16da562c347a354c55eb1ad5cd5094343afe7acdc12e5b526bf6c8cb12e866bc" { t.Fatalf("historical protocol fixture: %v", err) }
	...
	if hash, err := releaseConfigHash(fixture.base.cfg.Config, fixture.base.cfg.Public, fixture.base.cfg.Hyperparameters); err != nil || hash != fixture.base.plan.ConfigHash { t.Fatalf("original config approval differs: %v", err) }
	prepared, wire, err := fixture.executor.prepareRuntimeEvidenceActivationsV2(t.Context(), fixture.chain)
	if err != nil { t.Fatalf("prepare original455 signed activation: %v", err) }
...
func upgradeRuntimeEvidenceHistoricalTest(t *testing.T, fixture *runtimeEvidenceActivationRpcV2TestFixture) {
	...
	cfg.Public.Chain.ExpectedRuntimeSpec, cfg.Public.Chain.ConfigIdentityRuntimeSpec = 458, 455
	if hash, err := releaseConfigHash(cfg.Config, cfg.Public, cfg.Hyperparameters); err != nil || hash != cfg.ConfigHash { t.Fatalf("explicit runtime migration changed original config identity: %v", err) }
}

// The actual failing preparation reader must retain its original source hash,
// pinned native uid, signatures and exact persisted bytes under current458.
func TestRuntimeEvidenceHistoricalRuntimeReplaysOriginalPreparation(t *testing.T) {
	...
	prepared, wire, err := fixture.executor.prepareRuntimeEvidenceActivationsV2(t.Context(), fixture.chain)
	if err != nil { t.Fatalf("original455 preparation replay under458: %v", err) }
	if prepared.PlanHash != fixture.base.plan.PlanHash || !bytes.Equal(before, wire) { t.Fatal("historical preparation changed signed original bytes or approval") }
	if fixture.executor.substrate.chain.Meta != metadata || fixture.executor.substrate.chain.Runtime != runtime { t.Fatal("historical preparation changed signing metadata") }
}

func TestRuntimeEvidenceHistoricalRuntimeRetainsEligibilityAndConsentChecks(t *testing.T) {
	fixture := newRuntimeEvidenceHistoricalRuntimeTest(t)
	upgradeRuntimeEvidenceHistoricalTest(t, fixture)
	for _, test := range []struct { name string; mutate func(*runtimeEvidenceActivationPreparedV2) }{
		...
		if err := json.Unmarshal(fixture.base.preparedBytes, &candidate); err != nil { t.Fatal(err) }
		test.mutate(&candidate)
		if err := fixture.executor.authenticateRuntimeEvidencePreparedV2(t.Context(), fixture.chain, &candidate); err == nil { t.Fatalf("changed %s became original source authority", test.name) }
	}
	fixture.stateLock.Lock()
	fixture.permits[1] = false
	fixture.stateLock.Unlock()
	if err := fixture.executor.authenticateRuntimeEvidencePreparedV2(t.Context(), fixture.chain, fixture.base.prepared); err == nil || !strings.Contains(err.Error(), "native eligibility or historical uid differs") { t.Fatalf("historical permit refusal: %v", err) }
}
...
func TestRuntimeEvidenceHistoricalRuntimeCarriedActionUsesOriginalApproval(t *testing.T) {
	...
	if err != nil || source.PlanHash != fixture.base.plan.PlanHash { t.Fatalf("original approved carry reconstruction: %v", err) }
	...
	if err == nil || !strings.Contains(err.Error(), "activation postcondition has no finalized checkpoint") || fixture.callCount("state_call") <= before { t.Fatalf("carried action bypassed original historical eligibility: %v", err) }
}
...
func TestRuntimeEvidenceHistoricalRuntimeHorizonSeparatesOriginalAndFreshReads(t *testing.T) {
	...
	if epoch, err := relay.readHorizonNative(t.Context(), activation, evidenceRelayNativeOriginalSnapshot); err != nil || epoch == 0 { t.Fatalf("original relay native anchor: epoch=%d error=%v", epoch, err) }
	for _, mode := range []evidenceRelayNativeReadMode{evidenceRelayNativeCurrentHead, evidenceRelayNativeCurrentSnapshot, 0} {
		if _, err := relay.readHorizonNative(t.Context(), activation, mode); err == nil { t.Fatalf("fresh or invalid mode %d admitted old455", mode) }
	}
	fixture.stateLock.Lock()
	fixture.permits[1] = false
	fixture.stateLock.Unlock()
	if _, err := relay.readHorizonNative(t.Context(), activation, evidenceRelayNativeOriginalSnapshot); err == nil { t.Fatal("historical horizon ignored original eligibility") }
}

func TestRuntimeEvidenceHistoricalRuntimeFreshPreparationStillRequires458(t *testing.T) {
	fixture := newRuntimeEvidenceHistoricalRuntimeTest(t)
	upgradeRuntimeEvidenceHistoricalTest(t, fixture)
	...
	if err := ensurePrivateDir(fresh); err != nil { t.Fatal(err) }
	executor := *fixture.executor
	executor.stateDir = fresh
	if _, _, err := executor.prepareRuntimeEvidenceActivationsV2(t.Context(), fixture.chain); err == nil || !strings.Contains(err.Error(), "unreviewed identity") { t.Fatalf("fresh preparation used historical runtime: %v", err) }
	if _, err := os.Stat(filepath.Join(fresh, "evidence-v2-setup", "prepared.json")); !errors.Is(err, os.ErrNotExist) { t.Fatalf("refused fresh preparation persisted consent: %v", err) }
}
