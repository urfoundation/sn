...
func attemptSettlementRuntimeV2TestAncestorAlias(t *testing.T, physical string) (string, string) {
	...
	if err := os.Symlink(filepath.Dir(physical), link); err != nil { t.Fatal(err) }
...
func TestAttemptSettlementRuntimeV2LegacyRecoveryPreservesStableAncestorAliases(t *testing.T) {
	...
	for index := range participants { participants[index].StateDir, _ = attemptSettlementRuntimeV2TestAncestorAlias(t, participants[index].StateDir) }
	...
	if err := RecoverAttemptSettlementEpoch(fixture.coordinator, legacy); err != nil { t.Fatal(err) }
	for _, snapshot := range transaction.Snapshots {
		actual, err := os.ReadFile(snapshot.StatsPath)
		if err != nil || !bytes.Equal(actual, snapshot.StatsJSON) { t.Fatalf("stable alias changed legacy bytes: %v", err) }
	}
	if _, err := os.Lstat(attemptSettlementTransactionPath(fixture.coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("stable alias journal was not removed: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2LegacyAliasToV6TargetIsRejected(t *testing.T) {
	...
	if err := RecoverAttemptSettlementEpoch(fixture.coordinator, legacy); err == nil || !strings.Contains(err.Error(), "cannot replace compact") { t.Fatalf("ancestor alias concealed v6 target: %v", err) }
...
func TestAttemptSettlementRuntimeV2LegacyAliasRetargetRefusesBeforeFirstWrite(t *testing.T) {
	for _, variant := range []string{"operator", "coordinator"} {
		...
		participants := append([]AttemptSettlementRuntimeV2Participant(nil), fixture.participants...)
		var link string
		if variant == "operator" { participants[0].StateDir, link = attemptSettlementRuntimeV2TestAncestorAlias(t, participants[0].StateDir) } else { fixture.coordinator, link = attemptSettlementRuntimeV2TestAncestorAlias(t, fixture.coordinator) }
		transaction, legacy, journal := legacyAttemptSettlementRuntimeV2TestJournal(t, fixture, participants)
		publishLegacyAttemptSettlementRuntimeV2TestJournal(t, fixture, journal)
		before := make([][]byte, len(fixture.participants))
		for index, participant := range fixture.participants { var err error; before[index], err = os.ReadFile(filepath.Join(participant.StateDir, "stats.json")); if err != nil { t.Fatal(err) } }
		if bytes.Equal(before[0], transaction.Snapshots[0].StatsJSON) { t.Fatal("first target would not detect a premature write") }
		...
		if err := os.WriteFile(filepath.Join(other, "settlement-transaction.json"), marker, 0o600); err != nil { t.Fatal(err) }
		steps := 0
		err := recoverAttemptSettlementEpochOwned(fixture.coordinator, legacy, nil, func(step string) error {
			if step != "before-first-write" { return nil }
			steps++
			if err := os.Rename(link, link+".preserved"); err != nil { return err }
			return os.Symlink(filepath.Dir(other), link)
		})
		if err == nil || steps != 1 { t.Fatalf("%s alias retarget was not refused: %v/%d", variant, err, steps) }
		for index, participant := range fixture.participants {
			actual, err := os.ReadFile(filepath.Join(participant.StateDir, "stats.json"))
			if err != nil || !bytes.Equal(actual, before[index]) { t.Fatalf("%s retarget wrote original target: %v", variant, err) }
		}
		actual, err := os.ReadFile(attemptSettlementTransactionPath(physicalCoordinator))
		if err != nil || !bytes.Equal(actual, journal) { t.Fatalf("%s retarget changed original journal: %v", variant, err) }
		for _, name := range []string{"stats.json", "settlement-transaction.json"} {
			actual, err := os.ReadFile(filepath.Join(other, name))
			if err != nil || !bytes.Equal(actual, marker) { t.Fatalf("%s retarget wrote unrelated %s: %v", variant, name, err) }
		}
	}
}
...
func TestAttemptSettlementRuntimeV2PhysicalPathRequiredBeforePersistence(t *testing.T) {
	...
	if err := InitializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence()); err != nil { t.Fatalf("physical canonical namespace refused: %v", err) }
	alias, _ := attemptSettlementRuntimeV2TestAncestorAlias(t, fixture.coordinator)
	physical := attemptSettlementV2PhysicalIO()
	writes := 0
	physical.writeJournal = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected alias journal") }
	if err := initializeAttemptSettlementEpochV2(t.Context(), alias, fixture.participants, 42, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence(), physical); err == nil || writes != 0 { t.Fatalf("v2 normalized an alias candidate: %v/%d", err, writes) }
	if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("alias refusal changed actual v6 state") }
}
