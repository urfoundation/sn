...
func newReleaseMeasurementV2TestFixture(t *testing.T, completed int) *releaseMeasurementV2TestFixture {
	...
	for _, noID := range []uint64{9, 10} {
		...
		for _, record := range seal.recordTs {
			...
			if err != nil || !reflect.DeepEqual(appended, &record) { t.Fatalf("v1 oracle changed actual signed record: %v", err) }
		}
		...
		if err != nil || cut == nil { t.Fatalf("actual M8 compact seal: %v", err) }
		...
		for _, provider := range measurement.Providers {
			if provider.Assignments > unbound.Assignments { unbound = provider }
		}
...
func (self *releaseMeasurementV2TestFixture) rebuildLegacy(t *testing.T) {
	...
	for index := range legacy.Inputs {
		...
		if err != nil { t.Fatalf("genuine v1 statistics oracle: %v", err) }
	...
	if err != nil { t.Fatalf("complete genuine v1 measurement oracle: %v", err) }
...
func (self *releaseMeasurementV2TestFixture) options(t *testing.T) ReleaseMeasurementV2Options {
	...
	options := ReleaseMeasurementV2Options{
		...
		MaxHeadEntries: 32, MaxArtifactBytes: 1<<20, MaxControlBytes: 1<<20,
	}
	for _, input := range artifact.Inputs {
		...
		for _, binding := range artifact.Bindings {
			if binding.NoID != input.NoID || !binding.Active || !binding.LiveUIDFound || binding.RecordUID != binding.LiveUID { continue }
		...
		var hashes uint64
		for _, provider := range input.Stats.Providers { hashes += uint64(len(provider.EgressIPHashHexes)) }
...
func assertReleaseMeasurementV2Empty(t *testing.T, result VerifiedReleaseMeasurementV2) {
	t.Helper()
	if result.Decision != nil || result.ReplayByNO != nil { t.Fatal("failed compact artifact published partial decision or replay census") }
}
...
func TestReleaseMeasurementV2MatchesRealM8WholeArtifact(t *testing.T) {
	...
	if err != nil || !reflect.DeepEqual(result.Decision, fixture.want) { t.Fatalf("real v1/v2 artifact parity differs: %v", err) }
	positive := 0
	for noID, replayed := range result.ReplayByNO {
		if replayed.Records.ItemCount != 122 || replayed.CompleteCount != 15 || replayed.FailedCount != 1 { t.Fatalf("operator %d real census differs: %+v", noID, replayed) }
		for _, provider := range result.Decision.StatsByNO[noID].Providers { if provider.HasQuality && provider.QualityPPM > 0 { positive++ } }
	}
	if len(result.ReplayByNO) != 2 || positive < 2 { t.Fatal("complete M8/two-operator positive-quality precondition was lost") }
	after, _ := canonicalReleaseMeasurementBytes(fixture.artifact)
	if !bytes.Equal(before, after) { t.Fatal("compact verifier mutated its caller's artifact") }
}
...
func TestReleaseMeasurementV2UsesOneJointReplayPerOperator(t *testing.T) {
	...
	observe := func(options ReleaseMeasurementV2Options, counts map[string]int) ReleaseMeasurementV2Options {
		for noID, operator := range options.Operators {
			...
			operator.Measurement.Replay.ReadMetadata = func(ctx context.Context, hash string, size uint64) ([]byte,error) { counts[fmtReleaseMeasurementV2Read(noID,"metadata",hash)]++; return transport.metadata(ctx,hash,size) }
			operator.Measurement.Replay.OpenData = func(ctx context.Context, kind, hash string, size uint64) (io.ReadCloser,error) { counts[fmtReleaseMeasurementV2Read(noID,kind,hash)]++; return transport.data(ctx,kind,hash,size) }
	...
	for _, input := range fixture.artifact.Inputs {
		...
		if _, err := VerifyReleaseStatsAndHeadWithAttemptCutV2(t.Context(),input.Stats,*input.AttemptCutV2,operator.Expected,fixture.artifact.Policy,operator.Bounds,operator.Measurement); err != nil { t.Fatal(err) }
	}
	result, err := VerifyReleaseMeasurementArtifactV2(t.Context(),fixture.artifact,observe(fixture.options(t),gotKVs))
	if err != nil || result.Decision == nil || len(wantKVs) == 0 || !maps.Equal(gotKVs,wantKVs) { t.Fatalf("artifact duplicated complete stream work: want=%v got=%v error=%v",wantKVs,gotKVs,err) }
}
...
func TestReleaseMeasurementV2CanonicalWireAndLegacyRefusal(t *testing.T) {
	...
	if err != nil || hash != ReleaseMeasurementContentHash(encoded) { t.Fatalf("compact seal: %v",err) }
	decoded, result, err := DecodeReleaseMeasurementArtifactV2(t.Context(),encoded,fixture.options(t))
	if err != nil || !reflect.DeepEqual(decoded,fixture.artifact) || !reflect.DeepEqual(result.Decision,fixture.want) { t.Fatalf("compact canonical round trip: %v",err) }
	if _,_,err := DecodeReleaseMeasurementArtifact(encoded); err == nil { t.Fatal("legacy decoder accepted compact artifact") }
	forged := cloneReleaseMeasurementArtifact(t,fixture.artifact)
	forged.Schema = ReleaseMeasurementSchema
	if _,err := VerifyReleaseMeasurementArtifact(forged); err == nil { t.Fatal("v1 schema spoof accepted compact authority") }
	...
	if _,err:=decodeReleaseMeasurementInput(legacyJournalBytes);err!=nil{t.Fatalf("complete genuine legacy journal precondition failed: %v",err)}
	journal.MeasurementInput=forged.Inputs[0]
	journalBytes,_ := canonicalReleaseMeasurementInputBytes(journal)
	if _,err := decodeReleaseMeasurementInput(journalBytes); err == nil || !strings.Contains(err.Error(),"legacy measurement input journal rejects compact evidence") { t.Fatalf("legacy private journal did not refuse compact authority at its actual boundary: %v",err) }
	for _, malformed := range [][]byte{bytes.TrimSpace(encoded),append(slices.Clone(encoded),[]byte("{}\n")...),bytes.Replace(encoded,[]byte("\"stats\":{"),[]byte("\"stats\":{\"attempt_cut\":{},"),1)} {
		if _,_,err := DecodeReleaseMeasurementArtifactV2(t.Context(),malformed,fixture.options(t)); err == nil { t.Fatal("compact strict wire admitted malformed or competing legacy fields") }
	}
}
...
func TestReleaseMeasurementV2RejectsUntrustedAuthorityBeforeIO(t *testing.T) {
	...
	for _, edit := range []func(*ReleaseMeasurementArtifact,*ReleaseMeasurementV2Options){
		...
		func(a *ReleaseMeasurementArtifact,o *ReleaseMeasurementV2Options){ a.DepositAudits[0].ObservedDepositRao = "1" },
		func(a *ReleaseMeasurementArtifact,o *ReleaseMeasurementV2Options){ a.Policy.Verify.TrailDepth = 4 },
		func(a *ReleaseMeasurementArtifact,o *ReleaseMeasurementV2Options){ op:=o.Operators[9];op.Expected.Identity.ValidatorVPK=releaseHex32([32]byte{9});o.Operators[9]=op },
		func(a *ReleaseMeasurementArtifact,o *ReleaseMeasurementV2Options){ op:=o.Operators[9];op.Expected.Activation.Domain.ActivationHash[0]++;o.Operators[9]=op },
		func(a *ReleaseMeasurementArtifact,o *ReleaseMeasurementV2Options){ op:=o.Operators[9];op.CutNativeBlockHash=releaseHex32([32]byte{0x77});o.Operators[9]=op },
		func(a *ReleaseMeasurementArtifact,o *ReleaseMeasurementV2Options){ op:=o.Operators[9];op.Measurement.Replay.VisitRecord=func(AttemptRecord)error{return nil};o.Operators[9]=op },
	} {
		artifact := cloneReleaseMeasurementArtifact(t,fixture.artifact)
		options := fixture.options(t)
		reads := 0
		for noID,op := range options.Operators { op.Measurement.Replay.ReadMetadata=func(context.Context,string,uint64)([]byte,error){reads++;return nil,errors.New("unexpected external read")};options.Operators[noID]=op }
		edit(artifact,&options)
		result,err := VerifyReleaseMeasurementArtifactV2(t.Context(),artifact,options)
		if err == nil || reads != 0 { t.Fatalf("untrusted authority reached replay: reads=%d error=%v",reads,err) }
		assertReleaseMeasurementV2Empty(t,result)
		for _,op := range options.Operators { if _,err:=os.Lstat(op.Measurement.Replay.ScratchDirectory);!errors.Is(err,os.ErrNotExist){t.Fatal("authority refusal mutated scratch")} }
	}
}
...
func TestReleaseMeasurementV2LateRecordAndProofErrorsAreAtomic(t *testing.T) {
	...
	if err == nil { t.Fatal("compact artifact accepted invented exposure") }
	assertReleaseMeasurementV2Empty(t,result)
	...
	if !errors.Is(err,failure) || opens==0 {t.Fatalf("late proof error was not reached: opens=%d error=%v",opens,err)}
	assertReleaseMeasurementV2Empty(t,result)
}
...
func (self *releaseMeasurementV2ObservedReader) Read(buffer []byte) (int,error) {
	...
	if self.corrupt&&!self.changed&&count>0{buffer[0]^=1;self.changed=true}
	return count,err
}
...
func TestReleaseMeasurementV2RejectsChangedStreamsAndJoinsOwners(t *testing.T) {
	t.Parallel()
	fixture:=newReleaseMeasurementV2TestFixture(t,2)
	for _,test:=range []struct{name,kind string;truncate,withError bool}{
		{"changed-record",AttemptStreamV2Records,false,false},
		{"changed-proof",AttemptStreamV2Proofs,false,false},
		{"short-proof",AttemptStreamV2Proofs,true,false},
		{"reader-and-error",AttemptStreamV2Proofs,false,true},
	}{
		t.Run(test.name,func(t *testing.T){
			...
			operator.Measurement.Replay.OpenData=func(ctx context.Context,kind,hash string,size uint64)(io.ReadCloser,error){
				reader,err:=fixture.operators[10].data(ctx,kind,hash,size)
				if err!=nil||kind!=test.kind{return reader,err}
				opens++
				...
				if test.truncate{
					if size==0{return observed,errors.New("actual proof chunk is unexpectedly empty")}
					observed.reader=io.LimitReader(reader,int64(size)-1)
				}
				if test.withError{return observed,failure}
				return observed,nil
			}
			options.Operators[10]=operator
			result,err:=VerifyReleaseMeasurementArtifactV2(t.Context(),fixture.artifact,options)
			if err==nil||opens==0||opens!=closes||test.withError&&!errors.Is(err,failure){t.Fatalf("changed stream escaped full verification/ownership: opens=%d closes=%d error=%v",opens,closes,err)}
			assertReleaseMeasurementV2Empty(t,result)
		})
	}
}

// Cancellation after the final real proof close joins that resource and
// returns neither canonical bytes nor a partial whole-artifact decision.
func TestReleaseMeasurementV2CancellationJoinsAndClears(t *testing.T) {
	...
	if !errors.Is(err,context.Canceled)||closes==0||encoded!=nil||hash!=""{t.Fatalf("post-close cancellation published artifact: closes=%d error=%v",closes,err)}
	result,err:=VerifyReleaseMeasurementArtifactV2(ctx,fixture.artifact,fixture.options(t))
	if !errors.Is(err,context.Canceled){t.Fatal("pre-canceled artifact was admitted")}
	assertReleaseMeasurementV2Empty(t,result)
}
...
func TestReleaseMeasurementV2OwnsEveryOperatorBeforeFirstIO(t *testing.T) {
	...
	op.Measurement.Replay.ReadMetadata=func(ctx context.Context,hash string,size uint64)([]byte,error){
		if !changed {
			...
			for _,key:=range other.Measurement.Replay.ServerKeys{key[0]^=1}
	...
	if err!=nil||!changed||!bytes.Equal(encoded,expectedBytes){t.Fatalf("first callback changed later operator/wire: changed=%v error=%v",changed,err)}
}
...
func TestReleaseMeasurementV2BoundsAndFixedWidthAdmission(t *testing.T) {
	...
	for _,edit:=range []func(*ReleaseMeasurementArtifact,*ReleaseMeasurementV2Options){
		...
		func(a *ReleaseMeasurementArtifact,o *ReleaseMeasurementV2Options){a.Inputs[0].Stats.Providers[0].ClientID=strings.Repeat("a",100000)},
		func(a *ReleaseMeasurementArtifact,o *ReleaseMeasurementV2Options){a.Inputs[0].Stats.Providers[0].EgressIPHashHexes=[]string{strings.Repeat("aA",100000)}},
		func(a *ReleaseMeasurementArtifact,o *ReleaseMeasurementV2Options){op:=o.Operators[9];op.Measurement.MaxProviders=1;o.Operators[9]=op},
	} {
		a:=cloneReleaseMeasurementArtifact(t,fixture.artifact);o:=fixture.options(t);edit(a,&o)
		result,err:=VerifyReleaseMeasurementArtifactV2(t.Context(),a,o)
		if err==nil{t.Fatal("compact artifact exceeded an explicit bound")}
		assertReleaseMeasurementV2Empty(t,result)
	}
}
...
func TestReleaseMeasurementV2SameEpochLineageReplaysExactPrefix(t *testing.T) {
	...
	currentOptions:=fixture.options(t);currentOptions.Expected=releaseMeasurementV2Decision(current)
	result,err:=VerifyReleaseMeasurementLineageV2(t.Context(),previous,fixture.options(t),current,currentOptions)
	if err!=nil||!reflect.DeepEqual(result.Decision,fixture.want){t.Fatalf("real same-prefix lineage failed: %v",err)}
	current.Inputs[0].Stats.Providers[0].HasPriorQuality=true
	current.Inputs[0].Stats.Providers[0].PriorQualityPPM=12345
	currentOptions=fixture.options(t);currentOptions.Expected=releaseMeasurementV2Decision(current)
	result,err=VerifyReleaseMeasurementLineageV2(t.Context(),previous,fixture.options(t),current,currentOptions)
	if err==nil||!strings.Contains(err.Error(),"prior pool EMA"){t.Fatalf("lineage ignored prior state: %v",err)}
	assertReleaseMeasurementV2Empty(t,result)
}
...
func TestReleaseMeasurementV2TerminalBoundaryFailsClosed(t *testing.T) {
	...
	if err==nil||!strings.Contains(err.Error(),"terminal closure"){t.Fatalf("unintegrated compact terminal was accepted: %v",err)}
	assertReleaseMeasurementV2Empty(t,result)
}
...
func TestReleaseMeasurementV2JointSignedNativeCursorParity(t *testing.T) {
	...
	for index:=range fixture.artifact.Inputs {
		...
		for provider:=range input.Stats.Providers{input.Stats.Providers[provider].EgressIPHashHexes=nil}
	}
	fixture.rebuildLegacy(t)
	result,err:=VerifyReleaseMeasurementArtifactV2(t.Context(),fixture.artifact,fixture.options(t))
	if err!=nil||!reflect.DeepEqual(result.Decision,fixture.want){t.Fatalf("signed native cursor changed whole artifact parity: %v",err)}
	if len(result.Decision.EligibleHead)!=0||len(result.Decision.UIDs)!=2{t.Fatal("empty native window did not cede head to the two real positive pools")}
	for noID,stats:=range result.Decision.StatsByNO{
		if result.ReplayByNO[noID].Records.ItemCount!=122{t.Fatal("native cursor skipped cumulative settlement records")}
		for clientID,provider:=range stats.Providers{
			prior:=priorStats[noID].Providers[clientID]
			if provider.Exposure!=prior.Exposure||provider.QualityPPM!=prior.QualityPPM||provider.HasQuality!=prior.HasQuality||len(provider.EgressIPHashes)!=0{t.Fatal("native cursor changed settlement quality or retained old egress")}
		}
	}
}
...
func TestReleaseMeasurementV2LineageRejectsGenuineRewrittenPrefix(t *testing.T) {
	...
	if len(records)!=18||records[7].Disposition!=AttemptDispositionComplete||records[15].Disposition!=AttemptDispositionComplete{t.Fatal("two full M8 trails and one failed trail are required")}
	...
	for _,record:=range reordered{
		appended,err:=ledger.AppendContext(t.Context(),record)
		if err!=nil{t.Fatal(err)}
		if err:=VerifyAttemptRecord(appended,original.seal.expected.Identity,original.seal.key.Public().(ed25519.PublicKey),original.seal.server.serverPublicKeys());err!=nil{t.Fatalf("alternative signed ordering is not genuine: %v",err)}
	}
	...
	if err!=nil||cut==nil||cut.Root==fixture.artifact.Inputs[0].AttemptCutV2.Root{t.Fatalf("genuine fork did not produce a distinct root: %v",err)}
	...
	if err!=nil||!reflect.DeepEqual(standalone.Decision,fixture.want){t.Fatalf("valid independent fork precondition failed: %v",err)}
	result,err:=VerifyReleaseMeasurementLineageV2(t.Context(),previous,previousOptions,fixture.artifact,fixture.options(t))
	if err==nil||!strings.Contains(err.Error(),"rewrites its authenticated prior prefix"){t.Fatalf("genuine fork bypassed actual prefix checkpoint: %v",err)}
	assertReleaseMeasurementV2Empty(t,result)
}
...
func TestReleaseMeasurementV2StandaloneRechecksSignatures(t *testing.T) {
	...
	if err==nil{t.Fatal("standalone compact artifact reused old signature acceptance")}
	assertReleaseMeasurementV2Empty(t,result)
	result,err=VerifyReleaseMeasurementArtifactV2(t.Context(),fixture.artifact,fixture.options(t))
	if err!=nil||!reflect.DeepEqual(result.Decision,fixture.want){t.Fatalf("caller-mutated prior result contaminated next call: %v",err)}
}
...
func TestReleaseMeasurementV2CurrentUIDZeroAndControlledMaskParity(t *testing.T) {
	...
	for index:=range fixture.artifact.Bindings{
		...
		if binding.Active { binding.RecordUID,binding.LiveUID=0,0 }
	}
	fixture.artifact.ControlledNOIDs=[]uint64{9}
	fixture.rebuildLegacy(t)
	result,err:=VerifyReleaseMeasurementArtifactV2(t.Context(),fixture.artifact,fixture.options(t))
	if err!=nil||!reflect.DeepEqual(result.Decision,fixture.want){t.Fatalf("UID0/current-control math differs: %v",err)}
	if !slices.Contains(result.Decision.MaskedUIDs,uint16(0))||!slices.Contains(result.Decision.MaskedUIDs,uint16(109))||len(result.Decision.UIDs)!=1||result.Decision.UIDs[0]!=110{t.Fatal("shared eligible UID or controlled pool escaped exact masking")}
}
...
func (self *releaseMeasurementV2TestFixture) intent(t *testing.T, encoded []byte) *SteeringIntent {
	...
	for index,input:=range w.EligibleHead{headScores[index]=input.Score}
	eligible,err:=rationalJSON(headScores);if err!=nil{t.Fatal(err)}
	scores,err:=rationalJSON(w.Scores);if err!=nil{t.Fatal(err)}
...
func TestReleaseMeasurementV2IntentRequiresFullEvidenceAndContent(t *testing.T) {
	...
	if err:=VerifyReleaseMeasurementIntentV2(t.Context(),encoded,fixture.options(t),intent);err!=nil{t.Fatalf("real compact intent comparison failed: %v",err)}
	intent.MeasurementArtifactHash=ReleaseMeasurementContentHash([]byte("other"))
	if err:=VerifyReleaseMeasurementIntentV2(t.Context(),encoded,fixture.options(t),intent);err==nil{t.Fatal("intent accepted unrelated content address")}
}
...
func TestReleaseMeasurementV2IntentOwnsDecisionBeforeIO(t *testing.T) {
	...
	for _,initiallyValid:=range []bool{false,true}{
		...
		if !initiallyValid{intent.Scores[0]=RationalJSON{Numerator:"999",Denominator:"1"}}
		options:=fixture.options(t)
		...
		operator.Measurement.Replay.ReadMetadata=func(ctx context.Context,hash string,size uint64)([]byte,error){
			if !changed{
				if initiallyValid{intent.Scores[0]=RationalJSON{Numerator:"999",Denominator:"1"};intent.MeasurementArtifactHash="changed"}else{intent.Scores[0]=validScore}
				changed=true
			}
			return fixture.operators[9].metadata(ctx,hash,size)
		}
		...
		if !changed||initiallyValid&&err!=nil||!initiallyValid&&(err==nil||!strings.Contains(err.Error(),"decision differs")){t.Fatalf("transport changed admitted intent: initialValid=%v changed=%v error=%v",initiallyValid,changed,err)}
	}
}
...
func TestReleaseMeasurementV2DistinctOperatorActivationHashes(t *testing.T) {
	...
	if left.Identity.NoID==right.Identity.NoID||left.Activation.Domain.ActivationHash==right.Activation.Domain.ActivationHash||!equalAttemptCutV2CommonDomain(left.Activation.Domain,right.Activation.Domain){t.Fatal("two independent activation anchors/common deployment precondition changed")}
	before,_:=canonicalReleaseMeasurementBytes(fixture.artifact)
	result,err:=VerifyReleaseMeasurementArtifactV2(t.Context(),fixture.artifact,options)
	if err!=nil||len(result.ReplayByNO)!=2||!reflect.DeepEqual(result.Decision,fixture.want){t.Fatalf("complete distinct-activation operators refused: %v",err)}
	for _,replayed:=range result.ReplayByNO{if replayed.Records.ItemCount!=18||replayed.CompleteCount!=2||replayed.FailedCount!=1{t.Fatal("distinct anchors lost complete M8/failed census")}}
	after,_:=canonicalReleaseMeasurementBytes(fixture.artifact)
	if !bytes.Equal(before,after){t.Fatal("common-domain join normalized caller activation hashes")}
}
	...
	for _,name:=range []string{"signed-candidate","swapped-expected","altered-expected"}{
		t.Run(name,func(t *testing.T){
			...
			case "signed-candidate":
				...
				if err:=cut.VerifyHeader(cut.Context,fixture.operators[10].seal.bounds);err!=nil{t.Fatalf("changed-anchor candidate was not genuinely signed: %v",err)}
			...
			reads:=0
			for noID,operator:=range options.Operators{
				operator.Measurement.Replay.ReadMetadata=func(context.Context,string,uint64)([]byte,error){reads++;return nil,errors.New("unexpected activation read")}
				options.Operators[noID]=operator
			}
			result,err:=VerifyReleaseMeasurementArtifactV2(t.Context(),artifact,options)
			if err==nil||reads!=0||!strings.Contains(err.Error(),"differs from expected authority"){t.Fatalf("per-NO activation pin bypassed: reads=%d error=%v",reads,err)}
			assertReleaseMeasurementV2Empty(t,result)
		})
	}
}
...
func TestReleaseMeasurementV2RejectsCommonDomainDrift(t *testing.T) {
	t.Parallel()
	fixture:=newReleaseMeasurementV2TestFixture(t,2)
	for _,test:=range []struct{name string;change func(*AttemptCutV2Context)}{
		{"chain",func(c *AttemptCutV2Context){c.Identity.ChainID++;c.Activation.Domain.ChainID=c.Identity.ChainID}},
		{"genesis",func(c *AttemptCutV2Context){c.Activation.Domain.GenesisHash[0]^=1;c.Identity.GenesisHash=attemptHex32(c.Activation.Domain.GenesisHash)}},
		{"netuid",func(c *AttemptCutV2Context){c.Identity.Netuid++;c.Activation.Domain.Netuid=c.Identity.Netuid}},
		{"coordinator",func(c *AttemptCutV2Context){c.Activation.Domain.Coordinator[0]^=1}},
		{"vault",func(c *AttemptCutV2Context){c.Activation.Domain.SettlementVault[0]^=1}},
		{"deployment",func(c *AttemptCutV2Context){c.Identity.DeploymentID+="-other";c.Activation.Domain.DeploymentIDHash=sha256.Sum256([]byte(c.Identity.DeploymentID))}},
		{"policy",func(c *AttemptCutV2Context){c.Activation.Domain.PolicyHash[0]^=1}},
		{"epoch",func(c *AttemptCutV2Context){c.Activation.Domain.ActivationEpoch--}},
	}{
		t.Run(test.name,func(t *testing.T){
			artifact:=cloneReleaseMeasurementArtifact(t,fixture.artifact)
			options:=fixture.options(t)
			...
			if err!=nil{t.Fatalf("changed common field lost valid signed-header precondition: %v",err)}
			cut.Signature=signature
			if err:=cut.VerifyHeader(operator.Expected,operator.Bounds);err!=nil{t.Fatal(err)}
			options.Operators[10]=operator
			reads:=0
			for noID,observed:=range options.Operators{
				observed.Measurement.Replay.ReadMetadata=func(context.Context,string,uint64)([]byte,error){reads++;return nil,errors.New("unexpected mixed-domain read")}
				options.Operators[noID]=observed
			}
			result,err:=VerifyReleaseMeasurementArtifactV2(t.Context(),artifact,options)
			if err==nil||reads!=0||!strings.Contains(err.Error(),"do not share one authenticated activation"){t.Fatalf("common domain field was ignored: reads=%d error=%v",reads,err)}
			assertReleaseMeasurementV2Empty(t,result)
		})
	}
	// Keep the helper's explicit field census visible if the protocol grows.
	if reflect.TypeFor[protocol.ValidatorEvidenceDomain]().NumField()!=9{t.Fatal("activation common-domain field census requires explicit review")}
}
