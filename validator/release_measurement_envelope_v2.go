...
func releaseMeasurementEnvelopeV2WireSize(ctx context.Context, envelope *ReleaseMeasurementEnvelope, limit, reserved uint64) (uint64, error) {
	if ctx == nil || envelope == nil { return 0, errors.New("compact measurement envelope wire context is nil") }
	if err := ctx.Err(); err != nil { return 0, err }
	used := uint64(0)
	charge := func(count uint64) error {
		if count > limit-used { return errReleaseMeasurementEnvelopeV2WireBound }
		used += count
		return nil
	}
	stringBytes := func(value string) error {
		if err := charge(2); err != nil { return err }
		for index := 0; index < len(value); {
			...
			case char >= utf8.RuneSelf:
				...
				if runeValue == utf8.RuneError && width == 1 || runeValue == '\u2028' || runeValue == '\u2029' { encodedWidth = 6 }
			}
			if err := charge(encodedWidth); err != nil { return err }
			index += width
		}
		return nil
	}
	...
	if err := charge(3); err != nil { return 0, err }
	if err := charge(reserved); err != nil { return 0, err }
	value := reflect.ValueOf(envelope).Elem()
	for index := 0; index < value.NumField(); index++ {
		if err := ctx.Err(); err != nil { return 0, err }
		field := value.Type().Field(index)
		name := field.Tag.Get("json")
		if field.PkgPath != "" || name == "" || name == "-" || strings.Contains(name, ",") { return 0, errors.New("compact measurement envelope has unsupported JSON field behavior") }
		if index != 0 { if err := charge(1); err != nil { return 0, err } }
		if err := stringBytes(name); err != nil { return 0, err }
		if err := charge(1); err != nil { return 0, err }
		switch fieldValue := value.Field(index); fieldValue.Kind() {
		case reflect.String:
			if err := stringBytes(fieldValue.String()); err != nil { return 0, err }
		case reflect.Uint16, reflect.Uint64:
			if err := charge(uint64(len(strconv.FormatUint(fieldValue.Uint(), 10)))); err != nil { return 0, err }
		default:
...
func admitReleaseMeasurementEnvelopeV2Storage(ctx context.Context, envelope *ReleaseMeasurementEnvelope, maxControlBytes uint64, unsigned bool) error {
	if ctx == nil || envelope == nil || maxControlBytes == 0 || maxControlBytes > releaseMeasurementEnvelopeMaxArtifactSize { return errors.New("compact measurement envelope context or bound is invalid") }
	if err := ctx.Err(); err != nil { return err }
	if envelope.Schema != ReleaseMeasurementEnvelopeSchemaV2 || envelope.MeasurementSchema != ReleaseMeasurementSchemaV2 { return errors.New("compact measurement envelope schema is invalid") }
	reserved := uint64(0)
	if unsigned {
		if envelope.SigningHash != "" || envelope.Signature != "" { return errors.New("compact measurement unsigned envelope contains signing fields") }
		reserved = releaseMeasurementEnvelopeV2SigningFieldBytes
	}
	remaining := maxControlBytes
	if err := releaseMeasurementV2ControlStorage(ctx, reflect.ValueOf(envelope), &remaining); err != nil { return errors.Join(errReleaseMeasurementEnvelopeV2ControlBound, err) }
	if reserved > remaining { return errReleaseMeasurementEnvelopeV2ControlBound }
	if _, err := releaseMeasurementEnvelopeV2WireSize(ctx, envelope, maxControlBytes, reserved); err != nil { return err }
	return ctx.Err()
}
...
func validateReleaseMeasurementEnvelopeV2(ctx context.Context, envelope *ReleaseMeasurementEnvelope, maxControlBytes uint64) error {
	if err := admitReleaseMeasurementEnvelopeV2Storage(ctx, envelope, maxControlBytes, false); err != nil { return err }
	if err := validateReleaseMeasurementEnvelopeFieldsWithHexWork(envelope, canonicalHexWork{}, ReleaseMeasurementEnvelopeSigningDomainV2); err != nil { return err }
	return ctx.Err()
}
...
func ownReleaseMeasurementEnvelopeV2Hotkey(hotkey *crv4.Keypair) (owned *crv4.Keypair, publicKey [32]byte, resultErr error) {
	if hotkey == nil { return nil, publicKey, errors.New("compact measurement envelope signer is missing") }
	ownedHotkey := *hotkey
	owned = &ownedHotkey
	publicKey, resultErr = func() (key [32]byte, err error) {
		defer func() {
			if recover() != nil { key = [32]byte{}; err = errors.New("compact measurement envelope signer public key is unavailable") }
		}()
		return owned.PublicKey(), nil
	}()
	if resultErr != nil { return nil, [32]byte{}, resultErr }
	if publicKey == ([32]byte{}) { return nil, publicKey, errors.New("compact measurement envelope signer public key is zero") }
	return owned, publicKey, nil
}
...
func SealReleaseMeasurementEnvelopeV2(ctx context.Context, measurement []byte, validatorUID uint16, hotkey *crv4.Keypair, preparedExtrinsicHash string, signedAt time.Time, options ReleaseMeasurementV2Options) (encoded []byte, contentHash string, envelope *ReleaseMeasurementEnvelope, resultErr error) {
	if ctx == nil { return nil, "", nil, errors.New("compact measurement envelope context is nil") }
	defer func() {
		resultErr = errors.Join(resultErr, ctx.Err())
		if resultErr != nil { encoded, contentHash, envelope = nil, "", nil }
	}()
	...
	if err != nil { return nil, "", nil, err }
	if err := validateReleaseMeasurementEnvelopeV2Authority(options, publicKey, validatorUID); err != nil { return nil, "", nil, err }
	...
	if err != nil { return nil, "", nil, fmt.Errorf("compact measurement envelope artifact: %w", err) }
	...
	if err := admitReleaseMeasurementEnvelopeV2Storage(ctx, envelope, options.MaxControlBytes, true); err != nil { return nil, "", nil, err }
	if _, err := VerifyReleaseMeasurementArtifactV2(ctx, artifact, options); err != nil { return nil, "", nil, fmt.Errorf("compact measurement envelope artifact: %w", err) }
	...
	if err := validateReleaseMeasurementEnvelopeV2(ctx, envelope, options.MaxControlBytes); err != nil { return nil, "", nil, err }
...
func DecodeReleaseMeasurementEnvelopeV2(ctx context.Context, encoded []byte, maxControlBytes uint64) (*ReleaseMeasurementEnvelope, error) {
	...
	if err := validateReleaseMeasurementEnvelopeV2(ctx, envelope, maxControlBytes); err != nil { return nil, err }
	return envelope, nil
}
...
func VerifyReleaseMeasurementEnvelopeV2(ctx context.Context, envelope *ReleaseMeasurementEnvelope, measurement []byte, expectedHotkey [32]byte, expectedUID uint16, expectedPreparedExtrinsicHash string, options ReleaseMeasurementV2Options) (artifact *ReleaseMeasurementArtifact, verified VerifiedReleaseMeasurementV2, resultErr error) {
	if ctx == nil { return nil, verified, errors.New("compact measurement envelope context is nil") }
	defer func() {
		resultErr = errors.Join(resultErr, ctx.Err())
		if resultErr != nil { artifact, verified = nil, VerifiedReleaseMeasurementV2{} }
	}()
	if err := validateReleaseMeasurementEnvelopeV2(ctx, envelope, options.MaxControlBytes); err != nil { return nil, verified, err }
	ownedEnvelope := *envelope
	envelope = &ownedEnvelope
	if err := validateReleaseMeasurementEnvelopeV2Authority(options, expectedHotkey, expectedUID); err != nil { return nil, verified, err }
	...
	if err != nil { return nil, VerifiedReleaseMeasurementV2{}, fmt.Errorf("compact measurement envelope artifact: %w", err) }
