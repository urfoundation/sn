...
func validateAttemptSettlementV2CarriedLegacy(ctx context.Context, participants []AttemptSettlementRuntimeV2Participant, statsTs []*StatsEngine, authority AttemptSettlementV2Options, persistence AttemptSettlementRuntimeV2PersistenceBounds) error {
	...
	if len(closure.Transitions) != 0 {
		...
		if err != nil { return err }
		if _, err := DecodeAttemptSettlementClosureWithServerKeys(encoded, serverKeys); err != nil { return err }
	}
	return ctx.Err()
}
...
func validateAttemptSettlementV2Carried(ctx context.Context, coordinator string, participants []AttemptSettlementRuntimeV2Participant, statsTs []*StatsEngine, authority AttemptSettlementV2Options, persistence AttemptSettlementRuntimeV2PersistenceBounds, physical attemptSettlementV2IO) error {
	...
	for index, participant := range participants {
		...
		if _, err := countAttemptSettlementV2Engine(ctx, stats, persistence.MaxSnapshotBytes); err != nil { return err }
		if err := validateAttemptStatsV2Snapshot(stats.snapshotStats()); err != nil { return err }
	}
	if terminalCount == 0 {
		if err := validateAttemptSettlementV2CarriedLegacy(ctx, participants, statsTs, authority, persistence); err != nil { return err }
	} else {
	...
	for index, participant := range participants {
		if err := validateAttemptSettlementV2LedgerState(ctx, participant, statsTs[index], authority.Operators[participant.NoID], persistence.MaxSnapshotBytes); err != nil { return err }
	}
	return ctx.Err()
}
...
func recoverAttemptSettlementEpochV2(ctx context.Context, coordinator string, participants []AttemptSettlementRuntimeV2Participant, authority AttemptSettlementV2Options, persistence AttemptSettlementRuntimeV2PersistenceBounds, physical attemptSettlementV2IO) (resultErr error) {
	...
	if err := persistence.validate(); err != nil { return err }
	if err := validateAttemptSettlementV2MetadataLimit(authority.MaxClosureBytes); err != nil { return err }
	physical.ctx = ctx
	...
	if exists {
		...
		if err != nil { return err }
		if _, err := validateAttemptSettlementTransactionV2(ctx, transaction, batch.ordered, preimages, postimages, owned, persistence); err != nil { return err }
		...
		if err := validateAttemptSettlementV2Carried(ctx, coordinator, batch.ordered, candidates, owned, persistence, physical); err != nil { return err }
	}
	for index, participant := range batch.ordered {
	...
	for index, participant := range batch.ordered {
		if err := prepareAttemptSettlementV2Attachment(ctx, participant, candidates[index], owned.Operators[participant.NoID], persistence.MaxSnapshotBytes); err != nil { return err }
	}
	...
	for _, candidate := range candidates {
		if _, err := countAttemptSettlementV2Engine(ctx, candidate, persistence.MaxSnapshotBytes); err != nil { return err }
	}
	if err := physical.removeOwnedJournal(physical.roots[coordinator], filepath.Base(attemptSettlementTransactionV2Path(coordinator))); err != nil { return err }
	for index, participant := range batch.ordered {
		if err := checkHeads(); err != nil { return err }
		data, err := encodeAttemptSettlementV2Engine(ctx, candidates[index], persistence.MaxSnapshotBytes)
		if err != nil { return err }
		if err := physical.writeImage(physical.roots[participant.StateDir], "stats.json", data, persistence.MaxSnapshotBytes, physical.writeSnapshot); err != nil { return err }
	}
	if err := checkHeads(); err != nil { return err }
