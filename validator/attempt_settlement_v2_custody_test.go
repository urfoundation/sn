...
func TestAttemptSettlementRuntimeV2DirectoryFIFORefusedNatively(t *testing.T) {
	...
	if err := unix.Mkfifo(path, 0o600); err != nil { t.Fatal(err) }
	root, err := openAttemptSettlementV2Root(path)
	if root != nil || err == nil { t.Fatalf("FIFO acquired as directory: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2LeafFIFOSwapRefusedNatively(t *testing.T) {
	...
	physical.step = func(step string) error {
		if step != "before-open" { return nil }
		steps++
	...
	if err == nil || !exists || data != nil || steps != 1 { t.Fatalf("replacement FIFO was read or treated absent: %v/%t/%d", err, exists, steps) }
}
...
func TestAttemptSettlementRuntimeV2InRootSymlinkSwapRefusedNatively(t *testing.T) {
	...
	data, exists, err := readAttemptSettlementV2File(root, "stats.json", 64, physical)
	if !errors.Is(err, unix.ELOOP) || !exists || data != nil { t.Fatalf("in-root symlink was followed: %v/%t", err, exists) }
}
...
func TestAttemptSettlementRuntimeV2LateMissingLeafNeverBecomesAbsence(t *testing.T) {
	for _, boundary := range []string{"before-open", "after-read"} {
		...
		if err != nil { t.Fatal(err) }
		physical := attemptSettlementV2PhysicalIO()
		physical.step = func(step string) error { if step == boundary { return os.Rename(path, path+".preserved") }; return nil }
		data, exists, err := readAttemptSettlementV2File(root, "stats.json", 64, physical)
		closeErr := root.close()
		if !errors.Is(err, os.ErrNotExist) || !exists || data != nil || closeErr != nil { t.Fatalf("%s missing occupied leaf lost custody: %v/%t close=%v", boundary, err, exists, closeErr) }
	}
}
...
func TestAttemptSettlementRuntimeV2ClosureDirectoryFIFOSwapRefused(t *testing.T) {
	...
	physical := attemptSettlementV2PhysicalIO()
	physical.step = func(step string) error {
		if step != "before-closure-directory-open" { return nil }
		if err := os.Rename(path, path+".preserved"); err != nil { return err }
		return unix.Mkfifo(path, 0o600)
	}
	root, err := openAttemptSettlementV2ClosureDirectory(coordinator, false, physical)
	if err == nil || root != nil { t.Fatalf("replaced child directory acquired: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2CallbackDirectorySwapPreservesOwners(t *testing.T) {
	...
	physical.writeSnapshot = func(root *attemptPrivateDirectory, name string, data []byte) error {
		...
		if err := os.Rename(second, second+".preserved"); err != nil { return err }
	...
	if err == nil || closure != nil || writes != 1 || !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatalf("callback redirect partially published: %v/%d", err, writes) }
	fixture.assertReserved(t, 43)
	if _, err := os.Lstat(filepath.Join(second, "stats.json")); !errors.Is(err, os.ErrNotExist) { t.Fatalf("new occupant received old owner's snapshot: %v", err) }
	retained, err := os.ReadFile(filepath.Join(second+".preserved", "stats.json"))
	if err != nil || !bytes.Equal(retained, oldDisk) { t.Fatalf("retained second image changed: %v", err) }
...
func TestAttemptSettlementRuntimeV2LateClosureCloseRetainsCauseAndCancellation(t *testing.T) {
	...
	physical.closeFile = func(file *os.File) error {
		err := file.Close()
		if strings.HasPrefix(filepath.Base(file.Name()), ".closure-") { closes++; cancel(); return errors.Join(err, cause) }
		return err
	}
	...
	if closure != nil || !errors.Is(err, cause) || !errors.Is(err, context.Canceled) || closes != 1 { t.Fatalf("late file close/cancel cause lost: %v/%d", err, closes) }
	fixture.assertReserved(t, 43)
...
func TestAttemptSettlementRuntimeV2ClosureDirectorySyncFailureIsRetryable(t *testing.T) {
	...
	physical.syncDirectory = func(file *os.File) error {
		err := file.Sync()
		if filepath.Base(file.Name()) == "settlement-closures-v2" { syncs++; return errors.Join(err, cause) }
		return err
	}
	closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t), physical, true)
	if closure != nil || !errors.Is(err, cause) || syncs != 1 { t.Fatalf("late directory sync lost: %v/%d", err, syncs) }
	fixture.assertReserved(t, 43)
	transaction := fixture.transaction(t)
	data, err := os.ReadFile(AttemptSettlementClosureV2Path(fixture.coordinator, 42))
	if err != nil || !bytes.Equal(data, transaction.ClosureJSON) { t.Fatalf("actual renamed immutable bytes were not retained: %v", err) }
	if _, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t)); err != nil { t.Fatal(err) }
	assertAttemptSettlementRuntimeV2Postimages(t, fixture.coordinator, fixture.participants, transaction)
}
...
func TestAttemptSettlementRuntimeV2PhysicalOwnerCloseFailureKeepsAdmissionClosed(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		path := root.path
		if closed[path] { t.Error("owner was closed repeatedly") }
		closed[path] = true
		if path == fixture.participants[0].StateDir {
			if err := root.file.Close(); err != nil { return err }
	...
	if closure != nil || physicalErr == nil || !errors.Is(err, physicalErr) || !errors.As(physicalErr, &pathErr) || len(closed) != 4 { t.Fatalf("actual owner close lost or owners abandoned: %v owners=%d", err, len(closed)) }
	fixture.assertReserved(t, 43)
	before := runtimeAttemptSettlementV2TestImages(t, fixture.participants)
	if _, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t)); err != nil { t.Fatal(err) }
	if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("post-close retry changed exact completed bytes") }
}
...
func TestAttemptSettlementRuntimeV2FinalOwnerCloseCancellationKeepsGate(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		path := root.path
		err := root.close()
		if path == fixture.participants[0].StateDir { closes++; cancel() }
		return err
	}
	closure, err := advanceAttemptSettlementEpochV2(ctx, fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t), physical, true)
	if closure != nil || !errors.Is(err, context.Canceled) || closes != 1 { t.Fatalf("late owner cancellation was ignored: %v/%d", err, closes) }
	fixture.assertReserved(t, 43)
	before := runtimeAttemptSettlementV2TestImages(t, fixture.participants)
	if _, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t)); err != nil { t.Fatal(err) }
	if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("late-cancel retry changed exact completed state") }
}
...
func TestAttemptSettlementRuntimeV2MetadataCloseReplacementIsRefused(t *testing.T) {
	...
	data, exists, err := readAttemptSettlementV2File(root, "stats.json", 64, physical)
	if !errors.Is(err, os.ErrNotExist) || !exists || data != nil || closes != 1 { t.Fatalf("post-close replacement returned stale bytes or absence: %v/%t/%d", err, exists, closes) }
}
