...
func (self statsSnapshotIOHooks) observe(stage string, file *os.File) error {
	if self.after != nil { return self.after(stage, file) }
	return nil
}
...
func (self *statsWriteOwner) prepareSnapshot(dir string, physical bool) error {
	if self.snapshot != nil { return errors.New("statistics snapshot owner was prepared twice") }
	if physical { if err := validateStatsSnapshotPhysicalDirectory(dir); err != nil { return err } }
...
func (self *statsWriteOwner) finishSnapshot() error {
	if self.snapshot == nil { return nil }
...
func encodedStatsSnapshotWrite(path string, data []byte) (statsSnapshotWrite, error) {
	...
	if err != nil || start != json.Delim('{') { return statsSnapshotWrite{}, errors.Join(errors.New("statistics snapshot encoding has no object header"), err) }
	field, err := decoder.Token()
	if err != nil || field != "v" { return statsSnapshotWrite{}, errors.Join(errors.New("statistics snapshot encoding has no leading version"), err) }
	var version int
	if err := decoder.Decode(&version); err != nil { return statsSnapshotWrite{}, err }
	if version < 1 || version > 6 { return statsSnapshotWrite{}, errors.New("statistics snapshot encoding version is unsupported") }
...
func writeEncodedStatsSnapshot(path string, data []byte) (resultErr error) {
	...
	if owner != nil { defer func() { resultErr = errors.Join(resultErr, owner.finish()) }() }
	if err != nil { return err }
