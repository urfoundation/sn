...
// complete chain lineage and cross-settlement terminal-ID persistence remain
...
// shape reaches this function (both legacy pointers and terminal closure nil).
...
func releaseMeasurementV2ControlStorage(ctx context.Context, value reflect.Value, remaining *uint64) error {
	...
	case reflect.Pointer:
		...
		if err := charge(1, uint64(value.Type().Elem().Size())); err != nil { return err }
		return releaseMeasurementV2ControlStorage(ctx, value.Elem(), remaining)
	case reflect.Slice:
		if err := charge(uint64(value.Len()), uint64(value.Type().Elem().Size())); err != nil { return err }
		for index := 0; index < value.Len(); index++ {
			if err := releaseMeasurementV2ControlStorage(ctx, value.Index(index), remaining); err != nil { return err }
		}
	case reflect.Struct:
		for index := 0; index < value.NumField(); index++ {
			if err := releaseMeasurementV2ControlStorage(ctx, value.Field(index), remaining); err != nil { return err }
		}
	case reflect.Array:
...
func admitReleaseMeasurementV2ProviderCensus(ctx context.Context, input ReleaseMeasurementInput, options ReleaseMeasurementV2OperatorOptions) (map[connect.Id]bool, error) {
	...
	for _, provider := range input.Stats.Providers {
		...
		for _, encoded := range provider.EgressIPHashHexes {
			if len(encoded) != 66 { return nil, errors.New("compact measurement egress width is invalid") }
		}
...
func ownReleaseMeasurementV2(ctx context.Context, artifact *ReleaseMeasurementArtifact, options ReleaseMeasurementV2Options) (*ReleaseMeasurementArtifact, ReleaseMeasurementV2Options, error) {
	if ctx == nil { return nil, options, errors.New("compact release measurement context is nil") }
	if err := ctx.Err(); err != nil { return nil, options, err }
	...
	if artifact.SettlementClosureV2 != nil {
		return nil, options, errors.New("compact terminal closure integration is not enabled")
	}
	...
	if err := releaseMeasurementV2ControlStorage(ctx, reflect.ValueOf(artifact), &remaining); err != nil { return nil, options, err }
	...
	if err := verifyReleaseMeasurementCommonIdentity(artifact); err != nil { return nil, options, err }
	...
	for index, input := range artifact.Inputs {
		...
		if _, err := admitReleaseMeasurementV2ProviderCensus(ctx, input, operator); err != nil { return nil, options, err }
		...
		if _, err := parseReleaseHex32("compact native cut hash", input.CutNativeBlockHash, false); err != nil { return nil, options, err }
		...
		if _, err := attemptCutV2PolicyDepth(expected, options.Policy); err != nil { return nil, options, err }
		...
		if err := input.AttemptCutV2.Validate(operator.Bounds); err != nil { return nil, options, err }
	}
	...
	if err := json.Unmarshal(encoded, &owned); err != nil { return nil, options, err }
	...
	for noID, operator := range options.Operators {
		...
		for keyID, key := range operator.Measurement.Replay.ServerKeys {
			if len(key) != ed25519.PublicKeySize { return nil, options, errors.New("compact operator server key has invalid width") }
...
func (self *releaseMeasurementV2Checkpoint) visit(record AttemptRecord) error {
	if record.Sequence != self.sequence { return nil }
...
func verifyOwnedReleaseMeasurementV2(ctx context.Context, artifact *ReleaseMeasurementArtifact, options ReleaseMeasurementV2Options, checkpoints map[uint64]*releaseMeasurementV2Checkpoint) (VerifiedReleaseMeasurementV2, error) {
	...
	for _, input := range artifact.Inputs {
		...
		if err != nil { return VerifiedReleaseMeasurementV2{}, err }
	...
	for _, input := range artifact.Inputs {
		...
		if checkpoint := checkpoints[input.NoID]; checkpoint != nil { visit = checkpoint.visit }
		...
		if err != nil { return VerifiedReleaseMeasurementV2{}, fmt.Errorf("compact operator %d: %w", input.NoID, err) }
		...
		for key, prefixes := range projection.HeadPrefixes {
			...
			if destination == nil { return VerifiedReleaseMeasurementV2{}, errors.New("compact replay returned an ineligible fleet") }
			for hash := range prefixes { destination[hash] = true }
		}
	...
	if err != nil { return VerifiedReleaseMeasurementV2{}, err }
	if err := ctx.Err(); err != nil { return VerifiedReleaseMeasurementV2{}, err }
	return VerifiedReleaseMeasurementV2{Decision: decision, ReplayByNO: replayedKVs}, nil
}
...
func VerifyReleaseMeasurementArtifactV2(ctx context.Context, artifact *ReleaseMeasurementArtifact, options ReleaseMeasurementV2Options) (result VerifiedReleaseMeasurementV2, resultErr error) {
	if ctx == nil { return result, errors.New("compact release measurement context is nil") }
	defer func() { resultErr = errors.Join(resultErr, ctx.Err()); if resultErr != nil { result = VerifiedReleaseMeasurementV2{} } }()
	owned, ownedOptions, err := ownReleaseMeasurementV2(ctx, artifact, options)
	if err != nil { return result, err }
	return verifyOwnedReleaseMeasurementV2(ctx, owned, ownedOptions, nil)
}
...
func decodeReleaseMeasurementV2Bytes(encoded []byte, maxBytes uint64) (*ReleaseMeasurementArtifact, error) {
	if maxBytes == 0 || maxBytes > maxReleaseMeasurementArtifactBytes || uint64(len(encoded)) > maxBytes {
	...
	if wire.Schema != ReleaseMeasurementSchemaV2 || len(wire.SettlementClosureV2) != 0 {
		return nil, errors.New("compact measurement schema or terminal representation is unsupported")
	}
	artifact := wire.ReleaseMeasurementArtifact
	if wire.Inputs != nil { artifact.Inputs = make([]ReleaseMeasurementInput, len(wire.Inputs)) }
...
func DecodeReleaseMeasurementArtifactV2(ctx context.Context, encoded []byte, options ReleaseMeasurementV2Options) (*ReleaseMeasurementArtifact, VerifiedReleaseMeasurementV2, error) {
	if ctx == nil { return nil, VerifiedReleaseMeasurementV2{}, errors.New("compact release measurement context is nil") }
	if err := ctx.Err(); err != nil { return nil, VerifiedReleaseMeasurementV2{}, err }
	artifact, err := decodeReleaseMeasurementV2Bytes(encoded, options.MaxArtifactBytes)
	if err != nil { return nil, VerifiedReleaseMeasurementV2{}, err }
...
func SealReleaseMeasurementArtifactV2(ctx context.Context, artifact *ReleaseMeasurementArtifact, options ReleaseMeasurementV2Options) (encoded []byte, contentHash string, resultErr error) {
	if ctx == nil { return nil, "", errors.New("compact release measurement context is nil") }
	defer func() { resultErr = errors.Join(resultErr, ctx.Err()); if resultErr != nil { encoded, contentHash = nil, "" } }()
	owned, ownedOptions, err := ownReleaseMeasurementV2(ctx, artifact, options)
	if err != nil { return nil, "", err }
	if _, err := verifyOwnedReleaseMeasurementV2(ctx, owned, ownedOptions, nil); err != nil { return nil, "", err }
	encoded, err = canonicalReleaseMeasurementBytes(owned)
	if err != nil { return nil, "", err }
	return encoded, ReleaseMeasurementContentHash(encoded), nil
}

// Own only the fields compared by the existing measurement/intent join. Other
// intent transaction, private submission and persistence fields remain the
// outer workflow's responsibility and are not copied or interpreted here.
func ownReleaseMeasurementV2Intent(ctx context.Context, intent *SteeringIntent, maxBytes uint64) (*SteeringIntent, error) {
	...
	remaining := maxBytes
	if err := releaseMeasurementV2ControlStorage(ctx, reflect.ValueOf(owned), &remaining); err != nil { return nil, err }
...
func VerifyReleaseMeasurementIntentV2(ctx context.Context, encoded []byte, options ReleaseMeasurementV2Options, intent *SteeringIntent) error {
	if ctx == nil { return errors.New("compact release measurement context is nil") }
	...
	ownedIntent, err := ownReleaseMeasurementV2Intent(ctx, intent, options.MaxControlBytes)
	...
	if err != nil { return err }
	if err := VerifyReleaseMeasurementIntent(ownedIntent, artifact, result.Decision); err != nil { return err }
	return ctx.Err()
}

// Same-settlement lineage authenticates both source artifacts independently.
// Current replay proves the previous prefix at its actual signed record, not
// merely by comparing roots declared in two headers. No cross-call verdict is
// reused. Full compact terminal integration is a separately required slice.
func VerifyReleaseMeasurementLineageV2(ctx context.Context, previousEncoded []byte, previousOptions ReleaseMeasurementV2Options, current *ReleaseMeasurementArtifact, currentOptions ReleaseMeasurementV2Options) (result VerifiedReleaseMeasurementV2, resultErr error) {
	if ctx == nil { return result, errors.New("compact release lineage context is nil") }
	defer func() { resultErr = errors.Join(resultErr, ctx.Err()); if resultErr != nil { result = VerifiedReleaseMeasurementV2{} } }()
	if err := ctx.Err(); err != nil { return result, err }
	previous, err := decodeReleaseMeasurementV2Bytes(previousEncoded, previousOptions.MaxArtifactBytes)
	if err != nil { return result, err }
	...
	if current.SettlementEpoch != previous.SettlementEpoch {
		return result, errors.New("compact cross-settlement lineage integration is not enabled")
	}
	...
	if len(current.Inputs) != len(previous.Inputs) { return result, errors.New("compact measurement lineage changes operator coverage") }
	...
	for index, input := range current.Inputs {
		...
		if prior.NoID != input.NoID || !maps.Equal(releasePriorQualityState(prior.Stats), releasePriorQualityState(input.Stats)) {
			return result, errors.New("compact measurement lineage changes operator or prior pool EMA")
		}
		oldCut, newCut := prior.AttemptCutV2, input.AttemptCutV2
		if oldCut.Context.Identity != newCut.Context.Identity || oldCut.Context.Activation != newCut.Context.Activation || oldCut.Context.FirstSequence != newCut.Context.FirstSequence || oldCut.Context.PriorRoot != newCut.Context.PriorRoot || oldCut.LastSequence > newCut.LastSequence || !releaseBlockAtOrBefore(prior.CutEVMSnapshotBlock, prior.CutEVMSnapshotHash, input.CutEVMSnapshotBlock, input.CutEVMSnapshotHash) || !releaseBlockAtOrBefore(prior.CutNativeBlock, prior.CutNativeBlockHash, input.CutNativeBlock, input.CutNativeBlockHash) || oldCut.Context.EgressGeneration > newCut.Context.EgressGeneration || oldCut.Context.EgressFirstSequence > newCut.Context.EgressFirstSequence || oldCut.Context.EgressGeneration == newCut.Context.EgressGeneration && oldCut.Context.EgressFirstSequence != newCut.Context.EgressFirstSequence {
			return result, errors.New("compact measurement lineage changes or regresses its cumulative prefix")
		}
		checkpoints[input.NoID] = &releaseMeasurementV2Checkpoint{sequence: oldCut.LastSequence, root: oldCut.Root, seen: oldCut.RecordCount == 0}
	}
	if _, err := verifyOwnedReleaseMeasurementV2(ctx, previous, previousOptions, nil); err != nil { return result, fmt.Errorf("previous compact measurement: %w", err) }
	if err := verifyReleaseMeasurementHeadLineage(previous, current); err != nil { return result, err }
	return verifyOwnedReleaseMeasurementV2(ctx, current, currentOptions, checkpoints)
}
