...
func openAttemptPrivateDirectoryFile(path string) (*os.File, error) {
	...
	if err != nil { return nil, &os.PathError{Op: "open", Path: path, Err: err} }
	...
	for _, component := range strings.Split(strings.TrimPrefix(path, string(filepath.Separator)), string(filepath.Separator)) {
		...
		if openErr != nil { return nil, errors.Join(&os.PathError{Op: "openat", Path: path, Err: openErr}, closeErr) }
		current = os.NewFile(uintptr(next), path)
		if closeErr != nil { return nil, errors.Join(closeErr, current.Close()) }
	}
	return current, nil
}
...
func (self *attemptPrivateDirectory) stat(name string) (attemptPrivateFileState, error) {
	if self == nil || self.file == nil || !validAttemptPrivateLeaf(name) { return attemptPrivateFileState{}, errors.New("private metadata stat has invalid owner or name") }
	var state unix.Stat_t
	if err := unix.Fstatat(int(self.file.Fd()), name, &state, unix.AT_SYMLINK_NOFOLLOW); err != nil { return attemptPrivateFileState{}, &os.PathError{Op: "fstatat", Path: filepath.Join(self.path, name), Err: err} }
...
func (self *attemptPrivateDirectory) openFile(name string, flags int, mode uint32) (*os.File, error) {
	if self == nil || self.file == nil || !validAttemptPrivateLeaf(name) { return nil, errors.New("private metadata open has invalid owner or name") }
	...
	if err != nil { return nil, &os.PathError{Op: "openat", Path: path, Err: err} }
...
func statAttemptPrivateFile(file *os.File) (attemptPrivateFileState, error) {
	if file == nil { return attemptPrivateFileState{}, errors.New("private metadata descriptor is nil") }
	var state unix.Stat_t
	if err := unix.Fstat(int(file.Fd()), &state); err != nil { return attemptPrivateFileState{}, &os.PathError{Op: "fstat", Path: file.Name(), Err: err} }
	return attemptPrivateNativeFileState(&state), nil
}
