...
func oldCanonicalHex32(name, encoded string, zeroAllowed bool) ([32]byte, error) {
	...
	if err != nil || len(decoded) != 32 { return value, fmt.Errorf("%s is not 32-byte hex", name) }
	copy(value[:], decoded)
	if !zeroAllowed && value == ([32]byte{}) { return value, fmt.Errorf("%s is zero", name) }
	return value, nil
}
...
func oldCanonicalContentHash(encoded string) ([32]byte, error) {
	...
	if err != nil || len(decoded) != 32 { return value, errors.New("content hash is invalid") }
	copy(value[:], decoded)
	return value, nil
}
...
func oldCanonicalEnvelopeSignature(encoded string) ([]byte, error) {
	...
	if err != nil || len(signature) != 64 { return nil, errors.New("release measurement envelope signature is invalid") }
...
func canonicalHexRealM8Transition(t *testing.T) (*AttemptSettlementTransition, ed25519.PrivateKey) {
	...
	if _, err := engine.RunTrail(context.Background()); err != nil { t.Fatal(err) }
	participant := AttemptSettlementParticipant{NoID: ledger.identity.NoID, StateDir: stateDir, Stats: stats}
	if err := AdvanceAttemptSettlementEpoch(t.TempDir(), 43, attemptLedgerTestBoundary(), []AttemptSettlementParticipant{participant}); err != nil { t.Fatal(err) }
	...
	for index, record := range cut.Records {
		if record.M != 8 || record.Sequence != uint64(index+1) { t.Fatal("real M8 record depth or sequence changed") }
		if index < 7 && (record.Disposition != AttemptDispositionPending || len(record.Assignments) != index+1) { t.Fatal("real M8 pending checkpoint missing") }
	}
	if err := VerifyAttemptLedgerCut(cut, key.Public().(ed25519.PublicKey), server.serverPublicKeys()); err != nil { t.Fatal(err) }
	if err := VerifyAttemptSettlementTransition(transition); err != nil { t.Fatal(err) }
...
func TestAttemptCanonicalHexRealM8MemberWidthBeforeNormalization(t *testing.T) {
	...
	if err := json.Unmarshal(encoded, &changed); err != nil { t.Fatal(err) }
	...
	if !ed25519.Verify(key.Public().(ed25519.PublicKey), message, changed.Signature) { t.Fatal("changed batch lost its genuine signature") }
	normalizations := 0
	...
	if canonicalHexErrorText(err) != "settlement transition member digest is not canonical 32-byte hex" { t.Fatalf("wrong actual member refusal: %v", err) }
	if normalizations != 0 { t.Fatalf("real M8 settlement digest normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexSharedWidthBeforeNormalization(t *testing.T) {
	...
	if value != ([32]byte{}) || canonicalHexErrorText(err) != "shared is not canonical 32-byte hex" { t.Fatalf("wrong shared refusal: %x %v", value, err) }
	if normalizations != 0 { t.Fatalf("shared hex parser normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexReleaseWidthBeforeNormalization(t *testing.T) {
	...
	if value != ([32]byte{}) || canonicalHexErrorText(err) != "release is not canonical 32-byte hex" { t.Fatalf("wrong release refusal: %x %v", value, err) }
	if normalizations != 0 { t.Fatalf("release hex parser normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexContentWidthBeforeNormalization(t *testing.T) {
	...
	if value != ([32]byte{}) || canonicalHexErrorText(err) != "content hash is not canonical SHA-256" { t.Fatalf("wrong content refusal: %x %v", value, err) }
	if normalizations != 0 { t.Fatalf("content hash parser normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexDepositAddressWidthBeforeNormalization(t *testing.T) {
	...
	if canonicalHexErrorText(err) != "deposit is not a canonical nonzero address" { t.Fatalf("wrong address refusal: %v", err) }
	if normalizations != 0 { t.Fatalf("deposit address parser normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexMeasurementAddressesBeforeNormalization(t *testing.T) {
	...
	for _, field := range []string{"coordinator", "vault"} {
		...
		if field == "coordinator" { artifact.Coordinator = "0x" + strings.Repeat("aB", 64*1024) } else { artifact.SettlementVault = "0x" + strings.Repeat("aB", 64*1024) }
		err := verifyReleaseMeasurementIdentityWithHexWork(artifact, canonicalHexOversizeWork(&normalizations, 42))
		if canonicalHexErrorText(err) != "release measurement contract identity is invalid" { t.Fatalf("%s refused before actual address boundary: %v", field, err) }
	}
	if normalizations != 0 { t.Fatalf("measurement contract addresses normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexEnvelopeAddressesBeforeNormalization(t *testing.T) {
	...
	for _, field := range []string{"coordinator", "vault"} {
		envelope := &ReleaseMeasurementEnvelope{Schema: ReleaseMeasurementEnvelopeSchema, MeasurementSchema: ReleaseMeasurementSchema,
			...
			Coordinator: "0x" + strings.Repeat("ab", 20), SettlementVault: "0x" + strings.Repeat("cd", 20)}
		if field == "coordinator" { envelope.Coordinator = "0x" + strings.Repeat("aB", 64*1024) } else { envelope.SettlementVault = "0x" + strings.Repeat("aB", 64*1024) }
		err := validateReleaseMeasurementEnvelopeWithHexWork(envelope, canonicalHexOversizeWork(&normalizations, 42))
		if canonicalHexErrorText(err) != "release measurement envelope contract identity is invalid" { t.Fatalf("%s refused before actual envelope boundary: %v", field, err) }
	}
	if normalizations != 0 { t.Fatalf("envelope contract addresses normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexEnvelopeSignatureBeforeNormalization(t *testing.T) {
	...
	if value != nil || canonicalHexErrorText(err) != "release measurement envelope signature is not canonical 64-byte hex" { t.Fatalf("wrong signature refusal: %x %v", value, err) }
	if normalizations != 0 { t.Fatalf("envelope signature parser normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexPreparedWidthBeforeNormalization(t *testing.T) {
	...
	if _, err := parseReleaseHex32("prepared", normalized, false); err == nil { t.Fatal("oversized normalized prepared hash was admitted") }
	if normalizations != 0 { t.Fatalf("prepared hash boundary normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexStatsEgressBeforeNormalization(t *testing.T) {
	...
	if canonicalHexErrorText(err) != want || !reflect.DeepEqual(verified, VerifiedReleaseStats{}) { t.Fatalf("wrong egress boundary refusal: %v %+v", err, verified) }
	if normalizations != 0 { t.Fatalf("statistics egress parser normalized oversized input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexContentPreservesOldBytesAndErrors(t *testing.T) {
	...
	for _, encoded := range canonicalHexVectors("sha256:", 32) {
		...
		if got != want || canonicalHexErrorText(gotErr) != canonicalHexErrorText(wantErr) { t.Fatalf("content oracle changed for %x: %x/%v vs %x/%v", encoded, got, gotErr, want, wantErr) }
	}
}
...
func TestAttemptCanonicalHexSignaturePreservesOldBytesAndErrors(t *testing.T) {
	...
	for _, encoded := range canonicalHexVectors("0x", 64) {
		...
		if !bytes.Equal(got, want) || (got == nil) != (want == nil) || canonicalHexErrorText(gotErr) != canonicalHexErrorText(wantErr) { t.Fatalf("signature oracle changed for %x: %x/%v vs %x/%v", encoded, got, gotErr, want, wantErr) }
	}
}
...
func TestAttemptCanonicalHexAddressesPreserveOldForms(t *testing.T) {
	...
	old := func(encoded string) error {
		if encoded != strings.ToLower(encoded) || !common.IsHexAddress(encoded) || common.HexToAddress(encoded) == (common.Address{}) { return errors.New("deposit is not a canonical nonzero address") }
		return nil
	}
	for _, prefix := range []string{"", "0x"} {
		for _, encoded := range canonicalHexVectors(prefix, 20) {
			...
			if canonicalHexErrorText(got) != canonicalHexErrorText(want) { t.Fatalf("address oracle changed for %x: %v vs %v", encoded, got, want) }
		}
	}
	...
	if verifyCanonicalDepositAddress("deposit", bare) != nil || verifyCanonicalDepositAddress("deposit", "0x"+bare) != nil ||
		common.HexToAddress(bare) != common.HexToAddress("0x"+bare) { t.Fatal("original 40/42 address acceptance changed") }
}
...
func TestAttemptCanonicalHexObserverRunsActualNormalization(t *testing.T) {
	...
	for _, encoded := range []string{"aB", "İ", "K", "Σ", "\xffA", "0xABC", ""} {
		...
		if got != strings.ToLower(encoded) || !reflect.DeepEqual(lengths, []int{len(encoded)}) { t.Fatalf("observer replaced actual normalization for %x: %x %v", encoded, got, lengths) }
	}
}
...
func TestAttemptCanonicalHexObserversAreInvocationLocal(t *testing.T) {
	...
	work := canonicalHexWork{normalized: func(int) {
		first++
		if _, err := canonicalAttemptHex32WithWork("nested", releaseHex32([32]byte{2}), false, other); err != nil { t.Fatal(err) }
	}}
	for index := 0; index < 3; index++ {
		if _, err := parseReleaseContentHashWithWork("sha256:"+strings.Repeat("ab", 32), work); err != nil { t.Fatal(err) }
	}
	if first != 3 || second != 3 { t.Fatalf("normalization escaped call ownership: first=%d second=%d", first, second) }
	if _, err := canonicalAttemptHex32("standalone", releaseHex32([32]byte{1}), false); err != nil { t.Fatal(err) }
	if first != 3 || second != 3 { t.Fatal("standalone call reused another observer") }
}
...
func TestAttemptCanonicalHexDecodedOutputsRemainOwned(t *testing.T) {
	...
	if err != nil || second[0] != 0xab { t.Fatal("signature decode reused mutable output") }
	...
	if err != nil || again[0] != 9 { t.Fatal("hash decode reused mutable output") }
	for _, bad := range []string{strings.ToUpper(hash), hash + "0", "0x" + strings.Repeat("g", 64)} {
		if _, err := canonicalAttemptHex32("owned", bad, false); err == nil { t.Fatal("changed input reused prior admission") }
	}
}
...
func TestAttemptCanonicalHexRealM8RetainsFullVerification(t *testing.T) {
	...
	for _, mutation := range []string{"signature", "member", "record", "postfold", "identity"} {
		...
		if err := json.Unmarshal(encoded, &changed); err != nil { t.Fatal(err) }
		switch mutation {
		case "signature": changed.Signature[0] ^= 1
		case "member": changed.Batch[0].Digest = releaseHex32([32]byte{0xab})
		case "record": changed.PreFold.AttemptCut.Records[0].Signature[0] ^= 1
		case "postfold": changed.PostFold[0].QualityPPM ^= 1
		case "identity": changed.Identity.ValidatorID++
		}
		if err := VerifyAttemptSettlementTransition(&changed); err == nil { t.Fatalf("full terminal check bypassed for %s", mutation) }
	}
	if err := VerifyAttemptSettlementBatch([]*AttemptSettlementTransition{transition}); err != nil { t.Fatal(err) }
	after, err := json.Marshal(transition)
	if err != nil || !bytes.Equal(encoded, after) { t.Fatal("verification mutated real M8 input") }
}
...
func TestAttemptCanonicalHexPublicAddressFormsRemainValid(t *testing.T) {
	...
	for _, bare := range []bool{false, true} {
		...
		if err := verifyReleaseMeasurementIdentity(&changedArtifact); err != nil { t.Fatalf("old artifact address form bare=%t changed: %v", bare, err) }
		resignReleaseMeasurementEnvelope(t, &envelope, hotkey)
		if err := validateReleaseMeasurementEnvelope(&envelope); err != nil { t.Fatalf("old signed envelope address form bare=%t changed: %v", bare, err) }
		if common.HexToAddress(changedArtifact.Coordinator) != common.HexToAddress(artifact.Coordinator) ||
			common.HexToAddress(envelope.SettlementVault) != common.HexToAddress(original.SettlementVault) { t.Fatal("address representation changed decoded bytes") }
	}
	for _, encoded := range []string{
		...
		for _, vault := range []bool{false, true} {
			changedArtifact := *artifact
			envelope := *original
			if vault { changedArtifact.SettlementVault = encoded; envelope.SettlementVault = encoded } else { changedArtifact.Coordinator = encoded; envelope.Coordinator = encoded }
			artifactErr := verifyReleaseMeasurementIdentity(&changedArtifact)
			resignReleaseMeasurementEnvelope(t, &envelope, hotkey)
			envelopeErr := validateReleaseMeasurementEnvelope(&envelope)
			if oldBad {
				if canonicalHexErrorText(artifactErr) != "release measurement contract identity is invalid" || canonicalHexErrorText(envelopeErr) != "release measurement envelope contract identity is invalid" { t.Fatalf("old address rejection changed: bytes=%x vault=%t artifact=%v envelope=%v", encoded, vault, artifactErr, envelopeErr) }
			} else if artifactErr != nil || envelopeErr != nil { t.Fatalf("old address accepted form changed: bytes=%x vault=%t artifact=%v envelope=%v", encoded, vault, artifactErr, envelopeErr) }
		}
	}
}
...
func TestAttemptCanonicalHexPreparedPublicCaseAndErrors(t *testing.T) {
	...
	for _, expected := range []string{releaseMeasurementEnvelopeTestPreparedHash, strings.ToUpper(releaseMeasurementEnvelopeTestPreparedHash)} {
		if _, _, err := VerifyReleaseMeasurementEnvelope(envelope, measurement, hotkey.PublicKey(), uid, expected); err != nil { t.Fatalf("accepted expected-hash case changed: %x %v", expected, err) }
	}
	for _, invalid := range []string{
		...
		if _, _, _, err := SealReleaseMeasurementEnvelope(measurement, uid, hotkey, invalid, signedAt); canonicalHexErrorText(err) != "release measurement envelope prepared extrinsic hash is not canonical" { t.Fatalf("sealer prepared-hash error changed for %x: %v", invalid, err) }
		if invalid == strings.ToUpper(releaseMeasurementEnvelopeTestPreparedHash) { continue }
		if _, _, err := VerifyReleaseMeasurementEnvelope(envelope, measurement, hotkey.PublicKey(), uid, invalid); canonicalHexErrorText(err) != "release measurement envelope prepared extrinsic hash differs" { t.Fatalf("expected-hash mismatch changed for %x: %v", invalid, err) }
	}
}
...
func TestAttemptCanonicalHexIntentPreparedRulesRemainDistinct(t *testing.T) {
	...
	if err := verify(&intent); err != nil { t.Fatal(err) }
	for _, mutation := range []string{"uppercase extrinsic", "oversized extrinsic", "uppercase hotkey", "oversized hotkey"} {
		...
		switch mutation {
		case "uppercase extrinsic": prepared.ExtrinsicHash = strings.ToUpper(prepared.ExtrinsicHash)
		case "oversized extrinsic": prepared.ExtrinsicHash = "0x" + strings.Repeat("aB", 1024)
		case "uppercase hotkey": prepared.HotkeyHex = strings.ToUpper(prepared.HotkeyHex)
		case "oversized hotkey": prepared.HotkeyHex = "0x" + strings.Repeat("aB", 1024)
		}
		err := verify(&changed)
		switch mutation {
		case "uppercase extrinsic":
			if err != nil { t.Fatalf("intent stopped accepting expected uppercase extrinsic: %v", err) }
		case "oversized extrinsic":
			if canonicalHexErrorText(err) != "release measurement envelope prepared extrinsic hash differs" { t.Fatalf("intent extrinsic error changed: %v", err) }
		default:
			if canonicalHexErrorText(err) != "steering intent prepared validator hotkey is not canonical" { t.Fatalf("intent canonical hotkey error changed: %v", err) }
		}
	}
	if err := verify(&intent); err != nil { t.Fatalf("invalid intent damaged valid retry: %v", err) }
}
...
func TestAttemptCanonicalHexStatsPreservesOrderZeroAndErrors(t *testing.T) {
	...
	old := func(hashes []string) error {
		...
		for index, encoded := range hashes {
			if encoded != strings.ToLower(encoded) || len(encoded) != 66 || !strings.HasPrefix(encoded, "0x") || (prior != "" && encoded <= prior) { return fmt.Errorf("provider %s egress hash %d is not canonical", clientID, index) }
			decoded, err := hex.DecodeString(encoded[2:])
			if err != nil || len(decoded) != 32 { return fmt.Errorf("provider %s egress hash %d is invalid", clientID, index) }
			var hash [32]byte
			copy(hash[:], decoded)
			if hash == ([32]byte{}) { return fmt.Errorf("provider %s contains the zero egress hash", clientID) }
			prior = encoded
		}
		return nil
	}
	for _, hashes := range [][]string{
		...
		wantErr := old(hashes)
		if canonicalHexErrorText(gotErr) != canonicalHexErrorText(wantErr) { t.Fatalf("egress oracle changed for %x: got=%v old=%v", hashes, gotErr, wantErr) }
		if wantErr != nil {
			if !reflect.DeepEqual(got, VerifiedReleaseStats{}) { t.Fatal("invalid egress published partial scores") }
		} else {
			provider := got.Providers[connect.Id{1}]
			if provider.HasQuality || provider.QualityPPM != 0 || len(provider.EgressIPHashes) != len(hashes) { t.Fatal("valid sparse provider semantics changed") }
		}
	}
}
...
func TestAttemptCanonicalHexActualEntrypointEdges(t *testing.T) {
	...
	for _, edge := range []struct { file, function, callee string }{
		...
		{file: "release_measurement_envelope.go", function: "validateReleaseMeasurementEnvelopeWithHexWork", callee: "lower"},
		{file: "release_measurement_envelope.go", function: "validateReleaseMeasurementEnvelopeWithHexWork", callee: "Verify"},
		...
		for _, declaration := range file.Decls {
			...
			if !ok || function.Name.Name != edge.function { continue }
			ast.Inspect(function.Body, func(node ast.Node) bool {
				call, ok := node.(*ast.CallExpr)
				if !ok { return true }
				switch callee := call.Fun.(type) {
				case *ast.Ident: found = found || callee.Name == edge.callee
				case *ast.SelectorExpr: found = found || callee.Sel.Name == edge.callee
				}
				return true
			})
		}
		if !found { t.Errorf("actual canonical parser edge missing: %s %s -> %s", edge.file, edge.function, edge.callee) }
	}
}
...
func TestAttemptCanonicalHexSealerPreparedBeforeNormalization(t *testing.T) {
	...
	if raw != nil || hash != "" || envelope != nil || canonicalHexErrorText(err) != "release measurement envelope prepared extrinsic hash is not canonical" { t.Fatalf("wrong actual sealer refusal: %v", err) }
	if normalizations != 0 { t.Fatalf("measurement sealer normalized oversized prepared input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexVerifierPreparedBeforeNormalization(t *testing.T) {
	...
	if artifact != nil || verified != nil || canonicalHexErrorText(err) != "release measurement envelope prepared extrinsic hash differs" { t.Fatalf("wrong actual verifier refusal: %v", err) }
	if normalizations != 0 { t.Fatalf("measurement verifier normalized oversized expected input: got %d, want exactly 0", normalizations) }
}
...
func TestAttemptCanonicalHexIntentPreparedBeforeNormalization(t *testing.T) {
	...
	for _, field := range []string{"hotkey", "extrinsic"} {
		...
		if field == "hotkey" { prepared.HotkeyHex = "0x"+strings.Repeat("aB", 64*1024) } else {
		...
		if canonicalHexErrorText(err) != want { t.Fatalf("wrong actual intent %s refusal: %v", field, err) }
	}
	if normalizations != 0 { t.Fatalf("durable intent normalized oversized prepared fields: got %d, want exactly 0", normalizations) }
}
