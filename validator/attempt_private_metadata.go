...
func readAttemptPrivateMetadata(ctx context.Context, root *attemptPrivateDirectory, name string, limit uint64, physical attemptPrivateMetadataReadIO) (data []byte, exists bool, resultErr error) {
	...
	defer func() {
		...
		if resultErr != nil { data = nil }
	}()
	if err := ctx.Err(); err != nil { return nil, false, err }
	if err := root.check(); err != nil { return nil, false, err }
	before, err := root.stat(name)
	if errors.Is(err, os.ErrNotExist) { return nil, false, root.check() }
	...
	if physical.step != nil {
		if err := physical.step("before-open"); err != nil { return nil, true, err }
	}
	if err := ctx.Err(); err != nil { return nil, true, err }
	...
	defer func() {
		...
		if statErr != nil || before != afterClose { resultErr = errors.Join(resultErr, errors.New("private metadata changed through close"), statErr) }
		resultErr = errors.Join(resultErr, root.check())
		if resultErr != nil { data = nil }
	}()
	opened, err := statAttemptPrivateFile(file)
	if err != nil || before != opened { return nil, true, errors.Join(errors.New("private metadata changed during open"), err) }
	if physical.step != nil {
		if err := physical.step("after-open"); err != nil { return nil, true, err }
	}
	if err := ctx.Err(); err != nil { return nil, true, err }
	data, err = io.ReadAll(io.LimitReader(file, before.size+1))
	if err != nil || int64(len(data)) != before.size { return nil, true, errors.Join(errors.New("private metadata read length changed"), err) }
	if physical.step != nil {
		if err := physical.step("after-read"); err != nil { return nil, true, err }
	}
	after, err := root.stat(name)
	if err != nil || before != after { return nil, true, errors.Join(errors.New("private metadata changed or disappeared during read"), err) }
	final, err := statAttemptPrivateFile(file)
	if err != nil || before != final { return nil, true, errors.Join(errors.New("private metadata descriptor changed during read"), err) }
	if err := root.check(); err != nil { return nil, true, err }
