...
func readReleaseMeasurementInputV2(path string, limit uint64) (encoded []byte, resultErr error) {
	...
	if !before.Mode().IsRegular() || before.Mode().Perm()&0o077 != 0 || before.Size() < 0 || uint64(before.Size()) > limit {
	...
	after, err := file.Stat()
	if err != nil || after.Size() != before.Size() || int64(len(encoded)) != before.Size() {
	...
	if err != nil || !current.Mode().IsRegular() || !os.SameFile(before, current) {
		return nil, errors.Join(errors.New("compact input journal pathname changed during its read"), err)
	}
	return encoded, nil
}
...
func (self *ReleaseSteerer) loadOrDetachReleaseMeasurementInputV2(ctx context.Context, noID, subnetEpoch, nativeBlock uint64, nativeHash string, snapshot *ReleaseSnapshot, options releaseMeasurementInputV2Options) (ReleaseMeasurementInput, error) {
	...
	if err == nil {
		...
		if err := syncReleaseMeasurementInputV2Directory(path); err != nil {
			return ReleaseMeasurementInput{}, err
		}
		if err := measurement.Stats.reconcileReleaseStatsCutV2(ctx, measurementStateDir(self.cfg, noID), input.Stats, *cut, options.Stats); err != nil {
