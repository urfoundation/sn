...
func (self *ReleaseSteerer) loadOrDetachReleaseMeasurementInputV2WithReadHooks(ctx context.Context, noID, subnetEpoch, nativeBlock uint64, nativeHash string, snapshot *ReleaseSnapshot, options releaseMeasurementInputV2Options, hooks releaseMeasurementInputV2ReadHooks) (result ReleaseMeasurementInput, resultErr error) {
	...
	defer func() {
		...
		if resultErr != nil { result = ReleaseMeasurementInput{} }
	}()
	if err != nil { return ReleaseMeasurementInput{}, err }
	guard := func(phase string) error {
		if phase == "before-save" { return owner.check() }
		if phase == "after-save" { return errors.Join(owner.check(), owner.finish(), ctx.Err()) }
	...
	if err == nil {
		...
		if err := owner.sync(); err != nil { return ReleaseMeasurementInput{}, err }
		if err := measurement.Stats.reconcileReleaseStatsCutV2WithJournalGuard(ctx, measurementStateDir(self.cfg, noID), input.Stats, *cut, options.Stats, guard); err != nil { return ReleaseMeasurementInput{}, fmt.Errorf("reconcile compact input: %w", err) }
		return input, nil
	}
	if !owner.initialMissing || !releaseMeasurementInputV2OnlyMissing(err) { return ReleaseMeasurementInput{}, err }
	if err := owner.check(); err != nil { return ReleaseMeasurementInput{}, err }
