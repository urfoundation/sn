...
func (self attemptSettlementV2IO) validate() error {
	if self.writeSnapshot == nil || self.writeJournal == nil || self.syncDirectory == nil || self.closeFile == nil || self.closeRoot == nil || self.removeJournal == nil { return errors.New("compact settlement durability operations are incomplete") }
	return nil
}
...
func openAttemptSettlementV2Root(path string) (*attemptPrivateDirectory, error) {
	...
	if root.anchor.mode&0o077 != 0 || root.anchor.uid != uint32(os.Geteuid()) { return nil, errors.Join(errors.New("compact settlement state directory is not private and owned"), root.close()) }
	if err := root.check(); err != nil { return nil, errors.Join(err, root.close()) }
	return root, nil
}
...
func retainAttemptSettlementV2Roots(coordinator string, participants []AttemptSettlementRuntimeV2Participant, physical attemptSettlementV2IO) (attemptSettlementV2IO, error) {
	...
	for _, participant := range participants { paths = append(paths, participant.StateDir) }
	for _, path := range paths {
		root, err := openAttemptSettlementV2Root(path)
		if err != nil { return physical, errors.Join(err, physical.closeRoots()) }
		physical.roots[path] = root
		if err := physical.retainWitness(root); err != nil { return physical, errors.Join(err, physical.closeRoots()) }
	}
	...
	} else if !errors.Is(err, os.ErrNotExist) { return physical, errors.Join(err, physical.closeRoots()) }
	if err := physical.witnessLeaf(physical.roots[coordinator], "settlement-transaction.json", attemptSettlementV2LeafWitness{}); err != nil { return physical, errors.Join(err, physical.closeRoots()) }
	for _, participant := range participants {
		if participant.Ledger.directory == nil { return physical, errors.Join(errors.New("compact participant lost its physical directory owner"), physical.closeRoots()) }
		opened, err := physical.roots[participant.StateDir].file.Stat()
		if err != nil || !os.SameFile(opened, participant.Ledger.directory.anchor) { return physical, errors.Join(errors.New("compact participant directory differs from retained ledger"), err, physical.closeRoots()) }
	}
	return physical, nil
}
...
func (self attemptSettlementV2IO) closeRoots() error {
	...
	for path := range self.roots { paths = append(paths, path) }
	slices.Sort(paths)
	...
	if self.witnesses != nil {
		for _, root := range self.witnesses.owners { resultErr = errors.Join(resultErr, self.closeOwnedRoot(root)) }
...
func (self attemptSettlementV2IO) checkRoots() error {
	var resultErr error
	for _, root := range self.roots { resultErr = errors.Join(resultErr, root.check()) }
	return resultErr
}
...
func (self attemptSettlementV2IO) openRoot(path string) (*attemptPrivateDirectory, func() error, error) {
	if root := self.roots[path]; root != nil {
		if err := root.check(); err != nil { return nil, nil, err }
		return root, func() error { return nil }, nil
	}
	root, err := openAttemptSettlementV2Root(path)
	if err != nil { return nil, nil, err }
	if err := self.retainWitness(root); err != nil { return nil, nil, errors.Join(err, self.closeOwnedRoot(root)) }
...
func writeAttemptSettlementV2OwnedState(root *attemptPrivateDirectory, name string, data []byte) (resultErr error) {
	if root == nil || name != "stats.json" && name != "settlement-transaction-v2.json" { return errors.New("compact mutable state target is not owned metadata") }
	if err := root.check(); err != nil { return err }
	before, err := root.stat(name)
	if err == nil {
		if !before.regular() || before.mode&0o077 != 0 || before.links != 1 || before.uid != uint32(os.Geteuid()) { return errors.New("compact mutable state target is aliased or not private") }
	...
	defer func() { if err := unix.Unlinkat(int(root.file.Fd()), temporary, 0); err != nil && !errors.Is(err, os.ErrNotExist) { resultErr = errors.Join(resultErr, err) } }()
	written, writeErr := file.Write(data)
	if written != len(data) && writeErr == nil { writeErr = io.ErrShortWrite }
	if err := errors.Join(writeErr, file.Sync(), file.Close()); err != nil { return err }
	if err := root.check(); err != nil { return err }
	if err := unix.Renameat(int(root.file.Fd()), temporary, int(root.file.Fd()), name); err != nil { return &os.LinkError{Op: "renameat", Old: temporary, New: name, Err: err} }
...
func writeAttemptSettlementV2State(path string, data []byte) (resultErr error) {
	if err := validateAttemptSettlementV2Paths([]string{filepath.Dir(path)}); err != nil { return err }
...
func AttemptSettlementClosureV2Path(coordinator string, epoch uint64) string { return filepath.Join(coordinator, "settlement-closures-v2", strconv.FormatUint(epoch, 10)+".json") }
...
func openAttemptSettlementOwnedClosureDirectory(coordinator, name string, create bool, physical attemptSettlementV2IO) (result *attemptPrivateDirectory, resultErr error) {
	if name != "settlement-closures-v2" && name != "settlement-closures" { return nil, errors.New("settlement closure namespace is not owned") }
	if name == "settlement-closures-v2" { if err := validateAttemptSettlementV2Paths([]string{coordinator}); err != nil { return nil, err } }
	parent, release, err := physical.openRoot(coordinator)
	if err != nil { return nil, err }
	defer func() {
		resultErr = errors.Join(resultErr, release())
		if resultErr != nil && result != nil { resultErr = errors.Join(resultErr, physical.closeOwnedRoot(result)); result = nil }
	}()
	if create {
		if err := unix.Mkdirat(int(parent.file.Fd()), name, 0o700); err != nil && !errors.Is(err, os.ErrExist) { return nil, err }
		if err := physical.syncDirectory(parent.file); err != nil { return nil, err }
	}
	before, err := parent.stat(name)
	if err != nil || !before.directory() || before.mode&0o077 != 0 || before.uid != uint32(os.Geteuid()) { return nil, errors.Join(errors.New("compact closure directory is not private and owned"), err) }
	if physical.step != nil { if err := physical.step("before-closure-directory-open"); err != nil { return nil, err } }
	file, err := parent.openFile(name, unix.O_RDONLY|unix.O_DIRECTORY, 0)
	if err != nil { return nil, err }
	opened, err := statAttemptPrivateFile(file)
	if err != nil || before.dev != opened.dev || before.ino != opened.ino || before.mode != opened.mode || before.uid != opened.uid { return nil, errors.Join(errors.New("compact closure directory changed during open"), err, physical.closeFile(file)) }
	child := &attemptPrivateDirectory{file: file, path: filepath.Join(parent.path, name), anchor: opened}
	if err := errors.Join(physical.retainWitness(child), parent.check(), child.check()); err != nil { return nil, errors.Join(err, physical.closeOwnedRoot(child)) }
	return child, nil
}
...
func readAttemptSettlementV2File(root *attemptPrivateDirectory, name string, limit uint64, physical attemptSettlementV2IO) ([]byte, bool, error) {
	if err := validateAttemptSettlementV2MetadataLimit(limit); err != nil { return nil, false, err }
	if root == nil || !validAttemptPrivateLeaf(name) { return nil, false, errors.New("compact metadata read ownership is invalid") }
	before, statErr := root.stat(name)
	if statErr != nil && !errors.Is(statErr, os.ErrNotExist) { return nil, false, statErr }
	...
	if err != nil { return nil, exists, err }
	after, afterErr := root.stat(name)
	if exists != expected.exists || exists && (afterErr != nil || after != before) || !exists && !errors.Is(afterErr, os.ErrNotExist) { return nil, exists, errors.Join(errors.New("compact metadata witness changed around read"), afterErr) }
	if err := physical.witnessLeaf(root, name, expected); err != nil { return nil, exists, err }
...
func publishAttemptSettlementOwnedClosure(coordinator, namespace string, epoch uint64, encoded []byte, maxBytes uint64, physical attemptSettlementV2IO) (resultErr error) {
	if err := validateAttemptSettlementV2MetadataLimit(maxBytes); err != nil { return err }
	if uint64(len(encoded)) > maxBytes { return errors.New("compact settlement closure exceeds its existing byte bound") }
	...
	if err != nil { return err }
	if exists {
		if !bytes.Equal(prior, encoded) { return errors.New("compact immutable settlement closure differs from the transaction") }
	...
	if written != len(encoded) && writeErr == nil { writeErr = io.ErrShortWrite }
	if err := errors.Join(writeErr, file.Sync(), physical.closeFile(file)); err != nil { return err }
	if err := root.check(); err != nil { return err }
	if err := renameAttemptSettlementClosure(int(root.file.Fd()), temporary, name); err != nil { return fmt.Errorf("publish immutable compact settlement closure: %w", err) }
	if err := errors.Join(physical.syncDirectory(root.file), root.check()); err != nil { return err }
	actual, exists, err := readAttemptSettlementV2File(root, name, maxBytes, physical)
	if err != nil || !exists || !bytes.Equal(actual, encoded) { return errors.Join(errors.New("compact published closure differs from its exact image"), err) }
	return nil
}
...
func removeAttemptSettlementV2OwnedTransaction(root *attemptPrivateDirectory, name string) error {
	if root == nil || name != "settlement-transaction-v2.json" { return errors.New("compact transaction removal name is invalid") }
	if err := root.check(); err != nil { return err }
	if err := unix.Unlinkat(int(root.file.Fd()), name, 0); err != nil && !errors.Is(err, os.ErrNotExist) { return err }
	return errors.Join(root.file.Sync(), root.check())
}
...
func readAttemptSettlementClosureV2(ctx context.Context, coordinator string, epoch uint64, authority AttemptSettlementV2Options, physical attemptSettlementV2IO) (closure *AttemptSettlementClosureV2, verified VerifiedAttemptSettlementV2, resultErr error) {
	if ctx == nil { return nil, verified, errors.New("compact settlement read context is nil") }
	if err := validateAttemptSettlementV2Paths([]string{coordinator}); err != nil { return nil, verified, err }
	for _, operator := range authority.Operators {
		if err := validateAttemptSettlementV2Paths([]string{operator.Measurement.Replay.ScratchDirectory}); err != nil { return nil, verified, err }
	}
	if err := physical.validate(); err != nil { return nil, verified, err }
	if err := validateAttemptSettlementV2MetadataLimit(authority.MaxClosureBytes); err != nil { return nil, verified, err }
	owned, err := ownAttemptSettlementV2Options(ctx, authority)
	if err != nil { return nil, verified, err }
	physical = physical.withWitnesses(ctx)
	defer func() { resultErr = errors.Join(resultErr, physical.closeRoots(), ctx.Err()); if resultErr != nil { closure, verified = nil, VerifiedAttemptSettlementV2{} } }()
	...
	if err != nil || !exists { return nil, verified, errors.Join(errors.New("compact immutable closure is missing or unreadable"), err) }
	closure, verified, err = DecodeAttemptSettlementClosureV2(ctx, encoded, owned)
	if err != nil || closure.Epoch != epoch { return nil, VerifiedAttemptSettlementV2{}, errors.Join(errors.New("compact closure epoch or replay differs"), err) }
