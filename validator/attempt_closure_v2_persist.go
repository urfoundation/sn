...
func (self attemptSettlementV2IO) closeRoots() error {
	...
	for index := len(paths)-1; index >= 0; index-- {
		path := paths[index]
		resultErr = errors.Join(resultErr, self.closeRoot(self.roots[path]))
...
func (self attemptSettlementV2IO) openRoot(path string) (*attemptPrivateDirectory, func() error, error) {
	...
	return root, func() error { return self.closeRoot(root) }, nil
}
...
func writeAttemptSettlementV2State(path string, data []byte) (resultErr error) {
	root, err := openAttemptSettlementV2Root(filepath.Dir(path))
	if err != nil { return err }
	defer func() { resultErr = errors.Join(resultErr, root.close()) }()
	return writeAttemptSettlementV2OwnedState(root, filepath.Base(path), data)
}
...
func openAttemptSettlementOwnedClosureDirectory(coordinator, name string, create bool, physical attemptSettlementV2IO) (result *attemptPrivateDirectory, resultErr error) {
	...
	defer func() {
		...
		if resultErr != nil && result != nil { resultErr = errors.Join(resultErr, physical.closeRoot(result)); result = nil }
	}()
	if create {
	...
	if err := errors.Join(parent.check(), child.check()); err != nil { return nil, errors.Join(err, physical.closeRoot(child)) }
	return child, nil
}
...
func readAttemptSettlementV2File(root *attemptPrivateDirectory, name string, limit uint64, physical attemptSettlementV2IO) ([]byte, bool, error) {
	return readAttemptPrivateMetadata(context.Background(), root, name, limit, attemptPrivateMetadataReadIO{closeFile: physical.closeFile, step: physical.step})
}
...
func publishAttemptSettlementOwnedClosure(coordinator, namespace string, epoch uint64, encoded []byte, maxBytes uint64, physical attemptSettlementV2IO) (resultErr error) {
	if maxBytes == 0 || uint64(len(encoded)) > maxBytes { return errors.New("compact settlement closure exceeds its existing byte bound") }
	root, err := openAttemptSettlementOwnedClosureDirectory(coordinator, namespace, true, physical)
	if err != nil { return err }
	defer func() { resultErr = errors.Join(resultErr, physical.closeRoot(root)) }()
...
func removeAttemptSettlementTransactionV2(path string) (resultErr error) {
	...
	if err != nil { return err }
	defer func() { resultErr = errors.Join(resultErr, root.close()) }()
	return removeAttemptSettlementV2OwnedTransaction(root, filepath.Base(path))
}
...
func ReadAttemptSettlementClosureV2(ctx context.Context, coordinator string, epoch uint64, authority AttemptSettlementV2Options) (closure *AttemptSettlementClosureV2, verified VerifiedAttemptSettlementV2, resultErr error) {
	...
	if err != nil { return nil, verified, err }
	defer func() { resultErr = errors.Join(resultErr, root.check(), physical.closeRoot(root), ctx.Err()); if resultErr != nil { closure, verified = nil, VerifiedAttemptSettlementV2{} } }()
