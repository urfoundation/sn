...
func readAttemptSettlementV2Path(path string, limit uint64, physical attemptSettlementV2IO) (data []byte, exists bool, resultErr error) {
	if err := validateAttemptSettlementV2Paths([]string{filepath.Dir(path)}); err != nil { return nil, false, err }
	...
	if physical.witnesses == nil && physical.roots == nil {
		...
		defer func() { resultErr = errors.Join(resultErr, physical.closeRoots()); if resultErr != nil { data = nil } }()
	}
...
func decodeAttemptSettlementV2Stats(ctx context.Context, encoded []byte, config StatsConfig, limit uint64) (*StatsEngine, error) {
	if ctx == nil { return nil, errors.New("compact snapshot decode context is nil") }
	if err := validateAttemptSettlementV2MetadataLimit(limit); err != nil { return nil, err }
	if uint64(len(encoded)) > limit { return nil, errors.New("compact nested snapshot exceeds its independent byte allowance") }
...
func newAttemptSettlementTransactionV2(ctx context.Context, kind string, epoch uint64, batch *attemptSettlementV2Owners, postimages []*StatsEngine, closure *AttemptSettlementClosureV2, authority AttemptSettlementV2Options, physical attemptSettlementV2IO) (*attemptSettlementTransactionV2, error) {
	...
	for index, participant := range batch.ordered {
		...
		if exists {
			...
			if err != nil { return nil, err }
			if err := validateAttemptSettlementV2Original(ctx, originalStats, batch.candidates[index], batch.persistence.MaxSnapshotBytes); err != nil { return nil, err }
			if kind == "advance" {
				if err := validateAttemptSettlementV2LedgerState(ctx, participant, originalStats, authority.Operators[participant.NoID], batch.persistence.MaxSnapshotBytes); err != nil { return nil, err }
			}
...
func decodeAttemptSettlementTransactionV2(ctx context.Context, encoded []byte, participants []AttemptSettlementRuntimeV2Participant, persistence AttemptSettlementRuntimeV2PersistenceBounds) (*attemptSettlementTransactionV2, []*StatsEngine, []*StatsEngine, error) {
	if ctx == nil { return nil, nil, nil, errors.New("compact transaction decode context is nil") }
	if err := persistence.validate(); err != nil { return nil, nil, nil, err }
	if uint64(len(encoded)) > persistence.MaxJournalBytes { return nil, nil, nil, errors.New("compact journal exceeds its independent byte allowance") }
	if err := ctx.Err(); err != nil { return nil, nil, nil, err }
	...
	for index, entry := range transaction.Snapshots {
		...
		if len(entry.OriginalJSON) != 0 {
			...
			if err != nil { return nil, nil, nil, err }
			if err := validateAttemptSettlementV2Original(ctx, original, pre, persistence.MaxSnapshotBytes); err != nil { return nil, nil, nil, err }
		}
...
func checkAttemptSettlementV2DiskImages(ctx context.Context, transaction *attemptSettlementTransactionV2, batch *attemptSettlementV2Owners, physical attemptSettlementV2IO) error {
	for index, entry := range transaction.Snapshots {
		...
		if _, err := decodeAttemptSettlementV2Stats(ctx, encoded, batch.ordered[index].Stats.cfg, batch.persistence.MaxSnapshotBytes); err != nil { return err }
...
func finishAttemptSettlementTransactionV2(ctx context.Context, coordinator string, transaction *attemptSettlementTransactionV2, batch *attemptSettlementV2Owners, postimages []*StatsEngine, authority AttemptSettlementV2Options, physical attemptSettlementV2IO, publish bool) error {
	...
	if err := checkAttemptSettlementV2DiskImages(ctx, transaction, batch, physical); err != nil { return err }
	...
	for index, entry := range transaction.Snapshots {
		...
		if err := ctx.Err(); err != nil { return err }
		if err := physical.checkRoots(); err != nil { return err }
		if err := physical.writeImage(physical.roots[filepath.Dir(entry.StatsPath)], filepath.Base(entry.StatsPath), entry.StatsJSON, batch.persistence.MaxSnapshotBytes, physical.writeSnapshot); err != nil { return err }
		if err := physical.checkRoots(); err != nil { return err }
		if err := checkAttemptSettlementV2TransactionHeads(transaction, batch.ordered, postimages); err != nil { return err }
