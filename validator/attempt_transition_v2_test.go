...
func newAttemptSettlementV2TestOperator(t *testing.T, noID uint64, completed, failed int) *attemptCutV2StatsTestFixture {
	...
	if noID != fixture.expected.Identity.NoID {
		...
		if err := stats.AdvanceSettlementEpoch(42, state); err != nil { t.Fatal(err) }
		if err := stats.AttachAttemptLedger(ledger, state); err != nil { t.Fatal(err) }
	...
	for index := 0; index < completed; index++ {
		...
		if err != nil || proof == nil || proof.M != 8 || len(proof.Hops) != 8 { t.Fatalf("real M8 terminal fixture: %v", err) }
	}
	fixture.engine.transport = attemptCutV2SealTestTransport(func(ctx context.Context, hop connect.Id, raw []byte) ([]byte, error) {
		var envelope struct { TrailID *connect.Id `json:"trail_id"` }
		if err := json.Unmarshal(raw, &envelope); err != nil { return nil, err }
		if envelope.TrailID != nil { return nil, errors.New("terminal fixture actual extension refusal") }
		return fixture.server.PostVerify(ctx, hop, raw)
	})
	for index := 0; index < failed; index++ {
		proof, err := fixture.engine.RunTrail(t.Context())
		if err == nil || proof != nil { t.Fatal("failed real terminal produced a proof") }
	}
	fixture.engine.transport = fixture.server
	head, err := fixture.ledger.Head()
	if err != nil || head.LastSequence != uint64(8*completed+2*failed) { t.Fatalf("real terminal head: %+v %v", head, err) }
	...
	if err != nil || cut == nil { t.Fatalf("actual terminal compact sealer: %v", err) }
...
func sealAttemptSettlementV2Test(t *testing.T, fixtures ...*attemptCutV2StatsTestFixture) *AttemptSettlementClosureV2 {
	...
	if err != nil || closure == nil { t.Fatalf("complete real terminal batch: %v", err) }
	return closure
}
...
func resignAttemptSettlementV2Test(t *testing.T, closure *AttemptSettlementClosureV2, fixtures ...*attemptCutV2StatsTestFixture) {
	...
	if err != nil { t.Fatal(err) }
	keys := map[uint64]ed25519.PrivateKey{}
	for _, fixture := range fixtures { keys[fixture.cut.Context.Identity.NoID] = fixture.seal.key }
...
func TestAttemptSettlementV2RealM8CompleteBatchAndDistinctActivations(t *testing.T) {
	...
	if first.cut.Context.Activation.Domain.ActivationHash == second.cut.Context.Activation.Domain.ActivationHash || first.cut.Context.Identity.ValidatorVPK == second.cut.Context.Identity.ValidatorVPK { t.Fatal("fixture lacks distinct real operator identities and expected anchors") }
	...
	if err != nil || len(result.Operators) != 2 || result.Operators[9].Replay.Records.ItemCount != 122 || result.Operators[9].Replay.CompleteCount != 15 || result.Operators[9].Replay.FailedCount != 1 || result.Operators[10].Replay.Records.ItemCount != 8 { t.Fatalf("complete real terminal replay: %+v %v", result, err) }
	if first.measurement.Config.AMin != 8 || first.seal.policy.Verify.TrailDepth != 8 || len(closure.Transitions[0].PostFold) == 0 { t.Fatal("terminal fixture reduced M/minimum or lost actual positive quality") }
	if !reflect.DeepEqual(closure.Transitions[0].PreFold, first.measurement) || !reflect.DeepEqual(closure.Transitions[0].Cut, first.cut) { t.Fatal("terminal signer replaced pre-fold complete signed evidence") }
	for _, transition := range closure.Transitions {
		if transition.ToEpoch != 43 || !slices.Equal(transition.Batch, closure.Transitions[0].Batch) || !slices.Equal(transition.PostFold, attemptSettlementV2Qualities(result.Operators[transition.Identity.NoID].Stats)) { t.Fatal("terminal fold or complete signed census differs") }
	}
}
...
func TestAttemptSettlementV2EmptySignedWindowRetainsIdleAndZeroPriors(t *testing.T) {
	...
	if err != nil || result.Operators[9].Replay.Records.ItemCount != 0 || len(closure.Transitions[0].PostFold) != 2 || !closure.Transitions[0].PostFold[0].HasQuality || closure.Transitions[0].PostFold[0].QualityPPM != 0 || closure.Transitions[0].PostFold[1].QualityPPM != 765432 { t.Fatalf("empty terminal lost exact priors: %v", err) }
}
...
func TestAttemptSettlementV2SparseTerminalDoesNotFabricateQuality(t *testing.T) {
	...
	closure := sealAttemptSettlementV2Test(t, fixture)
	result, err := VerifyAttemptSettlementClosureV2(t.Context(), closure, attemptSettlementV2TestOptions(t, fixture))
	if err != nil || len(closure.Transitions[0].PostFold) != 0 || result.Operators[9].Replay.Records.ItemCount != 10 { t.Fatalf("sparse terminal quality: %v", err) }
	var exposure uint64
	for _, provider := range result.Operators[9].Stats.Providers { exposure += provider.Exposure }
	if exposure != 8 { t.Fatalf("sparse actual exposure=%d want8", exposure) }
}
...
func TestAttemptSettlementV2RejectsChangedOrMissingMemberBeforeReads(t *testing.T) {
	...
	for _, edit := range []func(*AttemptSettlementClosureV2){
		...
		func(value *AttemptSettlementClosureV2) { value.Transitions[0], value.Transitions[1] = value.Transitions[1], value.Transitions[0] },
		...
		for noID, operator := range options.Operators {
			operator.Measurement.Replay.ReadMetadata = func(context.Context, string, uint64) ([]byte, error) { reads++; return nil, errors.New("unexpected admission read") }
			options.Operators[noID] = operator
		}
		result, err := VerifyAttemptSettlementClosureV2(t.Context(), candidate, options)
		if err == nil || result.Operators != nil || reads != 0 { t.Fatalf("partial/changed member escaped admission: reads=%d error=%v", reads, err) }
	}
}
...
func TestAttemptSettlementV2ResignedFoldDriftPublishesNoParticipant(t *testing.T) {
	...
	result, err := VerifyAttemptSettlementClosureV2(t.Context(), closure, attemptSettlementV2TestOptions(t, first, second))
	if err == nil || result.Operators != nil { t.Fatal("re-signed invented fold published a complete or partial batch") }
}
...
func TestAttemptSettlementV2RejectsSwappedActivationAndCommonDomainDrift(t *testing.T) {
	...
	for _, edit := range []func(*AttemptSettlementV2OperatorOptions){
		func(operator *AttemptSettlementV2OperatorOptions) { operator.Expected.Activation.Domain.ActivationHash = first.cut.Context.Activation.Domain.ActivationHash },
		func(operator *AttemptSettlementV2OperatorOptions) { operator.Expected.Activation.Domain.ActivationHash[0] ^= 1 },
		func(operator *AttemptSettlementV2OperatorOptions) { operator.Expected.Activation.Domain.SettlementVault[0] ^= 1 },
		...
		if err == nil || result.Operators != nil { t.Fatal("foreign independently expected activation/domain was accepted") }
	}
}
...
func TestAttemptSettlementV2DigestAndSignatureAvoidHashCycle(t *testing.T) {
	...
	if before != after || bytes.Equal(message, changedMessage) { t.Fatal("digest/signature roles created a cycle or lost member binding") }
}
...
func TestAttemptSettlementV2SharesOnePolicyReplayPerMember(t *testing.T) {
	...
	if err != nil || closure == nil || !reflect.DeepEqual(wantReads, producerReads) { t.Fatalf("terminal producer duplicated or skipped complete replay: %v", err) }
	...
	result, err := verifyAttemptSettlementClosureV2(t.Context(), closure, options, func(noID uint64, record AttemptRecord) error {
		if noID != 9 || visited >= len(fixture.seal.recordTs) || !reflect.DeepEqual(record, fixture.seal.recordTs[visited]) { return errors.New("terminal prefix observer differs") }
		visited++
		return nil
	})
	if err != nil || !reflect.DeepEqual(wantReads, reads) || !reflect.DeepEqual(want, result.Operators[9]) || visited != 18 { t.Fatalf("terminal replay duplicated/skipped checked work: visits=%d %v", visited, err) }
}
...
func TestAttemptSettlementV2LateCloseCancellationDiscardsCompleteBatch(t *testing.T) {
	...
	if !errors.Is(err, context.Canceled) || closes == 0 || result.Operators != nil { t.Fatalf("late terminal cancellation escaped: closes=%d %v", closes, err) }
}
...
func TestAttemptSettlementV2PreCanceledAdmissionHasNoReplay(t *testing.T) {
	...
	reads := attemptSettlementV2TestObserveReads(&options, fixture)
	result, err := VerifyAttemptSettlementClosureV2(ctx, closure, options)
	if !errors.Is(err, context.Canceled) || result.Operators != nil || len(reads) != 0 { t.Fatalf("canceled terminal admission read or published: %v", err) }
	if _, err := os.Lstat(options.Operators[9].Measurement.Replay.ScratchDirectory); !errors.Is(err, os.ErrNotExist) { t.Fatalf("canceled terminal admission created scratch: %v", err) }
}
