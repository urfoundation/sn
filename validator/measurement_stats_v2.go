...
func (self *StatsEngine) releaseStatsV2Measurement(options AttemptCutV2StatsOptions) (ReleaseStatsMeasurement, error) {
	...
	admit := func(id connect.Id) bool {
		if !providerKVs[id] {
			if uint64(len(providerKVs)) >= options.MaxProviders { return false }
			providerKVs[id] = true
		}
		return true
	}
	for id := range self.window { if !admit(id) { return ReleaseStatsMeasurement{}, errors.New("compact native provider census exceeds its bound") } }
	for id := range self.emaPPM { if !admit(id) { return ReleaseStatsMeasurement{}, errors.New("compact native prior-provider census exceeds its bound") } }
	for id := range self.ema { if !admit(id) { return ReleaseStatsMeasurement{}, errors.New("compact native reporting-provider census exceeds its bound") } }
	var hashes uint64
	for id, values := range self.egress {
		if !admit(id) || uint64(len(values)) > options.MaxEgressHashes-hashes { return ReleaseStatsMeasurement{}, errors.New("compact native egress census exceeds its bound") }
		hashes += uint64(len(values))
	}
	measurement := self.releaseStatsMeasurementWithLock()
	measurement.SettlementTransition = nil
	if measurement.Config != options.ExpectedConfig { return ReleaseStatsMeasurement{}, errors.New("compact native statistics configuration differs") }
...
func releaseStatsV2PrefixRoot(ctx context.Context, ledger *AttemptLedger, sequence uint64) (string, error) {
	if sequence == 0 { return zeroAttemptHash(), nil }
	root, count := "", 0
	err := ledger.Walk(ctx, sequence, sequence, func(record AttemptRecord) error {
		if record.Sequence != sequence || record.Identity != ledger.identity { return errors.New("compact native cursor row differs from its ledger") }
		root, count = record.RecordHash, count+1
		return ctx.Err()
	})
	if err != nil { return "", err }
	if count != 1 { return "", errors.New("compact native cursor row is absent") }
	if _, err := canonicalAttemptHex32("compact native cursor root", root, false); err != nil { return "", err }
	return root, nil
}
...
func (self *StatsEngine) releaseStatsV2OwnedHead(dir string) (AttemptLedgerHead, error) {
	...
	if err != nil { return AttemptLedgerHead{}, err }
...
func (self *StatsEngine) releaseStatsV2Context(ctx context.Context, boundary AttemptBoundary, options releaseStatsV2Options) (AttemptCutV2Context, error) {
	if boundary.SettlementEpoch != self.settlementEpoch { return AttemptCutV2Context{}, errors.New("compact native boundary differs from active settlement") }
	prior, err := releaseStatsV2PrefixRoot(ctx, self.attemptLedger, self.attemptSettlementFirstSequence-1)
	if err != nil { return AttemptCutV2Context{}, err }
	...
	if _, err := attemptCutV2PolicyDepth(expected, options.Policy); err != nil { return AttemptCutV2Context{}, err }
	return expected, nil
}
...
func (self *StatsEngine) detachReleaseStatsMeasurementV2WithJournalGuard(ctx context.Context, dir string, boundary AttemptBoundary, options releaseStatsV2Options, persist func(ReleaseStatsMeasurement, AttemptCutV2) error, guard func(string) error) (ReleaseStatsMeasurement, *AttemptCutV2, error) {
	if persist == nil { return ReleaseStatsMeasurement{}, nil, errors.New("compact native journal writer is absent") }
	if err := validateReleaseStatsV2Scratch(options); err != nil { return ReleaseStatsMeasurement{}, nil, err }
	owner, err := self.acquireStatsWrite(ctx, "detach-native-v2")
	if err != nil { return ReleaseStatsMeasurement{}, nil, err }
	defer owner.release()
...
// cancellation boundary as well. The physical snapshot writer stays unchanged.
func releaseStatsV2JournalPersist(owner *statsWriteOwner, guard func(string) error) func(string, []byte) error {
	if guard == nil { return owner.persist }
	return func(path string, data []byte) error {
		return owner.persistChecked(path, data, func() error { return errors.Join(owner.check(), guard("before-save")) })
	}
}
...
func (self *StatsEngine) detachReleaseStatsMeasurementV2OwnedWithJournalGuard(owner *statsWriteOwner, dir string, boundary AttemptBoundary, measurement ReleaseStatsMeasurement, options releaseStatsV2Options, persist func(ReleaseStatsMeasurement, AttemptCutV2) error, guard func(string) error) (*AttemptCutV2, error) {
	if self.attemptSettlementCutPending { return nil, errAttemptCutPending }
	if !self.settlementEpochKnown || boundary.SettlementEpoch != self.settlementEpoch { return nil, errors.New("compact native boundary differs from active settlement") }
	if self.egressGeneration == ^uint64(0) { return nil, errors.New("compact native egress generation overflows") }
	head, err := self.releaseStatsV2OwnedHead(dir)
	if err != nil { return nil, err }
	self.attemptCutPending = true
	if self.activeAttemptCount != 0 { return nil, errAttemptCutPending }
	...
	if err != nil { return nil, err }
	if cut.LastSequence != head.LastSequence || cut.Root != head.Root { return nil, errors.New("compact native sealer changed the owned applied prefix") }
	if _, _, err := VerifyReleaseStatsMeasurementWithAttemptCutV2(owner.ctx, measurement, *cut, expected, options.Policy, options.Bounds, options.Stats); err != nil { return nil, err }
	currentHead, err := self.releaseStatsV2OwnedHead(dir)
	if err != nil || currentHead != head { return nil, errors.Join(errors.New("compact native head changed before journal publication"), err) }
	owner.step("before-journal")
	if err := owner.ctx.Err(); err != nil { return nil, err }
	if err := owner.check(); err != nil { return nil, err }
	currentHead, err = self.releaseStatsV2OwnedHead(dir)
	if err != nil || currentHead != head { return nil, errors.Join(errors.New("compact native head changed at journal admission"), err) }
	publication := *cut
	publication.Signature = bytes.Clone(cut.Signature)
	if err := persist(cloneReleaseStatsV2Measurement(measurement), publication); err != nil { return nil, err }
	currentHead, err = self.releaseStatsV2OwnedHead(dir)
	if err != nil || currentHead != head { return nil, errors.Join(errors.New("compact native head changed after journal publication"), err) }
	if guard != nil { if err := guard("before-save"); err != nil { return nil, err } }
...
func (self *StatsEngine) reconcileReleaseStatsCutV2WithJournalGuard(ctx context.Context, dir string, measurement ReleaseStatsMeasurement, cut AttemptCutV2, options releaseStatsV2Options, guard func(string) error) error {
	owner, err := self.acquireStatsWrite(ctx, "reconcile-native-v2")
	if err != nil { return err }
	defer owner.release()
	if _, err := self.releaseStatsV2Measurement(options.Stats); err != nil { return err }
	candidate := owner.clone()
...
// This guard closes callback-to-Save ordering, not the independent pathname
// custody of the unchanged Stats snapshot writer.
func (self *StatsEngine) reconcileReleaseStatsCutV2OwnedWithJournalGuard(owner *statsWriteOwner, dir string, measurement ReleaseStatsMeasurement, cut AttemptCutV2, options releaseStatsV2Options, guard func(string) error) error {
	if self.attemptSettlementCutPending { return errAttemptCutPending }
	head, err := self.releaseStatsV2OwnedHead(dir)
	if err != nil { return err }
	...
	if self.egressGeneration == cut.Context.EgressGeneration {
		if cut.Context.EgressFirstSequence != self.attemptEgressFirstSequence { return errors.New("compact native journal starts at another egress cursor") }
	} else {
		if self.attemptEgressFirstSequence < cut.LastSequence+1 { return errors.New("advanced compact statistics omit the signed cut cursor") }
		expected.EgressFirstSequence = cut.Context.EgressFirstSequence
	}
	if err := cut.VerifyHeader(expected, options.Bounds); err != nil { return err }
	root, err := releaseStatsV2PrefixRoot(owner.ctx, self.attemptLedger, cut.LastSequence)
	if err != nil || root != cut.Root { return errors.Join(errors.New("compact native journal is not the owned ledger prefix"), err) }
	if _, _, err := VerifyReleaseStatsMeasurementWithAttemptCutV2(owner.ctx, measurement, cut, expected, options.Policy, options.Bounds, options.Stats); err != nil { return err }
	currentHead, err := self.releaseStatsV2OwnedHead(dir)
	if err != nil || currentHead != head { return errors.Join(errors.New("compact native head changed during journal replay"), err) }
	if self.egressGeneration > cut.Context.EgressGeneration { return nil }
	if self.egressGeneration == ^uint64(0) { return errors.New("compact native journal egress generation overflows") }
	self.attemptCutPending = true
	if self.activeAttemptCount != 0 { return errAttemptCutPending }
	egress, err := self.releaseStatsV2SuffixEgress(owner.ctx, cut, head, options)
	if err != nil { return err }
	currentHead, err = self.releaseStatsV2OwnedHead(dir)
	if err != nil || currentHead != head { return errors.Join(errors.New("compact native head changed during journal reconciliation"), err) }
	if guard != nil { if err := guard("before-save"); err != nil { return err } }
	priorEgress, priorGeneration, priorFirst := self.egress, self.egressGeneration, self.attemptEgressFirstSequence
...
func (self *StatsEngine) releaseStatsV2SuffixEgress(ctx context.Context, cut AttemptCutV2, head AttemptLedgerHead, options releaseStatsV2Options) (map[connect.Id]map[[32]byte]bool, error) {
	...
	if cut.LastSequence == head.LastSequence { return egress, nil }
	if head.LastSequence-cut.LastSequence > options.Bounds.Records.MaxItems { return nil, errors.New("compact native suffix record bound exceeded") }
	...
	err = self.attemptLedger.Walk(ctx, cut.LastSequence+1, head.LastSequence, func(record AttemptRecord) error {
		...
		if err := VerifyAttemptRecord(&record, self.attemptLedger.identity, vpk[:], options.Stats.Replay.ServerKeys); err != nil { return err }
		if record.Disposition == AttemptDispositionComplete {
			if record.Proof == nil { return errors.New("compact native suffix complete record has no proof") }
			for index := 1; index < len(record.Proof.Hops); index++ {
				hop := record.Proof.Hops[index]
				if hop.EgressIpHash == ([32]byte{}) { continue }
				if egress[hop.ClientId] == nil {
					if uint64(len(egress)) >= options.Stats.MaxProviders { return errors.New("compact native suffix provider bound exceeded") }
					egress[hop.ClientId] = map[[32]byte]bool{}
				}
				if !egress[hop.ClientId][hop.EgressIpHash] {
					if hashes >= options.Stats.MaxEgressHashes { return errors.New("compact native suffix hash bound exceeded") }
	...
	if err != nil { return nil, err }
	if sequence != head.LastSequence || previous != head.Root { return nil, fmt.Errorf("compact native suffix differs from applied head %d", head.LastSequence) }
	return egress, nil
}
