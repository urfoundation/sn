// Current459 admission is an exact artifact boundary, not a spec-only upgrade.
package validator
...
// The one reviewed459 pair is accepted; every adjacent version or artifact
// mismatch is refused before its configured bytes can become signing authority.
func TestReleaseRuntime459RequiresExactReviewedArtifact(t *testing.T) {
...
// A formerly reviewed release lock is evidence, not authority for new459
// steering. Refusal happens before even a public read or chain-binding mutation.
func TestReleaseRuntime459RejectsHistoricalConfigBeforeRpc(t *testing.T) {
	calls := 0
