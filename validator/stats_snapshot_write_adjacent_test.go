...
func TestStatsSnapshotWriteLegacyRelativeAliasAndMissingCompatibility(t *testing.T) {
	for _, kind := range []string{"relative", "alias", "alias-missing"} {
		...
		if kind == "relative" {
			...
			if err != nil || filepath.IsAbs(dir) { t.Fatalf("actual relative path prerequisite: %v", err) }
		} else {
			alias := physical+"-alias"
			if err := os.Symlink(filepath.Base(physical), alias); err != nil { t.Fatal(err) }
			dir = alias
			if kind == "alias-missing" { dir, physical = filepath.Join(alias, "one", "two"), filepath.Join(physical, "one", "two") }
		}
		if err := stats.Save(dir); err != nil { t.Fatalf("legacy %s admission failed: %v", kind, err) }
		if !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(physical, "stats.json")), expected) { t.Fatalf("legacy %s changed encoding/routing", kind) }
	}
}
...
func TestStatsSnapshotWriteLegacyModeRepairAndV2NoRepair(t *testing.T) {
	for _, physical := range []bool{false, true} {
		...
		if err == nil { err = stats.saveOwned(dir, owner.persist) }
		err = errors.Join(err, owner.finishSnapshot())
		owner.release()
		info, statErr := os.Stat(dir)
		if statErr != nil { t.Fatal(statErr) }
		actual := statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json"))
		if physical {
			if err == nil || info.Mode().Perm() != 0o750 || !bytes.Equal(actual, before) { t.Fatalf("v2 repaired an existing non-private directory: %v", err) }
		} else if err != nil || info.Mode().Perm() != 0o700 || !bytes.Equal(actual, expected) { t.Fatalf("legacy owned mode repair changed: %v", err) }
	}
}
...
func TestStatsSnapshotWriteLegacyAliasRetargetRefused(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(other, "stats.json"), marker, 0o600); err != nil { t.Fatal(err) }
	steps := 0
	stats.writeHooks.step = func(operation, stage string) {
		if operation != "save" || stage != "before-snapshot" { return }
		steps++
		if err := os.Rename(alias, alias+"-preserved"); err != nil { t.Fatal(err) }
		if err := os.Symlink(other, alias); err != nil { t.Fatal(err) }
	}
	err := stats.Save(alias)
	if steps != 1 || err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(other, "stats.json")), marker) { t.Fatalf("legacy alias retarget acquired a new write authority: steps=%d error=%v", steps, err) }
}
...
func TestStatsSnapshotWriteEncodedVersionPinsPhysicalAuthority(t *testing.T) {
	...
	alias := dir+"-alias"
	if err := os.Symlink(dir, alias); err != nil { t.Fatal(err) }
	if err := writeEncodedStatsSnapshot(filepath.Join(alias, "stats.json"), encoded); err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) { t.Fatalf("encoded v6 inherited legacy alias admission: %v", err) }
	if err := writeEncodedStatsSnapshot(filepath.Join(dir, "stats.json"), encoded); err != nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), encoded) { t.Fatalf("actual physical v6 bytes did not survive writer transport: %v", err) }
	write, err := encodedStatsSnapshotWrite(filepath.Join(dir, "stats.json"), encoded)
	if err != nil || write.version != 6 || !bytes.Equal(write.data, encoded) { t.Fatalf("snapshot payload lost actual version: %v", err) }
	for _, invalid := range []string{`{}`, `{"v":0}`, `{"v":7}`, `{"ema":{},"v":5}`, `{"v":"6"}`, `[]`} {
		if err := writeEncodedStatsSnapshot(filepath.Join(dir, "stats.json"), []byte(invalid)); err == nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), encoded) { t.Fatalf("invalid private version header reached mutation: %s/%v", invalid, err) }
	}
}
...
func TestStatsSnapshotWriteV2InvalidPathAndAliasRefusedBeforeMutation(t *testing.T) {
	...
	for _, candidate := range []string{alias, dir+string([]byte{0xff}), dir+"/.", string(filepath.Separator)} {
		mutations := 0
		stats.writeHooks.snapshotIO.after = func(stage string, _ *os.File) error { if stage != "directory-admitted" && stage != "directory-closed" { mutations++ }; return nil }
		owner, err := stats.acquireStatsWrite(t.Context(), "snapshot-physical-control")
		if err != nil { t.Fatal(err) }
		err = owner.prepareSnapshot(candidate, true)
		err = errors.Join(err, owner.finishSnapshot())
		owner.release()
		if err == nil || mutations != 0 || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), before) { t.Fatalf("invalid physical snapshot authority mutated state: path=%q mutations=%d error=%v", candidate, mutations, err) }
	}
}
...
func TestStatsSnapshotWriteLegacyLeafReplacementDoesNotFollowOrBlock(t *testing.T) {
	for _, kind := range []string{"symlink", "fifo", "hardlink"} {
		...
		var err error
		switch kind {
		case "symlink": err = os.Symlink(filepath.Base(preserved), path)
		case "fifo": err = unix.Mkfifo(path, 0o600)
		default: err = os.Link(preserved, path)
		}
		if err != nil { t.Fatal(err) }
		if err := stats.Save(dir); err != nil || !bytes.Equal(statsSnapshotWriteTestRead(t, path), expected) || !bytes.Equal(statsSnapshotWriteTestRead(t, preserved), before) { t.Fatalf("legacy %s destination was followed or changed: %v", kind, err) }
	}
}
...
func TestStatsSnapshotWriteLegacyNativeJournalRetargetKeepsWindow(t *testing.T) {
	...
	if calls != 1 || err == nil || !reflect.DeepEqual(measurement, ReleaseStatsMeasurement{}) || !reflect.DeepEqual(stats.snapshotStats(), live) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir, "stats.json")), marker) || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(dir+"-preserved", "stats.json")), before) { t.Fatalf("legacy native callback retarget lost its snapshot/window: calls=%d error=%v", calls, err) }
}
...
func TestStatsSnapshotWritePublicBatchDefaultKeepsExactSnapshots(t *testing.T) {
	...
	if err := AdvanceAttemptSettlementEpoch(t.TempDir(), 43, attemptLedgerTestBoundary(), participants); err != nil { t.Fatal(err) }
	if err := VerifyAttemptSettlementBatch([]*AttemptSettlementTransition{first.Stats.settlementTransition, second.Stats.settlementTransition}); err != nil { t.Fatal(err) }
	for _, participant := range participants {
		expected, err := encodeStatsSnapshot(participant.Stats.snapshotStats())
		if err != nil || !bytes.Equal(statsSnapshotWriteTestRead(t, filepath.Join(participant.StateDir, "stats.json")), expected) { t.Fatalf("public default snapshot writer changed canonical batch bytes: %v", err) }
	}
}
