...
func checkHeadEMAStoreV2Leaf(directory *attemptPrivateDirectory, namespace headEMAStoreV2Namespace) error {
	if err := directory.check(); err != nil { return err }
	...
	if err != nil || current != namespace.leaf { return errors.Join(errors.New("bounded head EMA loaded leaf changed"), err) }
	return nil
}
...
func closeHeadEMAStoreV2File(file **os.File, hooks headEMAStoreV2RuntimeHooks) error {
	...
	*file = nil
	err := owned.Close()
	if hooks.afterClose != nil { err = errors.Join(err, hooks.afterClose(owned)) }
	return err
}
...
func witnessHeadEMAStoreV2Runtime(ctx context.Context, path string, namespace headEMAStoreV2Namespace, hooks headEMAStoreV2RuntimeHooks) (resultErr error) {
	...
	defer func() {
		...
		if hooks.afterClose != nil { resultErr = errors.Join(resultErr, hooks.afterClose(owned)) }
		witness, err := openHeadEMAStoreV2Directory(path, namespace)
		if err == nil { err = errors.Join(checkHeadEMAStoreV2Leaf(witness, namespace), requireHeadEMAStoreV2NoMarker(witness), witness.close()) }
		resultErr = errors.Join(resultErr, err, ctx.Err())
	}()
	if err := hooks.observe(ctx, "directory-opened", directory.file); err != nil { return err }
...
func hashHeadEMAStoreV2File(ctx context.Context, file *os.File, expected attemptPrivateFileState, limit uint64) ([32]byte, error) {
	var zero [32]byte
	if expected.size < 0 || uint64(expected.size) > limit || !expected.regular() { return zero, errors.New("bounded head EMA hash input exceeds its file allowance") }
	before, err := statAttemptPrivateFile(file)
	if err != nil || before != expected { return zero, errors.Join(errors.New("bounded head EMA hash descriptor changed"), err) }
	if _, err := file.Seek(0, io.SeekStart); err != nil { return zero, err }
	hasher := sha256.New()
	...
	for {
		...
		width := len(buffer)
		if remaining < int64(width) { width = int(remaining)+1 }
		count, readErr := file.Read(buffer[:width])
		consumed += int64(count)
		if consumed > expected.size { return zero, errors.New("bounded head EMA hash input grew") }
		if count != 0 { _, _ = hasher.Write(buffer[:count]) }
		if readErr == io.EOF { break }
		if readErr != nil { return zero, readErr }
		if count == 0 { return zero, io.ErrNoProgress }
	}
	after, err := statAttemptPrivateFile(file)
	if err != nil || after != expected || consumed != expected.size { return zero, errors.Join(errors.New("bounded head EMA hash input changed"), err) }
	var digest [32]byte
...
func createHeadEMAStoreV2Marker(directory *attemptPrivateDirectory) (*os.File, attemptPrivateFileState, error) {
	...
	if err != nil { return nil, attemptPrivateFileState{}, err }
...
func unlinkHeadEMAStoreV2Owned(directory *attemptPrivateDirectory, name string, expected attemptPrivateFileState) error {
	...
	if err != nil || state != expected { return errors.Join(errors.New("bounded head EMA cleanup name changed"), err) }
...
func finishHeadEMAStoreV2Write(path string, namespace headEMAStoreV2Namespace, marker attemptPrivateFileState, displaced *attemptPrivateFileState) (resultErr error) {
	...
	defer func() {
		resultErr = errors.Join(resultErr, directory.close())
		if resultErr != nil && markerRemoved { resultErr = errors.Join(resultErr, restoreHeadEMAStoreV2Marker(path, namespace)) }
	}()
	if err := checkHeadEMAStoreV2Leaf(directory, namespace); err != nil { return err }
	if displaced != nil { if err := unlinkHeadEMAStoreV2Owned(directory, headEMAStoreV2Candidate, *displaced); err != nil { return err } }
	if err := directory.file.Sync(); err != nil { return err }
	if err := unlinkHeadEMAStoreV2Owned(directory, headEMAStoreV2Marker, marker); err != nil { return err }
	markerRemoved = true
	if err := directory.file.Sync(); err != nil { return err }
...
func writeHeadEMAStoreV2(ctx context.Context, path string, encoded []byte, prior headEMAStoreV2Namespace, limits HeadEMAStoreV2Limits, hooks headEMAStoreV2RuntimeHooks) (next headEMAStoreV2Namespace, uncertain bool, resultErr error) {
	if err := ctx.Err(); err != nil { return next, false, err }
	if uint64(len(encoded)) > limits.MaxFileBytes { return next, false, errors.New("bounded head EMA encoded output exceeds its file allowance") }
	directory, err := openHeadEMAStoreV2Directory(path, prior)
	if err != nil { return next, true, err }
	...
	if err := hooks.observe(ctx, "directory-opened", directory.file); err != nil { return next, false, err }
	if err := checkHeadEMAStoreV2Leaf(directory, prior); err != nil { return next, true, err }
	if err := requireHeadEMAStoreV2NoMarker(directory); err != nil { return next, true, err }
	if !prior.missing {
		if err := hooks.observe(ctx, "predecessor-observed", nil); err != nil { return next, false, err }
		priorFile, err = directory.openFile("head-ema.json", unix.O_RDONLY, 0)
		if err != nil { return next, true, err }
		if err := hooks.observe(ctx, "predecessor-opened", priorFile); err != nil { return next, false, err }
		state, err := statAttemptPrivateFile(priorFile)
		if err != nil || state != prior.leaf { return next, true, errors.Join(errors.New("bounded head EMA predecessor changed before write"), err) }
	}
	...
	if err := hooks.observe(ctx, "marker-created", markerFile); err != nil { return next, true, err }
	if err := markerFile.Sync(); err != nil { return next, true, err }
	if err := closeHeadEMAStoreV2File(&markerFile, hooks); err != nil { return next, true, err }
	if err := directory.file.Sync(); err != nil { return next, true, err }
	if err := hooks.observe(ctx, "marker-synced", directory.file); err != nil { return next, true, err }
	candidateFile, err = directory.openFile(headEMAStoreV2Candidate, unix.O_WRONLY|unix.O_CREAT|unix.O_EXCL, 0o600)
	if err != nil { return next, true, err }
	if err := hooks.observe(ctx, "candidate-created", candidateFile); err != nil { return next, true, err }
	...
	if err != nil || written != len(encoded) { return next, true, errors.Join(io.ErrShortWrite, err) }
	if err := hooks.observe(ctx, "candidate-written", candidateFile); err != nil { return next, true, err }
	if err := candidateFile.Sync(); err != nil { return next, true, err }
	candidate, err = statAttemptPrivateFile(candidateFile)
	if err != nil || !sameHeadEMAStoreV2FileOwner(candidate, candidateOwner) || candidate.size != int64(len(encoded)) { return next, true, errors.Join(errors.New("bounded head EMA candidate write state differs"), err) }
	if err := hooks.observe(ctx, "candidate-synced", candidateFile); err != nil { return next, true, err }
	if err := closeHeadEMAStoreV2File(&candidateFile, hooks); err != nil { return next, true, err }
	if err := hooks.observe(ctx, "before-publish", directory.file); err != nil { return next, true, err }
	if err := checkHeadEMAStoreV2Leaf(directory, prior); err != nil { return next, true, err }
	...
	if err := hooks.observe(ctx, "publish-ready", directory.file); err != nil { return next, true, err }
	...
	if err := hooks.observe(ctx, "published", directory.file); err != nil { return next, true, err }
	published, err := directory.stat("head-ema.json")
	if err != nil || !sameHeadEMAStoreV2RenamedFile(published, candidate) { return next, true, errors.Join(errors.New("bounded head EMA published candidate differs"), err) }
	var displaced *attemptPrivateFileState
	if !prior.missing {
		state, err := directory.stat(headEMAStoreV2Candidate)
		if err != nil || !sameHeadEMAStoreV2RenamedFile(state, prior.leaf) { return next, true, errors.Join(errors.New("bounded head EMA exchanged predecessor differs from loaded authority"), err) }
		digest, err := hashHeadEMAStoreV2File(ctx, priorFile, state, limits.MaxFileBytes)
		if err != nil || digest != prior.digest { return next, true, errors.Join(errors.New("bounded head EMA exchanged predecessor bytes differ"), err) }
		displaced = &state
	}
	...
	if err != nil || digest != sha256.Sum256(encoded) { return next, true, errors.Join(errors.New("bounded head EMA published bytes differ"), err) }
	if err := closeHeadEMAStoreV2File(&candidateFile, hooks); err != nil { return next, true, err }
	if err := closeHeadEMAStoreV2File(&priorFile, hooks); err != nil { return next, true, err }
	next = headEMAStoreV2Namespace{directory: prior.directory, leaf: published, digest: digest}
	if err := checkHeadEMAStoreV2Leaf(directory, next); err != nil { return next, true, err }
	if displaced != nil {
		state, err := directory.stat(headEMAStoreV2Candidate)
		if err != nil || state != *displaced { return next, true, errors.Join(errors.New("bounded head EMA predecessor changed after Close"), err) }
	}
	if err := directory.file.Sync(); err != nil { return next, true, err }
	if err := hooks.observe(ctx, "directory-synced", directory.file); err != nil { return next, true, err }
	owned := directory.file
	if err := directory.close(); err != nil { return next, true, err }
	if hooks.afterClose != nil { if err := hooks.afterClose(owned); err != nil { return next, true, errors.Join(err, ctx.Err()) } }
	if err := ctx.Err(); err != nil { return next, true, err }
	...
	if err := finishHeadEMAStoreV2Write(path, next, marker, displaced); err != nil { return next, true, err }
