...
func newReleaseMeasurementEnvelopeV2TerminalTestFixture(t *testing.T, nonempty bool) *releaseMeasurementEnvelopeV2TestFixture {
	...
	if _, err := VerifyReleaseMeasurementLineageV2(t.Context(), previous, fixture.previous.options(t), fixture.current.artifact, fixture.options(t)); err != nil { t.Fatal(err) }
...
func (self *releaseMeasurementEnvelopeV2TestFixture) seal(t *testing.T) ([]byte, *ReleaseMeasurementEnvelope) {
	...
	if err != nil || envelope == nil || hash != ReleaseMeasurementEnvelopeContentHash(raw) { t.Fatalf("actual compact envelope seal: %v", err) }
	return raw, envelope
}
...
func TestReleaseMeasurementEnvelopeV2RoundTripGenuineM8(t *testing.T) {
	...
	if err != nil || !reflect.DeepEqual(decoded, envelope) { t.Fatalf("compact envelope round trip: %v", err) }
	...
	if err != nil || artifact == nil || *reads == 0 || len(verified.ReplayByNO) != 2 || verified.SettlementReplayByNO != nil { t.Fatalf("compact envelope real replay: reads=%d error=%v", *reads, err) }
	if !reflect.DeepEqual(verified.Decision, fixture.want) { t.Fatal("compact envelope changed the independent legacy decision") }
	for _, replay := range verified.ReplayByNO {
		if replay.Records.ItemCount != 18 || replay.CompleteCount != 2 || replay.FailedCount != 1 { t.Fatalf("actual M8 census changed: %+v", replay) }
	}
}
...
func TestReleaseMeasurementEnvelopeV2RetainsPositiveQuality(t *testing.T) {
	...
	if err != nil || !reflect.DeepEqual(verified.Decision, fixture.want) { t.Fatalf("positive-quality envelope changed exact decision: %v", err) }
	positive := 0
	for noID, replay := range verified.ReplayByNO {
		if replay.Records.ItemCount != 122 || replay.CompleteCount != 15 || replay.FailedCount != 1 { t.Fatal("positive M8 census changed") }
		for _, provider := range verified.Decision.StatsByNO[noID].Providers { if provider.HasQuality && provider.QualityPPM > 0 { positive++ } }
	}
	if positive < 2 { t.Fatal("real operator quality disappeared") }
}
...
func TestReleaseMeasurementEnvelopeV2TerminalNonemptyRestart(t *testing.T) {
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		if err != nil || !reflect.DeepEqual(decoded, envelope) { t.Fatalf("restart envelope decode: %v", err) }
		...
		if err != nil || len(verified.ReplayByNO) != 2 || len(verified.SettlementReplayByNO) != 2 { t.Fatalf("complete terminal/current envelope: %v", err) }
		for noID, terminal := range verified.SettlementReplayByNO {
			current := verified.ReplayByNO[noID]
			if terminal.Records.ItemCount != 18 || terminal.CompleteCount != 2 || terminal.FailedCount != 1 || current.Records.ItemCount != 8 || current.CompleteCount != 1 || current.FailedCount != 0 { t.Fatalf("restart lost actual terminal/current work: terminal=%+v current=%+v", terminal, current) }
		}
	}
}
...
func TestReleaseMeasurementEnvelopeV2TerminalRetainsPositiveQuality(t *testing.T) {
	...
	_, envelope := fixture.seal(t)
	_, verified, err := VerifyReleaseMeasurementEnvelopeV2(t.Context(), envelope, fixture.measurement, fixture.hotkey.PublicKey(), fixture.artifact.SelfUID, releaseMeasurementEnvelopeTestPreparedHash, fixture.options())
	if err != nil || len(verified.SettlementReplayByNO) != 2 { t.Fatalf("positive terminal envelope: %v", err) }
	positive := 0
	for noID, terminal := range verified.SettlementReplayByNO {
		if terminal.Records.ItemCount != 122 || verified.ReplayByNO[noID].Records.ItemCount != 0 { t.Fatal("terminal/empty-successor census changed") }
		for _, provider := range verified.Decision.StatsByNO[noID].Providers { if provider.HasQuality && provider.QualityPPM > 0 { positive++ } }
	}
	if positive < 2 { t.Fatal("terminal envelope invented an empty pool-quality result") }
}
...
func TestReleaseMeasurementEnvelopeV2SignatureBindsEveryField(t *testing.T) {
	...
	for index := 0; index < reflect.TypeOf(*original).NumField(); index++ {
		...
		switch field.Kind() {
		case reflect.String: field.SetString(field.String() + "x")
		case reflect.Uint16, reflect.Uint64: field.SetUint(field.Uint() + 1)
		default: t.Fatalf("new unsigned field kind: %s", reflect.TypeOf(candidate).Field(index).Name)
		}
		raw, err := canonicalReleaseMeasurementEnvelopeBytes(&candidate)
		if err != nil { t.Fatal(err) }
		if decoded, err := DecodeReleaseMeasurementEnvelopeV2(t.Context(), raw, fixture.options().MaxControlBytes); err == nil || decoded != nil { t.Fatalf("field %s escaped signature binding", reflect.TypeOf(candidate).Field(index).Name) }
	}
}
...
func TestReleaseMeasurementEnvelopeV2ResignedDecisionRefusesBeforeIO(t *testing.T) {
	...
	fixture := newReleaseMeasurementEnvelopeV2TestFixture(t, 2)
	_, original := fixture.seal(t)
	for _, change := range []struct { name string; mutate func(*ReleaseMeasurementEnvelope) }{
		...
		{name: "coordinator", mutate: func(value *ReleaseMeasurementEnvelope) { value.Coordinator = "0x0000000000000000000000000000000000000061" }},
		{name: "vault", mutate: func(value *ReleaseMeasurementEnvelope) { value.SettlementVault = "0x0000000000000000000000000000000000000062" }},
		...
		{name: "lineage", mutate: func(value *ReleaseMeasurementEnvelope) { value.PreviousArtifactHash = ReleaseMeasurementContentHash([]byte("another actual byte string")) }},
		{name: "artifact hash", mutate: func(value *ReleaseMeasurementEnvelope) { value.MeasurementArtifactHash = ReleaseMeasurementContentHash([]byte("different bytes")) }},
		...
		options := fixture.options()
		if err := validateReleaseMeasurementEnvelopeV2(t.Context(), &candidate, options.MaxControlBytes); err != nil { t.Fatalf("%s is not genuinely self-signed: %v", change.name, err) }
		reads := observeReleaseMeasurementV2SettlementTest(&options)
		...
		if err == nil || artifact != nil || *reads != 0 { t.Fatalf("%s competing decision reached IO: reads=%d error=%v", change.name, *reads, err) }
...
func TestReleaseMeasurementEnvelopeV2AuthorityRefusesBeforeIO(t *testing.T) {
	...
	fixture := newReleaseMeasurementEnvelopeV2TestFixture(t, 2)
	_, envelope := fixture.seal(t)
	for _, change := range []struct { name string; mutate func(*ReleaseMeasurementV2Options) }{
		{name: "zero native signer", mutate: func(options *ReleaseMeasurementV2Options) { op := options.Operators[10]; op.Expected.Activation.Hotkey = [32]byte{}; options.Operators[10] = op }},
		{name: "different native signer", mutate: func(options *ReleaseMeasurementV2Options) { op := options.Operators[10]; op.Expected.Activation.Hotkey = [32]byte{0x77}; options.Operators[10] = op }},
		...
		if err == nil || artifact != nil || *reads != 0 { t.Fatalf("%s reached IO: reads=%d error=%v", change.name, *reads, err) }
		assertReleaseMeasurementV2Empty(t, verified)
		...
		if err == nil || raw != nil || hash != "" || signed != nil || *reads != 0 { t.Fatalf("%s escaped producer admission: %v", change.name, err) }
	}
}
...
func TestReleaseMeasurementEnvelopeV2TerminalAuthorityRefusesBeforeIO(t *testing.T) {
	...
	for _, missing := range []bool{false, true} {
		options := fixture.options()
		if missing { delete(options.Settlement.Operators, 10) } else { op := options.Settlement.Operators[10]; op.Expected.Activation.Hotkey = [32]byte{0x73}; options.Settlement.Operators[10] = op }
		reads := observeReleaseMeasurementV2SettlementTest(&options)
		artifact, verified, err := VerifyReleaseMeasurementEnvelopeV2(t.Context(), envelope, fixture.measurement, fixture.hotkey.PublicKey(), fixture.artifact.SelfUID, releaseMeasurementEnvelopeTestPreparedHash, options)
		if err == nil || artifact != nil || *reads != 0 { t.Fatalf("terminal signer omission/substitution reached IO: %v", err) }
		assertReleaseMeasurementV2Empty(t, verified)
	}
}
...
func TestReleaseMeasurementEnvelopeV2RejectsLegacyAndDomainReplay(t *testing.T) {
	...
	fixture := newReleaseMeasurementEnvelopeV2TestFixture(t, 2)
	raw, envelope := fixture.seal(t)
	if value, err := DecodeReleaseMeasurementEnvelope(raw); err == nil || value != nil { t.Fatal("legacy decoder accepted compact envelope") }
	if _, _, err := VerifyReleaseMeasurementEnvelope(envelope, fixture.measurement, fixture.hotkey.PublicKey(), fixture.artifact.SelfUID, releaseMeasurementEnvelopeTestPreparedHash); err == nil { t.Fatal("legacy trusted verifier accepted compact evidence") }
	if value, hash, signed, err := SealReleaseMeasurementEnvelope(fixture.measurement, fixture.artifact.SelfUID, fixture.hotkey, releaseMeasurementEnvelopeTestPreparedHash, time.Unix(1, 0)); err == nil || value != nil || hash != "" || signed != nil { t.Fatal("legacy producer accepted compact evidence") }
	...
	if err != nil { t.Fatal(err) }
	if _, _, err := VerifyReleaseMeasurementEnvelope(legacyEnvelope, legacyBytes, legacyKey.PublicKey(), legacyUID, releaseMeasurementEnvelopeTestPreparedHash); err != nil { t.Fatalf("old route regressed: %v", err) }
	...
	if err != nil || legacyDigest != oldDigest { t.Fatalf("common envelope factoring changed exact old signing bytes: %v", err) }
	...
	if err != nil || compactDigest == oldDigest { t.Fatalf("compact signature domain is not distinct: %v", err) }
	if value, err := DecodeReleaseMeasurementEnvelopeV2(t.Context(), legacyRaw, fixture.options().MaxControlBytes); err == nil || value != nil { t.Fatal("compact decoder accepted legacy envelope") }
	for _, version := range []string{ReleaseMeasurementEnvelopeSchema, ReleaseMeasurementEnvelopeSchemaV2} {
		...
		if err != nil { t.Fatal(err) }
		if value, err := DecodeReleaseMeasurementEnvelopeV2(t.Context(), candidateRaw, fixture.options().MaxControlBytes); err == nil || value != nil { t.Fatal("legacy signature domain replayed into compact verifier") }
	}
}
...
func TestReleaseMeasurementEnvelopeV2RejectsNoncanonicalWire(t *testing.T) {
	...
	for _, candidate := range [][]byte{
		...
		if bytes.Equal(candidate, raw) { t.Fatal("wire mutation did not change the actual encoding") }
		if value, err := DecodeReleaseMeasurementEnvelopeV2(t.Context(), candidate, fixture.options().MaxControlBytes); err == nil || value != nil { t.Fatal("noncanonical compact envelope was accepted") }
	}
}
...
func TestReleaseMeasurementEnvelopeV2BoundsRefuseBeforeIO(t *testing.T) {
	...
	for _, bound := range []uint64{0, 1, uint64(len(raw)-1), releaseMeasurementEnvelopeMaxArtifactSize+1} {
		if value, err := DecodeReleaseMeasurementEnvelopeV2(t.Context(), raw, bound); err == nil || value != nil { t.Fatalf("wire bound %d was ignored", bound) }
	}
	for _, control := range []bool{false, true} {
		options := fixture.options()
		if control { options.MaxControlBytes = 1 } else { options.MaxArtifactBytes = uint64(len(fixture.measurement)-1) }
		reads := observeReleaseMeasurementV2SettlementTest(&options)
		artifact, verified, err := VerifyReleaseMeasurementEnvelopeV2(t.Context(), envelope, fixture.measurement, fixture.hotkey.PublicKey(), fixture.artifact.SelfUID, releaseMeasurementEnvelopeTestPreparedHash, options)
		if err == nil || artifact != nil || *reads != 0 { t.Fatalf("bounded verification reached streams: %v", err) }
		assertReleaseMeasurementV2Empty(t, verified)
		...
		if err == nil || value != nil || hash != "" || signed != nil || *reads != 0 { t.Fatalf("bounded seal reached streams: %v", err) }
	}
}
...
func TestReleaseMeasurementEnvelopeV2SealRetainsLateProofCloseFailure(t *testing.T) {
	...
	op.Measurement.Replay.OpenData = func(ctx context.Context, kind, hash string, size uint64) (io.ReadCloser, error) {
		...
		if err != nil || kind != AttemptStreamV2Proofs { return reader, err }
	...
	if !errors.Is(err, failure) || closes == 0 || raw != nil || hash != "" || envelope != nil { t.Fatalf("late proof close escaped producer: closes=%d error=%v", closes, err) }
}
...
func TestReleaseMeasurementEnvelopeV2VerifyRetainsLateProofCloseFailure(t *testing.T) {
	...
	options.Operators[10] = op
	artifact, verified, err := VerifyReleaseMeasurementEnvelopeV2(t.Context(), envelope, fixture.measurement, fixture.hotkey.PublicKey(), fixture.artifact.SelfUID, releaseMeasurementEnvelopeTestPreparedHash, options)
	if !errors.Is(err, failure) || closes == 0 || artifact != nil { t.Fatalf("late proof close escaped verifier: closes=%d error=%v", closes, err) }
	assertReleaseMeasurementV2Empty(t, verified)
}
...
func TestReleaseMeasurementEnvelopeV2CancellationAfterProofCloseClearsResults(t *testing.T) {
	...
	for _, producer := range []bool{false, true} {
		...
		if producer {
			...
			if !errors.Is(err, context.Canceled) || closes == 0 || raw != nil || hash != "" || signed != nil { cancel(); t.Fatalf("producer ignored proof-close cancellation: %v", err) }
		} else {
			...
			if !errors.Is(err, context.Canceled) || closes == 0 || artifact != nil { cancel(); t.Fatalf("verifier ignored proof-close cancellation: %v", err) }
			assertReleaseMeasurementV2Empty(t, verified)
		}
		cancel()
	}
...
func TestReleaseMeasurementEnvelopeV2OwnsInputsBeforeCallbacks(t *testing.T) {
	...
	if err != nil || !mutated || artifact == nil || len(verified.ReplayByNO) != 2 || !reflect.DeepEqual(verified.Decision, fixture.want) { t.Fatalf("callback changed owned envelope inputs: mutated=%t error=%v", mutated, err) }
	if artifact.NativeSnapshotBlock != fixture.artifact.NativeSnapshotBlock { t.Fatal("callback changed the returned immutable decision") }
}
...
func TestReleaseMeasurementEnvelopeV2RejectsChangedProofBytes(t *testing.T) {
	...
	options.Operators[10] = op
	artifact, verified, err := VerifyReleaseMeasurementEnvelopeV2(t.Context(), envelope, fixture.measurement, fixture.hotkey.PublicKey(), fixture.artifact.SelfUID, releaseMeasurementEnvelopeTestPreparedHash, options)
	if err == nil || !changed || artifact != nil { t.Fatalf("changed actual proof bytes escaped full replay: changed=%t error=%v", changed, err) }
	assertReleaseMeasurementV2Empty(t, verified)
}
...
func (self *releaseMeasurementEnvelopeV2ChangedReader) Read(buffer []byte) (int, error) {
	...
	if count > 0 && !*self.changed { buffer[0] ^= 1; *self.changed = true }
	return count, err
}
...
func releaseMeasurementEnvelopeV2TestControlBytes(t *testing.T, value any) uint64 {
	...
	if err := releaseMeasurementV2ControlStorage(t.Context(), reflect.ValueOf(value), &remaining); err != nil { t.Fatal(err) }
...
func TestReleaseMeasurementEnvelopeV2AdmissionChargesFixedRoot(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ENVELOPE-ADMISSION-v1 PASS TestReleaseMeasurementEnvelopeV2AdmissionChargesFixedRoot") } })
	fixture := newReleaseMeasurementEnvelopeV2TestFixture(t, 2)
	_, envelope := fixture.seal(t)
	stringsOnly := uint64(0)
	value := reflect.ValueOf(*envelope)
	for index := 0; index < value.NumField(); index++ { if field := value.Field(index); field.Kind() == reflect.String { stringsOnly += uint64(field.Len()) } }
	full := releaseMeasurementEnvelopeV2TestControlBytes(t, envelope)
	if full != stringsOnly+uint64(value.Type().Size()) { t.Fatal("flat envelope root census precondition differs") }
	if err := validateReleaseMeasurementEnvelopeV2(t.Context(), envelope, stringsOnly); err == nil || !strings.Contains(err.Error(), "control bound") { t.Fatalf("actual envelope validator omitted root storage: %v", err) }
}
...
func TestReleaseMeasurementEnvelopeV2AdmissionCountsExactEscapedWire(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ENVELOPE-ADMISSION-v1 PASS TestReleaseMeasurementEnvelopeV2AdmissionCountsExactEscapedWire") } })
	fixture := newReleaseMeasurementEnvelopeV2TestFixture(t, 2)
	_, original := fixture.seal(t)
	for _, sample := range []string{"plain", "\"\\\b\f\n\r\t", "\x00\x01\x0b\x1f", "<>&", "\u2028\u2029", "é🌍�", string([]byte{0xff, 0xc0, 0xaf})} {
		...
		if wire <= controls || wire > fixture.options().MaxControlBytes { t.Fatalf("escape wire/control witness is outside existing bounds: wire=%d control=%d", wire, controls) }
		if err := validateReleaseMeasurementEnvelopeV2(t.Context(), &candidate, wire); err != nil { t.Fatalf("exact encoding/json wire was rejected: %v", err) }
		if err := validateReleaseMeasurementEnvelopeV2(t.Context(), &candidate, wire-1); err == nil || !strings.Contains(err.Error(), "wire bound") { t.Fatalf("escaped canonical wire escaped its exact byte bound: %v", err) }
		if utf8.ValidString(candidate.DeploymentID) {
			decoded, err := DecodeReleaseMeasurementEnvelopeV2(t.Context(), raw, wire)
			if err != nil || !reflect.DeepEqual(decoded, &candidate) { t.Fatalf("exact valid canonical envelope did not decode: %v", err) }
		}
	}
}
...
func TestReleaseMeasurementEnvelopeV2AdmissionReservesExactSigningFields(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ENVELOPE-ADMISSION-v1 PASS TestReleaseMeasurementEnvelopeV2AdmissionReservesExactSigningFields") } })
	fixture := newReleaseMeasurementEnvelopeV2TestFixture(t, 2)
	...
	if actualFields != 71+130 || uint64(len(signedRaw)-len(unsignedRaw)) != actualFields || releaseMeasurementEnvelopeV2TestControlBytes(t, envelope)-releaseMeasurementEnvelopeV2TestControlBytes(t, &unsigned) != actualFields { t.Fatal("fixed signing-field reservation differs from actual signed control and JSON bytes") }
	if envelope.SigningHash == "" || envelope.Signature == "" { t.Fatal("reservation control did not produce real signature material") }
}
...
func TestReleaseMeasurementEnvelopeV2AdmissionExactCombinedBudget(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ENVELOPE-ADMISSION-v1 PASS TestReleaseMeasurementEnvelopeV2AdmissionExactCombinedBudget") } })
	for _, terminal := range []bool{false, true} {
		var fixture *releaseMeasurementEnvelopeV2TestFixture
		if terminal { fixture = newReleaseMeasurementEnvelopeV2TerminalTestFixture(t, true) } else { fixture = newReleaseMeasurementEnvelopeV2TestFixture(t, 2) }
		raw, envelope := fixture.seal(t)
		bound := releaseMeasurementEnvelopeV2TestControlBytes(t, fixture.artifact)
		if bound <= max(uint64(len(raw)), releaseMeasurementEnvelopeV2TestControlBytes(t, envelope)) { t.Fatal("the real artifact control census must dominate this adjacent unchanged-budget control") }
		if bound > fixture.options().MaxControlBytes || bound == 0 { t.Fatal("exact fixture budget would increase its original allowance") }
		...
		for _, producer := range []bool{false, true} {
			for _, exact := range []bool{true, false} {
				...
				if !exact { options.MaxControlBytes-- }
				reads := observeReleaseMeasurementV2SettlementTest(&options)
				if producer {
					...
					if exact {
						if err != nil || signed == nil || *reads == 0 || hash != ReleaseMeasurementEnvelopeContentHash(encoded) || uint64(len(encoded)) > bound { t.Fatalf("exact producer budget refused real replay: %v", err) }
					} else if err == nil || encoded != nil || hash != "" || signed != nil || *reads != 0 { t.Fatalf("short producer budget reached readers or outputs: %v", err) }
				} else {
					artifact, verified, err := VerifyReleaseMeasurementEnvelopeV2(t.Context(), envelope, fixture.measurement, fixture.hotkey.PublicKey(), fixture.artifact.SelfUID, releaseMeasurementEnvelopeTestPreparedHash, options)
					if exact {
						if err != nil || artifact == nil || verified.Decision == nil || len(verified.ReplayByNO) != 2 || *reads == 0 { t.Fatalf("exact verifier budget refused real replay: %v", err) }
						if terminal && len(verified.SettlementReplayByNO) != 2 { t.Fatal("exact budget omitted real terminal replay") }
					} else {
						if err == nil || artifact != nil || *reads != 0 { t.Fatalf("short verifier budget reached readers or output: %v", err) }
						assertReleaseMeasurementV2Empty(t, verified)
					}
				}
			}
...
func TestReleaseMeasurementEnvelopeV2AdmissionProducerWireBeforeReplay(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ENVELOPE-ADMISSION-v1 PASS TestReleaseMeasurementEnvelopeV2AdmissionProducerWireBeforeReplay") } })
	fixture := newReleaseMeasurementEnvelopeV2TestFixture(t, 2)
	...
	options := fixture.options()
	if bound >= options.MaxControlBytes || releaseMeasurementEnvelopeV2TestControlBytes(t, unsigned)+71+130 > bound || releaseMeasurementEnvelopeV2TestControlBytes(t, artifact) > bound { t.Fatal("wire amplification must be the first exhausted finite budget") }
	...
	if err == nil || !strings.Contains(err.Error(), "wire bound") || *reads != 0 || raw != nil || hash != "" || envelope != nil { t.Fatalf("producer reached identity/replay/signing before envelope wire admission: reads=%d error=%v", *reads, err) }
}
...
func TestReleaseMeasurementEnvelopeV2AdmissionDirectVerifyWireBeforeReplay(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ENVELOPE-ADMISSION-v1 PASS TestReleaseMeasurementEnvelopeV2AdmissionDirectVerifyWireBeforeReplay") } })
	fixture := newReleaseMeasurementEnvelopeV2TestFixture(t, 2)
	_, original := fixture.seal(t)
	envelope := *original
	...
	if err != nil { t.Fatal(err) }
	if decoded, err := DecodeReleaseMeasurementEnvelopeV2(t.Context(), raw, fixture.options().MaxControlBytes); err != nil || decoded == nil { t.Fatalf("wire negative is not genuinely canonical and self-signed: %v", err) }
	options := fixture.options()
	options.MaxControlBytes, options.Expected.DeploymentID = uint64(len(raw)-1), envelope.DeploymentID
	if releaseMeasurementEnvelopeV2TestControlBytes(t, &envelope) > options.MaxControlBytes { t.Fatal("direct wire witness exhausted control storage first") }
	reads := observeReleaseMeasurementV2SettlementTest(&options)
	artifact, verified, err := VerifyReleaseMeasurementEnvelopeV2(t.Context(), &envelope, fixture.measurement, fixture.hotkey.PublicKey(), fixture.artifact.SelfUID, releaseMeasurementEnvelopeTestPreparedHash, options)
	if err == nil || !strings.Contains(err.Error(), "wire bound") || artifact != nil || *reads != 0 { t.Fatalf("direct verifier missed pre-replay wire admission: reads=%d error=%v", *reads, err) }
	assertReleaseMeasurementV2Empty(t, verified)
}
...
func TestReleaseMeasurementEnvelopeV2AdmissionCanceledBeforeReaders(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ENVELOPE-ADMISSION-v1 PASS TestReleaseMeasurementEnvelopeV2AdmissionCanceledBeforeReaders") } })
	fixture := newReleaseMeasurementEnvelopeV2TestFixture(t, 2)
	...
	options := fixture.options()
	reads := observeReleaseMeasurementV2SettlementTest(&options)
	if value, err := DecodeReleaseMeasurementEnvelopeV2(ctx, encoded, options.MaxControlBytes); !errors.Is(err, context.Canceled) || value != nil { t.Fatalf("canceled decoder exposed an envelope: %v", err) }
	if raw, hash, signed, err := SealReleaseMeasurementEnvelopeV2(ctx, fixture.measurement, fixture.artifact.SelfUID, fixture.hotkey, releaseMeasurementEnvelopeTestPreparedHash, time.Unix(1, 0), options); !errors.Is(err, context.Canceled) || raw != nil || hash != "" || signed != nil { t.Fatalf("canceled producer exposed output: %v", err) }
	artifact, verified, err := VerifyReleaseMeasurementEnvelopeV2(ctx, envelope, fixture.measurement, fixture.hotkey.PublicKey(), fixture.artifact.SelfUID, releaseMeasurementEnvelopeTestPreparedHash, options)
	if !errors.Is(err, context.Canceled) || artifact != nil || *reads != 0 { t.Fatalf("canceled verifier reached readers or output: %v", err) }
	assertReleaseMeasurementV2Empty(t, verified)
}
...
func TestReleaseMeasurementEnvelopeV2AdmissionOwnsSignerBeforeCallbacks(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ENVELOPE-ADMISSION-v1 PASS TestReleaseMeasurementEnvelopeV2AdmissionOwnsSignerBeforeCallbacks") } })
	for _, terminal := range []bool{false, true} {
		var fixture *releaseMeasurementEnvelopeV2TestFixture
		if terminal { fixture = newReleaseMeasurementEnvelopeV2TerminalTestFixture(t, true) } else { fixture = newReleaseMeasurementEnvelopeV2TestFixture(t, 2) }
		...
		if err != nil { t.Fatal(err) }
		if replacement.PublicKey() == expectedHotkey { t.Fatal("real alternate signer precondition did not change the key") }
		options := fixture.options()
		reads := observeReleaseMeasurementV2SettlementTest(&options)
		changed := false
		replaceOnRead := func(replay *AttemptCutV2ReplayOptions) {
			...
			replay.ReadMetadata = func(ctx context.Context, hash string, size uint64) ([]byte, error) {
				if !changed { changed = true; *fixture.hotkey = *replacement }
				return read(ctx, hash, size)
			}
		}
		for noID, operator := range options.Operators { replaceOnRead(&operator.Measurement.Replay); options.Operators[noID] = operator }
		if options.Settlement != nil { for noID, operator := range options.Settlement.Operators { replaceOnRead(&operator.Measurement.Replay); options.Settlement.Operators[noID] = operator } }
		...
		if err != nil || !changed || *reads == 0 || envelope == nil || envelope.ValidatorHotkey != releaseHex32(expectedHotkey) || hash != ReleaseMeasurementEnvelopeContentHash(encoded) || fixture.hotkey.PublicKey() != replacement.PublicKey() { t.Fatalf("callback replaced the admitted real envelope signer: terminal=%t changed=%t reads=%d error=%v", terminal, changed, *reads, err) }
		decoded, err := DecodeReleaseMeasurementEnvelopeV2(t.Context(), encoded, fixture.options().MaxControlBytes)
		if err != nil || !reflect.DeepEqual(decoded, envelope) { t.Fatalf("owned real signer did not produce canonical authentic bytes: %v", err) }
		...
		if err != nil || artifact == nil || verified.Decision == nil || *verifyReads == 0 || len(verified.ReplayByNO) != 2 || terminal && len(verified.SettlementReplayByNO) != 2 { t.Fatalf("owned envelope lost fresh full replay: %v", err) }
		if !terminal && !reflect.DeepEqual(verified.Decision, fixture.want) { t.Fatal("owned signer changed exact genuine M8 scoring") }
		for _, invalid := range []struct { name string; hotkey *crv4.Keypair }{
			...
			if err == nil || raw != nil || hash != "" || signed != nil || *invalidReads != 0 { t.Fatalf("invalid initial %s signer reached readers or outputs: %v", invalid.name, err) }
		}
	}
}
