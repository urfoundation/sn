...
func newReleaseMeasurementV2SettlementTestFromTerminal(t *testing.T, previous *releaseMeasurementV2TestFixture, terminalTs []*attemptCutV2StatsTestFixture, sameNativeEpoch bool, generations uint64) *releaseMeasurementV2SettlementTestFixture {
	...
	if !sameNativeEpoch { current.SubnetEpoch++ }
	...
	for index, transition := range closure.Transitions {
		...
		for _, quality := range transition.PostFold { priors[quality.ClientID] = quality.QualityPPM }
...
func (self *releaseMeasurementV2SettlementTestFixture) rebuildHead(t *testing.T, sameNativeEpoch bool) {
	...
	for _, input := range current.Inputs {
		...
		for _, provider := range input.Stats.Providers {
			...
			if observedKVs[provider.ClientID] { continue }
	...
	sort.Slice(current.Bindings, func(i, j int) bool {
		if current.Bindings[i].NoID != current.Bindings[j].NoID { return current.Bindings[i].NoID < current.Bindings[j].NoID }
	...
	for _, input := range current.Inputs {
		...
		for key, hashes := range prefixKVs {
			for hash := range hashes { fleets[key][hash] = true }
		}
	}
	...
	if sameNativeEpoch {
		for _, record := range previous.HeadEMA {
			if record.HasPrior { t.Fatal("same-native fixture requires the actual empty pre-epoch EMA base") }
		}
	} else {
		if err := head.CommitForEpoch(previous.SubnetEpoch, previous.HeadEMA, current.Policy.Steering.HeadScoreEMA); err != nil { t.Fatal(err) }
	}
...
func newReleaseMeasurementV2SettlementNonemptyTestFixture(t *testing.T) *releaseMeasurementV2SettlementTestFixture {
	...
	for index := range fixture.current.artifact.Inputs {
		...
		if err := stats.AdvanceSettlementEpoch(expected.Boundary.SettlementEpoch, state); err != nil { t.Fatal(err) }
		...
		if err := stats.AttachAttemptLedgerContext(t.Context(), terminal.seal.ledger, state); err != nil { t.Fatal(err) }
		...
		cfg.AttemptBoundaryResolver = func(_ context.Context, pinned *AttemptBoundary, clients []connect.Id) (AttemptBoundary, []AttemptBinding, error) {
			if pinned != nil && *pinned != expected.Boundary { return AttemptBoundary{}, nil, errors.New("nonempty successor boundary pin changed") }
			bindings := make([]AttemptBinding, len(clients))
			for index, clientID := range clients { bindings[index] = attemptLedgerTestBinding(clientID, 1) }
		...
		if err != nil || proof == nil || proof.M != 8 || len(proof.Hops) != 8 { t.Fatalf("real nonempty successor M8 failed: %v", err) }
		measurement := func() ReleaseStatsMeasurement {
			stats.mu.Lock()
		...
		for _, provider := range measurement.Providers { providerKVs[provider.ClientID] = true }
		for _, provider := range input.Stats.Providers {
			if provider.HasPriorQuality || provider.PriorQualityPPM != 0 { t.Fatal("sparse successor bootstrap invented a quality prior") }
		...
		if err != nil || head.LastSequence != 26 { t.Fatalf("real successor durable census changed: %+v error=%v", head, err) }
		seal := *terminal.seal
		...
		if err := seal.ledger.Walk(t.Context(), expected.FirstSequence, head.LastSequence, func(record AttemptRecord) error {
			if err := VerifyAttemptRecord(&record, expected.Identity, seal.key.Public().(ed25519.PublicKey), seal.server.serverPublicKeys()); err != nil { return err }
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		for noID, terminal := range result.SettlementReplayByNO {
			...
			for _, provider := range result.Decision.StatsByNO[noID].Providers {
				if provider.HasQuality && provider.QualityPPM > 0 { positive++ }
			}
		}
		if positive < 2 { t.Fatal("actual above-minimum terminal qualities disappeared on restart") }
	}
}
...
func TestReleaseMeasurementV2SettlementLineageAcceptsRealPrefixAndNativeRetry(t *testing.T) {
	...
	for _, variation := range []struct {
		...
		if !variation.sameNative {
			...
			if positive != 0 { t.Fatal("sparse next-native fixture invented terminal quality") }
		}
	}
}
...
func TestReleaseMeasurementV2SettlementNativeRetryRejectsDoubleFoldedHead(t *testing.T) {
	...
	for _, record := range fixture.current.artifact.HeadEMA {
		if record.HasPrior { priorCount++ }
	}
	if priorCount == 0 { t.Fatal("double-fold candidate lacks genuine positive prior head scores") }
...
func TestReleaseMeasurementV2SettlementUsesOneReplayPerContainingCut(t *testing.T) {
	...
	if _, err := VerifyAttemptSettlementClosureV2(t.Context(), fixture.current.artifact.SettlementClosureV2, *terminalOptions.Settlement); err != nil { t.Fatal(err) }
...
func TestReleaseMeasurementV2SettlementAdmissionPrecedesAllIO(t *testing.T) {
	...
	for _, edit := range []func(*ReleaseMeasurementArtifact, *ReleaseMeasurementV2Options){
		...
		func(a *ReleaseMeasurementArtifact, o *ReleaseMeasurementV2Options) { delete(o.Settlement.Operators, 10) },
		func(a *ReleaseMeasurementArtifact, o *ReleaseMeasurementV2Options) { a.SettlementClosureV2.Transitions = a.SettlementClosureV2.Transitions[:1] },
		func(a *ReleaseMeasurementArtifact, o *ReleaseMeasurementV2Options) { a.SettlementClosureV2.Epoch-- },
		func(a *ReleaseMeasurementArtifact, o *ReleaseMeasurementV2Options) { o.Settlement.MaxClosureBytes = o.MaxArtifactBytes+1 },
		...
		func(a *ReleaseMeasurementArtifact, o *ReleaseMeasurementV2Options) { a.SettlementClosureV2.Transitions[0].PreFold.AttemptCut = &AttemptLedgerCut{} },
	} {
		...
		if err == nil || *reads != 0 { t.Fatalf("terminal admission read or accepted: reads=%d error=%v", *reads, err) }
		assertReleaseMeasurementV2Empty(t, result)
...
func TestReleaseMeasurementV2SettlementRejectsResignedInventedFold(t *testing.T) {
	...
	if len(transition.PostFold) != 0 { t.Fatal("small real fixture unexpectedly qualified a provider") }
	...
	options := fixture.options(t)
	if _, err := admitAttemptSettlementV2(t.Context(), artifact.SettlementClosureV2, *options.Settlement, true); err != nil { t.Fatalf("valid member-signature prerequisite: %v", err) }
	reads := observeReleaseMeasurementV2SettlementTest(&options)
	result, err := VerifyReleaseMeasurementArtifactV2(t.Context(), artifact, options)
	if err == nil || !strings.Contains(err.Error(), "post-fold EMA") || *reads == 0 { t.Fatalf("invented signed fold escaped real replay: reads=%d error=%v", *reads, err) }
	assertReleaseMeasurementV2Empty(t, result)
}
...
func TestReleaseMeasurementV2SettlementLineageRejectsCrossRoleScratchBeforeIO(t *testing.T) {
	...
	for _, carriedClosure := range []bool{false, true} {
		...
		if carriedClosure { previousArtifact = fixture.current.artifact }
		...
		for _, priorTerminal := range []bool{false, true} {
			...
			for _, nextTerminal := range []bool{false, true} {
				...
				if priorTerminal { priorPath = previous.Settlement.Operators[9].Measurement.Replay.ScratchDirectory }
				if nextTerminal {
...
func TestReleaseMeasurementV2SettlementDecodeRejectsCompetingTerminalWire(t *testing.T) {
	...
	for _, candidate := range [][]byte{
		...
		if bytes.Equal(candidate, raw) { t.Fatal("terminal wire mutation did not change actual bytes") }
		options := fixture.options(t)
		reads := observeReleaseMeasurementV2SettlementTest(&options)
		artifact, result, err := DecodeReleaseMeasurementArtifactV2(t.Context(), candidate, options)
		if err == nil || artifact != nil || *reads != 0 { t.Fatalf("competing terminal decoded or read: %v", err) }
		assertReleaseMeasurementV2Empty(t, result)
	}
}
...
func TestReleaseMeasurementV2SettlementOwnsTerminalAndSuccessorBeforeIO(t *testing.T) {
	...
	op.Measurement.Replay.ReadMetadata = func(ctx context.Context, hash string, size uint64) ([]byte, error) {
		if !changed {
			...
			for _, key := range options.Operators[10].Measurement.Replay.ServerKeys { key[0] ^= 1 }
	...
	if err != nil || !changed || !bytes.Equal(raw, want) { t.Fatalf("terminal callback altered successor/closure bytes: changed=%t error=%v", changed, err) }
	result, err := VerifyReleaseMeasurementArtifactV2(t.Context(), artifact, fixture.options(t))
	if err == nil { t.Fatal("a second operation reused the earlier owned verdict") }
	assertReleaseMeasurementV2Empty(t, result)
}
...
func TestReleaseMeasurementV2SettlementCancellationAfterProofCloseClearsAll(t *testing.T) {
	...
	if !errors.Is(err, context.Canceled) || closes == 0 { t.Fatalf("terminal close cancellation disappeared: closes=%d error=%v", closes, err) }
	assertReleaseMeasurementV2Empty(t, result)
}
...
func TestReleaseMeasurementV2SettlementLateCurrentFailureDiscardsTerminalCensus(t *testing.T) {
	...
	for noID, op := range options.Settlement.Operators {
		metadata := op.Measurement.Replay.ReadMetadata
		data := op.Measurement.Replay.OpenData
		op.Measurement.Replay.ReadMetadata = func(ctx context.Context, hash string, size uint64) ([]byte, error) { terminalReads++; return metadata(ctx, hash, size) }
		op.Measurement.Replay.OpenData = func(ctx context.Context, kind, hash string, size uint64) (io.ReadCloser, error) {
			reader, err := data(ctx, kind, hash, size)
			if err != nil || kind != AttemptStreamV2Proofs { return reader, err }
	...
	op.Measurement.Replay.ReadMetadata = func(ctx context.Context, hash string, size uint64) ([]byte, error) {
		...
		for _, transition := range fixture.current.artifact.SettlementClosureV2.Transitions {
			if transition.Cut.Proofs.ChunkCount == 0 || terminalProofCloses[transition.Identity.NoID] != transition.Cut.Proofs.ChunkCount { terminalCompleteAtFailure = false }
		}
	...
	result, err := VerifyReleaseMeasurementArtifactV2(t.Context(), fixture.current.artifact, options)
	if !errors.Is(err, failure) || terminalReads == 0 || currentReads == 0 || !terminalCompleteAtFailure { t.Fatalf("late current failure lost complete terminal/current traversal: closes=%v error=%v", terminalProofCloses, err) }
	assertReleaseMeasurementV2Empty(t, result)
}
...
func TestReleaseMeasurementV2SettlementRejectsPriorQualityDriftBeforeIO(t *testing.T) {
	...
	for _, mode := range []string{"changed", "omitted", "invented"} {
		source := fixture
		if mode == "invented" { source = sparse }
		...
		for index, provider := range artifact.Inputs[0].Stats.Providers {
			if provider.HasPriorQuality == (mode != "invented") { providerIndex = index; break }
		}
		if providerIndex < 0 { t.Fatalf("real prior-quality variation prerequisite absent: %s", mode) }
		provider := &artifact.Inputs[0].Stats.Providers[providerIndex]
		switch mode {
		case "changed": provider.PriorQualityPPM ^= 1
		case "omitted": provider.HasPriorQuality, provider.PriorQualityPPM = false, 0
		case "invented": provider.HasPriorQuality, provider.PriorQualityPPM = true, 12345
		}
		options := source.options(t)
		reads := observeReleaseMeasurementV2SettlementTest(&options)
		result, err := VerifyReleaseMeasurementArtifactV2(t.Context(), artifact, options)
		if err == nil || !strings.Contains(err.Error(), "prior EMA") || *reads != 0 { t.Fatalf("%s prior state escaped pre-I/O join: %v", mode, err) }
		assertReleaseMeasurementV2Empty(t, result)
	}
}
...
func TestReleaseMeasurementV2SettlementSameWindowRetryRetainsClosure(t *testing.T) {
	...
	if err != nil || !slices.Equal(current.HeadEMA, fixture.current.artifact.HeadEMA) || len(result.SettlementReplayByNO) != 2 { t.Fatalf("same-window complete-closure retry: %v", err) }
}
