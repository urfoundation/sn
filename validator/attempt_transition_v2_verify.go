...
func ownAttemptSettlementV2Options(ctx context.Context, options AttemptSettlementV2Options) (AttemptSettlementV2Options, error) {
	...
	for noID, operator := range options.Operators {
		if err := ctx.Err(); err != nil { return AttemptSettlementV2Options{}, err }
		...
		if _, err := attemptCutV2PolicyDepth(operator.Expected, operator.Policy); err != nil { return AttemptSettlementV2Options{}, err }
		...
		if err := operator.Measurement.Replay.Bounds.validate(operator.Bounds); err != nil { return AttemptSettlementV2Options{}, err }
		...
		}); err != nil { return AttemptSettlementV2Options{}, err }
		...
		if !filepath.IsAbs(scratch) || filepath.Clean(scratch) != scratch || scratchKVs[scratch] || operator.Measurement.Replay.ReadMetadata == nil || operator.Measurement.Replay.OpenData == nil {
		...
		for id, binding := range operator.Measurement.CurrentBindingKVs { bindings[id] = binding }
		...
		for id, key := range operator.Measurement.Replay.ServerKeys {
			if len(key) != ed25519.PublicKeySize { return AttemptSettlementV2Options{}, errors.New("compact settlement server-key history has an invalid key") }
...
func ownAttemptSettlementV2Transition(ctx context.Context, transition *AttemptSettlementTransitionV2, operator AttemptSettlementV2OperatorOptions, options AttemptSettlementV2Options, signed bool) (*AttemptSettlementTransitionV2, error) {
	...
	if err := transition.Cut.VerifyHeader(operator.Expected, operator.Bounds); err != nil { return nil, err }
	var hashes uint64
	for _, provider := range transition.PreFold.Providers {
		if err := ctx.Err(); err != nil { return nil, err }
		if uint64(len(provider.EgressIPHashHexes)) > options.MaxTransitionBytes-hashes { return nil, errors.New("compact settlement hash metadata exceeds its byte bound") }
	...
	for index, member := range transition.Batch {
		if member.NoID == 0 || index > 0 && member.NoID <= transition.Batch[index-1].NoID { return nil, errors.New("compact settlement member census is not canonical") }
		if _, err := canonicalAttemptHex32("compact settlement member digest", member.Digest, false); err != nil { return nil, err }
	}
	raw, err := json.Marshal(transition)
	if err != nil || uint64(len(raw))+1 > options.MaxTransitionBytes { return nil, errors.Join(errors.New("compact settlement transition exceeds its byte bound"), err) }
	owned := *transition
...
func admitAttemptSettlementV2(ctx context.Context, closure *AttemptSettlementClosureV2, options AttemptSettlementV2Options, signed bool) (*attemptSettlementV2Operation, error) {
	...
	if closure == nil || closure.Schema != AttemptSettlementClosureV2Schema || len(closure.Transitions) != len(ownedOptions.Operators) { return nil, errors.New("compact settlement closure omits or adds a configured participant") }
	...
	for index, transition := range closure.Transitions {
		if err := ctx.Err(); err != nil { return nil, err }
		if transition == nil || index > 0 && transition.Identity.NoID <= closure.Transitions[index-1].Identity.NoID { return nil, errors.New("compact settlement closure participant order is not canonical") }
		operator, ok := ownedOptions.Operators[transition.Identity.NoID]
		if !ok || operator.Expected.Boundary.SettlementEpoch != closure.Epoch { return nil, errors.New("compact settlement closure participant or epoch differs") }
		owned.Transitions[index], err = ownAttemptSettlementV2Transition(ctx, transition, operator, ownedOptions, signed)
		if err != nil { return nil, fmt.Errorf("compact settlement no_id %d: %w", transition.Identity.NoID, err) }
	}
	if err := boundAttemptSettlementV2Closure(owned, ownedOptions); err != nil { return nil, err }
	if signed {
		members, err := attemptSettlementV2Members(owned)
		if err != nil { return nil, err }
		for _, transition := range owned.Transitions {
			if !slices.Equal(members, transition.Batch) { return nil, errors.New("compact settlement complete member digest census differs") }
			...
			if err != nil || !ed25519.Verify(vpk[:], message, transition.Signature) { return nil, errors.New("compact settlement member signature is invalid") }
		}
	}
...
func boundAttemptSettlementV2Closure(closure *AttemptSettlementClosureV2, options AttemptSettlementV2Options) error {
	...
	for _, transition := range closure.Transitions {
		raw, err := json.Marshal(transition)
		if err != nil || uint64(len(raw))+1 > options.MaxTransitionBytes { return errors.Join(errors.New("compact settlement transition exceeds its byte bound"), err) }
		if uint64(len(raw)) > options.MaxClosureBytes-memberBytes { return errors.New("compact settlement closure exceeds its byte bound") }
		memberBytes += uint64(len(raw))
	}
	raw, err := json.Marshal(closure)
	if err != nil || uint64(len(raw))+1 > options.MaxClosureBytes { return errors.Join(errors.New("compact settlement closure exceeds its byte bound"), err) }
	return nil
}
...
func attemptSettlementV2Qualities(stats VerifiedReleaseStats) []AttemptSettlementQuality {
	...
	for id, provider := range stats.Providers {
		if provider.HasQuality { qualities = append(qualities, AttemptSettlementQuality{ClientID: id.String(), HasQuality: true, QualityPPM: provider.QualityPPM}) }
	}
...
func (self *attemptSettlementV2Operation) replay(ctx context.Context, visitRecord func(uint64, AttemptRecord) error, derive bool) (VerifiedAttemptSettlementV2, error) {
	...
	for _, transition := range self.closure.Transitions {
		if err := ctx.Err(); err != nil { return VerifiedAttemptSettlementV2{}, err }
		noID := transition.Identity.NoID
		operator := self.options.Operators[noID]
		var visit func(AttemptRecord) error
		if visitRecord != nil { visit = func(record AttemptRecord) error { return visitRecord(noID, record) } }
		...
		if err != nil { return VerifiedAttemptSettlementV2{}, fmt.Errorf("compact settlement no_id %d replay: %w", noID, err) }
		qualities := attemptSettlementV2Qualities(verified.Stats)
		if derive { transition.PostFold = qualities } else if !slices.Equal(qualities, transition.PostFold) { return VerifiedAttemptSettlementV2{}, errors.New("compact settlement post-fold EMA differs from the complete replay") }
...
func verifyAttemptSettlementClosureV2(ctx context.Context, closure *AttemptSettlementClosureV2, options AttemptSettlementV2Options, visitRecord func(uint64, AttemptRecord) error) (result VerifiedAttemptSettlementV2, resultErr error) {
	if ctx == nil { return result, errors.New("compact settlement context is nil") }
	defer func() { resultErr = errors.Join(resultErr, ctx.Err()); if resultErr != nil { result = VerifiedAttemptSettlementV2{} } }()
...
func SealAttemptSettlementBatchV2(ctx context.Context, inputs []AttemptSettlementV2Input, privateKeys map[uint64]ed25519.PrivateKey, options AttemptSettlementV2Options) (result *AttemptSettlementClosureV2, resultErr error) {
	...
	if err := ctx.Err(); err != nil { return nil, err }
	if len(inputs) == 0 || len(inputs) != len(options.Operators) || len(privateKeys) != len(inputs) || uint64(len(inputs)) > options.MaxParticipants { return nil, errors.New("compact settlement producer participant coverage differs") }
	...
	sort.Slice(closure.Transitions, func(i, j int) bool { return closure.Transitions[i].Identity.NoID < closure.Transitions[j].Identity.NoID })
	...
	for _, transition := range operation.closure.Transitions {
		...
		if err := attemptCutV2PrivateKey(key, vpk[:]); err != nil { return nil, err }
		ownedKeys[noID] = slices.Clone(key)
	}
	if _, err := operation.replay(ctx, nil, true); err != nil { return nil, err }
	...
	if err := boundAttemptSettlementV2Closure(operation.closure, operation.options); err != nil { return nil, err }
