...
func ObserveReleaseNativeSourcesV2(ctx context.Context, cfg *ReleaseConfig, native *crv4.Chain, hotkey [32]byte, adoption *ReleaseHistoryAdoptionV2) (result *ReleaseNativeSourceObservationV2, resultErr error) {
	...
	for index := range result.References {
		...
		if err := authenticateReleaseNativeSourceReferenceV2(ctx, native, cfg, &value.Intent, value.Artifact); err != nil {
			return nil, err
		}
		value.Lifecycle.MeasurementHash = value.Intent.MeasurementArtifactHash
		if err := observeReleaseDecisionLifecycleV2(ctx, native, cfg, &value.Intent, &value.Lifecycle); err != nil {
			return nil, err
		}
		if value.Intent.Status == "applied" {
			if err := authenticateAdoptedIntentApplicationV2(ctx, native, cfg, &value.Intent); err != nil {
				return nil, err
			}
		}
	}
...
func readReleaseNativeSourceReferencesV2(ctx context.Context, cfg *ReleaseConfig, hotkey [32]byte, adoption *ReleaseHistoryAdoptionV2) (result *ReleaseNativeSourceObservationV2, resultErr error) {
	...
	checkPublication := func() error {
		for _, name := range []string{releaseIntentV2Marker, releaseIntentV2Candidate} {
			...
			if !releaseMeasurementInputV2OnlyMissing(err) {
				return errors.Join(errors.New("V2 native publication is unresolved"), err)
			}
		}
		return nil
	}
	...
	defer func() {
		resultErr = errors.Join(resultErr, checkPublication())
		if resultErr != nil {
			result = nil
		}
	}()
	...
	if raw != nil {
		...
		if err != nil || file.Schema != SteeringIntentSchema || !bytes.Equal(canonical, raw) {
			return nil, errors.Join(errors.New("V2 native source store is not canonical"), err)
		}
	}
	...
	for index := range all {
		...
		if seen[intent.VectorHash] {
			return nil, errors.New("V2 native source repeats an intent")
		}
		...
		if err := matchObservedNativeSourceV2(cfg, hotkey, intent, artifact, envelope); err != nil {
			return nil, err
		}
		if previous == nil {
			if artifact.PreviousArtifactHash != "" {
				return nil, errors.New("V2 native source omits its first measurement")
			}
		} else {
...
func matchObservedNativeSourceV2(cfg *ReleaseConfig, hotkey [32]byte, intent *SteeringIntent, artifact *ReleaseMeasurementArtifact, envelope *ReleaseMeasurementEnvelope) error {
	if intent == nil || intent.Prepared == nil || artifact == nil || envelope == nil {
		return errors.New("V2 native observation reference is incomplete")
	}
