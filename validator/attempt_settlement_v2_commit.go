...
func initializeAttemptSettlementEpochV2(ctx context.Context, coordinator string, participants []AttemptSettlementRuntimeV2Participant, epoch uint64, authority AttemptSettlementV2Options, persistence AttemptSettlementRuntimeV2PersistenceBounds, physical attemptSettlementV2IO) (resultErr error) {
	...
	if err != nil { return err }
	defer batch.release()
	if err := admitAttemptSettlementV2LiveSnapshots(ctx, batch); err != nil { return err }
	...
	if exists {
		...
		if err := checkAttemptSettlementV2LiveImages(ctx, transaction, batch); err != nil { return err }
		if _, err := validateAttemptSettlementTransactionV2(ctx, transaction, batch.ordered, preimages, postimages, owned, batch.persistence); err != nil { return err }
	...
	if err := validateAttemptSettlementV2Activation(ctx, batch.ordered, batch.candidates, epoch, owned, batch.persistence); err != nil { return err }
	active := 0
	...
	if err := physical.writeImage(physical.roots[coordinator], filepath.Base(attemptSettlementTransactionV2Path(coordinator)), encoded, batch.persistence.MaxJournalBytes, physical.writeJournal); err != nil { return err }
	if err := finishAttemptSettlementTransactionV2(ctx, coordinator, transaction, batch, postimages, owned, physical, true); err != nil { return err }
	if err := physical.closeRoots(); err != nil { return err }
	if err := ctx.Err(); err != nil { return err }
	batch.clearReservation(epoch)
...
func advanceAttemptSettlementEpochV2(ctx context.Context, coordinator string, participants []AttemptSettlementRuntimeV2Participant, epoch uint64, terminalBoundary AttemptBoundary, options AttemptSettlementRuntimeV2Options, physical attemptSettlementV2IO, finishCurrent bool) (result *AttemptSettlementClosureV2, resultErr error) {
	...
	if err != nil { return nil, err }
	defer batch.release()
	if err := admitAttemptSettlementV2LiveSnapshots(ctx, batch); err != nil { return nil, err }
	allCurrent := true
	...
	if exists {
		...
		if err := checkAttemptSettlementV2LiveImages(ctx, transaction, batch); err != nil { return nil, err }
	...
	if err := physical.writeImage(physical.roots[coordinator], filepath.Base(attemptSettlementTransactionV2Path(coordinator)), encoded, batch.persistence.MaxJournalBytes, physical.writeJournal); err != nil { return nil, err }
	if err := finishAttemptSettlementTransactionV2(ctx, coordinator, transaction, batch, postimages, owned.Authority, physical, true); err != nil { return nil, err }
	if err := physical.closeRoots(); err != nil { return nil, err }
	if err := ctx.Err(); err != nil { return nil, err }
	batch.clearReservation(epoch)
...
func finishCurrentAttemptSettlementV2(ctx context.Context, coordinator string, epoch uint64, boundary AttemptBoundary, batch *attemptSettlementV2Owners, authority AttemptSettlementV2Options, physical attemptSettlementV2IO) (*AttemptSettlementClosureV2, error) {
	...
	for index, participant := range batch.ordered {
		...
		if err := validateAttemptSettlementV2LedgerState(ctx, participant, stats, authority.Operators[participant.NoID], batch.persistence.MaxSnapshotBytes); err != nil { return nil, err }
	...
	if err := ctx.Err(); err != nil { return nil, err }
	if err := physical.checkRoots(); err != nil { return nil, err }
	if err := physical.removeOwnedJournal(physical.roots[coordinator], filepath.Base(attemptSettlementTransactionV2Path(coordinator))); err != nil { return nil, err }
	if err := physical.checkRoots(); err != nil { return nil, err }
