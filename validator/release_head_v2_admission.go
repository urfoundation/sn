...
func (self *releaseHeadV2Budget) charge(count, width uint64) error {
	if self.used > self.limit || width != 0 && count > (self.limit-self.used)/width { return errors.New("compact head EMA control storage exceeds its bound") }
...
func (self *releaseHeadV2Budget) chargeOutput(count, width uint64) error {
	if err := self.charge(count, width); err != nil { return err }
...
func reserveReleaseHeadV2CollectionDerived(ctx context.Context, draft *ReleaseMeasurementArtifact, budget releaseHeadV2Budget) (releaseHeadV2Budget, error) {
	...
	if err := budget.charge(1, uint64(reflect.TypeFor[releaseHeadResult]().Size())); err != nil { return budget, err }
	if err := budget.charge(uint64(len(draft.Inputs)), 3*(8+mapBytes)); err != nil { return budget, err }
	if err := budget.charge(uint64(len(draft.ControlledNOIDs)), 8+1); err != nil { return budget, err }
	for _, input := range draft.Inputs {
		...
		if err := budget.charge(uint64(len(input.Stats.Providers)), width); err != nil { return budget, err }
	}
	return budget, nil
}
...
func orderedReleaseHeadV2Keys[T any](values map[FleetScoreKey]T) []FleetScoreKey {
	...
	sort.Slice(keys, func(i, j int) bool {
		...
		if order := bytes.Compare(left.FleetID[:], right.FleetID[:]); order != 0 { return order < 0 }
		if order := bytes.Compare(left.Hotkey[:], right.Hotkey[:]); order != 0 { return order < 0 }
...
func releaseHeadV2BitSum(parts ...uint64) (uint64, error) {
	...
	for _, part := range parts {
		if part > ^uint64(0)-total { return 0, errors.New("compact head EMA arithmetic growth overflows") }
		total += part
	}
	return total, nil
}
...
func releaseHeadV2DecimalBits(value string) (uint64, error) {
	if len(value) == 0 { return 0, errors.New("compact head EMA rational text is absent") }
	for _, digit := range value { if digit < '0' || digit > '9' { return 0, errors.New("compact head EMA rational text is not unsigned decimal") } }
	if value == "0" { return 0, nil }
	length := uint64(len(value))
	if length > (^uint64(0)-2)/10 { return 0, errors.New("compact head EMA rational text size overflows") }
...
func releaseHeadV2TextBound(value RationalJSON) (releaseHeadV2FractionBound, error) {
	...
	if err != nil { return releaseHeadV2FractionBound{}, err }
	denominator, err := releaseHeadV2DecimalBits(value.Denominator)
	if err != nil || denominator == 0 { return releaseHeadV2FractionBound{}, errors.Join(errors.New("compact head EMA denominator is absent or zero"), err) }
...
func (self releaseHeadV2FractionBound) reserve(budget *releaseHeadV2Budget, output bool) error {
	...
	if output { return budget.chargeOutput(words, wordBytes) }
...
func (self releaseHeadV2FractionBound) reserveArithmetic(budget *releaseHeadV2Budget) error {
	for range 8 { if err := self.reserve(budget, false); err != nil { return err } }
	return nil
}
...
func (self releaseHeadV2FractionBound) reserveText(budget *releaseHeadV2Budget) error {
	for _, length := range []uint64{self.numerator, self.denominator} {
		...
		if err := budget.chargeOutput(digits, 1); err != nil { return err }
		...
		if err := budget.charge(digits, 2); err != nil { return err }
	}
	return nil
}
...
func (self releaseHeadV2FractionBound) multiply(numerator, denominator uint64) (releaseHeadV2FractionBound, error) {
	if numerator == 0 || self.numerator == 0 { return releaseHeadV2FractionBound{denominator: 1}, nil }
...
func (self *HeadEMAStore) admitReleaseHeadV2KnownWithLock(ctx context.Context, epoch uint64, alpha protocol.Rational, maxEntries uint64, budget releaseHeadV2Budget) (releaseHeadV2Budget, error) {
	if err := ctx.Err(); err != nil { return budget, err }
	if err := alpha.Validate("head_score_ema"); err != nil || alpha.Numerator > alpha.Denominator { return budget, errors.New("invalid head EMA policy") }
	if uint64(len(self.values)) > maxEntries || uint64(len(self.lastFold)) > maxEntries { return budget, errors.New("compact head EMA census exceeds its bound") }
	if self.lastSubnetEpoch != nil {
		if epoch < *self.lastSubnetEpoch { return budget, errors.New("compact head EMA epoch regressed") }
		if epoch == *self.lastSubnetEpoch {
			if self.lastAlpha == nil || *self.lastAlpha != alpha { return budget, errors.New("same-epoch head EMA policy changed") }
		} else if *self.lastSubnetEpoch == ^uint64(0) || epoch != *self.lastSubnetEpoch+1 { return budget, errors.New("compact head EMA epoch jumped") }
	}
	remaining := budget.limit-budget.used
	for key, value := range self.values {
		if uint64(len(key)) > remaining { return budget, errors.New("compact head EMA key exceeds its control bound") }
		remaining -= uint64(len(key))
		if err := releaseMeasurementV2ControlStorage(ctx, reflect.ValueOf(&value), &remaining); err != nil { return budget, err }
	}
	if err := releaseMeasurementV2ControlStorage(ctx, reflect.ValueOf(self.lastFold), &remaining); err != nil { return budget, err }
...
func (self *HeadEMAStore) admitReleaseHeadV2Known(ctx context.Context, epoch uint64, alpha protocol.Rational, maxEntries uint64, budget releaseHeadV2Budget) error {
	if self == nil || ctx == nil || maxEntries == 0 || budget.limit == 0 || budget.limit > maxReleaseMeasurementArtifactBytes || budget.used > budget.limit { return errors.New("compact head EMA owner or bounds are invalid") }
	self.mu.Lock()
...
func admitReleaseHeadV2CurrentWithLock[T any](ctx context.Context, store *HeadEMAStore, epoch uint64, current map[FleetScoreKey]T, maxEntries uint64, budget releaseHeadV2Budget) (releaseHeadV2Budget, error) {
	...
	if count > maxEntries { return budget, errors.New("compact head EMA census exceeds its bound") }
	if err := budget.charge(count, releaseHeadV2KeyBytes); err != nil { return budget, err }
	entries := uint64(len(store.values))
	for key := range current {
		if err := ctx.Err(); err != nil { return budget, err }
		if _, found := store.values[key.String()]; !found {
			if entries == maxEntries { return budget, errors.New("compact head EMA current/history union exceeds its bound") }
			entries++
		}
	}
	if store.lastSubnetEpoch != nil && epoch == *store.lastSubnetEpoch {
		...
		for _, record := range store.lastFold {
			if record.HasRaw {
				expected++
				if _, found := current[record.Key]; !found { return budget, errors.New("same-epoch head EMA raw identities changed") }
			}
		}
		if expected != count { return budget, errors.New("same-epoch head EMA raw input cardinality changed") }
	...
	if err := budget.charge(count, 2*(keyBytes+pointerBytes+ratBytes)+keyBytes+boundBytes+2+uint64(reflect.TypeFor[releaseHeadV2UIDBound]().Size())); err != nil { return budget, err }
	...
	if err := budget.charge(entries, 2*releaseHeadV2KeyBytes+3*stringBytes+keyBytes+uint64(reflect.TypeFor[headEMAEntry]().Size())+10*ratBytes); err != nil { return budget, err }
	for _, entry := range store.values {
		if err := budget.charge(uint64(len(entry.Numerator))+uint64(len(entry.Denominator)), 1); err != nil { return budget, err }
	}
	if err := budget.chargeOutput(entries, uint64(reflect.TypeFor[HeadEMAMeasurement]().Size())); err != nil { return budget, err }
	if err := budget.chargeOutput(count, 2+ratBytes); err != nil { return budget, err }
	if budget.collection {
		...
		if err := budget.charge(count, 5*uint64(reflect.TypeFor[ExactWeightInput]().Size())+3*ratBytes+8*2+3); err != nil { return budget, err }
	}
	return budget, nil
}
...
func planReleaseHeadV2Raw(ctx context.Context, fleets map[FleetScoreKey]map[[32]byte]bool, budget releaseHeadV2Budget) (map[[32]byte]uint64, map[FleetScoreKey]releaseHeadV2FractionBound, releaseHeadV2Budget, error) {
	if err := budget.charge(uint64(len(fleets)), uint64(reflect.TypeFor[FleetScoreKey]().Size())); err != nil { return nil, nil, budget, err }
	claims := map[[32]byte]uint64{}
	for _, prefixes := range fleets {
		for prefix := range prefixes {
			if err := ctx.Err(); err != nil { return nil, nil, budget, err }
			if claims[prefix] == 0 { if err := budget.charge(1, 32+8); err != nil { return nil, nil, budget, err } }
			claims[prefix]++
		}
	}
	...
	for _, key := range orderedReleaseHeadV2Keys(fleets) {
		...
		for prefix := range prefixes {
			...
			if count > 1 && !seen[count] {
				if err := budget.charge(1, 8+1); err != nil { return nil, nil, budget, err }
				temporary += 8+1
				seen[count] = true
				var err error
				denominator, err = releaseHeadV2BitSum(denominator, uint64(bits.Len64(count-1)))
				if err != nil { return nil, nil, budget, err }
			}
		}
		...
		if len(prefixes) != 0 {
			...
			if err != nil { return nil, nil, budget, err }
			if err := work.reserveArithmetic(&budget); err != nil { return nil, nil, budget, err }
		}
	}
...
func (self *HeadEMAStore) planReleaseHeadV2PreviewWithLock(ctx context.Context, epoch uint64, raw map[FleetScoreKey]releaseHeadV2FractionBound, alpha protocol.Rational, budget releaseHeadV2Budget) (releaseHeadV2Budget, error) {
	if err := budget.charge(2, uint64(reflect.TypeFor[big.Rat]().Size())+2*uint64(reflect.TypeFor[big.Word]().Size())); err != nil { return budget, err }
	if err := budget.charge(uint64(len(raw)), uint64(reflect.TypeFor[FleetScoreKey]().Size())); err != nil { return budget, err }
	if err := budget.charge(uint64(len(self.values)), uint64(reflect.TypeFor[string]().Size())); err != nil { return budget, err }
	for _, value := range raw {
		if err := value.reserve(&budget, false); err != nil { return budget, errors.Join(errors.New("compact head EMA raw rational exceeds its control bound"), err) }
	}
	...
	if self.lastSubnetEpoch != nil && epoch == *self.lastSubnetEpoch {
		for _, record := range self.lastFold {
			...
			for _, value := range []RationalJSON{record.Raw, record.Prior, record.Next} {
				if err := budget.chargeOutput(uint64(len(value.Numerator))+uint64(len(value.Denominator)), 1); err != nil { return budget, err }
			}
			if record.HasRaw {
				current, err := releaseHeadV2TextBound(record.Raw)
				if err != nil { return budget, err }
				if err := current.reserveArithmetic(&budget); err != nil { return budget, err }
				next, err := releaseHeadV2TextBound(record.Next)
				if err != nil { return budget, err }
				if err := next.reserveArithmetic(&budget); err != nil { return budget, err }
				if err := addUID(record.Key.UID, next); err != nil { return budget, err }
			}
		}
	} else {
		planRecord := func(key FleetScoreKey, current, prior releaseHeadV2FractionBound, hasRaw, hasPrior bool) error {
			next := current
			if hasPrior {
				if err := prior.reserveArithmetic(&budget); err != nil { return err }
				...
				if err != nil { return err }
				if err := left.reserveArithmetic(&budget); err != nil { return err }
				if err := right.reserveArithmetic(&budget); err != nil { return err }
				next, err = left.add(right)
				if err != nil { return err }
			}
			if err := current.reserveArithmetic(&budget); err != nil { return err }
			if err := next.reserveArithmetic(&budget); err != nil { return err }
			for _, value := range []releaseHeadV2FractionBound{current, prior, next} { if err := value.reserveText(&budget); err != nil { return err } }
			if hasRaw {
				if err := next.reserve(&budget, false); err != nil { return err }
				if err := addUID(key.UID, next); err != nil { return err }
			}
			return nil
		}
		priorKeys := make([]string, 0, len(self.values))
		for key := range self.values { priorKeys = append(priorKeys, key) }
		...
		for _, id := range priorKeys {
			...
			if !found { current = releaseHeadV2FractionBound{denominator: 1} }
			if err := planRecord(entry.Key, current, prior, found, true); err != nil { return budget, err }
		}
		for _, key := range orderedReleaseHeadV2Keys(raw) {
			current := raw[key]
			if err := ctx.Err(); err != nil { return budget, err }
			if _, found := self.values[key.String()]; !found {
				if err := planRecord(key, current, releaseHeadV2FractionBound{denominator: 1}, true, false); err != nil { return budget, err }
			}
		}
	}
	...
	for _, value := range uidBounds {
		...
		if err := bound.reserveArithmetic(&budget); err != nil { return budget, err }
		if err := bound.reserve(&budget, true); err != nil { return budget, err }
		if budget.collection {
			...
			for range 3 { if err := bound.reserve(&budget, false); err != nil { return budget, err } }
		}
	...
	if budget.collection {
		...
		if err != nil { return budget, err }
		if err := comparison.reserveArithmetic(&budget); err != nil { return budget, err }
	}
	return budget, ctx.Err()
}
...
func buildReleaseHeadV2Raw(ctx context.Context, fleets map[FleetScoreKey]map[[32]byte]bool, claims map[[32]byte]uint64) (map[FleetScoreKey]*big.Rat, error) {
	...
	for key, prefixes := range fleets {
		...
		for prefix := range prefixes {
			if err := ctx.Err(); err != nil { return nil, err }
			if claims[prefix] == 0 { return nil, errors.New("compact head EMA prefix census is incomplete") }
...
func checkReleaseHeadV2Output(ctx context.Context, ema map[uint16]*big.Rat, head []HeadEMAMeasurement, budget releaseHeadV2Budget) error {
	...
	if err := releaseMeasurementV2ControlStorage(ctx, reflect.ValueOf(head), &remaining); err != nil { return err }
	output := releaseHeadV2Budget{limit: remaining}
	for _, value := range ema {
		if err := ctx.Err(); err != nil { return err }
		if value == nil { return errors.New("compact head EMA output rational is nil") }
		if err := output.charge(1, 2+uint64(reflect.TypeFor[big.Rat]().Size())); err != nil { return err }
		bound := releaseHeadV2FractionBound{numerator: uint64(value.Num().BitLen()), denominator: uint64(value.Denom().BitLen())}
		if err := bound.reserve(&output, false); err != nil { return err }
	}
	if budget.used > budget.limit || budget.output > budget.used { return errors.New("compact head EMA completed output exceeds combined control bound") }
	return ctx.Err()
}
...
func checkReleaseHeadV2Result(ctx context.Context, result releaseHeadResult, budget releaseHeadV2Budget) error {
	if budget.used > budget.limit { return errors.New("compact head completed result exceeds combined control bound") }
	remaining := budget.used
	fixed := uint64(reflect.TypeFor[releaseHeadResult]().Size())
	if fixed > remaining { return errors.New("compact head completed result fixed storage exceeds its bound") }
	remaining -= fixed
	for _, value := range []any{result.Inputs, result.Bindings, result.HeadEMA, result.StaleBindings, result.EligibleUIDs, result.SelectedUIDs, result.RejectedUIDs} {
		if err := releaseMeasurementV2ControlStorage(ctx, reflect.ValueOf(value), &remaining); err != nil { return err }
	}
	account := releaseHeadV2Budget{limit: remaining}
	for _, rows := range [][]ExactWeightInput{result.Weights, result.Eligible} {
		if err := account.charge(uint64(len(rows)), uint64(reflect.TypeFor[ExactWeightInput]().Size())); err != nil { return err }
		for _, row := range rows {
			if err := ctx.Err(); err != nil { return err }
			if row.Score == nil || row.Score.Sign() < 0 { return errors.New("compact head completed result score is nil or negative") }
			if err := account.charge(1, uint64(reflect.TypeFor[big.Rat]().Size())); err != nil { return err }
			value := releaseHeadV2FractionBound{numerator: uint64(row.Score.Num().BitLen()), denominator: uint64(row.Score.Denom().BitLen())}
			if err := value.reserve(&account, false); err != nil { return err }
		}
	}
	if err := account.charge(uint64(len(result.Bound)), 8+uint64(reflect.TypeFor[map[connect.Id]bool]().Size())); err != nil { return err }
	for _, members := range result.Bound {
		if err := ctx.Err(); err != nil { return err }
		if err := account.charge(uint64(len(members)), 16+1); err != nil { return err }
	}
	if err := account.charge(uint64(len(result.Controlled)), 2+1); err != nil { return err }
	return ctx.Err()
}
...
func (self *HeadEMAStore) previewForEpochV2WithBudget(ctx context.Context, epoch uint64, raw map[FleetScoreKey]*big.Rat, alpha protocol.Rational, maxEntries uint64, budget releaseHeadV2Budget) (map[uint16]*big.Rat, []HeadEMAMeasurement, error) {
	if self == nil || ctx == nil || maxEntries == 0 || budget.limit == 0 || budget.limit > maxReleaseMeasurementArtifactBytes || budget.used > budget.limit { return nil, nil, errors.New("compact head EMA owner or bounds are invalid") }
	self.mu.Lock()
	defer self.mu.Unlock()
	budget, err := self.admitReleaseHeadV2KnownWithLock(ctx, epoch, alpha, maxEntries, budget)
	if err != nil { return nil, nil, err }
	if uint64(len(raw)) > maxEntries { return nil, nil, errors.New("compact head EMA census exceeds its bound") }
	...
	for _, value := range raw {
		...
		if value == nil || value.Sign() < 0 { return nil, nil, errors.New("compact head EMA raw score is nil or negative") }
		bound := releaseHeadV2FractionBound{numerator: uint64(value.Num().BitLen()), denominator: uint64(value.Denom().BitLen())}
		if err := bound.reserve(&wordCheck, false); err != nil { return nil, nil, errors.Join(errors.New("compact head EMA raw rational exceeds its control bound"), err) }
	}
	...
	if err := checkReleaseHeadV2Output(ctx, ema, head, budget); err != nil { return nil, nil, err }
	return ema, head, nil
}

// The collector plans from actual authenticated prefixes while holding only
// the EMA state lock. No chain/client-key/proof callback runs under that lock.
func (self *HeadEMAStore) previewReleaseHeadV2Fleets(ctx context.Context, epoch uint64, fleets map[FleetScoreKey]map[[32]byte]bool, alpha protocol.Rational, maxEntries uint64, budget releaseHeadV2Budget) (map[uint16]*big.Rat, []HeadEMAMeasurement, releaseHeadV2Budget, error) {
	if self == nil || ctx == nil || maxEntries == 0 || budget.limit == 0 || budget.limit > maxReleaseMeasurementArtifactBytes || budget.used > budget.limit { return nil, nil, budget, errors.New("compact head EMA owner or bounds are invalid") }
	self.mu.Lock()
	defer self.mu.Unlock()
	budget, err := self.admitReleaseHeadV2KnownWithLock(ctx, epoch, alpha, maxEntries, budget)
	if err != nil { return nil, nil, budget, err }
	...
	if err != nil { return nil, nil, budget, err }
	ema, head, err := self.previewForEpochWithLock(epoch, raw, alpha)
	if err = errors.Join(err, ctx.Err()); err != nil { return nil, nil, budget, err }
	if err := checkReleaseHeadV2Output(ctx, ema, head, budget); err != nil { return nil, nil, budget, err }
