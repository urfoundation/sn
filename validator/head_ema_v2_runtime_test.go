...
func loadHeadEMAStoreV2RuntimeTest(t *testing.T, fixture *headEMAStoreV2TestFixture, limits HeadEMAStoreV2Limits) *HeadEMAStore {
	...
	if store.v2 == nil { t.Fatal("bounded constructor did not retain a runtime owner") }
	return store
}
...
func assertHeadEMAStoreV2Unchanged(t *testing.T, fixture *headEMAStoreV2TestFixture, store *HeadEMAStore) {
	...
	if err != nil || !bytes.Equal(encoded, fixture.encoded) { t.Fatalf("uncommitted head changed: %v", err) }
...
func assertHeadEMAStoreV2Fault(t *testing.T, fixture *headEMAStoreV2TestFixture, store *HeadEMAStore) {
	t.Helper()
	if store.v2.fault == nil { t.Fatal("uncertain durable operation left a reusable owner") }
	if store.lastSubnetEpoch == nil || *store.lastSubnetEpoch != 10 || !equalHeadEMAFolds(store.lastFold, fixture.records) {
	...
	if err != nil || !marker.Mode().IsRegular() { t.Fatalf("uncertain write lost its marker: %v", err) }
	callbacks := 0
	...
	if err == nil || out != nil || records != nil || callbacks != 0 { t.Fatalf("faulted owner continued: callbacks=%d error=%v", callbacks, err) }
	parsed := 0
	...
	if err == nil || reloaded != nil || parsed != 0 { t.Fatalf("unresolved marker was accepted on restart: parsed=%d error=%v", parsed, err) }
}
...
func TestHeadEMAStoreV2RuntimeLegacyPreviewRetainsLimits(t *testing.T) {
	...
	if err == nil || out != nil || records != nil { t.Fatalf("legacy preview bypassed v2 census: %v", err) }
...
func TestHeadEMAStoreV2RuntimeLegacyCommitRetainsLimits(t *testing.T) {
	...
	if err != nil || len(records) != 3 { t.Fatalf("real transcript fixture failed: %v", err) }
	if err := store.CommitForEpoch(11, records, fixture.alpha); err == nil { t.Fatal("legacy commit bypassed v2 census") }
	assertHeadEMAStoreV2Unchanged(t, fixture, store)
...
func TestHeadEMAStoreV2RuntimeLegacyFoldRetainsRationalLimits(t *testing.T) {
	...
	if err == nil || out != nil { t.Fatalf("legacy fold bypassed v2 arithmetic allowance: %v", err) }
	assertHeadEMAStoreV2Unchanged(t, fixture, store)
...
func TestHeadEMAStoreV2RuntimeLegacyFoldEpochRetainsOutputLimits(t *testing.T) {
	...
	if err == nil || out != nil || records != nil { t.Fatalf("legacy epoch fold bypassed v2 wire allowance: %v", err) }
	assertHeadEMAStoreV2Unchanged(t, fixture, store)
	if _, err := os.Lstat(filepath.Join(fixture.stateDir, headEMAStoreV2Marker)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("output refusal performed native publication: %v", err) }
...
func TestHeadEMAStoreV2RuntimeLegacyCommitRetainsLoadedIdentity(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := os.Rename(fixture.path, fixture.path+".held"); err != nil { t.Fatal(err) }
	if err := os.WriteFile(fixture.path, fixture.encoded, 0o600); err != nil { t.Fatal(err) }
	if err := store.CommitForEpoch(11, records, fixture.alpha); err == nil { t.Fatal("legacy commit replaced a different loaded inode") }
	encoded, err := os.ReadFile(fixture.path)
	if err != nil || !bytes.Equal(encoded, fixture.encoded) { t.Fatalf("identity refusal overwrote replacement: %v", err) }
...
func TestHeadEMAStoreV2RuntimeCommitsReloadsAndRetriesExactFold(t *testing.T) {
	...
	if err != nil || len(out) != 1 || out[7].Cmp(big.NewRat(15, 4)) != 0 || len(records) != 2 { t.Fatalf("exact preview differs: %v %v", out, err) }
	assertHeadEMAStoreV2Unchanged(t, fixture, store)
	if err := store.CommitForEpochV2(context.Background(), 11, records, fixture.alpha); err != nil { t.Fatal(err) }
	encoded, err := os.ReadFile(fixture.path)
	if err != nil { t.Fatal(err) }
	...
	if err != nil || !reflect.DeepEqual(again, out) || !equalHeadEMAFolds(retained, records) { t.Fatalf("restart repeated/changed fold: %v", err) }
	if err := reloaded.CommitForEpochV2(context.Background(), 11, records, fixture.alpha); err != nil { t.Fatal(err) }
	after, err := os.ReadFile(fixture.path)
	afterStatus, statErr := os.Stat(fixture.path)
	if err != nil || statErr != nil || !bytes.Equal(after, encoded) || !os.SameFile(status, afterStatus) { t.Fatalf("same-epoch retry rewrote durable head: %v %v", err, statErr) }
	...
	if err != nil || len(next) != 1 || next[7].Cmp(big.NewRat(15, 8)) != 0 || len(nextRecords) != 2 { t.Fatalf("successor decay differs: %v %v", next, err) }
	for _, name := range []string{headEMAStoreV2Marker, headEMAStoreV2Candidate} {
		if _, err := os.Lstat(filepath.Join(fixture.stateDir, name)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("clean commit left unresolved evidence %s: %v", name, err) }
	}
...
func TestHeadEMAStoreV2RuntimePreservesEpochAndUnscopedSemantics(t *testing.T) {
	...
	fixture := newHeadEMAStoreV2TestFixture(t)
	store := loadHeadEMAStoreV2RuntimeTest(t, fixture, headEMAStoreV2TestLimits())
	if out, records, err := store.PreviewForEpochV2(context.Background(), 12, fixture.raw, fixture.alpha); err == nil || out != nil || records != nil { t.Fatal("preview accepted an epoch jump") }
	if _, _, err := store.FoldForEpochV2(context.Background(), 12, fixture.raw, fixture.alpha); err != nil { t.Fatal(err) }
	if _, err := store.FoldV2(context.Background(), nil, fixture.alpha); err != nil { t.Fatal(err) }
	if store.lastSubnetEpoch != nil || store.lastAlpha != nil || store.lastFold != nil { t.Fatal("unscoped fold retained scoped metadata") }
	reloaded := loadHeadEMAStoreV2RuntimeTest(t, fixture, headEMAStoreV2TestLimits())
	if reloaded.lastSubnetEpoch != nil || reloaded.lastAlpha != nil || reloaded.lastFold != nil || len(reloaded.values) != 2 { t.Fatal("unscoped exact state changed across restart") }
...
func TestHeadEMAStoreV2RuntimeDoesNotRetagLegacyCreatedStores(t *testing.T) {
	t.Parallel()
	fixture := newHeadEMAStoreV2TestFixture(t)
	if fixture.store.v2 != nil { t.Fatal("legacy constructor acquired v2 ownership") }
	if _, _, err := fixture.store.PreviewForEpochV2(context.Background(), 11, fixture.raw, fixture.alpha); err == nil { t.Fatal("new API accepted legacy authority") }
	if _, _, err := fixture.store.PreviewForEpoch(11, fixture.raw, fixture.alpha); err != nil { t.Fatal(err) }
	if _, _, err := fixture.store.FoldForEpoch(12, fixture.raw, fixture.alpha); err != nil { t.Fatal(err) }
...
func TestHeadEMAStoreV2RuntimeRejectsKnownInvalidAdmissionBeforeCallbacks(t *testing.T) {
	...
	cancel()
	cases := []struct{ ctx context.Context; epoch uint64; alpha protocol.Rational }{
	...
	for index, candidate := range cases {
		...
		out, records, err := store.runHeadEMAStoreV2(candidate.ctx, headEMAStoreV2Preview, candidate.epoch, fixture.raw, nil, candidate.alpha,
			headEMAStoreV2RuntimeHooks{step: func(string, *os.File) error { callbacks++; return nil }})
		if err == nil || out != nil || records != nil || callbacks != 0 { t.Fatalf("known invalid case %d reached callbacks: %d %v", index, callbacks, err) }
	}
	assertHeadEMAStoreV2Unchanged(t, fixture, store)
...
func TestHeadEMAStoreV2RuntimeExactSharedReservationBoundary(t *testing.T) {
	...
	if err != nil || budget.used == 0 { t.Fatalf("complete reservation failed: %v", err) }
	for _, bound := range []uint64{budget.used-1, budget.used} {
		...
		out, records, err := bounded.runHeadEMAStoreV2(context.Background(), headEMAStoreV2Preview, 11, fixture.raw, nil, fixture.alpha,
			headEMAStoreV2RuntimeHooks{step: func(step string, _ *os.File) error { if step == "operands-owned" { owned++ }; return nil }})
		if bound < budget.used {
			if err == nil || out != nil || records != nil || owned != 0 { t.Fatalf("one-short shared reservation reached math: %d %v", owned, err) }
		} else if err != nil || len(out) != 2 || len(records) != 2 || owned != 1 { t.Fatalf("exact shared reservation failed: %d %v", owned, err) }
	}
...
func TestHeadEMAStoreV2RuntimePreservesTranscriptOnlyHistory(t *testing.T) {
	...
	if _, _, err := fixture.store.FoldForEpoch(11, nil, alpha); err != nil { t.Fatal(err) }
	store := loadHeadEMAStoreV2RuntimeTest(t, fixture, headEMAStoreV2TestLimits())
	if len(store.values) != 0 || len(store.lastFold) != 2 { t.Fatal("fixture lost actual transcript-only history") }
	out, records, err := store.PreviewForEpochV2(context.Background(), 11, nil, alpha)
	if err != nil || len(out) != 0 || len(records) != 2 { t.Fatalf("empty-live retry lost retained history: %v", err) }
	if err := store.CommitForEpochV2(context.Background(), 11, records, alpha); err != nil { t.Fatal(err) }
...
func TestHeadEMAStoreV2RuntimeOwnsInputsAndReturnedValues(t *testing.T) {
	...
	out, records, err := store.runHeadEMAStoreV2(context.Background(), headEMAStoreV2Preview, 11, raw, nil, fixture.alpha,
		headEMAStoreV2RuntimeHooks{step: func(step string, _ *os.File) error {
			if step == "operands-owned" { owned++; raw[fixture.keys[0]].SetInt64(99); delete(raw, fixture.keys[0]) }
			return nil
		}})
	if err != nil || owned != 1 || out[7].Cmp(big.NewRat(15, 4)) != 0 { t.Fatalf("raw ownership changed preview: %v", err) }
	out[7].SetInt64(123)
	...
	if err != nil || again[7].Cmp(big.NewRat(4, 1)) != 0 || !equalHeadEMAFolds(retained, fixture.records) { t.Fatalf("returned owner mutated durable state: %v", err) }
...
func TestHeadEMAStoreV2RuntimeOwnsCommitTranscriptBeforeCallbacks(t *testing.T) {
	...
	_, _, err = store.runHeadEMAStoreV2(context.Background(), headEMAStoreV2Commit, 11, nil, records, fixture.alpha,
		headEMAStoreV2RuntimeHooks{step: func(step string, _ *os.File) error {
			if step == "operands-owned" { records[0].Next.Numerator = "999" }
			return nil
		}})
	if err != nil || !equalHeadEMAFolds(store.lastFold, expected) { t.Fatalf("caller mutation changed committed transcript: %v", err) }
	reloaded := loadHeadEMAStoreV2RuntimeTest(t, fixture, headEMAStoreV2TestLimits())
	if !equalHeadEMAFolds(reloaded.lastFold, expected) { t.Fatal("restart changed owned commit transcript") }
...
func TestHeadEMAStoreV2RuntimeValidationRollbackAllowsGenuineRetry(t *testing.T) {
	...
	if err := store.CommitForEpochV2(context.Background(), 11, bad, fixture.alpha); err == nil { t.Fatal("invalid transcript was accepted") }
	assertHeadEMAStoreV2Unchanged(t, fixture, store)
	if store.v2.fault != nil { t.Fatal("pure validation failure became durable uncertainty") }
	if err := store.CommitForEpochV2(context.Background(), 11, records, fixture.alpha); err != nil { t.Fatal(err) }
...
func TestHeadEMAStoreV2RuntimeCallbacksOutsideMutexRejectReentry(t *testing.T) {
	...
	out, records, err := store.runHeadEMAStoreV2(context.Background(), headEMAStoreV2Preview, 11, fixture.raw, nil, fixture.alpha,
		headEMAStoreV2RuntimeHooks{step: func(step string, _ *os.File) error {
			if step != "operands-owned" { return nil }
			observed = true
			if !store.mu.TryLock() { t.Error("external callback ran under EMA state mutex") } else { store.mu.Unlock() }
			nested, transcript, err := store.PreviewForEpochV2(context.Background(), 11, fixture.raw, fixture.alpha)
			if !errors.Is(err, errHeadEMAStoreV2Busy) || nested != nil || transcript != nil { t.Errorf("reentrant owner did not refuse promptly: %v", err) }
			return nil
		}})
	if err != nil || !observed || len(out) != 2 || len(records) != 2 { t.Fatalf("outer operation changed after safe reentry refusal: %v", err) }
...
func TestHeadEMAStoreV2RuntimeConcurrentOwnerAdmission(t *testing.T) {
	...
	go func() {
		_, _, err := store.runHeadEMAStoreV2(context.Background(), headEMAStoreV2Preview, 11, fixture.raw, nil, fixture.alpha,
			headEMAStoreV2RuntimeHooks{step: func(step string, _ *os.File) error {
				if step == "operands-owned" { close(entered); <-release }
				return nil
			}})
		done <- err
	}()
	...
	close(release)
	outerErr := <-done
	if !errors.Is(err, errHeadEMAStoreV2Busy) || outerErr != nil { t.Fatalf("concurrent ownership differs: inner=%v outer=%v", err, outerErr) }
	assertHeadEMAStoreV2Unchanged(t, fixture, store)
...
func TestHeadEMAStoreV2RuntimeCancellationBeforeAcquisitionAllowsRetry(t *testing.T) {
	...
	out, records, err := store.runHeadEMAStoreV2(ctx, headEMAStoreV2FoldEpoch, 11, fixture.raw, nil, fixture.alpha,
		headEMAStoreV2RuntimeHooks{step: func(step string, _ *os.File) error {
			if step == "operands-owned" { cancel() }
			if step == "directory-opened" { acquired = true }
			return nil
		}})
	if !errors.Is(err, context.Canceled) || acquired || out != nil || records != nil || store.v2.fault != nil { t.Fatalf("pre-I/O cancellation changed ownership: %v", err) }
	assertHeadEMAStoreV2Unchanged(t, fixture, store)
	if _, _, err := store.PreviewForEpochV2(context.Background(), 11, fixture.raw, fixture.alpha); err != nil { t.Fatal(err) }
...
func TestHeadEMAStoreV2RuntimeCurrentPriorUnionPrecedesMath(t *testing.T) {
	...
	out, records, err := store.runHeadEMAStoreV2(context.Background(), headEMAStoreV2Preview, 11, raw, nil, fixture.alpha,
		headEMAStoreV2RuntimeHooks{step: func(string, *os.File) error { callbacks++; return nil }})
	if err == nil || !strings.Contains(err.Error(), "union") || out != nil || records != nil || callbacks != 0 { t.Fatalf("current/history union was not admitted first: %d %v", callbacks, err) }
...
func TestHeadEMAStoreV2RuntimeRetainsGenerationAndUIDArithmetic(t *testing.T) {
	...
	if err != nil || len(expected) != 3 { t.Fatalf("independent genuine fold fixture differs: %v", err) }
	out, records, err := store.FoldForEpochV2(context.Background(), 11, raw, fixture.alpha)
	if err != nil || !reflect.DeepEqual(out, want) || !equalHeadEMAFolds(records, expected) { t.Fatalf("bounded full fold changed generation/UID arithmetic: %v", err) }
	for _, record := range records {
		if record.Key == newKey && (record.HasPrior || record.Next != (RationalJSON{Numerator: "3", Denominator: "19"})) { t.Fatal("new generation inherited prior UID ownership") }
	}
...
func TestHeadEMAStoreV2RuntimeExactWireAndEscapedOutputBoundary(t *testing.T) {
	...
	if err := json.Unmarshal(fixture.encoded, &original); err != nil { t.Fatal(err) }
	for _, timestamp := range []string{"2026-09-06T00:00:00Z", "<>&\t\\\"\u2028\u2029☃", string([]byte{'x', 0xff, 0, 'y'})} {
		...
		for _, limit := range []uint64{uint64(len(want)-1), uint64(len(want))} {
			...
			if limit < uint64(len(want)) {
				if err == nil || encoded != nil || budget.used != 0 { t.Fatalf("one-short output was allocated/admitted: %v", err) }
			} else if err != nil || !bytes.Equal(encoded, want) || budget.used != uint64(len(want)) { t.Fatalf("exact legacy wire differs: %v", err) }
		}
	}
...
func TestHeadEMAStoreV2RuntimePreservesNilEmptyWireSemantics(t *testing.T) {
	...
	for index, file := range files {
		...
		if err != nil || !bytes.Equal(encoded, want) { t.Fatalf("nil/empty wire case %d changed: %v", index, err) }
	}
...
func TestHeadEMAStoreV2RuntimeFieldCensusPinsAdmissionOwners(t *testing.T) {
	t.Parallel()
	cases := []struct{ value reflect.Type; fields string }{
	...
	for _, candidate := range cases {
		var fields []string
		for index := 0; index < candidate.value.NumField(); index++ { fields = append(fields, candidate.value.Field(index).Name) }
		if strings.Join(fields, " ") != candidate.fields { t.Errorf("%s field census changed: %v", candidate.value.Name(), fields) }
	}
