...
func collectCadenceReplayObjectsV2(t *testing.T, artifact *ReleaseMeasurementArtifact, options ReleaseMeasurementV2Options, objects map[string][]byte) {
	...
	retain := func(kind, hash string, raw []byte) {
		key := kind + ":" + hash
		if previous, found := objects[key]; found && !bytes.Equal(previous, raw) {
			t.Fatal("fixture rewrote an immutable replay object")
		}
	...
	for id, operator := range options.Operators {
		wrap(&operator.Measurement.Replay)
		options.Operators[id] = operator
	}
	if options.Settlement != nil {
		for id, operator := range options.Settlement.Operators {
			wrap(&operator.Measurement.Replay)
			options.Settlement.Operators[id] = operator
		}
	}
	if _, err := VerifyReleaseMeasurementArtifactV2(t.Context(), artifact, options); err != nil {
		t.Fatalf("actual cadence fixture body failed: %v", err)
	}
}
...
func TestReleaseMeasurementV2CadenceReplaysTwoSettlementsForOneNativeSuccessor(t *testing.T) {
	...
	collectCadenceReplayObjectsV2(t, current.artifact, last.options(t), objects)
	if !consecutiveNativeSettlementGapV2(previous.artifact, current.artifact) {
	...
	for _, input := range previous.artifact.Inputs {
		...
		for _, quality := range history.terminals[previous.artifact.SettlementEpoch].Transitions[len(history.participants)-1].PostFold {
			positive = positive || quality.QualityPPM > 0
		}
	}
	if !positive {
		t.Fatal("cadence regression requires real positive pool quality")
	}
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/sn/attempt-artifact" || !found {
			http.NotFound(w, r)
			return
		}
		...
		if corrupt.Load() && kind == AttemptStreamV2Records {
			raw = bytes.Clone(raw)
			raw[0] ^= 1
		}
		_, _ = w.Write(raw)
	...
	verify := func() error {
		return store.verifyMeasurementLineageV2(t.Context(), previousBytes, previous.options(t), current.artifact, last.options(t))
	}
	if err := verify(); err != nil {
		t.Fatalf("consecutive native cadence failed complete terminal replay: %v", err)
	}
	if dataReads.Load() == 0 || store.v2.historyAdoption != nil || store.v2.provisionalEpochGaps || history.retainedStartup {
		t.Fatal("cadence did not use actual strict record replay")
	}
	corrupt.Store(true)
	if err := verify(); err == nil {
		t.Fatal("cadence accepted changed real terminal records")
	}
	corrupt.Store(false)
	...
	if err := verify(); err == nil {
		t.Fatal("cadence omitted an interior settlement")
	}
	history.terminals[interior] = original
	original.Transitions[0].Signature[0] ^= 1
	if err := verify(); err == nil {
		t.Fatal("cadence accepted changed interior signature")
	}
	original.Transitions[0].Signature[0] ^= 1
	current.artifact.SubnetEpoch++
	if err := verify(); err == nil {
		t.Fatal("natural cadence allowed a missed native epoch")
	}
	current.artifact.SubnetEpoch--
	if _, err := VerifyReleaseMeasurementLineageV2(t.Context(), previousBytes, previous.options(t), current.artifact, last.options(t)); err == nil {
		t.Fatal("standalone lineage fabricated missing interior terminal authority")
	}
}
