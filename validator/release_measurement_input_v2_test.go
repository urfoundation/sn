...
func (self *releaseMeasurementInputV2TestFixture) restart(t *testing.T) {
	t.Helper()
	if err := self.runtime.ledger.Close(); err != nil { t.Fatal(err) }
	...
	if err := stats.Load(self.runtime.dir); err != nil { t.Fatal(err) }
	if err := stats.AttachAttemptLedger(ledger, self.runtime.dir); err != nil { t.Fatal(err) }
...
func TestReleaseMeasurementInputV2RuntimeRestartsExactGenuineInput(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2RuntimeRestartsExactGenuineInput") } })
	...
	if err != nil { t.Fatal(err) }
	if input.AttemptCutV2 == nil || input.AttemptCutV2.RecordCount != 8 || input.AttemptCutV2.CompleteCount != 1 || input.Stats.AttemptCut != nil || input.Stats.SettlementTransition != nil || input.EgressGeneration != 1 || fixture.runtime.stats.egressGeneration != 2 { t.Fatal("ordinary journal lacks actual compact M8 evidence") }
	journal, err := decodeReleaseMeasurementInputV2(t.Context(), before, fixture.fresh(t))
	if err != nil || journal.Schema != releaseMeasurementInputV2Schema || !reflect.DeepEqual(journal.MeasurementInput, input) { t.Fatalf("canonical compact private input: %v", err) }
	...
	if err != nil || !os.SameFile(info, current) || !bytes.Equal(before, after) || !reflect.DeepEqual(restarted, input) || fixture.runtime.objects.writes != writes || fixture.runtime.objects.reads <= reads || fixture.runtime.stats.egressGeneration != 2 { t.Fatalf("restart changed immutable evidence or repeated rotation: %v", err) }
}
...
func TestReleaseMeasurementInputV2RuntimeRecoversActualRenameFailure(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2RuntimeRecoversActualRenameFailure") } })
	fixture := newReleaseMeasurementInputV2TestFixture(t)
	...
	if !errors.As(err, &rename) || rename.Op != "rename" || !reflect.DeepEqual(input, ReleaseMeasurementInput{}) || fixture.runtime.stats.egressGeneration != 1 || !fixture.runtime.stats.attemptCutPending { t.Fatalf("real journal-first failure escaped: %v", err) }
	path := releaseMeasurementInputV2Path(fixture.steerer.cfg.StateDir, 7, 9)
	before, err := readReleaseMeasurementInputV2(path, fixture.options.MaxJournalBytes)
	if err != nil { t.Fatal(err) }
	journal, err := decodeReleaseMeasurementInputV2(t.Context(), before, fixture.fresh(t))
	if err != nil || journal.MeasurementInput.AttemptCutV2.LastSequence != 8 { t.Fatalf("actual durable input missing: %v", err) }
	writes := fixture.runtime.objects.writes
	restore()
	fixture.restart(t)
	recovered := fixture.detach(t)
	after, err := readReleaseMeasurementInputV2(path, fixture.options.MaxJournalBytes)
	if err != nil || !bytes.Equal(before, after) || !reflect.DeepEqual(recovered, journal.MeasurementInput) || fixture.runtime.objects.writes != writes || fixture.runtime.stats.egressGeneration != 2 || fixture.runtime.stats.attemptCutPending { t.Fatalf("journal-first restart did not reconcile once: %v", err) }
}
...
func TestReleaseMeasurementInputV2RuntimePreservesV1JournalNamespace(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2RuntimePreservesV1JournalNamespace") } })
	fixture := newReleaseMeasurementInputV2TestFixture(t)
	...
	for _, record := range fixture.runtime.source.recordTs { if _, err := legacy.AppendContext(t.Context(), record); err != nil { t.Fatal(err) } }
	...
	if err != nil { t.Fatal(err) }
	if _, err := decodeReleaseMeasurementInput(encoded); err != nil { t.Fatalf("genuine v1 history is not replayable: %v", err) }
	oldPath := releaseMeasurementInputPath(cfg.StateDir, 7, 9)
	if err := atomicStateWrite(oldPath, encoded, 0o600); err != nil { t.Fatal(err) }
	...
	if err != nil || !bytes.Equal(encoded, preserved) || !os.SameFile(info, current) { t.Fatalf("compact input rewrote v1 history: %v", err) }
	compact, err := readReleaseMeasurementInputV2(releaseMeasurementInputV2Path(cfg.StateDir, 7, 9), fixture.options.MaxJournalBytes)
	if err != nil { t.Fatal(err) }
	if _, err := decodeReleaseMeasurementInput(compact); err == nil { t.Fatal("v1 decoder accepted compact journal") }
	if _, err := decodeReleaseMeasurementInputV2(t.Context(), encoded, fixture.fresh(t)); err == nil { t.Fatal("compact decoder materialized legacy journal history") }
}
...
func TestReleaseMeasurementInputV2ImmutablePublicationConflict(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2ImmutablePublicationConflict") } })
	fixture := newReleaseMeasurementInputV2TestFixture(t)
	fixture.detach(t)
	...
	for _, raw := range [][]byte{first, second} { go func(raw []byte) { ready.Done(); <-start; results <- writeReleaseMeasurementInputV2(path, raw, fixture.options.MaxJournalBytes) }(raw) }
	ready.Wait()
	close(start)
	left, right := <-results, <-results
	if (left == nil) == (right == nil) { t.Fatalf("immutable race must have exactly one winner: %v / %v", left, right) }
	winner, err := readReleaseMeasurementInputV2(path, fixture.options.MaxJournalBytes)
	if err != nil || !bytes.Equal(winner, first) && !bytes.Equal(winner, second) { t.Fatalf("race produced partial or invented bytes: %v", err) }
	info, err := os.Lstat(path)
	if err != nil { t.Fatal(err) }
	if err := writeReleaseMeasurementInputV2(path, winner, fixture.options.MaxJournalBytes); err != nil { t.Fatal(err) }
	current, err := os.Lstat(path)
	if err != nil || !os.SameFile(info, current) { t.Fatalf("identical retry replaced committed inode: %v", err) }
}
...
func TestReleaseMeasurementInputV2RejectsUnsafeBoundedFiles(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2RejectsUnsafeBoundedFiles") } })
	fixture := newReleaseMeasurementInputV2TestFixture(t)
	fixture.detach(t)
	...
	for _, kind := range []string{"symlink", "public", "directory", "oversize", "zero-bound", "overflow-bound"} {
		...
		case "symlink":
			...
			if err := os.WriteFile(target, encoded, 0o600); err != nil { t.Fatal(err) }
			if err := os.Symlink(target, path); err != nil { t.Fatal(err) }
		default:
			if err := os.WriteFile(path, encoded, 0o600); err != nil { t.Fatal(err) }
			if kind == "public" { if err := os.Chmod(path, 0o644); err != nil { t.Fatal(err) } }
			if kind == "oversize" { limit = uint64(len(encoded))-1 }
			if kind == "zero-bound" { limit = 0 }
			if kind == "overflow-bound" { limit = ^uint64(0) }
		}
		if raw, err := readReleaseMeasurementInputV2(path, limit); err == nil || raw != nil { t.Fatalf("unsafe %s input was admitted: %v", kind, err) }
	}
}
...
func TestReleaseMeasurementInputV2RejectsMalformedAndLegacyGraphs(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2RejectsMalformedAndLegacyGraphs") } })
	fixture := newReleaseMeasurementInputV2TestFixture(t)
	fixture.detach(t)
	encoded, err := readReleaseMeasurementInputV2(releaseMeasurementInputV2Path(fixture.steerer.cfg.StateDir, 7, 9), fixture.options.MaxJournalBytes)
	if err != nil { t.Fatal(err) }
	for _, kind := range []string{"legacy-cut", "legacy-transition", "unknown", "truncated", "trailing", "noncanonical"} {
		...
		case "legacy-cut", "legacy-transition", "unknown":
			...
			if err := json.Unmarshal(changed, &outer); err != nil { t.Fatal(err) }
			if kind == "unknown" { outer["unexpected"] = json.RawMessage("true") } else {
				var input, stats map[string]json.RawMessage
				if err := json.Unmarshal(outer["measurement_input"], &input); err != nil { t.Fatal(err) }
				if err := json.Unmarshal(input["stats"], &stats); err != nil { t.Fatal(err) }
				field := "attempt_cut"
				if kind == "legacy-transition" { field = "settlement_transition" }
			...
			if err != nil { t.Fatal(err) }
		case "truncated": changed = changed[:len(changed)/2]
		case "trailing": changed = append(changed, []byte("{}")...)
		case "noncanonical": changed = append([]byte(" "), changed...)
		}
		reads := fixture.runtime.objects.reads
		if journal, err := decodeReleaseMeasurementInputV2(t.Context(), changed, fixture.fresh(t)); err == nil || journal != nil || fixture.runtime.objects.reads != reads { t.Fatalf("%s compact wire was accepted or reached object I/O: %v", kind, err) }
	}
}
...
func TestReleaseMeasurementInputV2RejectsConflictingJournalLabels(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2RejectsConflictingJournalLabels") } })
	fixture := newReleaseMeasurementInputV2TestFixture(t)
	fixture.detach(t)
	path := releaseMeasurementInputV2Path(fixture.steerer.cfg.StateDir, 7, 9)
	...
	for _, edit := range []func(*releaseMeasurementInputJournal){
		...
		func(j *releaseMeasurementInputJournal) { j.MeasurementInput.CutNativeBlockHash = attemptHex32([32]byte{0x35}) },
		func(j *releaseMeasurementInputJournal) { j.MeasurementInput.CutEVMSnapshotHash = attemptHex32([32]byte{0x36}) },
	} {
		...
		if err != nil { t.Fatal(err) }
		if err := os.WriteFile(path, changed, 0o600); err != nil { t.Fatal(err) }
		reads := fixture.runtime.objects.reads
		if input, err := fixture.steerer.loadOrDetachReleaseMeasurementInputV2(t.Context(), 9, 7, 200, fixture.nativeHash, fixture.snapshot, fixture.fresh(t)); err == nil || !reflect.DeepEqual(input, ReleaseMeasurementInput{}) { t.Fatalf("conflicting label escaped admission: %v", err) }
		if fixture.runtime.objects.reads != reads || !reflect.DeepEqual(fixture.runtime.stats.snapshotStats(), before) { t.Fatal("outer label changed live ownership or reached object replay") }
	}
}
...
func TestReleaseMeasurementInputV2RejectsAlternateActivationPins(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2RejectsAlternateActivationPins") } })
	fixture := newReleaseMeasurementInputV2TestFixture(t)
	fixture.detach(t)
	...
	if err == nil || !reflect.DeepEqual(input, ReleaseMeasurementInput{}) || fixture.runtime.objects.reads != reads || !reflect.DeepEqual(before, fixture.runtime.stats.snapshotStats()) { t.Fatalf("journal chose a different activation: %v", err) }
}
...
func TestReleaseMeasurementInputV2RejectsForeignConfigAtFirstPublication(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2RejectsForeignConfigAtFirstPublication") } })
	for _, edit := range []func(*ReleaseConfig){
		...
		input, err := fixture.steerer.loadOrDetachReleaseMeasurementInputV2(t.Context(), 9, 7, 200, fixture.nativeHash, fixture.snapshot, fixture.fresh(t))
		if err == nil || !reflect.DeepEqual(input, ReleaseMeasurementInput{}) || fixture.runtime.stats.egressGeneration != 1 { t.Fatalf("foreign config published a compact window: %v", err) }
		if _, err := os.Lstat(releaseMeasurementInputV2Path(fixture.steerer.cfg.StateDir, 7, 9)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("foreign config left an input journal: %v", err) }
	}
}
...
func TestReleaseMeasurementInputV2HonorsAdmissionAndByteBounds(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("ORDINARY-V2-RUNTIME-v1 PASS TestReleaseMeasurementInputV2HonorsAdmissionAndByteBounds") } })
	for _, kind := range []string{"missing-bound", "relative-root", "canceled", "small-bound"} {
		fixture := newReleaseMeasurementInputV2TestFixture(t)
		options := fixture.fresh(t)
		ctx, cancel := context.WithCancel(t.Context())
		switch kind {
		case "missing-bound": options.MaxJournalBytes = 0
		case "relative-root": fixture.steerer.cfg.StateDir = "relative-state"
		case "canceled": cancel()
		case "small-bound": options.MaxJournalBytes = 1
		}
		input, err := fixture.steerer.loadOrDetachReleaseMeasurementInputV2(ctx, 9, 7, 200, fixture.nativeHash, fixture.snapshot, options)
		cancel()
		if err == nil || !reflect.DeepEqual(input, ReleaseMeasurementInput{}) || fixture.runtime.stats.egressGeneration != 1 { t.Fatalf("%s crossed journal admission: %v", kind, err) }
		if kind != "small-bound" && fixture.runtime.objects.writes != 0 { t.Fatalf("%s reached object writes", kind) }
	}
}
