...
func (self headEMAStoreV2RuntimeHooks) observe(ctx context.Context, step string, file *os.File) error {
	...
	if self.step != nil {
		if err := self.step(step, file); err != nil { return errors.Join(err, ctx.Err()) }
	}
	return ctx.Err()
}
...
func (self *HeadEMAStore) runHeadEMAStoreV2(ctx context.Context, operation headEMAStoreV2Operation, epoch uint64, raw map[FleetScoreKey]*big.Rat, records []HeadEMAMeasurement, alpha protocol.Rational, hooks headEMAStoreV2RuntimeHooks) (out map[uint16]*big.Rat, transcript []HeadEMAMeasurement, resultErr error) {
	if ctx == nil { return nil, nil, errors.New("bounded head EMA context is nil") }
	if err := ctx.Err(); err != nil { return nil, nil, err }
	if self == nil || self.v2 == nil { return nil, nil, errors.New("head EMA store was not created by the bounded constructor") }
	owner := self.v2
	if !owner.active.CompareAndSwap(false, true) { return nil, nil, errors.Join(errHeadEMAStoreV2Busy, ctx.Err()) }
	durablyPublished := false
	defer owner.active.Store(false)
	defer func() {
		resultErr = errors.Join(resultErr, ctx.Err())
		if resultErr != nil && durablyPublished { owner.fault = errors.Join(owner.fault, resultErr) }
		if resultErr != nil { out, transcript = nil, nil }
	}()
	if owner.fault != nil { return nil, nil, errors.Join(errors.New("bounded head EMA owner requires unresolved-write investigation or fresh admission"), owner.fault) }
	...
	err := func() error {
		...
		if self.lastSubnetEpoch != nil { value := *self.lastSubnetEpoch; scratch.lastSubnetEpoch = &value }
		if self.lastAlpha != nil { value := *self.lastAlpha; scratch.lastAlpha = &value }
	...
	if err := hooks.observe(ctx, "operands-owned", nil); err != nil { return nil, nil, err }
	if operation == headEMAStoreV2Commit && scratch.lastSubnetEpoch != nil && epoch == *scratch.lastSubnetEpoch {
		if !equalHeadEMAFolds(scratch.lastFold, ownedRecords) { return nil, nil, errors.New("committed same-epoch head EMA transcript changed") }
	...
	if err := checkHeadEMAStoreV2Completed(ctx, scratch, out, transcript, budget); err != nil { return nil, nil, err }
	if err := hooks.observe(ctx, "fold-complete", nil); err != nil { return nil, nil, err }
	if !write {
	...
	if err := ctx.Err(); err != nil { owner.fault = err; return nil, nil, err }
	return out, transcript, nil
}
