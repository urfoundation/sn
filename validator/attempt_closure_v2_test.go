...
func attemptSettlementV2TestEmptySuccessor(t *testing.T, previous *attemptCutV2StatsTestFixture, transition *AttemptSettlementTransitionV2, generations uint64) *attemptCutV2StatsTestFixture {
	...
	if err != nil || cut == nil || cut.RecordCount != 0 || cut.Root != previous.cut.Root { t.Fatalf("real empty successor: %v", err) }
...
func TestAttemptSettlementV2CanonicalRestartReplaysExactBytes(t *testing.T) {
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		if err != nil || !reflect.DeepEqual(decoded, closure) || result.Operators[9].Replay.Records.ItemCount != 18 || len(reads) == 0 { t.Fatalf("canonical restart replay: %v", err) }
		if attempt == 0 { firstReads = reads } else if !reflect.DeepEqual(firstReads, reads) { t.Fatal("restart reused a verdict or changed full replay work") }
	}
}
...
func TestAttemptSettlementV2DecodeRejectsCompetingAndNoncanonicalWire(t *testing.T) {
	...
	for _, candidate := range [][]byte{
		...
		if err == nil || decoded != nil || result.Operators != nil || len(reads) != 0 { t.Fatalf("competing/noncanonical closure accepted or read: %v", err) }
	}
}
...
func TestAttemptSettlementV2ConsecutiveClosureLineageAndLaterNativeRotation(t *testing.T) {
	...
	if err != nil || result.Operators[9].Replay.Records.ItemCount != 0 || !slices.Equal(prior.Transitions[0].PostFold, next.Transitions[0].PostFold) { t.Fatalf("consecutive terminal lineage lost retained quality: %v", err) }
	if err := verifyAttemptSettlementV2Successor(prior.Transitions[0], nextFixture.measurement, nextFixture.cut, true); err == nil { t.Fatal("later native rotation was mistaken for the immediate successor") }
	immediateFixture := attemptSettlementV2TestEmptySuccessor(t, fixture, prior.Transitions[0], 1)
	if err := verifyAttemptSettlementV2Successor(prior.Transitions[0], immediateFixture.measurement, immediateFixture.cut, true); err != nil { t.Fatalf("exact immediate settlement rotation refused: %v", err) }
}
...
func TestAttemptSettlementV2LineageRejectsResignedPriorEMADrift(t *testing.T) {
	...
	next := sealAttemptSettlementV2Test(t, nextFixture)
	result, err := VerifyAttemptSettlementClosureV2Lineage(t.Context(), prior, next, attemptSettlementV2TestOptions(t, fixture), attemptSettlementV2TestOptions(t, nextFixture))
	if err == nil || result.Operators != nil { t.Fatal("validly signed new window replaced the actual preceding fold") }
}
...
func TestAttemptSettlementV2SuccessorRejectsCursorBoundaryAndPriorOmission(t *testing.T) {
	...
	for _, edit := range []func(*ReleaseStatsMeasurement, *AttemptCutV2){
		...
		func(raw *ReleaseStatsMeasurement, cut *AttemptCutV2) { cut.Context.PriorRoot = attemptHex32([32]byte{1}) },
		func(raw *ReleaseStatsMeasurement, cut *AttemptCutV2) { cut.Context.Boundary.SettlementEpoch++ },
		func(raw *ReleaseStatsMeasurement, cut *AttemptCutV2) { cut.Context.Boundary.EVMBlock = prior.Transitions[0].FromBoundary.EVMBlock },
		func(raw *ReleaseStatsMeasurement, cut *AttemptCutV2) { cut.Context.EgressGeneration = prior.Transitions[0].Cut.Context.EgressGeneration },
		func(raw *ReleaseStatsMeasurement, cut *AttemptCutV2) { cut.Context.Activation.Domain.ActivationHash[0] ^= 1 },
	} {
		measurement := cloneAttemptCutV2StatsTestMeasurement(t, next.measurement)
		cut := next.cut
		edit(&measurement, &cut)
		if err := verifyAttemptSettlementV2Successor(prior.Transitions[0], measurement, cut, true); err == nil { t.Fatal("terminal successor lost exact fold/cursor/epoch continuity") }
	}
}
...
func TestAttemptSettlementV2OwnsAllMembersBeforeFirstReader(t *testing.T) {
	...
	firstOptions.Measurement.Replay.ReadMetadata = func(ctx context.Context, hash string, size uint64) ([]byte, error) {
		if !changed {
			...
			for id, key := range secondOptions.Measurement.Replay.ServerKeys { key[0] ^= 1; secondOptions.Measurement.Replay.ServerKeys[id] = key }
	...
	if err != nil || !changed || len(result.Operators) != 2 || result.Operators[10].Replay.Records.ItemCount != 8 { t.Fatalf("later candidate/authority mutation reached owned replay: %v", err) }
	result, err = VerifyAttemptSettlementClosureV2(t.Context(), closure, options)
	if err == nil || result.Operators != nil { t.Fatal("new operation reused an old verdict over changed authority") }
}
...
func TestAttemptSettlementV2ProducerOwnsKeysAndInputsBeforeReplay(t *testing.T) {
	...
	if err != nil || closure == nil || !changed { t.Fatalf("producer key ownership: %v", err) }
	result, err := VerifyAttemptSettlementClosureV2(t.Context(), closure, attemptSettlementV2TestOptions(t, first, second))
	if err != nil || len(result.Operators) != 2 { t.Fatalf("producer signed mutated instead of owned bytes: %v", err) }
}
...
func TestAttemptSettlementV2IndependentMetadataBoundsBeforeReads(t *testing.T) {
	...
	for _, edit := range []func(*AttemptSettlementV2Options){
		...
		func(options *AttemptSettlementV2Options) { operator := options.Operators[9]; operator.Measurement.MaxProviders = 1; options.Operators[9] = operator },
		func(options *AttemptSettlementV2Options) { operator := options.Operators[9]; operator.Measurement.Replay.VisitRecord = func(AttemptRecord) error { return nil }; options.Operators[9] = operator },
	} {
		options := attemptSettlementV2TestOptions(t, fixture)
		reads := attemptSettlementV2TestObserveReads(&options, fixture)
		edit(&options)
		result, err := VerifyAttemptSettlementClosureV2(t.Context(), closure, options)
		if err == nil || result.Operators != nil || len(reads) != 0 { t.Fatalf("terminal metadata cap read/published: %v", err) }
	}
}
...
func TestAttemptSettlementV2CanonicalMetadataExactBounds(t *testing.T) {
	...
	result, err := VerifyAttemptSettlementClosureV2(t.Context(), closure, options)
	if err != nil || len(result.Operators) != 1 { t.Fatalf("exact canonical metadata bounds refused genuine evidence: %v", err) }
	for _, transitionLimit := range []bool{true, false} {
		options := attemptSettlementV2TestOptions(t, fixture)
		options.MaxTransitionBytes, options.MaxClosureBytes = uint64(len(transitionRaw))+1, uint64(len(closureRaw))+1
		if transitionLimit { options.MaxTransitionBytes-- } else { options.MaxClosureBytes-- }
		reads := attemptSettlementV2TestObserveReads(&options, fixture)
		result, err := VerifyAttemptSettlementClosureV2(t.Context(), closure, options)
		if err == nil || result.Operators != nil || len(reads) != 0 { t.Fatalf("correct signed bytes waived independent terminal bound: transition=%t error=%v", transitionLimit, err) }
	}
}
...
func TestAttemptSettlementV2LineageRejectsGenuineChangedParticipant(t *testing.T) {
	...
	if err != nil || len(valid.Operators) != 2 || next.Transitions[1].Identity.NoID != 11 { t.Fatalf("replacement fixture lacks its own genuine complete signed census: %v", err) }
	...
	if err == nil || result.Operators != nil { t.Fatal("genuine replacement operator was accepted as consecutive complete membership") }
	for _, options := range []AttemptSettlementV2Options{priorOptions, nextOptions} {
		for _, operator := range options.Operators {
			if _, err := os.Lstat(operator.Measurement.Replay.ScratchDirectory); !errors.Is(err, os.ErrNotExist) { t.Fatalf("changed-participant refusal created replay scratch: %v", err) }
		}
	}
}
...
func TestAttemptSettlementV2SecondMemberCloseFailureIsAtomic(t *testing.T) {
	...
	result, err := VerifyAttemptSettlementClosureV2(t.Context(), closure, options)
	if !errors.Is(err, failure) || closes == 0 || result.Operators != nil { t.Fatalf("second member close exposed first accepted member: %v", err) }
}
...
func TestAttemptSettlementV2RejectsInconsistentSigningKeyBeforeIO(t *testing.T) {
	...
	if err == nil || closure != nil || len(reads) != 0 { t.Fatalf("invalid key crossed terminal admission: %v", err) }
	if _, err := os.Lstat(options.Operators[9].Measurement.Replay.ScratchDirectory); !errors.Is(err, os.ErrNotExist) { t.Fatalf("invalid key allocated replay scratch: %v", err) }
}
