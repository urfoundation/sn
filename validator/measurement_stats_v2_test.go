...
func TestReleaseStatsV2RuntimeDrainsAndRetainsReservation(t *testing.T) {
	...
	_, cut, err = fixture.stats.detachReleaseStatsMeasurementV2(t.Context(), fixture.dir, fixture.source.expected.Boundary, fixture.fresh(t), persist)
	if err != nil || cut == nil || fixture.stats.attemptCutPending || fixture.stats.egressGeneration != 2 {
...
func TestReleaseStatsV2RuntimeOwnsWriterAcrossJournal(t *testing.T) {
	...
	if !errors.Is(waitErr, context.Canceled) || before.EgressGeneration != 1 || fixture.stats.egressGeneration != 2 {
...
func TestReleaseStatsV2RuntimeCancelBeforeJournal(t *testing.T) {
	...
	if !errors.Is(err, context.Canceled) || cut != nil || calls != 0 || fixture.objects.closes == 0 || fixture.stats.egressGeneration != 1 || !fixture.stats.attemptCutPending {
...
func TestReleaseStatsV2RuntimePublishesSuccessfulCanceledCommit(t *testing.T) {
	...
	defer cancel()
	writes := 0
	fixture.stats.writeHooks.writeSnapshot = func(path string, encoded []byte) error {
		writes++
		err := atomicStateWrite(path, encoded, 0o600)
		cancel()
		return err
	}
	var captured releaseStatsV2RuntimeTestInput
	...
	if err != nil || cut == nil || writes != 1 || ctx.Err() == nil || fixture.stats.egressGeneration != 2 || fixture.stats.attemptCutPending {
		t.Fatalf("committed cancellation lost publication: %v", err)
	}
	loaded := NewStatsEngine(fixture.stats.cfg)
	if err := loaded.Load(fixture.dir); err != nil || loaded.egressGeneration != 2 {
...
func TestReleaseStatsV2RuntimeFullyReplaysBeforePublication(t *testing.T) {
	...
	if err == nil || cut != nil || corrupted == 0 || closed != corrupted || publications != 0 || fixture.stats.egressGeneration != 1 {
...
func TestReleaseStatsV2RuntimeRejectsObjectCallbackHeadDrift(t *testing.T) {
	...
	if err == nil || cut != nil || !appended || publications != 0 || headErr != nil || head.LastSequence != 16 || fixture.stats.attemptLastAppliedSequence != 8 || fixture.stats.egressGeneration != 1 {
...
func TestReleaseStatsV2RuntimeReconcilesRealSuffixAfterRenameFailure(t *testing.T) {
	...
	if after.EgressGeneration != 2 || after.AttemptEgressFirstSequence != 9 || after.AttemptLastAppliedSequence != 16 || !reflect.DeepEqual(after.Egress, expectedEgress) || !reflect.DeepEqual(after.Window, full.Window) || !reflect.DeepEqual(after.EmaPPM, full.EmaPPM) || stats.attemptCutPending {
...
func TestReleaseStatsV2RuntimePreservesTerminalPendingOwner(t *testing.T) {
	...
	fixture := newReleaseStatsV2RuntimeTestFixture(t, 1, 0)
	var captured releaseStatsV2RuntimeTestInput
	if _, _, err := fixture.stats.detachReleaseStatsMeasurementV2(t.Context(), fixture.dir, fixture.source.expected.Boundary, fixture.fresh(t), releaseStatsV2RuntimeTestCapture(filepath.Join(t.TempDir(), "input.json"), &captured)); err != nil {
		t.Fatal(err)
	}
	if err := fixture.stats.beginAttempt(42, fixture.ledger); err != nil {
		t.Fatal(err)
	...
	if fixture.objects.reads != reads || fixture.stats.egressGeneration != 2 || !fixture.stats.attemptCutPending || !fixture.stats.attemptSettlementCutPending || fixture.stats.attemptSettlementCutEpoch != 43 {
...
func TestReleaseStatsV2RuntimeRejectsAliasedScratchAndCensus(t *testing.T) {
	...
	for _, edit := range []func(*releaseStatsV2Options){
		...
		if err == nil || cut != nil || fixture.objects.writes != 0 || fixture.stats.egressGeneration != 1 {
...
func TestReleaseStatsV2RuntimePublicationReceivesOwnedCopies(t *testing.T) {
	...
	if fixture.stats.egressGeneration != 2 {
