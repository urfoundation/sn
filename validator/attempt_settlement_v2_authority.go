...
func attemptSettlementV2LedgerRoot(ctx context.Context, ledger *AttemptLedger, before uint64) (string, error) {
	if before == 0 { return zeroAttemptHash(), nil }
	var root string
	if err := ledger.Walk(ctx, before, before, func(record AttemptRecord) error {
		if record.Sequence != before { return errors.New("compact runtime cursor row differs") }
		root = record.RecordHash
		return nil
	}); err != nil { return "", err }
	if root == "" { return "", errors.New("compact runtime cursor row is missing") }
	return root, nil
}
...
func requireAttemptSettlementV2NoPending(ctx context.Context, ledger *AttemptLedger) error {
	if err := ledger.begin(ctx); err != nil { return err }
...
func validateAttemptSettlementV2Activation(ctx context.Context, participants []AttemptSettlementRuntimeV2Participant, statsTs []*StatsEngine, epoch uint64, authority AttemptSettlementV2Options) error {
	...
	for index, participant := range participants {
		...
		if _, err := attemptSettlementV2RawMeasurement(stats, operator); err != nil { return err }
		if err := requireAttemptSettlementV2NoPending(ctx, participant.Ledger); err != nil { return err }
		...
		if legacyCount == 0 { legacy.Epoch = transition.FromBoundary.SettlementEpoch }
	...
	if legacyCount != 0 {
		if legacyCount != len(participants) { return errors.New("compact activation legacy history omits a configured operator") }
...
func validateAttemptSettlementV2LedgerState(ctx context.Context, participant AttemptSettlementRuntimeV2Participant, stats *StatsEngine, operator AttemptSettlementV2OperatorOptions) error {
	...
	for _, cursor := range []struct { first uint64; root string }{
		...
		if cursor.first == 0 || cursor.first > head.LastSequence+1 { return errors.New("compact runtime retained prefix is unavailable") }
		root, err := attemptSettlementV2LedgerRoot(ctx, participant.Ledger, cursor.first-1)
		if err != nil { return err }
		if root != cursor.root { return errors.New("compact runtime retained root differs from actual ledger prefix") }
	}
	...
	if stats.attemptSettlementFirstSequence <= stats.attemptLastAppliedSequence {
		if err := participant.Ledger.Walk(ctx, stats.attemptSettlementFirstSequence, stats.attemptLastAppliedSequence, func(record AttemptRecord) error {
			if record.M != depth || record.Boundary.SettlementEpoch != stats.settlementEpoch { return errors.New("compact runtime replay policy or epoch differs") }
			...
			if record.Disposition == AttemptDispositionPending { return nil }
			if err := replayed.validateAttemptStatsWithLock(&record); err != nil { return err }
			replayed.applyAttemptStatsWithLock(&record)
			if record.Sequence < stats.attemptEgressFirstSequence { replayed.egress = map[connect.Id]map[[32]byte]bool{} }
	...
	if err != nil { return err }
	wantJSON, err := json.Marshal(measurement)
	if err != nil { return err }
	actualJSON, err := json.Marshal(actual)
	if err != nil || !bytes.Equal(wantJSON, actualJSON) { return errors.Join(errors.New("compact runtime saved counters differ from actual signed rows"), err) }
	return ctx.Err()
}
...
func validateAttemptSettlementV2Terminal(ctx context.Context, participant AttemptSettlementRuntimeV2Participant, stats *StatsEngine, epoch uint64, boundary AttemptBoundary, operator AttemptSettlementV2OperatorOptions) (AttemptLedgerHead, error) {
	...
	if err := validateAttemptStatsV2Snapshot(stats.snapshotStats()); err != nil { return zero, err }
	if err := validateAttemptSettlementV2LedgerState(ctx, participant, stats, operator); err != nil { return zero, err }
	head, err := participant.Ledger.checkedHead()
	if err != nil || head.LastSequence != stats.attemptLastAppliedSequence { return zero, errors.Join(errors.New("compact runtime terminal has unapplied ledger rows"), err) }
	return head, nil
}

// Binds every canonical postimage to the independently replayed full closure
// and recomputes the real Stats fold from the recorded raw live preimage.
func validateAttemptSettlementTransactionV2(ctx context.Context, transaction *attemptSettlementTransactionV2, participants []AttemptSettlementRuntimeV2Participant, preimages, postimages []*StatsEngine, authority AttemptSettlementV2Options) (*AttemptSettlementClosureV2, error) {
	if transaction.Kind == "activate" {
		if err := validateAttemptSettlementV2Activation(ctx, participants, preimages, transaction.Epoch, authority); err != nil { return nil, err }
		for index, pre := range preimages {
			entry := transaction.Snapshots[index]
			if len(entry.OriginalJSON) != 0 && !bytes.Equal(entry.OriginalJSON, entry.PreImageJSON) { return nil, errors.New("compact activation original checkpoint differs from its drained live image") }
			if pre.attemptV2 != nil { return nil, errors.New("compact activation journal replaces already active state") }
			...
			encoded, err := encodeStatsSnapshot(pre.snapshotStats())
			if err != nil || !bytes.Equal(encoded, transaction.Snapshots[index].StatsJSON) { return nil, errors.Join(errors.New("compact activation postimage differs from exact original state"), err) }
		}
		return nil, nil
	}
	closure, _, err := DecodeAttemptSettlementClosureV2(ctx, transaction.ClosureJSON, authority)
	if err != nil { return nil, err }
	if closure.Epoch == ^uint64(0) || closure.Epoch+1 != transaction.Epoch || len(closure.Transitions) != len(participants) { return nil, errors.New("compact transaction closed epoch differs") }
	for index, participant := range participants {
		...
		if encoded := transaction.Snapshots[index].OriginalJSON; len(encoded) != 0 {
			original, err := decodeAttemptSettlementV2Stats(encoded, participant.Stats.cfg)
			if err != nil { return nil, err }
			if err := validateAttemptSettlementV2LedgerState(ctx, participant, original, authority.Operators[participant.NoID]); err != nil { return nil, err }
		}
		if _, err := validateAttemptSettlementV2Terminal(ctx, participant, pre, transaction.Epoch, terminal.FromBoundary, authority.Operators[participant.NoID]); err != nil { return nil, err }
		raw, err := attemptSettlementV2RawMeasurement(pre, authority.Operators[participant.NoID])
		if err != nil { return nil, err }
		left, err := json.Marshal(raw)
		if err != nil { return nil, err }
		right, err := json.Marshal(terminal.PreFold)
		if err != nil || !bytes.Equal(left, right) || terminal.Cut.LastSequence != pre.attemptLastAppliedSequence { return nil, errors.Join(errors.New("compact transaction live preimage differs from full replay"), err) }
		candidate, err := attemptSettlementV2Postimage(pre, terminal, false)
		if err != nil { return nil, err }
		...
		encoded, err := encodeStatsSnapshot(candidate.snapshotStats())
		if err != nil || !bytes.Equal(encoded, transaction.Snapshots[index].StatsJSON) { return nil, errors.Join(fmt.Errorf("compact transaction no_id %d postimage differs from real fold", participant.NoID), err) }
		if post.attemptV2.Activation != authority.Operators[participant.NoID].Expected.Activation { return nil, errors.New("compact transaction successor activation differs") }
	}
	return closure, nil
}
...
func attemptSettlementV2Postimage(pre *StatsEngine, terminal *AttemptSettlementTransitionV2, fold bool) (*StatsEngine, error) {
	...
	if err := json.Unmarshal(encodedTerminal, &retained); err != nil { return nil, err }
	pre.mu.Lock()
	post := pre.cloneStatsWithLock()
	pre.mu.Unlock()
	if pre.attemptV2 == nil || pre.egressGeneration == ^uint64(0) || terminal.Cut.LastSequence == ^uint64(0) || terminal.ToEpoch != pre.settlementEpoch+1 { return nil, errors.New("compact successor generation overflows or differs") }
	if fold {
		post.foldStatsOwned()
		if !slices.Equal(sortedAttemptEMAQualities(post.emaPPM), terminal.PostFold) { return nil, errors.New("compact signed post-fold differs from actual Stats fold") }
	} else {
