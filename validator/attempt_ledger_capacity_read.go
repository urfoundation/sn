...
func stoppedAttemptFiles(root *os.Root, limits AttemptLedgerDiskLimits) ([]stoppedAttemptFile,uint64,error) {
	...
	if err := errors.Join(readErr,directory.Close()); err != nil { return nil,0,err }
	if uint64(len(entries)) > limits.MaxStorageFiles { return nil,0,errAttemptRecordStoreLimit }
	files := make([]stoppedAttemptFile,0,len(entries))
	used,metadataBytes := uint64(attemptStoreMetadataReserve),uint64(0)
	for _, entry := range entries {
		info, err := root.Lstat(entry.Name())
		if err != nil { return nil,0,err }
		if !info.Mode().IsRegular() || info.Size() < 0 { return nil,0,errors.New("stopped ledger contains a nonregular storage entry") }
		if !attemptStoreMetadataName(entry.Name()) {
			if uint64(info.Size()) > limits.MaxStorageBytes || used > limits.MaxStorageBytes-uint64(info.Size()) { return nil,0,errAttemptRecordStoreLimit }
			used += uint64(info.Size())
		} else {
			if uint64(info.Size())>attemptStoreMetadataReserve || metadataBytes>attemptStoreMetadataReserve-uint64(info.Size()) { return nil,0,errAttemptRecordStoreLimit }
...
func ReadStoppedAttemptLedgerCapacity(ctx context.Context,stateDir string,identity AttemptLedgerIdentity,coordinator string,vpk ed25519.PublicKey,limits AttemptLedgerDiskLimits) (result StoppedAttemptLedgerCapacity, resultErr error) {
	...
	if limits.MaxRecordBytes==0 || limits.MaxRecordBytes>uint64(^uint(0)>>1)/8 || limits.MaxRecordCount==0 || limits.MaxTrailCount==0 || limits.MaxTrailCount>limits.MaxRecordCount || limits.MaxRawRecordBytes<limits.MaxRecordBytes || limits.MaxStorageBytes<=attemptStoreMetadataReserve || limits.MaxStorageFiles<8 || limits.MaxStorageFiles>=uint64(^uint(0)>>1) { return result,errors.New("stopped ledger capacity bounds are incomplete") }
	path := filepath.Join(stateDir,attemptLedgerStoreName)
	physical, err := filepath.EvalSymlinks(path)
	if err != nil || physical!=path { return result,errors.Join(errors.New("stopped ledger path is missing or aliased"),err) }
	pinned, err := os.Lstat(path)
	if err != nil || !pinned.IsDir() || pinned.Mode().Perm()&0o077!=0 { return result,errors.Join(errors.New("stopped ledger is not a private original directory"),err) }
	...
	if err != nil { return result,err }
	closed := false
	defer func(){ if !closed { resultErr=errors.Join(resultErr,db.Close()) } }()
	...
	if err != nil || attemptStoreDecode(raw,&storedIdentity)!=nil || storedIdentity!=store.identity { return result,errors.Join(errors.New("stopped ledger differs from original activation identity"),err) }
	...
	if err != nil || !os.SameFile(pinned,current) || used!=afterUsed || !reflect.DeepEqual(before,after) { return result,errors.Join(errors.New("stopped ledger storage changed during read-only replay"),err) }
