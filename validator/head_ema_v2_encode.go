...
func (self *headEMAStoreV2JSONWriter) emit(value string) error {
	...
	if self.used > self.limit || uint64(len(value)) > self.limit-self.used { return errors.New("bounded head EMA encoded output exceeds its file allowance") }
	self.used += uint64(len(value))
	if !self.countOnly { self.encoded = append(self.encoded, value...) }
	return nil
}
...
func (self *headEMAStoreV2JSONWriter) indent(depth int) error {
	if err := self.emit("\n"); err != nil { return err }
	for range depth { if err := self.emit("  "); err != nil { return err } }
	return nil
}
...
func (self *headEMAStoreV2JSONWriter) quoted(value string) error {
	if err := self.emit("\""); err != nil { return err }
	for index := 0; index < len(value); {
		...
		if b < utf8.RuneSelf {
			...
			default:
				...
				} else { escaped = value[index:index+1] }
			}
			if err := self.emit(escaped); err != nil { return err }
			index++
			continue
		}
		...
		if runeValue == utf8.RuneError && width == 1 {
			...
			if err := self.emit("\\u2029"); err != nil { return err }
		} else if err := self.emit(value[index:index+width]); err != nil { return err }
		index += width
	}
	return self.emit("\"")
}
...
func (self *headEMAStoreV2JSONWriter) value(value reflect.Value, depth int) error {
	if err := self.ctx.Err(); err != nil { return err }
	if depth > 8 { return errors.New("bounded head EMA output exceeds fixed schema depth") }
	if value.Kind() == reflect.Pointer {
		if value.IsNil() { return self.emit("null") }
	...
	case reflect.Struct:
		...
		for index := 0; index < value.NumField(); index++ {
			...
			if options == "omitempty" {
				if child.Kind() != reflect.Pointer { return errors.New("bounded head EMA output has an unknown omission rule") }
				if child.IsNil() { continue }
			}
			if count != 0 { if err := self.emit(","); err != nil { return err } }
			if err := self.indent(depth+1); err != nil { return err }
			if err := self.quoted(name); err != nil { return err }
			if err := self.emit(": "); err != nil { return err }
			if err := self.value(child, depth+1); err != nil { return err }
			count++
		}
		if count != 0 { if err := self.indent(depth); err != nil { return err } }
		return self.emit("}")
	case reflect.Array, reflect.Slice:
		if value.Kind() == reflect.Slice && value.IsNil() { return self.emit("null") }
		if err := self.emit("["); err != nil { return err }
		for index := 0; index < value.Len(); index++ {
			if index != 0 { if err := self.emit(","); err != nil { return err } }
			if err := self.indent(depth+1); err != nil { return err }
			if err := self.value(value.Index(index), depth+1); err != nil { return err }
		}
		if value.Len() != 0 { if err := self.indent(depth); err != nil { return err } }
		return self.emit("]")
	case reflect.String:
		return self.quoted(value.String())
	case reflect.Bool:
		if value.Bool() { return self.emit("true") }
...
func marshalHeadEMAStoreV2File(ctx context.Context, file headEMAFile, budget *headEMAStoreV2Budget, limit uint64) ([]byte, error) {
	if ctx == nil { return nil, errors.New("bounded head EMA context is nil") }
	if limit == 0 || limit > maxHeadEMAStoreV2Bytes { return nil, errors.New("bounded head EMA output allowance is invalid") }
	count := headEMAStoreV2JSONWriter{ctx: ctx, limit: limit, countOnly: true}
	if err := count.value(reflect.ValueOf(file), 0); err != nil { return nil, err }
	if err := count.emit("\n"); err != nil { return nil, err }
	if err := budget.charge(count.used, 1); err != nil { return nil, err }
	writer := headEMAStoreV2JSONWriter{ctx: ctx, limit: count.used, encoded: make([]byte, 0, int(count.used))}
	if err := writer.value(reflect.ValueOf(file), 0); err != nil { return nil, err }
	if err := writer.emit("\n"); err != nil { return nil, err }
	if writer.used != count.used || uint64(len(writer.encoded)) != count.used { return nil, errors.New("bounded head EMA exact output accounting changed") }
...
func encodeHeadEMAStoreV2File(ctx context.Context, store *HeadEMAStore, budget *headEMAStoreV2Budget, limit uint64) ([]byte, error) {
	...
	if len(keys) != 0 { file.Entries = make([]headEMAEntry, 0, len(keys)) }
	for _, key := range keys { file.Entries = append(file.Entries, store.values[key]) }
