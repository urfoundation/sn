...
func releaseHistoricalReconcileEvents(t *testing.T, metadata *types.Metadata, events []releaseHistoricalReconcileEvent) []byte {
	...
	for _, event := range events {
		...
		for _, pallet := range metadata.AsMetadataV14.Pallets {
			if string(pallet.Name) != event.pallet || !pallet.HasEvents { continue }
			definition := metadata.AsMetadataV14.EfficientLookup[pallet.Events.Type.Int64()]
			if definition == nil || !definition.Def.IsVariant { t.Fatalf("original %s event metadata is absent", event.pallet) }
			for _, variant := range definition.Def.Variant.Variants {
				if string(variant.Name) != event.name { continue }
				if len(variant.Fields) != len(event.fields) { t.Fatalf("original %s.%s field census differs", event.pallet, event.name) }
		...
		if !found { t.Fatalf("original %s.%s event index is absent", event.pallet, event.name) }
	}
	return encoded
}
...
func installReleaseHistoricalReconcileReceipt(t *testing.T, native *releaseNativeValidatorTestFixture, metadataHex string, prepared *crv4.PreparedSubmission, events []byte) *releaseHistoricalReconcileReceipt {
	...
	native.chain.API.Client = &releaseHistoricalReconcileClient{receipt: result, validatorRuntimeIdentityTestClient: &validatorRuntimeIdentityTestClient{callContext: func(ctx context.Context, target any, method string, args ...any) error {
		...
		case "chain_getFinalizedHead":
			if len(args) != 0 { return errors.New("finalized head request changed shape") }
			return assign(target, result.hash.Hex())
		case "chain_getHeader":
			if len(args) == 1 && args[0] == result.hash.Hex() { return assign(target, types.Header{Number: types.BlockNumber(result.number)}) }
		case "chain_getBlockHash":
			if len(args) == 1 && args[0] == result.number { return assign(target, result.hash.Hex()) }
		case "chain_getBlock":
			if len(args) != 1 { return errors.New("receipt body request changed shape") }
			result.blocks++
			if args[0] == native.block.Hex() { return assign(target, block.SignedBlock{Block: block.Block{Header: types.Header{Number: types.BlockNumber(native.blockNumber)}, Extrinsics: []string{}}}) }
			if args[0] == result.hash.Hex() { return assign(target, block.SignedBlock{Block: block.Block{Header: types.Header{Number: types.BlockNumber(result.number)}, Extrinsics: []string{prepared.ExtrinsicHex}}}) }
			return errors.New("receipt body escaped its canonical source blocks")
		case "state_getRuntimeVersion":
			if len(args) == 1 && args[0] == result.hash.Hex() { result.runtimeReads++; return assign(target, native.expected.Version) }
		case "state_getStorageHash":
			if len(args) == 2 && args[0] == "0x3a636f6465" && args[1] == result.hash.Hex() { return assign(target, native.expected.CodeHash) }
		case "state_getMetadata":
			if len(args) == 1 && args[0] == result.hash.Hex() { return assign(target, metadataHex) }
		case "state_getStorage":
			if len(args) == 2 && args[0] == eventsKey.Hex() {
				if args[1] != result.hash.Hex() { return errors.New("receipt events used preparation or moving block") }
				result.eventReads++
			...
			if prepared.SourceCommitment != nil && len(args) == 2 && (args[0] == commitmentKey.Hex() || args[0] == lastKey.Hex()) {
				if args[1] != result.hash.Hex() { return errors.New("source commitment used preparation or moving block") }
				result.commitmentReads++
				if args[0] == commitmentKey.Hex() { return assign(target, hexutil.Encode(result.registration)) }
...
func releaseHistoricalReconcileLegacyPending(t *testing.T) (*ReleaseSteerer, *SteeringIntent, *releaseHistoricalReconcileReceipt) {
	...
	if err != nil { t.Fatalf("original signed legacy pending store: %v", err) }
...
func TestReleaseEvidenceV2HistoricalRuntimeLegacyReconcilePersistsExactReceipt(t *testing.T) {
	...
	if err != nil || !done { t.Fatalf("original455 finalized legacy recovery: done=%t error=%v", done, err) }
	reopened, err := NewIntentStore(steerer.intents.stateDir)
	if err != nil { t.Fatal(err) }
	current, err := reopened.Current()
	if err != nil || current == nil || current.Status != "finalized" || current.VectorHash != pending.VectorHash || current.FinalizedBlock != receipt.number || current.FinalizedBlockHash != receipt.hash.Hex() || current.ExtrinsicHash != pending.Prepared.ExtrinsicHash { t.Fatalf("reopened original finalized receipt differs: %+v %v", current, err) }
	retained, err := json.Marshal(current.Prepared)
	if err != nil || !bytes.Equal(prepared, retained) { t.Fatalf("recovery changed original signed preparation: %v", err) }
	if receipt.runtimeReads == 0 || receipt.eventReads == 0 || receipt.submissions != 0 || receipt.subscriptions != 0 { t.Fatalf("legacy recovery crossed wrong boundary: %+v", receipt) }
	if steerer.native.Meta != metadata || steerer.native.Runtime != runtime { t.Fatal("legacy receipt recovery changed current signing view") }
}
...
func TestReleaseEvidenceV2HistoricalRuntimeLegacyReconcileRejectsDispatchFailure(t *testing.T) {
	...
	if done || !errors.As(err, &dispatch) || dispatch.BlockHash != receipt.hash { t.Fatalf("historical failure became local finality: done=%t error=%v", done, err) }
	after, readErr := os.ReadFile(steerer.intents.path)
	if readErr != nil || !bytes.Equal(before, after) || receipt.submissions != 0 || receipt.subscriptions != 0 { t.Fatalf("failed receipt changed source or submitted: %v", readErr) }
}
...
func TestReleaseEvidenceV2HistoricalRuntimeSignedReconcileAuthenticatesReceiptBeforePublication(t *testing.T) {
	for _, changedCommitment := range []bool{false, true} {
			...
			if changedCommitment { receipt.registration[len(receipt.registration)-1] ^= 1 }
			...
			if changedCommitment {
				if err == nil || errors.Is(err, publicationFault) || !strings.Contains(err.Error(), "source metadata readback") { t.Fatalf("changed original commitment reached publication: %v", err) }
			} else if !errors.Is(err, publicationFault) { t.Fatalf("valid original455 source did not finish receipt proof: %v", err) }
			if done || receipt.runtimeReads == 0 || receipt.eventReads != 2 || receipt.commitmentReads != 2 || receipt.submissions != 0 || receipt.subscriptions != 0 { t.Fatalf("V2 historical receipt boundary differs: changed_commitment=%t done=%t receipt=%+v error=%v", changedCommitment, done, receipt, err) }
			after, marshalErr := json.Marshal(prepared)
			if marshalErr != nil || !bytes.Equal(before, after) || fixture.native.chain.Meta != liveMetadata || fixture.native.chain.Runtime != liveRuntime { t.Fatalf("V2 receipt proof changed original source or signing view: changed_commitment=%t error=%v", changedCommitment, marshalErr) }
			if _, statErr := os.Stat(store.path); !errors.Is(statErr, os.ErrNotExist) { t.Fatalf("publication refusal wrote intent state: changed_commitment=%t error=%v", changedCommitment, statErr) }
	}
}
