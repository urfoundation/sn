...
func releaseHeadV2AccountingInputBudget(t *testing.T, store *HeadEMAStore, raw map[FleetScoreKey]*big.Rat) uint64 {
	...
	for key, value := range store.values {
		if uint64(len(key)) > remaining { t.Fatal("input budget fixture exceeds its existing allowance") }
	...
	if err := releaseMeasurementV2ControlStorage(t.Context(), reflect.ValueOf(store.lastFold), &remaining); err != nil { t.Fatal(err) }
	used := uint64(1024 * 1024)-remaining
	for _, value := range raw { used += (uint64(len(value.Num().Bits()))+uint64(len(value.Denom().Bits())))*uint64(reflect.TypeFor[big.Word]().Size()) }
	return used
}

// Reconstruct the unfinished draft/census plus the repair's explicitly tracked
// derived binding owners, leaving exactly zero allowance for EMA. The causal
// old collector still fits these inputs; it incorrectly restarts that allowance.
func releaseHeadV2AccountingCollectionBudget(t *testing.T, fixture *releaseHeadV2TestFixture) uint64 {
	t.Helper()
	...
	remaining := uint64(1024 * 1024)
	if err := releaseMeasurementV2ControlStorage(t.Context(), reflect.ValueOf(draft), &remaining); err != nil { t.Fatal(err) }
	...
	for _, input := range draft.Inputs { used += uint64(len(input.Stats.Providers))*(uint64(reflect.TypeFor[ReleaseBindingMeasurement]().Size())+36+5*66+16+1) }
...
func releaseHeadV2AccountingCommit(t *testing.T, fixture *releaseHeadV2TestFixture, epoch uint64, raw map[FleetScoreKey]*big.Rat, alpha protocol.Rational) []byte {
	...
	_, transcript, err := store.PreviewForEpoch(epoch, raw, alpha)
	if err != nil { t.Fatal(err) }
	if err := store.CommitForEpoch(epoch, transcript, alpha); err != nil { t.Fatal(err) }
	encoded, err := os.ReadFile(store.path)
	if err != nil { t.Fatal(err) }
	reloaded, err := NewHeadEMAStore(filepath.Dir(store.path))
	if err != nil { t.Fatal(err) }
...
func TestReleaseHeadV2AccountingRejectsUnreservedFixedOutput(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingRejectsUnreservedFixedOutput") } })
	store, err := NewHeadEMAStore(t.TempDir())
	if err != nil { t.Fatal(err) }
	raw := releaseHeadV2AccountingRaw(1)
	for key := range raw { raw[key] = big.NewRat(1, 1) }
	bound := 2*uint64(reflect.TypeFor[big.Word]().Size())
	out, head, err := store.previewForEpochV2(t.Context(), 7, raw, exactPolicy(t).Steering.HeadScoreEMA, 1, bound)
	if err == nil || out != nil || head != nil { t.Fatalf("input-only word allowance produced an unreserved transcript: bytes=%d error=%v", bound, err) }
	if uint64(reflect.TypeFor[HeadEMAMeasurement]().Size()) <= bound { t.Fatal("fixed-output witness no longer distinguishes input/output storage") }
}
...
func TestReleaseHeadV2AccountingKeepsOneGenuineCollectionBudget(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingKeepsOneGenuineCollectionBudget") } })
	fixture := newReleaseHeadV2TestFixture(t, 15)
	options := fixture.options(t)
	options.MaxControlBytes = releaseHeadV2AccountingCollectionBudget(t, fixture)
	...
	if err == nil || requests == 0 || *reads != 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("collection restarted its consumed allowance: requests=%d reads=%d error=%v", requests, *reads, err) }
	options = fixture.options(t)
	reads = observeReleaseMeasurementV2SettlementTest(&options)
	result, err = fixture.gather(t.Context(), options)
	if err != nil || *reads == 0 || !reflect.DeepEqual(result.Weights, fixture.measurement.want.SelectedHead) || !reflect.DeepEqual(result.Bindings, fixture.measurement.artifact.Bindings) { t.Fatalf("bounded repair bypassed genuine complete replay or exact math: reads=%d error=%v", *reads, err) }
...
func TestReleaseHeadV2AccountingAdmitsPersistedCountBeforeCallbacks(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingAdmitsPersistedCountBeforeCallbacks") } })
	...
	result, err := fixture.gather(t.Context(), options)
	requests, _ := fixture.rpc.counts()
	if err == nil || requests != 0 || *reads != 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("known retained census reached callbacks: requests=%d reads=%d error=%v", requests, *reads, err) }
	after, readErr := os.ReadFile(fixture.steerer.headEMA.path)
	if readErr != nil || !bytes.Equal(before, after) { t.Fatalf("retained census refusal changed disk: %v", readErr) }
}
...
func TestReleaseHeadV2AccountingAdmitsKnownEpochAndAlphaBeforeCallbacks(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingAdmitsKnownEpochAndAlphaBeforeCallbacks") } })
	for _, kind := range []string{"regressed", "jumped", "same alpha"} {
		...
		switch kind {
		case "regressed": epoch++
		case "jumped": epoch -= 2
		case "same alpha":
			replacement := protocol.Rational{Numerator: 1, Denominator: 1}
			if alpha == replacement { replacement.Numerator = 0 }
			alpha = replacement
		}
		...
		result, err := fixture.gather(t.Context(), options)
		requests, _ := fixture.rpc.counts()
		if err == nil || requests != 0 || *reads != 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("%s known state reached callbacks: requests=%d reads=%d error=%v", kind, requests, *reads, err) }
		after, readErr := os.ReadFile(fixture.steerer.headEMA.path)
		if readErr != nil || !bytes.Equal(before, after) { t.Fatalf("%s refusal changed disk: %v", kind, readErr) }
	}
}
...
func TestReleaseHeadV2AccountingAdmitsRealUnionBeforeProofReplay(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingAdmitsRealUnionBeforeProofReplay") } })
	fixture := newReleaseHeadV2TestFixture(t, 2)
	...
	if len(fixture.measurement.artifact.HeadEMA) != 1 { t.Fatal("genuine two-operator fixture no longer shares one live fleet") }
	options := fixture.options(t)
	options.MaxHeadEntries = 1
	reads := observeReleaseMeasurementV2SettlementTest(&options)
	result, err := fixture.gather(t.Context(), options)
	requests, batches := fixture.rpc.counts()
	if err == nil || requests == 0 || batches != 2 || *reads != 0 || !strings.Contains(err.Error(), "current/history union exceeds") || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("real current/history union was delayed until proof replay: requests=%d batches=%d reads=%d error=%v", requests, batches, *reads, err) }
}
...
func TestReleaseHeadV2AccountingAdmitsRetainedStorageBeforeCallbacks(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingAdmitsRetainedStorageBeforeCallbacks") } })
	fixture := newReleaseHeadV2TestFixture(t, 2)
	raw := releaseHeadV2AccountingRaw(1)
	large, ok := new(big.Int).SetString(strings.Repeat("7", 256), 10)
	if !ok { t.Fatal("large integer fixture is invalid") }
	for key := range raw { raw[key] = new(big.Rat).SetInt(large) }
	...
	result, err := fixture.gather(t.Context(), options)
	requests, _ := fixture.rpc.counts()
	if err == nil || requests != 0 || *reads != 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("retained string storage acquired another full budget: requests=%d reads=%d error=%v", requests, *reads, err) }
	after, readErr := os.ReadFile(fixture.steerer.headEMA.path)
	if readErr != nil || !bytes.Equal(before, after) { t.Fatalf("combined storage refusal changed disk: %v", readErr) }
}
...
func TestReleaseHeadV2AccountingReservesSameEpochGeneratedOutput(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingReservesSameEpochGeneratedOutput") } })
	fixture := newReleaseHeadV2TestFixture(t, 2)
	...
	if err == nil || out != nil || head != nil { t.Fatalf("same-epoch output bypassed consumed input allowance: %v", err) }
	want, wantHead, err := store.PreviewForEpoch(7, raw, alpha)
	if err != nil { t.Fatal(err) }
	out, head, err = store.previewForEpochV2(t.Context(), 7, raw, alpha, 8, 1024 * 1024)
	if err != nil || !reflect.DeepEqual(out, want) || !reflect.DeepEqual(head, wantHead) { t.Fatalf("same-epoch exact transcript changed: %v", err) }
	after, readErr := os.ReadFile(store.path)
	if readErr != nil || !bytes.Equal(before, after) { t.Fatalf("same-epoch admission rewrote history: %v", readErr) }
}
...
func TestReleaseHeadV2AccountingReservesSuccessorRationalOutput(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingReservesSuccessorRationalOutput") } })
	fixture := newReleaseHeadV2TestFixture(t, 2)
	raw, alpha := releaseHeadV2AccountingRaw(1), protocol.Rational{Numerator: 1, Denominator: 3}
	before := releaseHeadV2AccountingCommit(t, fixture, 7, raw, alpha)
	for key := range raw { raw[key] = big.NewRat(5, 7) }
	store := fixture.steerer.headEMA
	bound := releaseHeadV2AccountingInputBudget(t, store, raw)
	out, head, err := store.previewForEpochV2(t.Context(), 8, raw, alpha, 8, bound)
	if err == nil || out != nil || head != nil { t.Fatalf("successor output bypassed consumed input allowance: %v", err) }
	want, wantHead, err := store.PreviewForEpoch(8, raw, alpha)
	if err != nil { t.Fatal(err) }
	out, head, err = store.previewForEpochV2(t.Context(), 8, raw, alpha, 8, 1024 * 1024)
	if err != nil || !reflect.DeepEqual(out, want) || !reflect.DeepEqual(head, wantHead) { t.Fatalf("successor rational formula changed: %v", err) }
	after, readErr := os.ReadFile(store.path)
	if readErr != nil || !bytes.Equal(before, after) { t.Fatalf("successor preview rewrote history: %v", readErr) }
}

// Rechecking after callbacks is separate from initial admission. The first
// client-key mutation must precede proof reads; a later proof mutation is
// rechecked at final preview and never runs under the EMA state lock.
func TestReleaseHeadV2AccountingRechecksRealHistoryAfterCallbacks(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingRechecksRealHistoryAfterCallbacks") } })
	for _, phase := range []string{"client key", "proof"} {
		...
		mutate := func() {
			if mutated { return }
			mutated = true
			releaseHeadV2AccountingCommit(t, fixture, fixture.measurement.artifact.SubnetEpoch-1, releaseHeadV2AccountingRaw(3), fixture.steerer.cfg.Policy.Steering.HeadScoreEMA)
		}
		if phase == "client key" {
			...
			operator.Measurement.Replay.ReadMetadata = func(ctx context.Context, hash string, size uint64) ([]byte, error) { mutate(); return read(ctx, hash, size) }
			options.Operators[9] = operator
		}
		result, err := fixture.gather(t.Context(), options)
		if !mutated || err == nil || phase == "client key" && *reads != 0 || phase == "proof" && *reads == 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("%s history mutation escaped fresh admission: changed=%t reads=%d error=%v", phase, mutated, *reads, err) }
	}
}
...
func assertReleaseHeadV2AccountingFraction(t *testing.T, label string, bound releaseHeadV2FractionBound, value *big.Rat) {
	t.Helper()
	if uint64(value.Num().BitLen()) > bound.numerator || uint64(value.Denom().BitLen()) > bound.denominator { t.Fatalf("%s exact rational exceeds planned bit growth: n=%d/%d d=%d/%d", label, value.Num().BitLen(), bound.numerator, value.Denom().BitLen(), bound.denominator) }
	budget := releaseHeadV2Budget{limit: 1024 * 1024}
	if err := bound.reserveText(&budget); err != nil { t.Fatal(err) }
	if uint64(len(value.Num().String())+len(value.Denom().String())) > budget.output { t.Fatalf("%s generated decimal bytes exceed their reservation", label) }
}
...
func TestReleaseHeadV2AccountingBoundsArithmeticBeforeConstruction(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingBoundsArithmeticBeforeConstruction") } })
	for _, pair := range []struct { current, prior RationalJSON }{
		...
		parse := func(encoded RationalJSON) *big.Rat {
			...
			if !nOK || !dOK { t.Fatal("independent numeric fixture is invalid") }
	...
	for _, value := range []RationalJSON{{Numerator: "", Denominator: "1"}, {Numerator: "-1", Denominator: "1"}, {Numerator: "1", Denominator: "0"}, {Numerator: "1", Denominator: "a"}} {
		if _, err := releaseHeadV2TextBound(value); err == nil { t.Fatal("invalid decimal shape passed numeric planning") }
	}
	if _, err := releaseHeadV2BitSum(^uint64(0), 1); err == nil { t.Fatal("arithmetic growth overflow was accepted") }
}
...
func TestReleaseHeadV2AccountingPlansSharedPrefixesWithoutExponentInflation(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingPlansSharedPrefixesWithoutExponentInflation") } })
	...
	for index := 0; index < 4; index++ {
		...
		for prefix := 0; prefix < 128; prefix++ { prefixes[[32]byte{1, byte(prefix)}] = true }
		if index < 2 { for prefix := 0; prefix < 4; prefix++ { prefixes[[32]byte{2, byte(prefix)}] = true } }
		prefixes[[32]byte{3, byte(index)}] = true
		copied += uint64(len(prefixes))*(32+1)
		fleets[key] = prefixes
	}
	store := &HeadEMAStore{values: map[string]headEMAEntry{}}
	...
	if err != nil { t.Fatal(err) }
	if claims[[32]byte{1}] != 4 || claims[[32]byte{2}] != 2 || claims[[32]byte{3}] != 1 { t.Fatal("actual shared-prefix multiplicities changed") }
	raw, err := buildReleaseHeadV2Raw(t.Context(), fleets, claims)
	if err != nil || !reflect.DeepEqual(raw, releaseRawHeadScores(fleets)) { t.Fatalf("planned raw formula differs from independent existing implementation: %v", err) }
	for key, value := range raw {
		assertReleaseHeadV2AccountingFraction(t, "shared prefix score", planned[key], value)
		if planned[key].denominator > 4 { t.Fatal("repeated equal multiplicities inflated denominator growth") }
	}
	reversed := map[FleetScoreKey]map[[32]byte]bool{}
	keys := orderedReleaseHeadV2Keys(fleets)
	for index := len(keys)-1; index >= 0; index-- { reversed[keys[index]] = fleets[keys[index]] }
	againClaims, again, againBudget, err := planReleaseHeadV2Raw(t.Context(), reversed, budget)
	if err != nil || charged != againBudget || !reflect.DeepEqual(claims, againClaims) || !reflect.DeepEqual(planned, again) { t.Fatalf("insertion order changed the same exact plan: %v", err) }
	...
	if err != nil || len(out) != 1 { t.Fatalf("shared UID numeric output differs: %v", err) }
	if err := checkReleaseHeadV2Output(t.Context(), out, head, budget); err != nil { t.Fatal(err) }
}
...
func TestReleaseHeadV2AccountingExactPlannedBudgetKeepsUIDMath(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingExactPlannedBudgetKeepsUIDMath") } })
	store, err := NewHeadEMAStore(t.TempDir())
	if err != nil { t.Fatal(err) }
	...
	if err != nil || plan.used == 0 || plan.output == 0 { t.Fatalf("complete numeric plan is absent: %v", err) }
	want, wantHead, err := store.PreviewForEpoch(7, raw, alpha)
	if err != nil { t.Fatal(err) }
	out, head, err := store.previewForEpochV2WithBudget(t.Context(), 7, raw, alpha, 3, releaseHeadV2Budget{limit: plan.used})
	if err != nil || !reflect.DeepEqual(out, want) || !reflect.DeepEqual(head, wantHead) || len(out) != 1 { t.Fatalf("exact admitted numeric plan differs from original UID fold: %v", err) }
	out, head, err = store.previewForEpochV2WithBudget(t.Context(), 7, raw, alpha, 3, releaseHeadV2Budget{limit: plan.used-1})
	if err == nil || out != nil || head != nil { t.Fatalf("one-byte-short growth plan returned partial output: %v", err) }
	ctx, cancel := context.WithCancel(t.Context())
	cancel()
	out, head, err = store.previewForEpochV2WithBudget(ctx, 7, raw, alpha, 3, releaseHeadV2Budget{limit: plan.used})
	if !errors.Is(err, context.Canceled) || out != nil || head != nil { t.Fatalf("canceled numeric admission returned partial output: %v", err) }
	if len(store.values) != 0 || store.lastSubnetEpoch != nil { t.Fatal("numeric boundary preview committed live state") }
}
...
func TestReleaseHeadV2AccountingMeasuresExactCompletedOutput(t *testing.T) {
	t.Parallel()
	t.Cleanup(func() { if !t.Failed() { t.Log("HEAD-ACCOUNTING-v1 PASS TestReleaseHeadV2AccountingMeasuresExactCompletedOutput") } })
	fixture := newReleaseHeadV2TestFixture(t, 2)
	result, err := fixture.gather(t.Context(), fixture.options(t))
	if err != nil { t.Fatal(err) }
	if len(result.Weights) == 0 || len(result.Eligible) == 0 || len(result.HeadEMA) == 0 { t.Fatal("genuine output witness lost its positive rational owners") }
	...
	for _, rows := range [][]ExactWeightInput{result.Weights, result.Eligible} {
		...
		for _, row := range rows { exact += uint64(reflect.TypeFor[big.Rat]().Size())+(uint64(len(row.Score.Num().Bits()))+uint64(len(row.Score.Denom().Bits())))*uint64(reflect.TypeFor[big.Word]().Size()) }
	}
	exact += uint64(len(result.Bound))*(8+uint64(reflect.TypeFor[map[connect.Id]bool]().Size()))+uint64(len(result.Controlled))*(2+1)
	for _, members := range result.Bound { exact += uint64(len(members))*(16+1) }
	if err := checkReleaseHeadV2Result(t.Context(), result, releaseHeadV2Budget{limit: exact, used: exact}); err != nil { t.Fatalf("exact completed result was refused: %v", err) }
	if err := checkReleaseHeadV2Result(t.Context(), result, releaseHeadV2Budget{limit: exact-1, used: exact-1}); err == nil { t.Fatal("one-byte-short completed result allowance passed") }
	if err := checkReleaseHeadV2Result(t.Context(), result, releaseHeadV2Budget{limit: exact-1, used: exact}); err == nil { t.Fatal("completed result escaped combined budget invariant") }
	ctx, cancel := context.WithCancel(t.Context())
	cancel()
	if err := checkReleaseHeadV2Result(ctx, result, releaseHeadV2Budget{limit: exact, used: exact}); !errors.Is(err, context.Canceled) { t.Fatalf("completed output ignored cancellation: %v", err) }
	fixture.assertNoEMACommit(t)
}
