...
func (self *releaseHeadV2RPCFixture) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	if err := json.NewDecoder(request.Body).Decode(&raw); err != nil { http.Error(writer, err.Error(), 400); return }
	reply := func(call chainBatchRPCRequest) map[string]any {
		...
		refuse := func(message string) map[string]any { response["error"] = map[string]any{"code": -32000, "message": message}; return response }
		if call.Method == "eth_getBlockByHash" {
			var hash common.Hash
			if len(call.Params) != 2 || json.Unmarshal(call.Params[0], &hash) != nil || hash != common.HexToHash(self.artifact.EVMSnapshotHash) { return refuse("fixture head hash differs") }
			response["result"] = map[string]any{"number": hexutil.Uint64(self.artifact.EVMSnapshotBlock), "hash": hash}
			return response
		}
		if call.Method != "eth_call" || len(call.Params) != 2 { return refuse("unexpected fixture call") }
		var payload struct { To common.Address; Data hexutil.Bytes; Input hexutil.Bytes }
		var block gethrpc.BlockNumberOrHash
		if json.Unmarshal(call.Params[0], &payload) != nil || json.Unmarshal(call.Params[1], &block) != nil || block.BlockHash == nil || *block.BlockHash != common.HexToHash(self.artifact.EVMSnapshotHash) || !block.RequireCanonical || payload.To != common.HexToAddress(self.artifact.Coordinator) { return refuse("fixture binding is not at the pinned coordinator/hash") }
		calldata := payload.Data
		if len(calldata) == 0 { calldata = payload.Input }
		coordinator := stabi.NewSTCoordinator()
		selector := coordinator.PackBindingAt([16]byte{}, big.NewInt(1))
		if len(calldata) != 68 || !bytes.Equal(calldata[:4], selector[:4]) || new(big.Int).SetBytes(calldata[36:]).Cmp(new(big.Int).SetUint64(self.artifact.SettlementEpoch)) != 0 { return refuse("fixture binding selector/epoch differs") }
		var clientID connect.Id
		copy(clientID[:], calldata[4:20])
		if clientID == self.failClient && self.failClient != (connect.Id{}) { return refuse("test-owned exact binding failure") }
		binding, found := self.bindingKVs[clientID]
		if !found { return refuse("fixture client absent from independent binding census") }
		parsed, err := stabi.STCoordinatorMetaData.ParseABI()
		if err != nil { return refuse(err.Error()) }
	...
	if trimmed := bytes.TrimSpace(raw); len(trimmed) != 0 && trimmed[0] == '[' {
		...
		if err := json.Unmarshal(raw, &calls); err != nil { http.Error(writer, err.Error(), 400); return }
		func() { self.stateLock.Lock(); defer self.stateLock.Unlock(); self.batches++ }()
		results := make([]map[string]any, len(calls))
		for index, call := range calls { results[len(calls)-1-index] = reply(call) }
		_ = json.NewEncoder(writer).Encode(results)
		return
	}
	var call chainBatchRPCRequest
	if err := json.Unmarshal(raw, &call); err != nil { http.Error(writer, err.Error(), 400); return }
...
func newReleaseHeadV2TestFixture(t *testing.T, completed int) *releaseHeadV2TestFixture {
	...
	for _, binding := range artifact.Bindings {
		...
		if binding.LiveUIDFound { hotkeys[common.HexToHash(binding.Hotkey)] = binding.LiveUID }
	}
...
func (self *releaseHeadV2TestFixture) assertNoEMACommit(t *testing.T) {
	...
	store.mu.Unlock()
	if !empty { t.Fatal("head preview committed live EMA state before an intent") }
	if _, err := os.Lstat(store.path); !errors.Is(err, os.ErrNotExist) { t.Fatalf("head preview changed durable EMA state: %v", err) }
}
...
func TestReleaseHeadV2CollectsRealM8WithPinnedBindingBatches(t *testing.T) {
	...
	if requests == 0 || batches != 2 || len(result.Inputs) != 2 { t.Fatalf("real pinned binding census was not consumed: requests=%d batches=%d", requests, batches) }
...
func TestReleaseHeadV2RebindingKeepsZeroHeadAndPoolExclusion(t *testing.T) {
	...
	for clientID, binding := range fixture.rpc.bindingKVs {
		if binding.Active { binding.Generation++; fixture.rpc.bindingKVs[clientID] = binding }
	}
	result, err := fixture.gather(t.Context(), fixture.options(t))
	if err != nil || len(result.Weights) != 0 { t.Fatalf("new generation inherited old head work: %v", err) }
	for _, binding := range fixture.rpc.bindingKVs {
		clientID, _ := connect.ParseId(binding.ClientID)
		if result.Bound[binding.NoID][clientID] != binding.Active { t.Fatal("live unselected binding did not retain pool exclusion") }
	}
	fixture.assertNoEMACommit(t)
}
...
func TestReleaseHeadV2StaleUIDReturnsProviderToPool(t *testing.T) {
	t.Parallel()
	fixture := newReleaseHeadV2TestFixture(t, 2)
	for hotkey := range fixture.hotkeys { if hotkey != fixture.steerer.hotkey.PublicKey() { delete(fixture.hotkeys, hotkey) } }
	result, err := fixture.gather(t.Context(), fixture.options(t))
	if err != nil || len(result.Weights) != 0 || len(result.StaleBindings) == 0 { t.Fatalf("stale native binding was not rejected: %v", err) }
	for _, providers := range result.Bound { if len(providers) != 0 { t.Fatal("deregistered provider remained excluded from pool") } }
	if !reflect.DeepEqual(result.Inputs, fixture.measurement.artifact.Inputs) { t.Fatal("stale head altered signed pool exposure") }
	fixture.assertNoEMACommit(t)
}
...
func TestReleaseHeadV2AdmitsAllOperatorAuthorityBeforeIO(t *testing.T) {
	...
	if err == nil || requests != 0 || *reads != 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("later authority reached I/O: requests=%d reads=%d err=%v", requests, *reads, err) }
	fixture.assertNoEMACommit(t)
}
...
func TestReleaseHeadV2BindingFailurePrecedesAllProofReaders(t *testing.T) {
	t.Parallel()
	fixture := newReleaseHeadV2TestFixture(t, 2)
	for clientID, binding := range fixture.rpc.bindingKVs { if binding.NoID == 10 { fixture.rpc.failClient = clientID; break } }
	options := fixture.options(t)
	reads := observeReleaseMeasurementV2SettlementTest(&options)
	result, err := fixture.gather(t.Context(), options)
	requests, _ := fixture.rpc.counts()
	if err == nil || requests == 0 || *reads != 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("binding failure produced partial head: requests=%d reads=%d err=%v", requests, *reads, err) }
	fixture.assertNoEMACommit(t)
}
...
func TestReleaseHeadV2ChangedProofDiscardsAllHeadOutput(t *testing.T) {
	...
	options.Operators[10] = operator
	result, err := fixture.gather(t.Context(), options)
	if err == nil || !changed || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("changed real proof escaped complete replay: changed=%v error=%v", changed, err) }
	fixture.assertNoEMACommit(t)
}
...
func TestReleaseHeadV2OwnsInputsAcrossProofCallbacks(t *testing.T) {
	...
	operator.Measurement.Replay.ReadMetadata = func(ctx context.Context, hash string, size uint64) ([]byte, error) {
		if !mutated {
			...
			for _, key := range later.Measurement.Replay.ServerKeys { key[0] ^= 1; break }
	...
	result, err := fixture.gather(t.Context(), options)
	if err != nil || !mutated || !reflect.DeepEqual(result.Inputs, wantInputs) || !reflect.DeepEqual(result.Bindings, wantBindings) { t.Fatalf("callback changed owned head evidence: mutated=%v err=%v", mutated, err) }
	fixture.assertNoEMACommit(t)
}
...
func TestReleaseHeadV2OwnsInputsBeforeFirstClientKeyCallback(t *testing.T) {
	...
	fixture.steerer.contexts[9].ClientKey = func(clientID connect.Id) ([32]byte, bool, error) {
		if !mutated {
			...
			later := options.Operators[10]
			for _, key := range later.Measurement.Replay.ServerKeys { key[0] ^= 1; break }
			for key := range fixture.hotkeys { delete(fixture.hotkeys, key) }
			fixture.snapshot.BlockNumber++
			fixture.snapshot.Epoch.Add(fixture.snapshot.Epoch, big.NewInt(1))
			fixture.steerer.contexts[10].ClientKey = func(connect.Id) ([32]byte, bool, error) { return [32]byte{}, false, errors.New("borrowed later lookup must not execute") }
		}
		return lookup(clientID)
	}
	result, err := fixture.gather(t.Context(), options)
	_, batches := fixture.rpc.counts()
	if err != nil || !mutated || batches != 2 || !reflect.DeepEqual(result.Inputs, wantInputs) || !reflect.DeepEqual(result.Bindings, wantBindings) { t.Fatalf("first client-key callback retargeted owned head evidence: mutated=%v batches=%d err=%v", mutated, batches, err) }
	fixture.assertNoEMACommit(t)
}
...
func TestReleaseHeadV2RejectsCensusStorageBeforeBindingIO(t *testing.T) {
	...
	result, err := fixture.gather(t.Context(), options)
	requests, _ := fixture.rpc.counts()
	if err == nil || !strings.Contains(err.Error(), "compact live head census exceeds its control bound") || requests != 0 || *reads != 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("generated census bypassed bounded pre-I/O admission: requests=%d reads=%d err=%v", requests, *reads, err) }
	fixture.assertNoEMACommit(t)
}
...
func TestReleaseHeadV2RejectsInvalidSignerBeforeIO(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	for _, test := range []struct { name string; hotkey *crv4.Keypair }{
		...
		result, err := fixture.gather(t.Context(), options)
		requests, _ := fixture.rpc.counts()
		if err == nil || requests != 0 || *reads != 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("%s signer reached I/O or yielded head: requests=%d reads=%d err=%v", test.name, requests, *reads, err) }
	}
	fixture.assertNoEMACommit(t)
}
...
type releaseHeadV2LateReader struct { io.ReadCloser; cancel context.CancelFunc; failure error; closed *int }
...
func TestReleaseHeadV2LateCloseCancellationRetainsFailure(t *testing.T) {
	...
	if !errors.Is(err, failure) || !errors.Is(err, context.Canceled) || closed == 0 || !reflect.DeepEqual(result, releaseHeadResult{}) { t.Fatalf("late failure/cancellation escaped result clearing: closes=%d err=%v", closed, err) }
	fixture.assertNoEMACommit(t)
}
...
func TestReleaseHeadV2SharedSelectionKeepsTop200Of1000(t *testing.T) {
	...
	if policy.Steering.MaximumHeadFleets != 200 { t.Fatal("existing top-200 policy changed") }
	...
	for key, prefixes := range fleets { original[key] = maps.Clone(prefixes) }
	...
	if err != nil || len(result.SelectedUIDs) != 200 || len(result.RejectedUIDs) != 800 || len(result.EligibleUIDs) != 1000 { t.Fatalf("top-200 census differs: %v", err) }
	for index, uid := range result.SelectedUIDs { if uid != uint16(index) { t.Fatalf("exact tie selection is not by uid: %v", result.SelectedUIDs) } }
	for uid, providerMembers := range members {
		for _, member := range providerMembers {
			if !result.Bound[member.NoID][member.ClientID] { t.Fatalf("registered selected/rejected fleet %d re-entered pool", uid) }
			if result.Controlled[uid] != (member.NoID == 10) { t.Fatal("controlled operator masking changed") }
		}
	}
	if !reflect.DeepEqual(original, fleets) { t.Fatal("selection changed its input fleet/prefix sets") }
}
...
func TestReleaseHeadV2BoundedEMAPreviewPreservesDurableSameEpoch(t *testing.T) {
	...
	want, transcript, err := store.PreviewForEpoch(7, raw, alpha)
	if err != nil || want[key.UID] == nil || want[key.UID].Cmp(big.NewRat(5, 3)) != 0 { t.Fatalf("initial exact EMA control differs: %v", err) }
	if err := store.CommitForEpoch(7, transcript, alpha); err != nil { t.Fatal(err) }
	before, err := os.ReadFile(store.path)
	if err != nil { t.Fatal(err) }
	store, err = NewHeadEMAStore(stateDir)
	if err != nil { t.Fatal(err) }
	got, replay, err := store.previewForEpochV2(t.Context(), 7, raw, alpha, 8, 1024 * 1024)
	if err != nil || !reflect.DeepEqual(got, want) || !reflect.DeepEqual(replay, transcript) { t.Fatalf("bounded same-epoch preview changed committed values: %v", err) }
	successorRaw := map[FleetScoreKey]*big.Rat{key: big.NewRat(2, 3)}
	wantNext, wantTranscript, err := store.PreviewForEpoch(8, successorRaw, alpha)
	if err != nil { t.Fatal(err) }
	gotNext, gotTranscript, err := store.previewForEpochV2(t.Context(), 8, successorRaw, alpha, 8, 1024 * 1024)
	if err != nil || !reflect.DeepEqual(gotNext, wantNext) || !reflect.DeepEqual(gotTranscript, wantTranscript) { t.Fatalf("bounded successor differs from exact ordinary preview: %v", err) }
	after, err := os.ReadFile(store.path)
	if err != nil || !bytes.Equal(before, after) { t.Fatalf("bounded preview rewrote durable history: %v", err) }
	if store.lastSubnetEpoch == nil || *store.lastSubnetEpoch != 7 || !reflect.DeepEqual(store.lastFold, transcript) { t.Fatal("bounded preview advanced live history") }
}
...
func TestReleaseHeadV2BoundsEMAUnionAndCensuses(t *testing.T) {
	...
	for _, test := range []struct { name string; values map[string]headEMAEntry; fold []HeadEMAMeasurement; raw map[FleetScoreKey]*big.Rat; message string }{
		...
		store := &HeadEMAStore{values: test.values, lastFold: test.fold}
		...
		if err == nil || !strings.Contains(err.Error(), test.message) || got != nil || transcript != nil { t.Fatalf("%s did not refuse exact census before preview: %v", test.name, err) }
		if !reflect.DeepEqual(store.values, beforeValues) || !reflect.DeepEqual(store.lastFold, beforeFold) { t.Fatalf("%s admission mutated history", test.name) }
	}
}
...
func TestReleaseHeadV2BoundsRetainedEMARationalStorage(t *testing.T) {
	...
	if !ok { t.Fatal("deterministic large integer is invalid") }
	for _, transcriptOnly := range []bool{false, true} {
		...
		store, err = NewHeadEMAStore(stateDir)
		if err != nil { t.Fatal(err) }
		if transcriptOnly && (len(store.values) != 0 || len(store.lastFold) != 1 || len(store.lastFold[0].Prior.Numerator) != 1024) { t.Fatal("real zero-weight commit did not retain large prior in transcript only") }
		before, err := os.ReadFile(store.path)
		if err != nil { t.Fatal(err) }
		got, replay, err := store.previewForEpochV2(t.Context(), epoch, raw, alpha, 8, 512)
		if err == nil || !strings.Contains(err.Error(), "control storage exceeds its bound") || got != nil || replay != nil { t.Fatalf("retained rational bypassed bounded preview (transcript only %v): %v", transcriptOnly, err) }
		after, err := os.ReadFile(store.path)
		if err != nil || !bytes.Equal(before, after) { t.Fatalf("refused retained rational changed disk (transcript only %v): %v", transcriptOnly, err) }
	}
}
...
func TestReleaseHeadV2BoundsCurrentEMARationalStorage(t *testing.T) {
	...
	got, transcript, err := store.previewForEpochV2(t.Context(), 7, map[FleetScoreKey]*big.Rat{key: large}, exactPolicy(t).Steering.HeadScoreEMA, 1, 512)
	if err == nil || !strings.Contains(err.Error(), "raw rational exceeds its control bound") || got != nil || transcript != nil { t.Fatalf("current rational bypassed word bound: %v", err) }
	if len(store.values) != 0 || store.lastSubnetEpoch != nil { t.Fatal("refused current rational changed live history") }
	if _, err := os.Lstat(store.path); !errors.Is(err, os.ErrNotExist) { t.Fatalf("refused current rational changed disk: %v", err) }
}
