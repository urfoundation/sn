...
func validateAttemptSettlementV2Paths(paths []string) error {
	for index, path := range paths {
		if !filepath.IsAbs(path) || filepath.Clean(path) != path || filepath.Dir(path) == path {
			return errors.New("compact runtime path is not clean, absolute and non-root")
		}
...
func acquireAttemptSettlementV2Owners(ctx context.Context, coordinator string, participants []AttemptSettlementRuntimeV2Participant, authority AttemptSettlementV2Options, fresh bool) (*attemptSettlementV2Owners, error) {
	...
	batch := &attemptSettlementV2Owners{ordered: ordered, engineOrder: engineOrder, candidates: make([]*StatsEngine, len(ordered))}
...
func attemptSettlementV2RawMeasurement(stats *StatsEngine, operator AttemptSettlementV2OperatorOptions) (ReleaseStatsMeasurement, error) {
	...
	for id := range stats.window {
		if err := add(id); err != nil { return ReleaseStatsMeasurement{}, err }
	}
...
func ownAttemptSettlementRuntimeV2Options(ctx context.Context, coordinator string, participants []AttemptSettlementRuntimeV2Participant, options AttemptSettlementRuntimeV2Options) (AttemptSettlementRuntimeV2Options, error) {
	...
	owned := AttemptSettlementRuntimeV2Options{Authority: authority, Seal: map[uint64]AttemptCutV2SealOptions{}, PrivateKeys: map[uint64]ed25519.PrivateKey{}}
	paths := []string{}
...
func InitializeAttemptSettlementEpochV2(ctx context.Context, coordinator string, participants []AttemptSettlementRuntimeV2Participant, epoch uint64, authority AttemptSettlementV2Options) error {
	return initializeAttemptSettlementEpochV2(ctx, coordinator, participants, epoch, authority, attemptSettlementV2PhysicalIO())
}
...
func RecoverAttemptSettlementEpochV2(ctx context.Context, coordinator string, participants []AttemptSettlementRuntimeV2Participant, authority AttemptSettlementV2Options) error {
	return recoverAttemptSettlementEpochV2(ctx, coordinator, participants, authority, attemptSettlementV2PhysicalIO())
}
