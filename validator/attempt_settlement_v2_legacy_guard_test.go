...
func activateAttemptSettlementRuntimeV2TestSecond(t *testing.T, fixture *attemptSettlementRuntimeV2TestFixture) {
	...
	if err := InitializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants[1:], 42, authority, runtimeAttemptSettlementV2TestPersistence()); err != nil { t.Fatal(err) }
}
...
func publishLegacyAttemptSettlementRuntimeV2TestJournal(t *testing.T, fixture *attemptSettlementRuntimeV2TestFixture, encoded []byte) {
	t.Helper()
	if err := atomicStateWrite(attemptSettlementTransactionPath(fixture.coordinator), encoded, 0o600); err != nil { t.Fatal(err) }
}
...
func assertLegacyAttemptSettlementRuntimeV2TestUnchanged(t *testing.T, coordinator string, transaction *attemptSettlementTransaction, journal []byte, before [][]byte) {
	...
	if err != nil || !bytes.Equal(actual, journal) || attemptSettlementV2ImageHash(actual) != attemptSettlementV2ImageHash(journal) { t.Fatalf("refusal changed original journal: %v", err) }
	for index, snapshot := range transaction.Snapshots {
		actual, err := os.ReadFile(snapshot.StatsPath)
		if err != nil || !bytes.Equal(actual, before[index]) || attemptSettlementV2ImageHash(actual) != attemptSettlementV2ImageHash(before[index]) { t.Fatalf("refusal changed no_id %d target: %v", snapshot.NoID, err) }
	}
}
...
func TestAttemptSettlementRuntimeV2LegacyBatchCurrentCannotClearOwnership(t *testing.T) {
	for _, mixed := range []bool{false, true} {
		...
		if mixed { activateAttemptSettlementRuntimeV2TestSecond(t, fixture) }
		...
		if err := AdvanceAttemptSettlementEpoch(fixture.coordinator, 42, fixture.fixtures[0].expected.Boundary, legacy); err == nil || !strings.Contains(err.Error(), "v2 settlement coordinator") { t.Fatalf("mixed=%t legacy current bypass: %v", mixed, err) }
		if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("legacy current changed exact cursor/epoch/EMA/history") }
		for _, participant := range fixture.participants { if !participant.Stats.attemptCutPending || participant.Stats.attemptSettlementCutPending { t.Fatal("legacy current changed native reservation") } }
		if _, err := os.Lstat(attemptSettlementTransactionPath(fixture.coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("legacy current created journal: %v", err) }
	}
}
...
func TestAttemptSettlementRuntimeV2LegacyBatchAdvanceCannotPartiallyMutate(t *testing.T) {
	for _, mixed := range []bool{false, true} {
		...
		legacy := make([]AttemptSettlementParticipant, len(fixture.participants))
		for index, participant := range fixture.participants { legacy[index] = AttemptSettlementParticipant{NoID: participant.NoID, StateDir: participant.StateDir, Stats: participant.Stats} }
		before := runtimeAttemptSettlementV2TestImages(t, fixture.participants)
		if err := AdvanceAttemptSettlementEpoch(fixture.coordinator, 43, fixture.fixtures[0].expected.Boundary, legacy); err == nil || !strings.Contains(err.Error(), "v2 settlement coordinator") { t.Fatalf("mixed=%t legacy advance bypass: %v", mixed, err) }
		if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("legacy advance changed exact cursor/epoch/EMA/history") }
		for _, participant := range fixture.participants { if participant.Stats.attemptCutPending || participant.Stats.attemptSettlementCutPending { t.Fatal("legacy advance partially reserved participants") } }
	}
}
...
func TestAttemptSettlementRuntimeV2LegacyPerEngineAdvanceRefusesDowngrade(t *testing.T) {
	...
	for _, epoch := range []uint64{42, 43} {
		...
		if err := participant.Stats.AdvanceSettlementEpoch(epoch, participant.StateDir); err == nil { t.Fatalf("legacy engine accepted epoch%d", epoch) }
		if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("legacy engine mutated compact state") }
		disk, err := os.ReadFile(filepath.Join(participant.StateDir, "stats.json"))
		if err != nil || !bytes.Equal(disk, before[0]) { t.Fatalf("legacy engine changed v6 disk: %v", err) }
	}
}
...
func TestAttemptSettlementRuntimeV2LegacyRecoveryMixedTargetsStayExact(t *testing.T) {
	...
	for index, snapshot := range transaction.Snapshots { var err error; before[index], err = os.ReadFile(snapshot.StatsPath); if err != nil { t.Fatal(err) } }
	if bytes.Equal(before[0], transaction.Snapshots[0].StatsJSON) { t.Fatal("earlier v1 target would not prove a partial write") }
	if err := RecoverAttemptSettlementEpoch(fixture.coordinator, participants); err == nil { t.Fatal("v1 journal overwrote a later v6 target") }
...
func TestAttemptSettlementRuntimeV2LegacyRecoverySingleTargetStaysExact(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := RecoverAttemptSettlementEpoch(fixture.coordinator, participants); err == nil { t.Fatal("single v6 target was overwritten by v1 recovery") }
...
func TestAttemptSettlementRuntimeV2LegacyRecoveryRejectsV6JournalInjection(t *testing.T) {
	...
	for index, participant := range fixture.participants { var err error; original[index], err = os.ReadFile(filepath.Join(participant.StateDir, "stats.json")); if err != nil { t.Fatal(err) } }
	activateAttemptSettlementRuntimeV2TestSecond(t, fixture)
	transaction, participants, journal := legacyAttemptSettlementRuntimeV2TestJournal(t, fixture, fixture.participants)
	publishLegacyAttemptSettlementRuntimeV2TestJournal(t, fixture, journal)
	if err := atomicStateWrite(transaction.Snapshots[1].StatsPath, original[1], 0o600); err != nil { t.Fatal(err) }
	if err := RecoverAttemptSettlementEpoch(fixture.coordinator, participants); err == nil || !strings.Contains(err.Error(), "legacy settlement snapshot") { t.Fatalf("v6 journal admission was bypassed: %v", err) }
...
func TestAttemptSettlementRuntimeV2LegacyRecoveryRejectsUnknownTargets(t *testing.T) {
	for _, variant := range []string{"invalid-json", "unknown-version", "fifo", "symlink"} {
		...
		switch variant {
		case "invalid-json": err = os.WriteFile(path, []byte("{"), 0o600)
		case "unknown-version": err = os.WriteFile(path, []byte("{\"v\":999}\n"), 0o600)
		case "fifo": err = unix.Mkfifo(path, 0o600)
		case "symlink": err = os.Symlink("stats.json.preserved", path)
		}
		if err != nil { t.Fatal(err) }
		if err := RecoverAttemptSettlementEpoch(fixture.coordinator, participants); err == nil { t.Fatalf("%s occupied target was repaired as absent", variant) }
		actual, err := os.ReadFile(transaction.Snapshots[0].StatsPath)
		if err != nil || !bytes.Equal(actual, first) { t.Fatalf("%s wrote earlier valid target: %v", variant, err) }
		actual, err = os.ReadFile(attemptSettlementTransactionPath(fixture.coordinator))
		if err != nil || !bytes.Equal(actual, journal) { t.Fatalf("%s consumed original journal: %v", variant, err) }
	}
}
...
func TestAttemptSettlementRuntimeV2LegacyRecoveryKeepsValidV1Behavior(t *testing.T) {
	...
	transaction, participants, journal := legacyAttemptSettlementRuntimeV2TestJournal(t, fixture, fixture.participants)
	publishLegacyAttemptSettlementRuntimeV2TestJournal(t, fixture, journal)
	if err := RecoverAttemptSettlementEpoch(fixture.coordinator, participants); err != nil { t.Fatal(err) }
	for _, snapshot := range transaction.Snapshots {
		actual, err := os.ReadFile(snapshot.StatsPath)
		if err != nil || !bytes.Equal(actual, snapshot.StatsJSON) { t.Fatalf("valid v1 recovery output changed: %v", err) }
	}
	if _, err := os.Lstat(attemptSettlementTransactionPath(fixture.coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("valid v1 recovery retained journal: %v", err) }
}
