...
func openAttemptPrivateDirectory(path string) (*attemptPrivateDirectory, error) {
	if !filepath.IsAbs(path) || filepath.Clean(path) != path || filepath.Dir(path) == path { return nil, errors.New("private directory path is not canonical absolute non-root") }
	file, err := openAttemptPrivateDirectoryFile(path)
	if err != nil { return nil, err }
	state, err := statAttemptPrivateFile(file)
	if err != nil || !state.directory() { return nil, errors.Join(errors.New("private directory descriptor differs"), err, file.Close()) }
...
func (self *attemptPrivateDirectory) check() error {
	if self == nil || self.file == nil { return errors.New("private directory owner is closed") }
	current, err := openAttemptPrivateDirectory(self.path)
	if err != nil { return err }
	err = current.close()
	if self.anchor.dev != current.anchor.dev || self.anchor.ino != current.anchor.ino || self.anchor.mode != current.anchor.mode || self.anchor.uid != current.anchor.uid { return errors.Join(errors.New("private directory namespace changed after acquisition"), err) }
	state, statErr := statAttemptPrivateFile(self.file)
	if statErr != nil || state.dev != self.anchor.dev || state.ino != self.anchor.ino || state.mode != self.anchor.mode || state.uid != self.anchor.uid { return errors.Join(errors.New("private directory descriptor changed after acquisition"), err, statErr) }
	return err
}

// Closing does not suppress a late descriptor failure.
func (self *attemptPrivateDirectory) close() error {
	if self == nil || self.file == nil { return nil }
	file := self.file
	self.file = nil
	return file.Close()
}
