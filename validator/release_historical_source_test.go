...
func newReleaseHistoricalSourceTestFixture(t *testing.T) *releaseHistoricalSourceTestFixture {
	...
	native.chain.API.Client = &validatorRuntimeIdentityTestClient{callContext: func(ctx context.Context, result any, method string, args ...any) error {
		if method == "state_getStorage" && len(args) == 2 && args[0] == epochKey.Hex() {
			if args[1] != native.block.Hex() { return errors.New("source schedule changed original block") }
	...
	if err := signer.ValidatePreparedSource(prepared); err != nil { t.Fatalf("original455 SDK signed source: %v", err) }
...
func TestReleaseEvidenceV2HistoricalRuntimeSourceAuthenticatesOriginalSignedBatch(t *testing.T) {
	...
	if err := authenticateReleaseNativeSourceReferenceV2(t.Context(), fixture.native.chain, &fixture.config, fixture.intent, fixture.artifact); err != nil { t.Fatalf("original455 signed source replay under458: %v", err) }
	if fixture.native.chain.Meta != metadata || fixture.native.chain.Runtime != runtime { t.Fatal("signed source replay changed current signing owner") }
	fixture.intent.Prepared.SourceCommitment.RuntimeSpec = 458
	if err := authenticateReleaseNativeSourceReferenceV2(t.Context(), fixture.native.chain, &fixture.config, fixture.intent, fixture.artifact); err == nil { t.Fatal("original signature was relabeled as runtime458") }
}
...
func TestReleaseEvidenceV2HistoricalRuntimeCaptureRetainsExactOriginalMetadata(t *testing.T) {
	...
	err := CaptureReleaseNativeSourceV2(t.Context(), fixture.native.chain, &fixture.config, fixture.intent, fixture.measurement, func(_ context.Context, read ReleaseEvidenceV2NativeRead) error { reads = append(reads, read); return nil })
	if err != nil { t.Fatalf("original455 source capture under458: %v", err) }
	...
	for _, read := range reads {
		if read.Method == "state_getMetadata" {
			metadataCount++
			if !bytes.Equal(read.Result, want) { t.Fatal("capture replaced original metadata bytes") }
		}
	}
	if metadataCount == 0 { t.Fatal("capture omitted historical metadata after authentication") }
}
...
func TestReleaseEvidenceV2HistoricalRuntimePendingSignedV2RefusesNewSubmission(t *testing.T) {
	...
	fixture.native.chain.API.Client = &validatorRuntimeIdentityTestClient{callContext: func(ctx context.Context, result any, method string, args ...any) error {
		if strings.HasPrefix(method, "author_") { sends++; return errors.New("historical signed source attempted broadcast") }
		if method == "chain_getBlock" {
			if len(args) != 1 || args[0] != fixture.native.block.Hex() { return errors.New("pending signed source changed block") }
			blocks++
	...
	if _, err := steerer.reconcilePendingV2(t.Context(), fixture.intent, &crv4.EpochScheduleState{SubnetEpochIndex: 1}); err == nil || !strings.Contains(err.Error(), "pending steering replay uses a historical signing runtime") || blocks != 1 || sends != 0 { t.Fatalf("old signed source replay: blocks=%d sends=%d error=%v", blocks, sends, err) }
}
...
func TestReleaseEvidenceV2HistoricalRuntimeFinalizedSourceReachesOriginalEventVerifier(t *testing.T) {
	...
	fixture.native.chain.API.Client = &validatorRuntimeIdentityTestClient{callContext: func(ctx context.Context, result any, method string, args ...any) error {
		if method == "chain_getBlock" {
			if len(args) != 1 || args[0] != fixture.native.block.Hex() { return errors.New("finalized source changed original block") }
		...
		if method == "state_getStorage" && len(args) == 2 && args[0] == eventKey.Hex() {
			if args[1] != fixture.native.block.Hex() { return errors.New("source events changed original block") }
			events++
			return canary
		}
		return original.CallContext(ctx, result, method, args...)
	}}
	if err := authenticateReleaseNativeSourceReferenceV2(t.Context(), fixture.native.chain, &fixture.config, fixture.intent, fixture.artifact); !errors.Is(err, canary) || events != 1 { t.Fatalf("finalized original source did not reach event verification: events=%d error=%v", events, err) }
}
