...
func DecodeAttemptSettlementClosureV2(ctx context.Context, raw []byte, options AttemptSettlementV2Options) (closure *AttemptSettlementClosureV2, result VerifiedAttemptSettlementV2, resultErr error) {
	if ctx == nil { return nil, result, errors.New("compact settlement context is nil") }
	defer func() { resultErr = errors.Join(resultErr, ctx.Err()); if resultErr != nil { closure, result = nil, VerifiedAttemptSettlementV2{} } }()
	if err := ctx.Err(); err != nil { return nil, result, err }
	if options.MaxClosureBytes == 0 || uint64(len(raw)) > options.MaxClosureBytes { return nil, result, errors.New("compact settlement encoded closure exceeds its byte bound") }
	var wire struct {
		Schema string `json:"schema"`
	...
	if err := decoder.Decode(&wire); err != nil { return nil, result, err }
	var trailing any
	if err := decoder.Decode(&trailing); !errors.Is(err, io.EOF) { return nil, result, errors.New("compact settlement closure has trailing data") }
	if uint64(len(wire.Transitions)) > options.MaxParticipants { return nil, result, errors.New("compact settlement encoded participant census exceeds its bound") }
	...
	for index, transition := range wire.Transitions {
		if err := ctx.Err(); err != nil { return nil, result, err }
		if transition == nil { return nil, result, errors.New("compact settlement closure contains nil") }
	...
	if err != nil || !bytes.Equal(raw, append(encoded, '\n')) { return nil, result, errors.New("compact settlement closure is not canonical") }
	...
	if err != nil { return nil, VerifiedAttemptSettlementV2{}, err }
...
func verifyAttemptSettlementV2Successor(transition *AttemptSettlementTransitionV2, measurement ReleaseStatsMeasurement, cut AttemptCutV2, immediate bool) error {
	...
	for _, quality := range transition.PostFold { wantKVs[quality.ClientID] = quality.QualityPPM }
	seenKVs := map[string]bool{}
	for _, provider := range measurement.Providers {
		want, exists := wantKVs[provider.ClientID]
		if provider.HasPriorQuality != exists || exists && provider.PriorQualityPPM != want || !exists && provider.PriorQualityPPM != 0 { return fmt.Errorf("compact settlement successor provider %s prior EMA differs", provider.ClientID) }
		if exists { seenKVs[provider.ClientID] = true }
	}
	if len(seenKVs) != len(wantKVs) { return errors.New("compact settlement successor prior EMA census is incomplete") }
	return nil
}
...
func VerifyAttemptSettlementClosureV2Lineage(ctx context.Context, prior, current *AttemptSettlementClosureV2, priorOptions, currentOptions AttemptSettlementV2Options) (result VerifiedAttemptSettlementV2, resultErr error) {
	...
	if err != nil { return result, err }
	if previous.closure.Epoch == ^uint64(0) || next.closure.Epoch != previous.closure.Epoch+1 || len(previous.closure.Transitions) != len(next.closure.Transitions) { return result, errors.New("compact settlement closure lineage skips an epoch or participant") }
	for index, transition := range next.closure.Transitions {
		old := previous.closure.Transitions[index]
		if old.Identity.NoID != transition.Identity.NoID { return result, errors.New("compact settlement closure lineage changes a participant") }
		if err := verifyAttemptSettlementV2Successor(old, transition.PreFold, transition.Cut, false); err != nil { return result, err }
	}
	if _, err := previous.replay(ctx, nil, false); err != nil { return result, err }
