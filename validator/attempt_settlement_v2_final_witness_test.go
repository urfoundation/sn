...
func runtimeAttemptSettlementV2RetargetDirectory(t *testing.T, path string) func() {
	...
	return func() {
		if restored { return }
		restored = true
		if err := os.Remove(path); err != nil { t.Error(err); return }
		if err := os.Rename(preserved, path); err != nil { t.Error(err) }
	}
}
...
func runtimeAttemptSettlementV2CloseForWitness(t *testing.T, fixture *attemptSettlementRuntimeV2TestFixture) *AttemptSettlementClosureV2 {
	...
	if err != nil || closure == nil { t.Fatalf("actual signed witness closure prerequisite: %v", err) }
	return closure
}
...
func TestAttemptSettlementRuntimeV2FinalWitnessInitializationCrossRoot(t *testing.T) {
	...
	var earlier string
	var restore func()
	defer func() { if restore != nil { restore() } }()
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		path := root.path
		err := root.close()
		closes++
		if closes == 1 { earlier = path }
		if closes == 3 {
			if earlier == "" || earlier == path { t.Fatal("cross-root close prerequisite differs") }
	...
	if err == nil || closes != 3 || !strings.Contains(err.Error(), "compact final directory witness") { t.Fatalf("last initialization close escaped complete-registry witness: %v/%d", err, closes) }
...
func TestAttemptSettlementRuntimeV2FinalWitnessAdvanceClosureChild(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		...
		err := root.close()
		closes++
		if filepath.Base(path) == "settlement-closures-v2" { child = path }
		if closes == 4 {
			if child == "" || child == path { t.Fatal("closed child prerequisite missing") }
	...
	if restore != nil { restore() }
	if err == nil || closure != nil || closes != 4 || !strings.Contains(err.Error(), "compact final directory witness") { t.Fatalf("last advance close lost immutable-child witness: %v/%d", err, closes) }
	...
	if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("final witness retry refolded actual completed state") }
}
...
func TestAttemptSettlementRuntimeV2FinalWitnessCurrentRetryCrossRoot(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		...
		err := root.close()
		closes++
		if earlier == "" && filepath.Base(path) != "settlement-closures-v2" { earlier = path }
		if closes == 5 {
			if earlier == "" || earlier == path { t.Fatal("current retry cross-root prerequisite differs") }
			restore = runtimeAttemptSettlementV2RetargetDirectory(t, earlier)
		}
		return err
	}
	closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t), physical, true)
	if restore != nil { restore() }
	if err == nil || closure != nil || closes != 5 || !strings.Contains(err.Error(), "compact final directory witness") { t.Fatalf("last current retry close escaped cross-root witness: %v/%d", err, closes) }
	fixture.assertReserved(t, 43)
}
...
func TestAttemptSettlementRuntimeV2FinalWitnessRecoveryBeforeAttachment(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		...
		if closes == 1 { earlier = path }
		if closes == 3 {
			if earlier == path { t.Fatal("fresh recovery cross-root prerequisite differs") }
			restore = runtimeAttemptSettlementV2RetargetDirectory(t, earlier)
		}
		return err
	}
	...
	if restore != nil { restore() }
	if err == nil || closes != 3 || !strings.Contains(err.Error(), "compact final directory witness") { t.Fatalf("last recovery close escaped before attachment: %v/%d", err, closes) }
	for _, participant := range restarted { if participant.Stats.attemptLedger != nil || participant.Stats.attemptV2 != nil { t.Fatal("late recovery close published a partial fresh owner") } }
...
func TestAttemptSettlementRuntimeV2FinalWitnessPublicReadClearsReplayResult(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		...
		closes++
		if closes == 2 {
			if filepath.Base(path) != "settlement-closures-v2" || proofCloses == 0 { t.Fatal("public read did not actually replay before final child close") }
	...
	if restore != nil { restore() }
	if err == nil || closure != nil || !reflect.DeepEqual(verified, VerifiedAttemptSettlementV2{}) || closes != 2 || proofCloses == 0 { t.Fatalf("public replay result escaped last-close retarget: %v/%d/%d", err, closes, proofCloses) }
}
...
func TestAttemptSettlementRuntimeV2FinalWitnessLateSameBytesLeafReplacement(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		...
		if closes == 4 {
			...
			if writeErr := os.WriteFile(path+".same-bytes-replacement", actual, 0o600); writeErr != nil { return errors.Join(err, writeErr) }
			return errors.Join(err, os.Rename(path+".same-bytes-replacement", path))
		}
		return err
	}
	closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t), physical, true)
	if err == nil || closure != nil || closes != 4 || !strings.Contains(err.Error(), "compact final leaf witness") { t.Fatalf("same-byte late leaf replacement escaped witness: %v/%d", err, closes) }
	fixture.assertReserved(t, 43)
	persisted, readErr := os.ReadFile(path)
	if readErr != nil || !bytes.Equal(actual, persisted) { t.Fatalf("replacement prerequisite changed actual bytes: %v", readErr) }
}
...
func TestAttemptSettlementRuntimeV2FinalWitnessLateClosureLeafAbsence(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		err := root.close()
		closes++
		if closes == 4 { return errors.Join(err, os.Rename(path, path+".preserved")) }
		return err
	}
	closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t), physical, true)
	if err == nil || !errors.Is(err, os.ErrNotExist) || closure != nil || closes != 4 || !strings.Contains(err.Error(), "compact final leaf witness") { t.Fatalf("late immutable leaf absence escaped witness: %v/%d", err, closes) }
	fixture.assertReserved(t, 43)
	if err := os.Rename(path+".preserved", path); err != nil { t.Fatal(err) }
	if _, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t)); err != nil { t.Fatal(err) }
}
...
func TestAttemptSettlementRuntimeV2FinalWitnessJournalAbsenceRecreated(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		...
		closes++
		if closes == 4 {
			if len(journal) == 0 { t.Fatal("actual journal recreation prerequisite missing") }
			return errors.Join(err, os.WriteFile(attemptSettlementTransactionV2Path(fixture.coordinator), journal, 0o600))
		}
		return err
	}
	closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t), physical, true)
	if err == nil || closure != nil || closes != 4 || !strings.Contains(err.Error(), "compact final leaf witness") { t.Fatalf("late recreated journal escaped absence witness: %v/%d", err, closes) }
	fixture.assertReserved(t, 43)
...
func TestAttemptSettlementRuntimeV2FinalWitnessAnchorIsOwnedCopy(t *testing.T) {
	...
	err := initializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence(), physical)
	if restore != nil { restore() }
	if err == nil || closes != 3 || !strings.Contains(err.Error(), "compact final directory witness") { t.Fatalf("callback-mutated anchor replaced owned final witness: %v/%d", err, closes) }
	fixture.assertReserved(t, 42)
}
...
func TestAttemptSettlementRuntimeV2FinalWitnessJoinsAllLateCauses(t *testing.T) {
	...
	physical.closeRoot = func(root *attemptPrivateDirectory) error {
		...
		err := root.close()
		closes++
		if closes == 1 { earlier = path; return errors.Join(err, firstCause) }
		if closes == 3 { restore = runtimeAttemptSettlementV2RetargetDirectory(t, earlier); cancel(); return errors.Join(err, lastCause) }
		return err
	}
	...
	if restore != nil { restore() }
	if !errors.Is(err, firstCause) || !errors.Is(err, lastCause) || !errors.Is(err, context.Canceled) || closes != 3 || !strings.Contains(err.Error(), "compact final directory witness") { t.Fatalf("final witness lost a close/cancellation/namespace cause: %v/%d", err, closes) }
	fixture.assertReserved(t, 42)
}
...
func TestAttemptSettlementRuntimeV2FinalWitnessStableFullLifecycle(t *testing.T) {
	...
	if err := initializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence(), physical); err != nil || closes != 3 { t.Fatalf("stable initialization witness changed close ownership: %v/%d", err, closes) }
	...
	if err != nil || !reflect.DeepEqual(read, closure) { t.Fatalf("stable full closure read differs: %v", err) }
	restarted := fixture.reopen(t)
	if err := RecoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, restarted, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence()); err != nil { t.Fatal(err) }
	for _, participant := range restarted { if participant.Stats.attemptLedger != participant.Ledger || participant.Stats.attemptCutPending { t.Fatal("stable final witness failed coherent fresh attachment") } }
}
