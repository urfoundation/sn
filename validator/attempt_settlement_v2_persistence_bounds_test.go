...
func (self attemptSettlementV2CountCustom) MarshalJSON() ([]byte, error) { *self.calls = *self.calls+1; return []byte(`"custom"`), nil }
...
func (self attemptSettlementV2CountText) MarshalText() ([]byte, error) { *self.calls = *self.calls+1; return []byte("custom text"), nil }
...
func runtimeAttemptSettlementV2CountMetadataReads(physical *attemptSettlementV2IO, name string, reads *int) {
	physical.closeFile = func(file *os.File) error {
		if filepath.Base(file.Name()) == name { *reads = *reads+1 }
		return file.Close()
	}
}
...
func runtimeAttemptSettlementV2OversizeSnapshot(t *testing.T, participant AttemptSettlementRuntimeV2Participant, limit uint64) {
	...
	if _, err := os.Lstat(path); err != nil { t.Fatal(err) }
	if err := os.Rename(path, path+".before-bound-control"); err != nil { t.Fatal(err) }
	file, err := os.OpenFile(path, os.O_CREATE|os.O_EXCL|os.O_WRONLY, 0o600)
	if err != nil { t.Fatal(err) }
	if err := errors.Join(file.Truncate(int64(limit+1)), file.Close()); err != nil { t.Fatal(err) }
}
...
func runtimeAttemptSettlementV2ActivationJournal(t *testing.T, fixture *attemptSettlementRuntimeV2TestFixture) []byte {
	...
	if err := initializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence(), physical); !errors.Is(err, cause) { t.Fatalf("actual activation journal prerequisite: %v", err) }
	encoded, err := os.ReadFile(attemptSettlementTransactionV2Path(fixture.coordinator))
	if err != nil || len(encoded) < 2 { t.Fatalf("actual activation journal missing: %v", err) }
	return encoded
}
...
func TestAttemptSettlementRuntimeV2PersistenceLimitsBeforeIO(t *testing.T) {
	...
	for _, persistence := range []AttemptSettlementRuntimeV2PersistenceBounds{
		...
		physical.writeSnapshot = func(*attemptPrivateDirectory, string, []byte) error { calls++; return errors.New("unexpected snapshot") }
		...
		if err := initializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, options.Authority, persistence, physical); err == nil { t.Fatal("invalid persistence allowance entered initialization") }
		if closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, options, physical, true); err == nil || closure != nil { t.Fatal("invalid persistence allowance entered advance") }
		fresh := append([]AttemptSettlementRuntimeV2Participant(nil), fixture.participants...)
		for index := range fresh { fresh[index].Stats = NewStatsEngine(fresh[index].Stats.cfg) }
		if err := recoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fresh, options.Authority, persistence, physical); err == nil { t.Fatal("invalid persistence allowance entered recovery") }
		if calls != 0 || !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatalf("invalid persistence bound reached IO or changed state: %d", calls) }
		for _, participant := range fixture.participants { if participant.Stats.attemptCutPending { t.Fatal("invalid persistence bound reserved a live owner") } }
	}
}
...
func TestAttemptSettlementRuntimeV2PersistenceReaderRejectsZeroBeforeOpen(t *testing.T) {
	...
	for _, limit := range []uint64{0, uint64(^uint(0)>>1), ^uint64(0)} {
		...
		if err == nil || data != nil || reads != 0 { t.Fatalf("unsafe v2 read bound reached leaf IO: %d/%d/%v", limit, reads, err) }
	}
}
...
func TestAttemptSettlementRuntimeV2PersistenceInitJournalReadBound(t *testing.T) {
	...
	physical.writeSnapshot = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected bounded init write") }
	...
	if err == nil || reads != 0 || writes != 0 { t.Fatalf("oversized initialization journal reached IO: %v/%d/%d", err, reads, writes) }
	fixture.assertReserved(t, 42)
	persistence.MaxJournalBytes++
	if err := InitializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority, persistence); err != nil { t.Fatalf("exact journal allowance refused: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2PersistenceAdvanceJournalReadBound(t *testing.T) {
	...
	physical, reads, writes := attemptSettlementV2PhysicalIO(), 0, 0
	runtimeAttemptSettlementV2CountMetadataReads(&physical, "settlement-transaction-v2.json", &reads)
	physical.writeSnapshot = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected bounded retry write") }
	...
	if err == nil || closure != nil || reads != 0 || writes != 0 { t.Fatalf("oversized terminal journal reached IO: %v/%d/%d", err, reads, writes) }
	fixture.assertReserved(t, 43)
	options = fixture.options(t)
	options.Persistence.MaxJournalBytes = uint64(len(encoded))
	if _, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, options); err != nil { t.Fatal(err) }
...
func TestAttemptSettlementRuntimeV2PersistenceRecoverJournalReadBound(t *testing.T) {
	...
	physical, reads, writes := attemptSettlementV2PhysicalIO(), 0, 0
	runtimeAttemptSettlementV2CountMetadataReads(&physical, "settlement-transaction-v2.json", &reads)
	physical.writeSnapshot = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected bounded recovery write") }
	...
	if err == nil || reads != 0 || writes != 0 { t.Fatalf("oversized startup journal reached IO: %v/%d/%d", err, reads, writes) }
	for _, participant := range restarted { if participant.Stats.attemptLedger != nil { t.Fatal("bounded startup refusal attached an operator") } }
	persistence.MaxJournalBytes++
	if err := RecoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, restarted, fixture.options(t).Authority, persistence); err != nil { t.Fatal(err) }
...
func TestAttemptSettlementRuntimeV2PersistenceCurrentActivationSnapshotReadBound(t *testing.T) {
	...
	runtimeAttemptSettlementV2CountMetadataReads(&physical, "stats.json", &reads)
	err := initializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority, persistence, physical)
	if err == nil || reads != 0 { t.Fatalf("oversized current activation snapshot was opened: %v/%d", err, reads) }
}
...
func TestAttemptSettlementRuntimeV2PersistenceCurrentTerminalSnapshotReadBound(t *testing.T) {
	...
	runtimeAttemptSettlementV2CountMetadataReads(&physical, "stats.json", &reads)
	closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, options, physical, true)
	if err == nil || closure != nil || reads != 0 { t.Fatalf("oversized current terminal snapshot was opened: %v/%d", err, reads) }
	fixture.assertReserved(t, 43)
}
...
func TestAttemptSettlementRuntimeV2PersistenceRecoverySnapshotReadBound(t *testing.T) {
	...
	runtimeAttemptSettlementV2CountMetadataReads(&physical, "stats.json", &reads)
	err := recoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, restarted, fixture.options(t).Authority, persistence, physical)
	if err == nil || reads != 0 { t.Fatalf("oversized startup snapshot was opened: %v/%d", err, reads) }
	for _, participant := range restarted { if participant.Stats.attemptLedger != nil { t.Fatal("oversized snapshot startup attached a partial operator") } }
}
...
func TestAttemptSettlementRuntimeV2PersistenceOriginalSnapshotReadBound(t *testing.T) {
	...
	runtimeAttemptSettlementV2CountMetadataReads(&physical, "stats.json", &reads)
	closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, options, physical, true)
	if err == nil || closure != nil || reads != 0 { t.Fatalf("oversized original snapshot was opened: %v/%d", err, reads) }
	if _, err := os.Lstat(attemptSettlementTransactionV2Path(fixture.coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("refused original image published journal: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2PersistenceKnownImageSnapshotReadBound(t *testing.T) {
	...
	physical.writeJournal = func(root *attemptPrivateDirectory, name string, data []byte) error {
		if err := writeAttemptSettlementV2OwnedState(root, name, data); err != nil { return err }
		journals++
		runtimeAttemptSettlementV2OversizeSnapshot(t, fixture.participants[0], options.Persistence.MaxSnapshotBytes)
		reads = 0
		return nil
	}
	physical.writeSnapshot = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected post-growth write") }
	closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, options, physical, true)
	if err == nil || closure != nil || journals != 1 || reads != 0 || writes != 0 { t.Fatalf("oversized known image reached IO: %v/%d/%d/%d", err, journals, reads, writes) }
	fixture.assertReserved(t, 43)
}
...
func TestAttemptSettlementRuntimeV2PersistenceNestedSnapshotBound(t *testing.T) {
	...
	persistence := runtimeAttemptSettlementV2TestPersistence()
	var largest int
	for _, entry := range transaction.Snapshots { for _, image := range [][]byte{entry.OriginalJSON, entry.PreImageJSON, entry.StatsJSON} { if len(image) > largest { largest = len(image) } } }
	if largest < 2 { t.Fatal("real nested snapshot prerequisite missing") }
	persistence.MaxSnapshotBytes = uint64(largest-1)
	if result, _, _, err := decodeAttemptSettlementTransactionV2(t.Context(), encoded, fixture.participants, persistence); err == nil || result != nil { t.Fatal("nested snapshot exceeded independent allowance") }
	persistence.MaxSnapshotBytes++
	if _, _, _, err := decodeAttemptSettlementTransactionV2(t.Context(), encoded, fixture.participants, persistence); err != nil { t.Fatalf("exact nested snapshot allowance refused: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2PersistenceSnapshotOutputBeforeWrite(t *testing.T) {
	...
	for _, data := range before { if uint64(len(data)) > persistence.MaxSnapshotBytes { persistence.MaxSnapshotBytes = uint64(len(data)) } }
	physical, writes := attemptSettlementV2PhysicalIO(), 0
	physical.writeJournal = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected oversized output journal") }
	err := initializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority, persistence, physical)
	if err == nil || writes != 0 || !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatalf("oversized activation postimage reached persistence: %v/%d", err, writes) }
}
...
func TestAttemptSettlementRuntimeV2PersistenceJournalOutputBeforeWrite(t *testing.T) {
	...
	physical, writes := attemptSettlementV2PhysicalIO(), 0
	physical.writeJournal = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected oversized journal output") }
	err := initializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority, persistence, physical)
	if err == nil || writes != 0 { t.Fatalf("oversized journal output reached persistence: %v/%d", err, writes) }
}
...
func TestAttemptSettlementRuntimeV2PersistenceRealSnapshotCountsExact(t *testing.T) {
	...
	for _, participant := range fixture.participants {
		...
		if err != nil || size != uint64(len(actual)) { t.Fatalf("real indented snapshot count differs: %d/%d/%v", size, len(actual), err) }
		encoded, err := encodeAttemptSettlementV2Stats(t.Context(), snapshot, uint64(len(actual)))
		if err != nil || !bytes.Equal(encoded, actual) { t.Fatalf("bounded snapshot changed existing bytes: %v", err) }
		if encoded, err := encodeAttemptSettlementV2Stats(t.Context(), snapshot, uint64(len(actual)-1)); err == nil || encoded != nil { t.Fatal("one-short indented snapshot allowance accepted") }
	}
}
...
func TestAttemptSettlementRuntimeV2PersistenceRealJournalCountsExact(t *testing.T) {
	...
	if err != nil || size != uint64(len(actual)) { t.Fatalf("real nested journal count differs: %d/%d/%v", size, len(actual), err) }
	encoded, err := canonicalAttemptSettlementTransactionV2(t.Context(), transaction, uint64(len(actual)))
	if err != nil || !bytes.Equal(encoded, actual) { t.Fatalf("bounded journal changed actual bytes: %v", err) }
	if encoded, err := canonicalAttemptSettlementTransactionV2(t.Context(), transaction, uint64(len(actual)-1)); err == nil || encoded != nil { t.Fatal("one-short nested journal allowance accepted") }
}
...
func TestAttemptSettlementRuntimeV2PersistenceEncodingAdjacentExact(t *testing.T) {
	...
	for _, value := range []sample{
		...
		for _, indent := range []bool{false, true} {
			var actual []byte
			var err error
			if indent { actual, err = json.MarshalIndent(value, "", "  ") } else { actual, err = json.Marshal(value) }
			if err != nil { t.Fatal(err) }
			actual = append(actual, '\n')
			encoded, err := marshalAttemptSettlementV2JSON(t.Context(), value, uint64(len(actual)), indent, true)
			if err != nil || !bytes.Equal(encoded, actual) { t.Fatalf("adjacent JSON count/encoding differs indent=%t: %v", indent, err) }
			if _, err := countAttemptSettlementV2JSON(t.Context(), value, uint64(len(actual)-1), indent, true); err == nil { t.Fatal("one-short adjacent JSON allowance accepted") }
		}
	}
}
...
func TestAttemptSettlementRuntimeV2PersistenceCustomEncodersRefused(t *testing.T) {
	...
	for _, value := range []any{attemptSettlementV2CountCustom{calls: &calls}, &attemptSettlementV2CountCustom{calls: &calls}, (*attemptSettlementV2CountCustom)(nil), attemptSettlementV2CountText{calls: &calls}, []attemptSettlementV2CountByte{1}, json.RawMessage(`{"x":1}`), json.Number("1"), map[string]any{"dynamic": "value"}} {
		if encoded, err := marshalAttemptSettlementV2JSON(t.Context(), value, 1024, false, true); err == nil || encoded != nil || calls != 0 { t.Fatalf("unsupported encoder was evaluated: %T/%v/%d", value, err, calls) }
	}
}
...
func TestAttemptSettlementRuntimeV2PersistenceCounterUnsafeInputsRefused(t *testing.T) {
	for _, limit := range []uint64{0, uint64(^uint(0)>>1), ^uint64(0)} {
		if encoded, err := marshalAttemptSettlementV2JSON(t.Context(), "", limit, false, true); err == nil || encoded != nil { t.Fatal("unsafe metadata arithmetic allowance accepted") }
	}
	ctx, cancel := context.WithCancel(t.Context())
	cancel()
	if _, err := countAttemptSettlementV2JSON(ctx, "actual", 1024, false, true); !errors.Is(err, context.Canceled) { t.Fatalf("counter cancellation lost: %v", err) }
	type cycle struct { Next *cycle `json:"next"` }
	loop := &cycle{}
	loop.Next = loop
	for _, value := range []any{loop, math.Inf(1), math.NaN()} {
		if encoded, err := marshalAttemptSettlementV2JSON(t.Context(), value, 1024, false, true); err == nil || encoded != nil { t.Fatalf("unsafe metadata value accepted: %T", value) }
	}
}
