...
func releaseHistoricalTestMetadata(t *testing.T) string {
	...
	if err := errors.Join(err, reader.Close()); err != nil { t.Fatal(err) }
...
func installReleaseHistoricalTestNative(t *testing.T, native *releaseNativeValidatorTestFixture) *string {
	...
	native.chain.API.Client = &validatorRuntimeIdentityTestClient{callContext: func(ctx context.Context, result any, method string, args ...any) error {
		...
		if method == "state_getMetadata" {
			if len(args) != 1 || args[0] != native.block.Hex() { return errors.New("historical metadata changed original block") }
...
// when the source is455 and the currently approved release is458.
func TestReleaseEvidenceV2HistoricalRuntimeActivationAuthenticatesOriginalConsent(t *testing.T) {
	...
	if _, err := fixture.read(t.Context()); err != nil { t.Fatalf("original455 activation replay: %v", err) }
...
func TestReleaseEvidenceV2HistoricalRuntimeActivationRejectsChangedAuthority(t *testing.T) {
	for _, test := range []struct { name string; mutate func(*releaseActivationV2TestFixture, *string) }{
		...
		{name: "runtime-authority", mutate: func(f *releaseActivationV2TestFixture, _ *string) { f.authority.NativeRuntime.CodeHash = types.Hash{98}.Hex() }},
	} {
		fixture := newReleaseActivationV2TestFixture(t, "")
		raw := installReleaseHistoricalTestNative(t, fixture.native)
		fixture.authority.NativeRuntime = releaseHistoricalTestCurrentArtifact()
		test.mutate(fixture, raw)
		if _, err := fixture.read(t.Context()); err == nil { t.Fatalf("changed %s became historical activation authority", test.name) }
	}
}
...
func TestReleaseEvidenceV2HistoricalRuntimeSelectionPreservesExplicitAuthority(t *testing.T) {
	...
	want := []crv4.RuntimeArtifactIdentity{current, releaseHistoricalTestArtifact()}
	if got := HistoricalReleaseRuntimeArtifacts(current); !reflect.DeepEqual(got, want) { t.Fatalf("reviewed history selection differs: %+v", got) }
	for _, test := range []struct { name string; mutate func(*crv4.RuntimeArtifactIdentity) }{
		...
		input := current
		test.mutate(&input)
		if got := HistoricalReleaseRuntimeArtifacts(input); !reflect.DeepEqual(got, []crv4.RuntimeArtifactIdentity{input}) { t.Fatalf("%s gained implicit runtime authority", test.name) }
	}
	first := HistoricalReleaseRuntimeArtifacts(current)
	first[1].CodeHash = "changed"
	if !reflect.DeepEqual(HistoricalReleaseRuntimeArtifacts(current), want) { t.Fatal("caller mutated shared historical authority") }
}
...
func TestReleaseEvidenceV2HistoricalRuntimeKeepsFreshSigningCurrent(t *testing.T) {
	...
	if err := authenticatePinnedNativeRuntimeAtContext(t.Context(), native.chain, &cfg, native.block); err == nil { t.Fatal("fresh signing admitted455") }
	if native.chain.Meta != metadata || native.chain.Runtime != runtime { t.Fatal("refused signing mutated binding") }
	owned := *native.chain
	if err := authenticateHistoricalNativeRuntimeAtContext(t.Context(), &owned, &cfg, native.block); err != nil { t.Fatalf("historical private source binding: %v", err) }
	if owned.Runtime.SpecVersion != 455 || native.chain.Meta != metadata || native.chain.Runtime != runtime { t.Fatal("historical binding escaped its private owner") }
}
...
func releaseHistoricalTestPendingReplay(t *testing.T, versionTwo bool) {
	...
	native.chain.API.Client = &validatorRuntimeIdentityTestClient{callContext: func(ctx context.Context, result any, method string, args ...any) error {
		if strings.HasPrefix(method, "author_") { sendCalls++; return errors.New("historical replay attempted a write") }
		if method == "chain_getBlock" {
			if len(args) != 1 || args[0] != native.block.Hex() { return errors.New("pending scan changed original hash") }
			blockReads++
	...
	if _, err := prepared.Validate(); err != nil { t.Fatal(err) }
	cfg := runtime458ValidatorTestConfig()
	steerer := &ReleaseSteerer{cfg: &cfg, native: native.chain}
	intent := &SteeringIntent{Status: "pending", Prepared: prepared, SubnetEpoch: 1}
	metadata, runtime := native.chain.Meta, native.chain.Runtime
	var err error
	if versionTwo { _, err = steerer.reconcilePendingV2(t.Context(), intent, &crv4.EpochScheduleState{SubnetEpochIndex: 1}) } else { _, err = steerer.reconcilePending(t.Context(), intent, &crv4.EpochScheduleState{SubnetEpochIndex: 1}) }
	if err == nil || !strings.Contains(err.Error(), "pending steering replay uses a historical signing runtime") || blockReads != 1 || sendCalls != 0 { t.Fatalf("old preparation replay guard: blocks=%d sends=%d error=%v", blockReads, sendCalls, err) }
	if native.chain.Meta != metadata || native.chain.Runtime != runtime { t.Fatal("pending history changed active signing view") }
}

func TestReleaseEvidenceV2HistoricalRuntimePendingLegacyRefusesNewSubmission(t *testing.T) { releaseHistoricalTestPendingReplay(t, false) }
func TestReleaseEvidenceV2HistoricalRuntimePendingV2RefusesNewSubmission(t *testing.T) { releaseHistoricalTestPendingReplay(t, true) }
...
func TestReleaseEvidenceV2HistoricalRuntimeDecisionRetainsOriginalSources(t *testing.T) {
	...
	installReleaseHistoricalTestNative(t, fixture.native)
	current := releaseHistoricalTestCurrentArtifact()
	fixture.history.cfg.RuntimeSpec, fixture.history.cfg.RuntimeCodeHash, fixture.history.cfg.RuntimeMetadataHash = 458, current.CodeHash, current.MetadataHash
	if err := fixture.history.authenticateIntentChainReference(t.Context(), fixture.decision.chain, fixture.native.chain, current, fixture.intent, fixture.artifact); err != nil { t.Fatalf("original455 decision replay under458: %v", err) }
	fixture.artifact.NativeSnapshotHash = types.Hash{98}.Hex()
	if err := fixture.history.authenticateIntentChainReference(t.Context(), fixture.decision.chain, fixture.native.chain, current, fixture.intent, fixture.artifact); err == nil { t.Fatal("historical decision accepted a changed original block") }
}
...
func TestReleaseEvidenceV2HistoricalRuntimeStartupChecksOriginalSchedule(t *testing.T) {
	...
	if err := authenticateReleaseStartupNativeV2Context(t.Context(), native.chain, initial, journal, releaseHistoricalTestCurrentArtifact(), false); err != nil { t.Fatalf("original455 startup schedule: %v", err) }
	journal.SubnetEpoch++
	if err := authenticateReleaseStartupNativeV2Context(t.Context(), native.chain, initial, journal, releaseHistoricalTestCurrentArtifact(), false); err == nil { t.Fatal("historical startup accepted changed subnet epoch") }
}
...
func TestReleaseEvidenceV2HistoricalRuntimeAdoptionChecksExactAppliedRow(t *testing.T) {
	...
	native.chain.API.Client = &validatorRuntimeIdentityTestClient{callContext: func(ctx context.Context, result any, method string, args ...any) error {
		if method == "state_getStorage" && len(args) == 2 && args[0] == key.Hex() {
			if args[1] != native.block.Hex() { return errors.New("applied row changed original block") }
	...
	if err := authenticateAdoptedIntentApplicationV2(t.Context(), native.chain, &cfg, intent); err != nil { t.Fatalf("original455 applied receipt: %v", err) }
	intent.Values[1]++
	if err := authenticateAdoptedIntentApplicationV2(t.Context(), native.chain, &cfg, intent); err == nil { t.Fatal("adopted history accepted changed applied weights") }
}
...
func TestReleaseEvidenceV2HistoricalRuntimeArchiveReplaysOriginalConfig(t *testing.T) {
	...
	if err := fixture.options.Config.Validate(); err == nil { t.Fatal("original archive config became fresh launch authority") }
	archive, err := openReleaseEvidenceV2ArchiveHistory(t.Context(), fixture.options)
	if err != nil { t.Fatalf("original455 closed archive replay: %v", err) }
	defer archive.Close()
	closure, err := archive.TerminalClosure(fixture.last.Epoch)
	if err != nil || !bytes.Equal(mustArchiveV2JSONTest(t, closure), mustArchiveV2JSONTest(t, fixture.last)) { t.Fatalf("original archive lost signed terminal closure: %v", err) }
	if !bytes.Equal(before, mustArchiveV2JSONTest(t, fixture.options.Config)) { t.Fatal("archive rewrote original config bytes") }
}

func TestReleaseEvidenceV2HistoricalRuntimeArchiveRejectsChangedOriginalConfig(t *testing.T) {
	fixture := newReleaseArchiveV2TestFixtureWithTrails(t, 1)
	for _, test := range []struct { name string; mutate func(*ReleaseConfig) }{
		...
		options.Config = &cfg
		reads := 0
		options.ReadSource = func(context.Context, ReleaseEvidenceV2CaptureSource) ([]byte, error) { reads++; return nil, errors.New("invalid authority reached archive sources") }
		if archive, err := openReleaseEvidenceV2ArchiveHistory(t.Context(), options); err == nil || archive != nil || reads != 0 { t.Fatalf("changed %s escaped archive admission: reads=%d error=%v", test.name, reads, err) }
	}
}
