...
func TestReleaseRuntime459AuthenticatesOriginal458BlockWithoutCurrentAuthority(t *testing.T) {
	...
	if err != nil || metadataHash != artifact.MetadataHash { t.Fatalf("historical metadata differs: %v", err) }
	block := types.Hash{41}
	var calls []string
	client := &validatorRuntimeIdentityTestClient{callContext: func(_ context.Context, result any, method string, args ...any) error {
		if len(args) == 0 || args[len(args)-1] != block.Hex() { return errors.New("historical request lost its exact block") }
		calls = append(calls, method)
		switch method {
		case "state_getRuntimeVersion": return setReleaseHistoricalTestResult(result, artifact.Version)
		case "state_getStorageHash":
			if len(args) != 2 || args[0] != "0x3a636f6465" { return errors.New("historical request changed storage key") }
			return setReleaseHistoricalTestResult(result, artifact.CodeHash)
		case "state_getMetadata": return setReleaseHistoricalTestResult(result, metadataHex)
		default: return errors.New("unexpected historical method")
		}
	}}
