...
// which invokes the observer immediately before its one real generic call.
func TestAttemptCutV2StatsPublicPathRetainsObservedGenericBoundary(t *testing.T) {
	...
	var public, helper *ast.FuncDecl
	...
	if public == nil || public.Body == nil || len(public.Body.List) != 1 || helper == nil || helper.Body == nil {
	...
	genericCalls, boundaryPairs, policyReplayCalls, statelessReplayCalls := 0, 0, 0, 0
	...
	for index, statement := range helper.Body.List {
		...
		observed, ok := helper.Body.List[index-1].(*ast.IfStmt)
	...
	if genericCalls != 1 || boundaryPairs != 1 {
		t.Fatalf("actual generic boundary count changed: calls=%d pairs=%d", genericCalls, boundaryPairs)
	}
