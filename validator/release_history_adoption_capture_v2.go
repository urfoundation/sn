...
func CaptureReleaseHistoryAdoptionV2(ctx context.Context, configPath string, configBytes []byte, approvedPlanHash, sourcePlanHash string, firstNativeEpoch uint64) (encoded []byte, resultErr error) {
	...
	defer func() { resultErr = errors.Join(resultErr, closeOwner(), ctx.Err()); if resultErr != nil { encoded = nil } }()
	...
	if _, err := request.matchPrefix(ctx, file, raw, cfg.EvidenceV2.Bounds.IntentFileLimit()); err != nil { return nil, err }
	encoded, err = json.MarshalIndent(request, "", "  ")
	if err != nil { return nil, err }
	encoded = append(encoded, '\n')
	if _, err := DecodeReleaseHistoryAdoptionV2(encoded, ReleaseMeasurementContentHash(encoded)); err != nil { return nil, err }
	return encoded, nil
}
...
func CheckReleaseHistoryAdoptionV2Source(ctx context.Context, configPath string, configBytes, encoded []byte, expectedSHA256 string) (resultErr error) {
	...
	cfg, err := decodeReleaseConfigBytes(configPath, bytes.Clone(configBytes))
	if err != nil { return err }
	if cfg.ProvisionalDeferClosedNativeInput || cfg.ChainID != 945 || cfg.Policy.NetworkProfile != "testnet" || cfg.GenesisHash != "0x8f9cf856bf558a14440e75569c9e58594757048d7b3a84b5d25f6bd978263105" || cfg.DeploymentID != request.DeploymentID || cfg.ValidatorID != request.ValidatorID { return errors.New("strict history source changes the testnet owner") }
	file, raw, closeOwner, err := readHistoryAdoptionPrefixV2(ctx, cfg, request.CoordinatorStateDir)
...
func readHistoryAdoptionPrefixV2(ctx context.Context, cfg *ReleaseConfig, root string) (*steeringIntentFile, []byte, func() error, error) {
	...
	if err != nil { return nil, nil, finish, err }
	file := &steeringIntentFile{Schema: steeringIntentSchema}
	if raw == nil { return file, nil, finish, nil }
	if err := json.Unmarshal(raw, file); err != nil { return nil, nil, finish, err }
	canonical, err := marshalAttemptSettlementV2JSON(ctx, file, cfg.EvidenceV2.Bounds.IntentFileLimit(), true, true)
	if err != nil || file.Schema != steeringIntentSchema || !bytes.Equal(raw, canonical) { return nil, nil, finish, errors.Join(errors.New("strict history capture requires the original canonical intent file"), err) }
