...
func (self *attemptSettlementRuntimeV2TestFixture) nextWindow(t *testing.T, closure *AttemptSettlementClosureV2) {
	...
	for index, fixture := range self.fixtures {
		...
		resolver := func(_ context.Context, pinned *AttemptBoundary, ids []connect.Id) (AttemptBoundary, []AttemptBinding, error) {
			if pinned != nil && *pinned != expected.Boundary { return AttemptBoundary{}, nil, errors.New("runtime successor fixture changed finalized pin") }
...
func newAttemptSettlementRuntimeV2TestFixture(t *testing.T, activate bool) *attemptSettlementRuntimeV2TestFixture {
	...
	for _, noID := range []uint64{9, 11} {
		operator := newAttemptCutV2SealTestFixtureForOperator(t, 8, 0, 0, noID)
		if operator.policy.Verify.ReliabilityAMin != 8 || operator.policy.Verify.TrailDepth != 8 { t.Fatal("runtime fixture changed the real policy") }
	...
	if activate {
		if err := InitializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority); err != nil { t.Fatalf("actual empty activation: %v", err) }
	}
	return fixture
}
...
func (self *attemptSettlementRuntimeV2TestFixture) options(t *testing.T) AttemptSettlementRuntimeV2Options {
	t.Helper()
	options := AttemptSettlementRuntimeV2Options{Authority: AttemptSettlementV2Options{Operators: map[uint64]AttemptSettlementV2OperatorOptions{}, MaxParticipants: 2, MaxTransitionBytes: 256*1024, MaxClosureBytes: 1024*1024}, Seal: map[uint64]AttemptCutV2SealOptions{}, PrivateKeys: map[uint64]ed25519.PrivateKey{}}
	for index, fixture := range self.fixtures {
		...
		for _, id := range fixture.server.providers { bindings[id] = attemptCutV2HeadTestFleetKey(t, attemptLedgerTestBinding(id, 1)) }
...
func (self *attemptSettlementRuntimeV2TestFixture) trails(t *testing.T, index, completed, failed int) {
	...
	for range completed {
		...
		if err != nil || proof == nil || proof.M != 8 || len(proof.Hops) != 8 { t.Fatalf("actual runtime M8 trail: %v", err) }
	}
	fixture.engine.transport = attemptCutV2SealTestTransport(func(ctx context.Context, hop connect.Id, raw []byte) ([]byte, error) {
		var request struct { TrailID *connect.Id `json:"trail_id"` }
		if err := json.Unmarshal(raw, &request); err != nil { return nil, err }
		if request.TrailID != nil { return nil, errors.New("runtime fixture actual extension refusal") }
		return fixture.server.PostVerify(ctx, hop, raw)
	})
	for range failed {
		proof, err := fixture.engine.RunTrail(t.Context())
		if err == nil || proof != nil { t.Fatal("actual refused extension produced a proof") }
	}
...
func (self *attemptSettlementRuntimeV2TestFixture) assertReserved(t *testing.T, epoch uint64) {
	...
	for _, participant := range self.participants {
		...
		stats.mu.Unlock()
		if !reserved { t.Fatalf("no_id %d was not reserved for %d", participant.NoID, epoch) }
	}
}
...
func (self *attemptSettlementRuntimeV2TestFixture) transaction(t *testing.T) *attemptSettlementTransactionV2 {
	...
	if err != nil { t.Fatal(err) }
	transaction, _, _, err := decodeAttemptSettlementTransactionV2(raw, self.participants)
	if err != nil { t.Fatal(err) }
	return transaction
}
...
func assertAttemptSettlementRuntimeV2Postimages(t *testing.T, coordinator string, participants []AttemptSettlementRuntimeV2Participant, transaction *attemptSettlementTransactionV2) {
	...
	for index, participant := range participants {
		...
		if err != nil || !bytes.Equal(data, transaction.Snapshots[index].StatsJSON) { t.Fatalf("no_id %d durable postimage differs: %v", participant.NoID, err) }
		live, err := encodeStatsSnapshot(participant.Stats.snapshotStats())
		if err != nil || !bytes.Equal(live, data) || participant.Stats.attemptCutPending || participant.Stats.attemptSettlementCutPending || participant.Stats.attemptLedger != participant.Ledger { t.Fatalf("no_id %d live postimage or attachment differs: %v", participant.NoID, err) }
	}
	if _, err := os.Lstat(attemptSettlementTransactionV2Path(coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("completed journal remains: %v", err) }
	if len(transaction.ClosureJSON) != 0 {
		closure, err := os.ReadFile(AttemptSettlementClosureV2Path(coordinator, transaction.Epoch-1))
		if err != nil || !bytes.Equal(closure, transaction.ClosureJSON) { t.Fatalf("immutable postimage differs: %v", err) }
	}
}
...
func (self *attemptSettlementRuntimeV2TestFixture) partialWrite(t *testing.T) *attemptSettlementTransactionV2 {
	...
	physical.writeSnapshot = func(root *attemptPrivateDirectory, name string, raw []byte) error {
		calls++
		if calls == 2 { return errors.New("runtime second snapshot failure") }
	...
	if err == nil || closure != nil || calls != 2 || !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, self.participants)) { t.Fatalf("partial durability published a partial generation: %v", err) }
...
func (self *attemptSettlementRuntimeV2TestFixture) pendingRestart(t *testing.T) []AttemptSettlementRuntimeV2Participant {
	...
	for index, participant := range self.participants {
		...
		if record.Disposition != AttemptDispositionPending || record.M != 8 { t.Fatal("real pending M8 prefix prerequisite differs") }
		if err := participant.Ledger.Close(); err != nil { t.Fatal(err) }
