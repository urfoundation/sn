...
func acquireStatsSnapshotDirectory(path string, physical bool, _ statsSnapshotIOHooks) (*statsSnapshotDirectory, error) {
	if physical { return nil, errAttemptPrivateDirectoryPlatform }
...
func (self *statsSnapshotDirectory) admit(write statsSnapshotWrite) error {
	if write.version >= 6 { return errAttemptPrivateDirectoryPlatform }
	absolute, err := filepath.Abs(write.path)
	if err != nil { return err }
	if self == nil || self.written || absolute != self.path { return errors.New("statistics compatibility snapshot target differs") }
	return nil
}
...
func writeStatsSnapshotOwned(self *statsSnapshotDirectory, write statsSnapshotWrite) error {
	...
	if err := atomicStateWrite(write.path, write.data, 0o600); err != nil { return err }
	self.written = true
	return nil
}
