...
func TestAttemptSettlementRuntimeV2InitializesExactEmptyPrefix(t *testing.T) {
	...
	for index, participant := range fixture.participants {
		...
		if err != nil || snapshot.Version != 6 || snapshot.AttemptV2 == nil || snapshot.AttemptV2.Activation != fixture.fixtures[index].expected.Activation || snapshot.AttemptV2.Terminal != nil || snapshot.AttemptSettlementFirstSequence != head.LastSequence+1 || snapshot.AttemptV2.SettlementPriorRoot != head.Root || participant.Stats.attemptCutPending { t.Fatalf("actual activation prefix differs: %+v %v", snapshot, err) }
	}
	if _, err := os.Lstat(attemptSettlementTransactionV2Path(fixture.coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("completed activation retained journal: %v", err) }
	if _, err := os.Lstat(filepath.Join(fixture.coordinator, "settlement-closures-v2")); !errors.Is(err, os.ErrNotExist) { t.Fatalf("activation invented a closed epoch: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2ActivationRefusesNonemptySignedWindow(t *testing.T) {
	...
	if err := InitializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence()); err == nil { t.Fatal("activation discarded a real signed window") }
	if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("refused activation changed raw state") }
	if _, err := os.Lstat(attemptSettlementTransactionV2Path(fixture.coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("refused activation wrote a journal: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2ActivationRejectsMismatchedPin(t *testing.T) {
	...
	writes := 0
	physical.writeJournal = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected activation write") }
	if err := initializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 42, options, runtimeAttemptSettlementV2TestPersistence(), physical); err == nil || writes != 0 { t.Fatalf("candidate-selected activation entered persistence: %v/%d", err, writes) }
}
...
func TestAttemptSettlementRuntimeV2PersistsCompleteRealM8Batch(t *testing.T) {
	...
	if err != nil || closure == nil || transaction == nil { t.Fatalf("actual complete batch: %v", err) }
	if closure.Transitions[0].Cut.RecordCount != 122 || closure.Transitions[0].Cut.CompleteCount != 15 || closure.Transitions[0].Cut.FailedCount != 1 || closure.Transitions[1].Cut.RecordCount != 8 || len(closure.Transitions[0].PostFold) == 0 { t.Fatal("real complete/failed M8 census or positive quality was lost") }
	for _, quality := range closure.Transitions[0].PostFold { if !quality.HasQuality || quality.QualityPPM == 0 { t.Fatal("guaranteed real score was not positive") } }
	...
	if err != nil || !reflect.DeepEqual(read, closure) || len(verified.Operators) != 2 || verified.Operators[9].Replay.Records.ItemCount != 122 || verified.Operators[11].Replay.Records.ItemCount != 8 { t.Fatalf("independent immutable full replay differs: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2OwnsRetainedTerminalSeparately(t *testing.T) {
	...
	if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("public closure mutated retained terminal state") }
}
...
func TestAttemptSettlementRuntimeV2PartialWriteSaveThenActualRestart(t *testing.T) {
	...
	for index, participant := range fixture.participants {
		if transaction.Snapshots[index].OriginalHash == transaction.Snapshots[index].PreImageHash { t.Fatal("fixture did not exercise a newer live preimage") }
		if err := participant.Stats.Save(participant.StateDir); err != nil { t.Fatal(err) }
		data, err := os.ReadFile(filepath.Join(participant.StateDir, "stats.json"))
		if err != nil || !bytes.Equal(data, transaction.Snapshots[index].PreImageJSON) { t.Fatalf("actual shutdown Save differs from recorded preimage: %v", err) }
	}
	restarted := fixture.reopen(t)
	if err := RecoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, restarted, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence()); err != nil { t.Fatal(err) }
...
func TestAttemptSettlementRuntimeV2RetryFinishesWithoutResealing(t *testing.T) {
	...
	if err != nil || closure == nil || writes != 0 { t.Fatalf("journal retry resealed or failed: %v writes=%d", err, writes) }
	assertAttemptSettlementRuntimeV2Postimages(t, fixture.coordinator, fixture.participants, transaction)
}
...
func TestAttemptSettlementRuntimeV2PublishesAllBeforeJournalRemoval(t *testing.T) {
	...
	physical.removeJournal = func(*attemptPrivateDirectory, string) error {
		for _, participant := range fixture.participants {
			if participant.Stats.snapshotStats().SettlementEpoch == nil || participant.Stats.settlementEpoch != 43 || !participant.Stats.attemptCutPending || !participant.Stats.attemptSettlementCutPending { t.Error("removal observed partial or admitted publication") }
		}
		return cause
	}
	closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t), physical, true)
	if !errors.Is(err, cause) || closure != nil { t.Fatalf("removal result escaped: %v", err) }
	fixture.assertReserved(t, 43)
	transaction := fixture.transaction(t)
	if _, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t)); err != nil { t.Fatal(err) }
	assertAttemptSettlementRuntimeV2Postimages(t, fixture.coordinator, fixture.participants, transaction)
}
...
func TestAttemptSettlementRuntimeV2RetriesAfterRemovedJournalFailure(t *testing.T) {
	...
	physical.removeJournal = func(root *attemptPrivateDirectory, name string) error { return errors.Join(removeAttemptSettlementV2OwnedTransaction(root, name), cause) }
	if closure, err := advanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t), physical, true); !errors.Is(err, cause) || closure != nil { t.Fatalf("removed journal failure: %v", err) }
	fixture.assertReserved(t, 43)
	before := runtimeAttemptSettlementV2TestImages(t, fixture.participants)
	if _, err := os.Lstat(attemptSettlementTransactionV2Path(fixture.coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("real removal did not happen: %v", err) }
	if _, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t)); err != nil { t.Fatal(err) }
	if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("no-journal retry refolded or changed exact snapshots") }
}
...
func TestAttemptSettlementRuntimeV2CancellationAfterFirstSnapshotRetainsJournal(t *testing.T) {
	...
	if closure, err := advanceAttemptSettlementEpochV2(ctx, fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t), physical, true); !errors.Is(err, context.Canceled) || closure != nil || writes != 1 { t.Fatalf("canceled partial snapshot: %v writes=%d", err, writes) }
	fixture.assertReserved(t, 43)
	transaction := fixture.transaction(t)
	if _, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t)); err != nil { t.Fatal(err) }
	assertAttemptSettlementRuntimeV2Postimages(t, fixture.coordinator, fixture.participants, transaction)
...
func TestAttemptSettlementRuntimeV2RetainsCanceledPhysicalWriteCause(t *testing.T) {
	...
	physical.writeSnapshot = func(root *attemptPrivateDirectory, name string, raw []byte) error {
		...
		if err := os.Mkdir(path, 0o700); err != nil { return err }
	...
	if closure != nil || !errors.Is(err, context.Canceled) || physicalErr == nil || !errors.Is(err, physicalErr) || !errors.As(physicalErr, &renameErr) { t.Fatalf("cancellation hid the actual snapshot failure: %v", err) }
	fixture.assertReserved(t, 43)
...
func TestAttemptSettlementRuntimeV2ReservesEveryOperatorBeforeDrain(t *testing.T) {
	...
	if err := first.Stats.beginAttempt(42, first.Ledger); err != nil { t.Fatal(err) }
	defer first.Stats.abortAttempt()
	if closure, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t)); !errors.Is(err, errAttemptCutPending) || closure != nil { t.Fatalf("active drain result: %v", err) }
	fixture.assertReserved(t, 43)
	if err := fixture.participants[1].Stats.beginAttempt(42, fixture.participants[1].Ledger); !errors.Is(err, errAttemptCutPending) { t.Fatalf("later operator was not reserved: %v", err) }
	if _, err := os.Lstat(attemptSettlementTransactionV2Path(fixture.coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("undrained batch journaled: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2RefusesNativeReservationWithoutMutation(t *testing.T) {
	...
	owner.release()
	before := runtimeAttemptSettlementV2TestImages(t, fixture.participants)
	if closure, err := AdvanceAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.fixtures[0].expected.Boundary, fixture.options(t)); !errors.Is(err, errAttemptCutPending) || closure != nil { t.Fatalf("native reservation result: %v", err) }
	if !stats.attemptCutPending || stats.attemptSettlementCutPending || fixture.participants[0].Stats.attemptCutPending || !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("settlement stole or partially published a native reservation") }
}
...
func TestAttemptSettlementRuntimeV2UnchangedRefreshPreservesNativeReservation(t *testing.T) {
	...
	if err != nil || closure != nil || !stats.attemptCutPending || stats.attemptSettlementCutPending || !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatalf("unchanged refresh stole native ownership: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2RecoveryRejectsLiveAndRepeatedAttachment(t *testing.T) {
	fixture := newAttemptSettlementRuntimeV2TestFixture(t, true)
	before := runtimeAttemptSettlementV2TestImages(t, fixture.participants)
	if err := RecoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence()); err == nil { t.Fatal("recovery reset live owners") }
	if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("refused live recovery mutated state") }
	restarted := fixture.reopen(t)
	if err := RecoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, restarted, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence()); err != nil { t.Fatal(err) }
	if err := RecoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, restarted, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence()); err == nil { t.Fatal("repeated recovery reattached running owners") }
}
...
func TestAttemptSettlementRuntimeV2RecoveryRejectsUnknownSameEpochImage(t *testing.T) {
	...
	if err := json.Unmarshal(transaction.Snapshots[1].PreImageJSON, &snapshot); err != nil { t.Fatal(err) }
	snapshot.EgressGeneration++
	changed, err := encodeStatsSnapshot(snapshot)
	if err != nil { t.Fatal(err) }
	if err := atomicStateWrite(transaction.Snapshots[1].StatsPath, changed, 0o600); err != nil { t.Fatal(err) }
	restarted := fixture.reopen(t)
	physical := attemptSettlementV2PhysicalIO()
	writes := 0
	physical.writeSnapshot = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected recovery write") }
	if err := recoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, restarted, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence(), physical); err == nil || writes != 0 || !strings.Contains(err.Error(), "exact known image") { t.Fatalf("unrelated generation entered recovery: %v writes=%d", err, writes) }
	for _, participant := range restarted { if participant.Stats.attemptLedger != nil || participant.Stats.attemptV2 != nil { t.Fatal("refused recovery attached a partial owner") } }
}
...
func TestAttemptSettlementRuntimeV2RecoveryReplaysBeforeAnySnapshotWrite(t *testing.T) {
	...
	if !errors.Is(err, cause) || writes != 0 || closes == 0 { t.Fatalf("late replay close published recovery: %v writes=%d closes=%d", err, writes, closes) }
}
