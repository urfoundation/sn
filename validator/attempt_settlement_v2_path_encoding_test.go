...
func runtimeAttemptSettlementV2MoveEmptyOperator(t *testing.T, fixture *attemptSettlementRuntimeV2TestFixture, index int, path string) {
	...
	if err != nil || head.LastSequence != 0 || head.Root != zeroAttemptHash() || participant.Stats.attemptV2 != nil { t.Fatalf("path fixture requires an actual empty legacy prefix: %+v/%v", head, err) }
	if err := participant.Ledger.Close(); err != nil { t.Fatal(err) }
	if err := os.Rename(participant.StateDir, path); err != nil { t.Fatal(err) }
	...
	if err != nil || reopened != head || ledger.identity != participant.Ledger.identity { t.Fatalf("path fixture changed its real identity or prefix: %v", err) }
	stats := NewStatsEngine(participant.Stats.cfg)
	if err := stats.Load(path); err != nil { t.Fatal(err) }
	if err := stats.AttachAttemptLedger(ledger, path); err != nil { t.Fatal(err) }
	store, err := NewProofStore(path)
	if err != nil { t.Fatal(err) }
	config := operator.engine.cfg; config.AttemptLedger = ledger
...
func TestAttemptSettlementRuntimeV2PathEncodingCoordinatorBeforeIO(t *testing.T) {
	for _, role := range []string{"initialize", "advance", "recover", "closure"} {
		...
		if role == "advance" || role == "closure" { fixture.trails(t, 0, 2, 1) }
		if role == "closure" {
		...
		if utf8.ValidString(path) { t.Fatal("invalid-byte coordinator fixture became UTF-8") }
		if err := os.Rename(fixture.coordinator, path); err != nil { t.Fatal(err) }
		fixture.coordinator = path
		participants := fixture.participants
		if role == "recover" { participants = fixture.reopen(t) }
		...
		var err error
		switch role {
		case "initialize": err = initializeAttemptSettlementEpochV2(t.Context(), path, participants, 42, options.Authority, options.Persistence, physical)
		case "advance": _, err = advanceAttemptSettlementEpochV2(t.Context(), path, participants, 43, fixture.fixtures[0].expected.Boundary, options, physical, true)
		case "recover": err = recoverAttemptSettlementEpochV2(t.Context(), path, participants, options.Authority, options.Persistence, physical)
		case "closure":
			closure, verified, readErr := readAttemptSettlementClosureV2(t.Context(), path, 42, options.Authority, physical)
			err = readErr
			if closure != nil || !reflect.DeepEqual(verified, VerifiedAttemptSettlementV2{}) { t.Fatal("invalid-byte closure coordinator returned a result") }
		}
		if err == nil || !strings.Contains(err.Error(), "lossless UTF-8") || events != 0 { t.Fatalf("invalid-byte coordinator reached IO or was normalized: role=%s events=%d err=%v", role, events, err) }
	}
}
...
func TestAttemptSettlementRuntimeV2PathEncodingOperatorBeforeIO(t *testing.T) {
	...
	if utf8.ValidString(path) { t.Fatal("invalid-byte operator fixture became UTF-8") }
	...
	if err == nil || !strings.Contains(err.Error(), "lossless UTF-8") || events != 0 { t.Fatalf("invalid-byte operator reached IO or lossy journal encoding: events=%d err=%v", events, err) }
	if !reflect.DeepEqual(before, runtimeAttemptSettlementV2TestImages(t, fixture.participants)) { t.Fatal("invalid-byte operator refusal mutated live state") }
	if _, err := os.Lstat(attemptSettlementTransactionV2Path(fixture.coordinator)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("invalid-byte operator wrote a journal: %v", err) }
}
...
func TestAttemptSettlementRuntimeV2PathEncodingRealUnicodeRestart(t *testing.T) {
	...
	var journal []byte
	physical.writeJournal = func(root *attemptPrivateDirectory, name string, data []byte) error { journal = append([]byte(nil), data...); return writeAttemptSettlementV2OwnedState(root, name, data) }
	...
	if err := json.Unmarshal(journal, &transaction); err != nil { t.Fatal(err) }
	if len(transaction.Snapshots) != 2 || !bytes.Contains(journal, []byte(`\u003c`)) { t.Fatal("real Unicode journal lost its path/HTML-escape prerequisite") }
	for index, entry := range transaction.Snapshots {
		if entry.StatsPath != filepath.Join(fixture.participants[index].StateDir, "stats.json") { t.Fatal("real Unicode journal changed path bytes") }
	}
	...
	if err != nil || closure == nil { t.Fatalf("real Unicode terminal failed: %v", err) }
	if _, _, err := ReadAttemptSettlementClosureV2(t.Context(), fixture.coordinator, 42, fixture.options(t).Authority); err != nil { t.Fatal(err) }
	images := runtimeAttemptSettlementV2TestImages(t, fixture.participants)
	participants := fixture.reopen(t)
	if err := RecoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, participants, fixture.options(t).Authority, runtimeAttemptSettlementV2TestPersistence()); err != nil { t.Fatal(err) }
	if !reflect.DeepEqual(images, runtimeAttemptSettlementV2TestImages(t, participants)) { t.Fatal("real Unicode restart changed signed postimages") }
}
