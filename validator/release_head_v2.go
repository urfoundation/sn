...
func (self *ReleaseSteerer) gatherHeadV2(ctx context.Context, snapshot *ReleaseSnapshot, subnetEpoch, nativeBlock uint64, nativeHash string, hotkeyUIDs map[[32]byte]uint16, inputs []ReleaseMeasurementInput, options ReleaseMeasurementV2Options) (result releaseHeadResult, resultErr error) {
	...
	for _, input := range inputs {
		if input.Stats.AttemptCut != nil || input.Stats.SettlementTransition != nil { return result, errors.New("compact live head contains competing legacy authority") }
	}
	budget, err := admitReleaseHeadV2Controls(ctx, draft, uint64(len(hotkeyUIDs)), options.MaxControlBytes)
	if err != nil { return result, err }
	headEMA := self.headEMA
	if err := headEMA.admitReleaseHeadV2Known(ctx, subnetEpoch, draft.Policy.Steering.HeadScoreEMA, options.MaxHeadEntries, budget); err != nil { return result, err }
	...
	if err != nil { return result, err }
	if err := headEMA.admitReleaseHeadV2Current(ctx, subnetEpoch, fleets, draft.Policy.Steering.HeadScoreEMA, options.MaxHeadEntries, budget); err != nil { return result, err }
	for _, input := range draft.Inputs {
		operator := options.Operators[input.NoID]
		...
		for key, prefixes := range verified.HeadPrefixes {
			...
			for prefix := range prefixes {
				if err := ctx.Err(); err != nil { return result, err }
				if !fleets[key][prefix] { if err := budget.charge(1, 32+1); err != nil { return result, err } }
	...
	if err != nil { return result, err }
	if err := checkReleaseHeadV2Result(ctx, result, budget); err != nil { return result, err }
	return result, nil
}
...
func admitReleaseHeadV2Controls(ctx context.Context, draft *ReleaseMeasurementArtifact, nativeCount, maximum uint64) (releaseHeadV2Budget, error) {
	...
	if ctx == nil || draft == nil || maximum == 0 || maximum > maxReleaseMeasurementArtifactBytes { return budget, errors.New("compact live head control admission is incomplete") }
	remaining := maximum
	if err := releaseMeasurementV2ControlStorage(ctx, reflect.ValueOf(draft), &remaining); err != nil { return budget, err }
	...
	if err := charge(nativeCount, 32+2); err != nil { return budget, err }
	if err := charge(uint64(len(draft.Inputs)), 8+uint64(reflect.TypeFor[ClientKeyFunc]().Size())); err != nil { return budget, err }
	for _, input := range draft.Inputs {
		...
		if err := charge(uint64(len(input.Stats.Providers)), width); err != nil { return budget, err }
	}
