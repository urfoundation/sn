...
func newAttemptSettlementRuntimeV2LegacyHistoryFixture(t *testing.T, positive bool) *attemptSettlementRuntimeV2TestFixture {
	...
	for index, participant := range fixture.participants {
		...
		if err := participant.Ledger.Walk(t.Context(), 1, head.LastSequence, func(record AttemptRecord) error { records = append(records, record); return nil }); err != nil { t.Fatal(err) }
		image, err := encodeStatsSnapshot(participant.Stats.snapshotStats())
		if err != nil { t.Fatal(err) }
		if err := participant.Ledger.Close(); err != nil { t.Fatal(err) }
		dir := newAttemptSettlementRuntimeV2TestStateDir(t)
		if err := os.WriteFile(filepath.Join(dir, "attempt-ledger.jsonl"), attemptLedgerDiskTestJSONL(t, records), 0o600); err != nil { t.Fatal(err) }
		if err := os.WriteFile(filepath.Join(dir, "stats.json"), image, 0o600); err != nil { t.Fatal(err) }
		ledger, err := NewAttemptLedger(dir, operator.expected.Identity, operator.key)
		if err != nil { t.Fatal(err) }
		t.Cleanup(func() { _ = ledger.Close() })
		stats := NewStatsEngine(participant.Stats.cfg)
		if err := stats.Load(dir); err != nil { t.Fatal(err) }
	...
	if err := AdvanceAttemptSettlementEpoch(fixture.coordinator, 43, fixture.fixtures[0].expected.Boundary, legacy); err != nil { t.Fatalf("actual complete v1 close: %v", err) }
	for index, participant := range legacy {
		operator := fixture.fixtures[index]
		head, err := legacyLedgers[index].Head()
		if err != nil { t.Fatal(err) }
		if participant.Stats.settlementTransition == nil || participant.Stats.settlementTransition.PreFold.AttemptCut.LastSequence != head.LastSequence || len(participant.Stats.window) != 0 || len(participant.Stats.egress) != 0 { t.Fatal("real legacy close did not empty its exact signed window") }
		if err := legacyLedgers[index].Close(); err != nil { t.Fatal(err) }
		...
		if err != nil || imported != head { t.Fatalf("actual legacy migration changed signed prefix: %v", err) }
		stats := NewStatsEngine(participant.Stats.cfg)
		if err := stats.Load(participant.StateDir); err != nil { t.Fatal(err) }
		if err := stats.AttachAttemptLedger(disk, participant.StateDir); err != nil { t.Fatal(err) }
		store, err := NewProofStore(participant.StateDir)
		if err != nil { t.Fatal(err) }
		if err := store.ReconcileAttemptProofsContext(t.Context(), disk); err != nil { t.Fatal(err) }
		...
		config.AttemptBoundaryResolver = func(_ context.Context, pinned *AttemptBoundary, ids []connect.Id) (AttemptBoundary, []AttemptBinding, error) {
			if pinned != nil && *pinned != expected.Boundary { return AttemptBoundary{}, nil, errors.New("legacy migration fixture changed finalized pin") }
			bindings := make([]AttemptBinding, len(ids))
			for index, id := range ids { bindings[index] = attemptLedgerTestBinding(id, 1) }
...
func TestAttemptSettlementRuntimeV2ActivationPreservesRealLegacyPositiveHistory(t *testing.T) {
	...
	if len(fixture.participants[0].Stats.emaPPM) == 0 { t.Fatal("real a_min8 legacy close did not establish positive prior") }
	if err := InitializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.options(t).Authority); err != nil { t.Fatal(err) }
	for index, participant := range fixture.participants {
		snapshot := participant.Stats.snapshotStats()
		if snapshot.Version != 6 || snapshot.AttemptV2 == nil || snapshot.SettlementTransition == nil { t.Fatal("activation omitted the actual carried legacy history") }
		snapshot.Version, snapshot.AttemptV2 = 5, nil
		legacy, err := encodeStatsSnapshot(snapshot)
		if err != nil || !bytes.Equal(legacy, before[index]) { t.Fatalf("activation rewrote v1 history or manufactured prior: %v", err) }
	}
	...
	if !reflect.DeepEqual(runtimeAttemptSettlementV2TestImages(t, fixture.participants), runtimeAttemptSettlementV2TestImages(t, restarted)) { t.Fatal("restart changed genuine legacy-derived prior") }
}
...
func TestAttemptSettlementRuntimeV2TerminalPreservesLegacyHistoryWithRealNewRows(t *testing.T) {
	...
	for index, participant := range fixture.participants { var err error; legacy[index], err = json.Marshal(participant.Stats.settlementTransition); if err != nil { t.Fatal(err) } }
	...
	if err != nil { t.Fatal(err) }
	for index, participant := range fixture.participants {
		if closure.Transitions[index].Cut.RecordCount != 8 || closure.Transitions[index].PreFold.SettlementTransition != nil || closure.Transitions[index].PreFold.AttemptCut != nil { t.Fatal("compact terminal did not use exactly the actual new M8 window") }
		retained, err := json.Marshal(participant.Stats.settlementTransition)
		if err != nil || !bytes.Equal(retained, legacy[index]) { t.Fatalf("compact terminal rewrote original legacy bytes: %v", err) }
	}
	restarted := fixture.reopen(t)
	if err := RecoverAttemptSettlementEpochV2(t.Context(), fixture.coordinator, restarted, fixture.options(t).Authority); err != nil { t.Fatal(err) }
}
...
func TestAttemptSettlementRuntimeV2ActivationRejectsMutatedLegacyHistory(t *testing.T) {
	...
	if err == nil { err = json.Unmarshal(encoded, &changed) }
	if err == nil && len(changed.Signature) == 0 { err = errors.New("real legacy terminal signature prerequisite is missing") }
	if err == nil { changed.Signature[0] ^= 1; participant.Stats.settlementTransition = &changed }
	participant.Stats.mu.Unlock()
	owner.release()
	if err != nil { t.Fatal(err) }
	physical := attemptSettlementV2PhysicalIO()
	writes := 0
	physical.writeJournal = func(*attemptPrivateDirectory, string, []byte) error { writes++; return errors.New("unexpected legacy mutation journal") }
	if err := initializeAttemptSettlementEpochV2(t.Context(), fixture.coordinator, fixture.participants, 43, fixture.options(t).Authority, physical); err == nil || writes != 0 { t.Fatalf("mutated signed legacy history entered activation: %v/%d", err, writes) }
}
