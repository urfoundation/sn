...
func TestRuntimeArtifactMetadataCatalogPreservesExactManifest(t *testing.T) {
	...
	var manifest struct { Artifacts []struct {
	...
	if len(artifacts) != len(manifest.Artifacts) || len(artifacts) != len(wantSpecs) { t.Fatal("catalog lost an exact retained artifact") }
	for index, row := range manifest.Artifacts {
		...
		if row.SpecVersion != wantSpecs[index] || artifacts[index] != want { t.Fatalf("catalog artifact%d changed: %+v", index, artifacts[index]) }
		if got, ok := ReviewedRuntimeArtifact(want.Version); !ok || got != want { t.Fatal("exact catalog lookup differs") }
		...
		} { if _, ok := ReviewedRuntimeArtifact(version); ok { t.Fatal("changed version inherited a reviewed artifact") } }
	}
	...
	if !ok || current != artifacts[len(artifacts)-1] || current.CodeHash != ReviewedRuntimeCodeHash || current.MetadataHash != ReviewedRuntimeMetadataHash { t.Fatal("current selection is not the exact latest catalog artifact") }
	copyArtifacts := append([]RuntimeArtifactIdentity(nil), artifacts...)
	artifacts[0] = RuntimeArtifactIdentity{}
	if !reflect.DeepEqual(ReviewedRuntimeArtifacts(), copyArtifacts) { t.Fatal("caller mutated reviewed artifact authority") }
	for _, spec := range []uint32{456,457,461} { if _, ok := ReviewedRuntimeArtifact(RuntimeVersionIdentity{SpecName:"node-subtensor", SpecVersion:spec, TransactionVersion:1, StateVersion:1}); ok { t.Fatal("unreviewed runtime entered the catalog") } }
}
